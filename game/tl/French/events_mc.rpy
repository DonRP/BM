#     old ""
#     ""
# "Plural-Forms: nplurals=2; plural=(n > 1);\n"
# "X-french-Project: bad-memories\n"
# "X-french-Project-ID: 507994\n"
# "X-french-Language: fr\n"
# "X-french-File: /main/game/tl/french/events_mc.po\n"
# "X-french-File-ID: 3720\n"
# "Project-Id-Version: bad-memories\n"
# "Content-Type: text/plain; charset=UTF-8\n"
# "Language-Team: French\n"
# "Language: fr_FR\n"
# "PO-Revision-Date: 2022-05-03 20:31\n"

# TODO: Translation updated at 2022-04-03 HH:HH

# game/events_mc.rpy:5
translate french day0Morning_65a592f9:

    # pc "YAAAHHWN..."
    pc "YAAAHHWN..."

# game/events_mc.rpy:6
translate french day0Morning_3b638262:

    # pcthink "Gawd, I needed that sleep..."
    pcthink "Bon sang, j'avais besoin de ce sommeil..."

# game/events_mc.rpy:8
translate french day0Morning_fe495e49:

    # pcthink "Huh?! Who put the blanket on me?"
    pcthink "Huh ?! Qui a mis la couverture sur moi ?"

# game/events_mc.rpy:9
translate french day0Morning_1a156d9c:

    # pcthink "...and took my shoes off?"
    pcthink "...et j'ai enlevé mes chaussures ?"

# game/events_mc.rpy:12
translate french day0Morning_aff1c6b1:

    # pcthink "Uhh ouch, my back..."
    pcthink "Uhh ouch, mon dos..."

# game/events_mc.rpy:14
translate french day0Morning_35a70f21:

    # pcthink "Argh... wasn't the best idea to fall asleep in a chair!"
    pcthink "Argh... ce n'était pas la meilleure idée de s'endormir sur une chaise !"

# game/events_mc.rpy:16
translate french day0Morning_7159aa77:

    # n "You struggle to get up from the chair, but eventually make it."
    n "Vous luttez pour vous lever de la chaise, mais vous y arrivez finalement."

# game/events_mc.rpy:18
translate french day0Morning_63d6be43:

    # pcthink "Ugh... So this is how old people feel."
    pcthink "Ugh... Donc c'est ce que ressentent les personnes âgées."

# game/events_mc.rpy:19
translate french day0Morning_1f6ff3b9:

    # pcthink "I better try not to sit in that chair again when I'm tired..."
    pcthink "Je ferais mieux d'essayer de ne plus m'asseoir sur cette chaise quand je suis fatigué..."

# game/events_mc.rpy:20
translate french day0Morning_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:21
translate french day0Morning_f92b90eb:

    # pcthink "...back home after all these years..."
    pcthink "...de retour à la maison après toutes ces années..."

# game/events_mc.rpy:22
translate french day0Morning_07ad280b:

    # pcthink "I'll have to have a look for [e] later..."
    pcthink "Je vais devoir chercher [e] plus tard..."

# game/events_mc.rpy:23
translate french day0Morning_eed7ec06:

    # pcthink "But first things first. I really need to use the bathroom!"
    pcthink "Mais chaque chose en son temps. J'ai vraiment besoin d'utiliser les toilettes !"

# game/events_mc.rpy:30
translate french childRoomDay0_5f0b8470:

    # pc "Hey [e], [m] told me to..."
    pc "Hé [e], [m] m'a dit de..."

# game/events_mc.rpy:31
translate french childRoomDay0_ad420ce1:

    # pcthink "Huh? She's not here?"
    pcthink "Huh ? Elle n'est pas là ?"

# game/events_mc.rpy:32
translate french childRoomDay0_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:33
translate french childRoomDay0_eac0ec9f:

    # pcthink "Maybe she is in the bathroom?"
    pcthink "Peut-être qu'elle est dans la salle de bain ?"

# game/events_mc.rpy:35
translate french childRoomDay0_4c93f6e9:

    # pcthink "Hmm... the door is not locked and there is nothing to hear."
    pcthink "Hmm... la porte n'est pas verrouillée et il n'y a rien à entendre."

# game/events_mc.rpy:37
translate french childRoomDay0_70d4d8a9:

    # pcthink "No one here..."
    pcthink "Personne ici..."

# game/events_mc.rpy:39
translate french childRoomDay0_b76c7bc0:

    # pcthink "Okay, that's weird... did I miss her?"
    pcthink "Ok, c'est bizarre... est-ce que je l'ai manquée ?"

# game/events_mc.rpy:40
translate french childRoomDay0_8ad047f3:

    # pcthink "Maybe in the Livingroom?"
    pcthink "Peut-être dans le salon ?"

# game/events_mc.rpy:42
translate french childRoomDay0_50f4faf2:

    # pcthink "Nope, not here either... where is she?"
    pcthink "Non, pas ici non plus... où est-elle ?"

# game/events_mc.rpy:51
translate french kitchenBreakfastDay0_719102df:

    # n "A bit later" with dissolve
    n "Un peu plus tard" with dissolve

# game/events_mc.rpy:55
translate french kitchenBreakfastDay0_024f1747:

    # m "Oh, so you only need to go to work every few days?"
    m "Oh, donc tu n'as besoin d'aller au travail que quelques jours ?"

# game/events_mc.rpy:56
translate french kitchenBreakfastDay0_d3fae95e:

    # pc "Yeah, you could say I'm an employed freelancer."
    pc "Oui, on peut dire que je suis un freelance salarié."

# game/events_mc.rpy:57
translate french kitchenBreakfastDay0_da8f7c38:

    # pcthink "What's up with [e]?"
    pcthink "Qu'est-ce qui se passe avec [e]?"

# game/events_mc.rpy:58
translate french kitchenBreakfastDay0_48b558b0:

    # m "Sounds interesting, but does this job really pay that well?"
    m "Ça a l'air intéressant, mais ce travail paie-t-il vraiment si bien ?"

# game/events_mc.rpy:59
translate french kitchenBreakfastDay0_4358ca4b:

    # pc "The pay is okay, I manage..."
    pc "Le salaire est correct, je gère..."

# game/events_mc.rpy:60
translate french kitchenBreakfastDay0_7433034f:

    # pcthink "...actually more than okay, but why would I tell her that..."
    pcthink "...en fait plus que bien, mais pourquoi je lui dirais ça..."

# game/events_mc.rpy:61
translate french kitchenBreakfastDay0_44098f14:

    # m "Well, if you like your job, who am I to judge. But if you ever need something, just tell me, okay?"
    m "Eh bien, si tu aimes ton travail, qui suis-je pour juger. Mais si jamais tu as besoin de quelque chose, dis-le moi, ok ?"

# game/events_mc.rpy:62
translate french kitchenBreakfastDay0_77b9401c:

    # pcthink "Not gonna happen..."
    pcthink "Ça n'arrivera pas..."

# game/events_mc.rpy:63
translate french kitchenBreakfastDay0_deb9672a:

    # pc "Don't worry, I won't need anything."
    pc "Ne t'inquiètes pas, je n'aurai besoin de rien."

# game/events_mc.rpy:64
translate french kitchenBreakfastDay0_473b0738:

    # m "Alright..."
    m "Très bien..."

# game/events_mc.rpy:66
translate french kitchenBreakfastDay0_ad73f7a1:

    # m "[e], don't you want to eat something?"
    m "[e], tu ne veux pas manger quelque chose ?"

# game/events_mc.rpy:68
translate french kitchenBreakfastDay0_8fbec38d:

    # m "Alright, I'm going to make myself ready for work."
    m "Très bien, je vais me préparer pour le travail."

# game/events_mc.rpy:70
translate french kitchenBreakfastDay0_46a844ac:

    # m "[e] eat up, okay?"
    m "[e] mange, ok ?"

# game/events_mc.rpy:72
translate french kitchenBreakfastDay0_b3d1ca42:

    # pcthink "Ugh... she's doing it again..."
    pcthink "Ugh... elle le fait encore..."

# game/events_mc.rpy:74
translate french kitchenBreakfastDay0_deadf7f8:

    # pc "Err... what's up [e]?"
    pc "Err... quoi de neuf [e]?"

# game/events_mc.rpy:75
translate french kitchenBreakfastDay0_72a33094:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:76
translate french kitchenBreakfastDay0_2ce091cd:

    # pcthink "Damn, she doesn't talk. I keep forgetting that..."
    pcthink "Merde, elle ne parle pas. J'oublie toujours ça..."

# game/events_mc.rpy:77
translate french kitchenBreakfastDay0_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:78
translate french kitchenBreakfastDay0_72a33094_1:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:79
translate french kitchenBreakfastDay0_8b857d1c:

    # pc "Uh, hey I'm sorry I didn't recognize you back in the city..."
    pc "Uh, hé je suis désolé de ne pas t'avoir reconnu en ville..."

# game/events_mc.rpy:80
translate french kitchenBreakfastDay0_72a33094_2:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:81
translate french kitchenBreakfastDay0_8096f560:

    # pcthink "This is awkward..."
    pcthink "C'est gênant..."

# game/events_mc.rpy:82
translate french kitchenBreakfastDay0_ec8a779d:

    # pcthink "Why is she staring at me like this?!"
    pcthink "Pourquoi est-ce qu'elle me regarde comme ça ?!"

# game/events_mc.rpy:83
translate french kitchenBreakfastDay0_0ecb835a:

    # pcthink "It's almost creepy..."
    pcthink "C'est presque effrayant..."

# game/events_mc.rpy:85
translate french kitchenBreakfastDay0_d4887870:

    # m "[e], get ready for school, we need to leave in five minutes!"
    m "[e], prépare-toi pour l'école, on doit partir dans cinq minutes !"

# game/events_mc.rpy:88
translate french kitchenBreakfastDay0_cf0767d9:

    # n "She jumps up immediately and leaves the room."
    n "Elle se lève immédiatement et quitte la pièce."

# game/events_mc.rpy:89
translate french kitchenBreakfastDay0_142c6d46:

    # pcthink "Phew... thanks [m]..."
    pcthink "Ouf... merci [m]..."

# game/events_mc.rpy:90
translate french kitchenBreakfastDay0_e8626d4a:

    # n "A few minutes later" with dissolve
    n "Quelques minutes plus tard" with dissolve

# game/events_mc.rpy:94
translate french kitchenBreakfastDay0_71b5423a:

    # pcthink "Damn I don't know how to handle the situation with [e]..."
    pcthink "Bon sang, je ne sais pas comment gérer la situation avec [e]..."

# game/events_mc.rpy:95
translate french kitchenBreakfastDay0_1784afc6:

    # pcthink "But at least she doesn't seem to be mad at me..."
    pcthink "Mais au moins elle ne semble pas être en colère contre moi..."

# game/events_mc.rpy:96
translate french kitchenBreakfastDay0_3881cbd8:

    # pcthink "Maybe I shouldn't think too much about it."
    pcthink "Peut-être que je ne devrais pas trop y penser."

# game/events_mc.rpy:97
translate french kitchenBreakfastDay0_e9849b15:

    # pcthink "I mean, she's just a normal girl...right?"
    pcthink "Je veux dire, c'est juste une fille normale...non ?"

# game/events_mc.rpy:98
translate french kitchenBreakfastDay0_4883c233:

    # pcthink "Aside from not talking..."
    pcthink "A part ne pas parler..."

# game/events_mc.rpy:99
translate french kitchenBreakfastDay0_8e2aeca0:

    # pcthink "...and staring at me like a creep..."
    pcthink "...et me regardant fixement comme un sale type..."

# game/events_mc.rpy:100
translate french kitchenBreakfastDay0_88a665a7:

    # pcthink "...and sleeping in my bed..."
    pcthink "...et dormir dans mon lit..."

# game/events_mc.rpy:101
translate french kitchenBreakfastDay0_f0aa75e3:

    # pcthink "Ugh... don't think about it, she's just a normal girl! No sweat!"
    pcthink "Ugh... n'y pense pas, c'est juste une fille normale ! Pas de problème !"

# game/events_mc.rpy:103
translate french kitchenBreakfastDay0_4f2c56a3:

    # n "*KNOCK* *KNOCK"
    n "*TOC* *TOC"

# game/events_mc.rpy:105
translate french kitchenBreakfastDay0_c48e5fe0:

    # m "We are leaving, will you manage?"
    m "Nous partons, tu te débrouilleras ?"

# game/events_mc.rpy:106
translate french kitchenBreakfastDay0_39d36142:

    # pc "Yeah, sure."
    pc "Oui, bien sûr."

# game/events_mc.rpy:107
translate french kitchenBreakfastDay0_f03b848b:

    # pcthink "Damn, [e] looks like she's trying to seduce someone..."
    pcthink "Bon sang, [e] on dirait qu'elle essaie de séduire quelqu'un..."

# game/events_mc.rpy:108
translate french kitchenBreakfastDay0_592d8410:

    # m "Food is in the fridge, take whatever you want, okay?"
    m "La nourriture est dans le frigo, prends ce que tu veux, ok ?"

# game/events_mc.rpy:109
translate french kitchenBreakfastDay0_ac935b76:

    # pc "Yeah, okay. Thanks."
    pc "Ouais, ok. Merci."

# game/events_mc.rpy:110
translate french kitchenBreakfastDay0_848f5afd:

    # m "See you later, Hun."
    m "A plus tard, Hun."

# game/events_mc.rpy:111
translate french kitchenBreakfastDay0_19392efe:

    # pc "Later..."
    pc "Plus tard..."

# game/events_mc.rpy:113
translate french kitchenBreakfastDay0_15f55841:

    # pcthink "What's up with that \"Hun\" thing of hers? She never called me that back then..."
    pcthink "Qu'est-ce qui se passe avec son truc de 'Hun'? Elle ne m'a jamais appelé comme ça à l'époque..."

# game/events_mc.rpy:115
translate french kitchenBreakfastDay0_00c77948:

    # pcthink "Anyway, time to relax..."
    pcthink "Bref, il est temps de se détendre..."

# game/events_mc.rpy:122
translate french pcDream1_428d1889:

    # n "You lay down in your bed."
    n "Vous vous allongez dans votre lit."

# game/events_mc.rpy:124
translate french pcDream1_ac247afb:

    # pcthink "It feels so weird to be back..."
    pcthink "Ca fait tellement bizarre d'être de retour..."

# game/events_mc.rpy:125
translate french pcDream1_e59fabcc:

    # pcthink "...seeing [e] and [m] again..."
    pcthink "...voyant [e] et [m] à nouveau..."

# game/events_mc.rpy:126
translate french pcDream1_f7c84076:

    # pcthink "e "surely has grown up, but... actually I thought she should be a bit older by now."
    pcthink "e "a sûrement grandi, mais... en fait je pensais qu'elle devrait être un peu plus âgée maintenant."

# game/events_mc.rpy:127
translate french pcDream1_825c8758:

    # pcthink "How old is she anyway?"
    pcthink "Quel âge a-t-elle de toute façon ?"

# game/events_mc.rpy:128
translate french pcDream1_72087c33:

    # pcthink "Uhh..."
    pcthink "Ouhh..."

# game/events_mc.rpy:129
translate french pcDream1_f7cb6db2:

    # pcthink "Shit, I can't remember..."
    pcthink "Merde, je ne me souviens pas..."

# game/events_mc.rpy:130
translate french pcDream1_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:131
translate french pcDream1_b45b7ed4:

    # pcthink "...and [m]... did she really change?"
    pcthink "...et [m]... a-t-elle vraiment changé ?"

# game/events_mc.rpy:132
translate french pcDream1_d1c86b16:

    # pcthink "Something is bugging me about her... I don't trust her."
    pcthink "Quelque chose me chiffonne chez elle... Je n'ai pas confiance en elle."

# game/events_mc.rpy:133
translate french pcDream1_0c94bab4:

    # pcthink "I bet she's... *yaaawn* ...hiding something..."
    pcthink "Je parie qu'elle... *yaaawn* ...cache quelque chose..."

# game/events_mc.rpy:136
translate french pcDream1_8f03e192:

    # n "You close your eyes and fall asleep."
    n "Vous fermez les yeux et vous vous endormez."

# game/events_mc.rpy:138
translate french pcDream1_d0053a27:

    # n "In your dreams" with dissolve
    n "Dans vos rêves" with dissolve

# game/events_mc.rpy:141
translate french pcDream1_6834e5d1:

    # m "Welcome home [e]."
    m "Bienvenue à la maison [e]."

# game/events_mc.rpy:142
translate french pcDream1_ddb0e6fa:

    # e "Hello, Mommy."
    e "Bonjour, maman."

# game/events_mc.rpy:143
translate french pcDream1_1812dc36:

    # m "Wait a minute, show me your face..."
    m "Attends une minute, montre-moi ton visage..."

# game/events_mc.rpy:144
translate french pcDream1_2abbc6c7:

    # e "Um, it's nothing, Mommy. I just..."
    e "Um, c'est rien, maman. J'ai juste..."

# game/events_mc.rpy:145
translate french pcDream1_24dc9c21:

    # m "[bpc]! WHAT DID YOU DO TO HER?!"
    m "[bpc]! QU'EST-CE QUE TU LUI AS FAIT ?!"

# game/events_mc.rpy:146
translate french pcDream1_d25692ca:

    # pc "Huh? I didn't do anything!"
    pc "Huh ? Je n'ai rien fait !"

# game/events_mc.rpy:147
translate french pcDream1_25928455:

    # e "No. Mommy, it wasn't..."
    e "Non. Maman, ce n'était pas..."

# game/events_mc.rpy:148
translate french pcDream1_43ef6ea4:

    # m "Shut up [e]! [pc] what the hell?!"
    m "La fermes [e]! [pc] c'est quoi ce bordel ?!"

# game/events_mc.rpy:149
translate french pcDream1_71977e73:

    # pc "I-I didn't do anything, I swear! There were some guys bullying her and I..."
    pc "Je-je n'ai rien fait, je le jure ! Il y avait des gars qui l'intimidaient et je..."

# game/events_mc.rpy:150
translate french pcDream1_86b1fd11:

    # m "SHUT UP!"
    m "TAIS-TOI!"

# game/events_mc.rpy:151
translate french pcDream1_a19c3737:

    # m "I don't want to hear any excuses from you! You stupid [ds] of an asshole!"
    m "Je ne veux pas entendre d'excuses de ta part ! Espèce de stupide [ds] de trou du cul !"

# game/events_mc.rpy:152
translate french pcDream1_10b5ce69:

    # e "Mommy, please, [heshe] helped me!"
    e "Maman, s'il te plaît, [heshe] m'a aidé !"

# game/events_mc.rpy:153
translate french pcDream1_c2cdcf05:

    # m "Go to your room [pc]! I don't want to see you leave your room until you go to school tomorrow!"
    m "Va dans ta chambre [pc]! Je ne veux pas te voir quitter ta chambre jusqu'à ce que tu ailles à l'école demain !"

# game/events_mc.rpy:154
translate french pcDream1_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:157
translate french pcDream1_e2ee86ba:

    # pc "Uhh..."
    pc "Uhh..."

# game/events_mc.rpy:158
translate french pcDream1_f4d55998:

    # pcthink "Shit... the memories keep coming back..."
    pcthink "Merde... les souvenirs reviennent sans cesse..."

# game/events_mc.rpy:160
translate french pcDream1_149222e3:

    # pcthink "I wish I could just keep them locked somewhere in the back of my brain."
    pcthink "J'aimerais pouvoir les garder enfermés quelque part au fond de mon cerveau."

# game/events_mc.rpy:161
translate french pcDream1_f0cac90d_1:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:162
translate french pcDream1_e4520362:

    # pcthink "Damn, it really doesn't help to wallow in self-pity..."
    pcthink "Merde, ça ne sert vraiment à rien de s'apitoyer sur soi-même...."

# game/events_mc.rpy:163
translate french pcDream1_f45797d4:

    # pcthink "I better get up and do something useful..."
    pcthink "Je ferais mieux de me lever et de faire quelque chose d'utile..."

# game/events_mc.rpy:164
translate french pcDream1_3d2a2575:

    # pcthink "...well, or whatever."
    pcthink "...bien, ou peu importe."

# game/events_mc.rpy:174
translate french wakeupDay1_8bf6dc88:

    # n "*Knock* *Knock*"
    n "*Toc* *Toc*"

# game/events_mc.rpy:175
translate french wakeupDay1_73d106f4:

    # m "[pc], are you awake?"
    m "[pc], es-tu réveillé ?"

# game/events_mc.rpy:176
translate french wakeupDay1_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:178
translate french wakeupDay1_dabeb1e2:

    # n "*Knock* *Knock* *Knock*"
    n "*Toc* *Toc* *Toc*"

# game/events_mc.rpy:179
translate french wakeupDay1_4a57ea09:

    # m "[pc]?"
    m "[pc]?"

# game/events_mc.rpy:180
translate french wakeupDay1_d4803e34:

    # pc "Uhhh..."
    pc "Uhhh..."

# game/events_mc.rpy:182
translate french wakeupDay1_4300e10e:

    # pc "Uhnnn... yeah..."
    pc "Uhnnn... ouais..."

# game/events_mc.rpy:184
translate french wakeupDay1_60a71439:

    # m "I can't find [e], do you..."
    m "Je ne trouve pas [e], est-ce que tu..."

# game/events_mc.rpy:186
translate french wakeupDay1_f6b14dde:

    # m "What the?! [pc]?"
    m "Qu'est-ce que c'est ?! [pc]?"

# game/events_mc.rpy:187
translate french wakeupDay1_225fadf5:

    # pc "Huh?"
    pc "Huh ?"

# game/events_mc.rpy:189
translate french wakeupDay1_d1ae462b:

    # n "You turn around."
    n "Vous vous retournez."

# game/events_mc.rpy:193
translate french wakeupDay1_7d60a407:

    # pc "WHOA!!!"
    pc "WOUAA !!!"

# game/events_mc.rpy:195
translate french wakeupDay1_820ffab4:

    # pc "What the hell are you doing here, [e]?!"
    pc "Que diable fais-tu ici, [e]?!"

# game/events_mc.rpy:196
translate french wakeupDay1_990aec05:

    # m "Hahaha..."
    m "Hahaha..."

# game/events_mc.rpy:199
translate french wakeupDay1_ffc63eee:

    # m "I guess you didn't know she was here either."
    m "Je suppose que tu ne savais pas non plus qu'elle était là."

# game/events_mc.rpy:200
translate french wakeupDay1_4781350b:

    # m "Looks like someone wanted to cuddle a bit with her big [bs]."
    m "On dirait que quelqu'un a voulu faire un peu de câlin avec son grand [bs]."

# game/events_mc.rpy:202
translate french wakeupDay1_251417f1:

    # m "But, [e], you really shouldn't sneak into your [bs]'s room like this."
    m "Mais, [e], tu ne devrais vraiment pas te faufiler dans la chambre de ton [bs] comme ça."

# game/events_mc.rpy:208
translate french wakeupDay1_fd57a2d8:

    # n "[e] jumps over you and leaves the room."
    n "[e] saute par-dessus vous et quitte la pièce."

# game/events_mc.rpy:211
translate french wakeupDay1_5d70399b:

    # m "By the way, you should start wearing some pants in bed."
    m "Au fait, tu devrais commencer à porter un pantalon au lit."

# game/events_mc.rpy:213
translate french wakeupDay1_89d409bd:

    # m "By the way, you should start wearing pyjamas in bed."
    m "Au fait, tu devrais commencer à porter des pyjamas au lit."

# game/events_mc.rpy:214
translate french wakeupDay1_61e4f5ac:

    # pc "Huh?!"
    pc "Huh ?!"

# game/events_mc.rpy:218
translate french wakeupDay1_dbd30717:

    # pc "FUCK!"
    pc "PUTAIN!"

# game/events_mc.rpy:219
translate french wakeupDay1_01fbb572:

    # m "Hahaha, don't worry, Hun. I have seen that before... even though, you've obviously become a [pcgender]..."
    m "Hahaha, ne t'inquiète pas, Hun. J'ai déjà vu ça avant... même si, de toute évidence, tu es devenu un [pcgender]..."

# game/events_mc.rpy:221
translate french wakeupDay1_d82c55e9:

    # pc "Yeah, could you leave the room please!"
    pc "Ouais, pourrais-tu quitter la pièce s'il te plaît !"

# game/events_mc.rpy:222
translate french wakeupDay1_1d3c410e:

    # m "Hahaha, of course, I'm sorry honey."
    m "Hahaha, bien sûr, je suis désolé chéri."

# game/events_mc.rpy:224
translate french wakeupDay1_0d7416ed:

    # pcthink "Shit! I was so surprised by [e] that I didn't think about sleeping naked..."
    pcthink "Merde ! J'étais tellement surpris par [e] que je n'ai pas pensé que je dormais nu..."

# game/events_mc.rpy:225
translate french wakeupDay1_31a769fc:

    # pcthink "That was embarassing..."
    pcthink "C'était embarrassant..."

# game/events_mc.rpy:227
translate french wakeupDay1_6ba40074:

    # pcthink "Especially with that morning wood..."
    pcthink "Surtout avec cette trique matinal..."

# game/events_mc.rpy:228
translate french wakeupDay1_8cd64871:

    # pcthink "Wait, did [e] see that too?"
    pcthink "Attends, est-ce que [e] a vu ça aussi ?"

# game/events_mc.rpy:229
translate french wakeupDay1_f22ad3c6:

    # pcthink "Damn it..."
    pcthink "Merde..."

# game/events_mc.rpy:231
translate french wakeupDay1_503241c0:

    # n "You get dressed and take a quick shower."
    n "Vous vous habillez et prenez une douche rapide."

# game/events_mc.rpy:232
translate french wakeupDay1_42ed6300:

    # n "After breakfast, [m] and [e] leave for work and school.{p}You go back to your room."
    n "Après le petit déjeuner, [m] et [e] partent au travail et à l'école.{p}Vous retournez dans votre chambre."

# game/events_mc.rpy:233
translate french wakeupDay1_66b94188:

    # n "After a while" with dissolve
    n "Après un certain temps" with dissolve

# game/events_mc.rpy:237
translate french wakeupDay1_341ebece:

    # pcthink "Damn it's so boring..."
    pcthink "Merde, c'est tellement ennuyeux..."

# game/events_mc.rpy:238
translate french wakeupDay1_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:239
translate french wakeupDay1_c7fd02d6:

    # pcthink "Maybe I should look for an apartment again..."
    pcthink "Peut-être que je devrais chercher un appartement à nouveau..."

# game/events_mc.rpy:240
translate french wakeupDay1_a69377ef_2:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:241
translate french wakeupDay1_088ca266:

    # pcthink "It's nice to see [e] again, but I don't want to stay here with [m]..."
    pcthink "C'est agréable de revoir [e] , mais je ne veux pas rester ici avec [m]..."

# game/events_mc.rpy:242
translate french wakeupDay1_a69377ef_3:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:243
translate french wakeupDay1_15ac3a2e:

    # pcthink "I can still visit them from time to time..."
    pcthink "Je peux encore leur rendre visite de temps en temps..."

# game/events_mc.rpy:244
translate french wakeupDay1_a69377ef_4:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:245
translate french wakeupDay1_f22ad3c6_1:

    # pcthink "Damn it..."
    pcthink "Merde..."

# game/events_mc.rpy:246
translate french wakeupDay1_3203a608:

    # pcthink "Tomorrow my stuff comes...maybe I'll wait until then, so I can at least use my computer instead of the phone for that..."
    pcthink "Demain mes affaires arrivent...je vais peut-être attendre jusque là, comme ça je pourrai au moins utiliser mon ordinateur au lieu du téléphone pour ça...."

# game/events_mc.rpy:247
translate french wakeupDay1_ae6d2505:

    # pcthink "...yeah I guess that's a good idea..."
    pcthink "...oui je suppose que c'est une bonne idée..."

# game/events_mc.rpy:248
translate french wakeupDay1_a69377ef_5:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:252
translate french wakeupDay1_c2505a61:

    # n "You go over to the living room and zap around the channels..."
    n "Vous allez au salon et zappez sur les chaînes..."

# game/events_mc.rpy:255
translate french wakeupDay1_612659db:

    # pcthink "Commercials."
    pcthink "Publicités."

# game/events_mc.rpy:256
translate french wakeupDay1_8225006e:

    # pcthink "Commercials..."
    pcthink "Publicités..."

# game/events_mc.rpy:257
translate french wakeupDay1_f4244b81:

    # pcthink "Yaawn..."
    pcthink "Yaawn..."

# game/events_mc.rpy:264
translate french dinnerDay2_7e01f61f:

    # m "So... I know it's just your second day back home, but how do you feel about it? Have you settled in a bit already?"
    m "Alors... je sais que ce n'est que ton deuxième jour de retour à la maison, mais comment te sens-tu ? Tu t'es déjà un peu installé ?"

# game/events_mc.rpy:265
translate french dinnerDay2_e3fd9646:

    # pc "Err... it's still a bit weird to be honest."
    pc "Err... c'est encore un peu bizarre pour être honnête."

# game/events_mc.rpy:267
translate french dinnerDay2_a94c74f3:

    # m "Oh, I'm sure you will come to like it here after a while."
    m "Oh, je suis sûr que tu te plairas ici après un certain temps."

# game/events_mc.rpy:268
translate french dinnerDay2_ef30c1d2:

    # pcthink "After a while? For how long does she expect me to stay?"
    pcthink "Après un certain temps ? Combien de temps veut-elle que je reste ?"

# game/events_mc.rpy:270
translate french dinnerDay2_3c8e055f:

    # m "I..."
    m "Je..."

# game/events_mc.rpy:271
translate french dinnerDay2_e612037c:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:273
translate french dinnerDay2_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh ?"

# game/events_mc.rpy:274
translate french dinnerDay2_8746b622:

    # m "I know you don't have many good memories in this house..."
    m "Je sais que tu n'as pas beaucoup de bons souvenirs dans cette maison..."

# game/events_mc.rpy:276
translate french dinnerDay2_7ce7e5c3:

    # m "But I'm sure that will change from now on!"
    m "Mais je suis sûr que cela va changer à partir de maintenant !"

# game/events_mc.rpy:277
translate french dinnerDay2_267d5d43:

    # pc "Eh... yeah... maybe..."
    pc "Eh... ouais... peut-être..."

# game/events_mc.rpy:278
translate french dinnerDay2_1568d1ae:

    # pcthink "Damn I think she really means it... I'm not sure that it's possible though."
    pcthink "Bon sang, je pense qu'elle le pense vraiment... Je ne suis pas sûr que ce soit possible cependant."

# game/events_mc.rpy:280
translate french dinnerDay2_40da31b4:

    # m "Anyway, it must be boring being alone here half of the day?"
    m "Bref, ça doit être ennuyeux d'être seul ici la moitié de la journée ?"

# game/events_mc.rpy:281
translate french dinnerDay2_a367d625:

    # pc "Yeah, kinda."
    pc "Ouais, en quelque sorte."

# game/events_mc.rpy:283
translate french dinnerDay2_79065927:

    # m "When did you say your stuff comes?"
    m "Quand as-tu dit que tes affaires arrivent ?"

# game/events_mc.rpy:285
translate french dinnerDay2_b0155ac1:

    # pc "...should be delivered by tomorrow. I don't know the exact time though."
    pc "...devrait être livré demain. Mais je ne connais pas l'heure exacte."

# game/events_mc.rpy:287
translate french dinnerDay2_cafd2cdd:

    # m "Have you been able to change your address?"
    m "As-tu pu changer ton adresse ?"

# game/events_mc.rpy:289
translate french dinnerDay2_7c8a8431:

    # pc "Yeah, I sent them the new address via email."
    pc "Oui, je leur ai envoyé la nouvelle adresse par email."

# game/events_mc.rpy:290
translate french dinnerDay2_13506d52:

    # m "Good. That makes it easier at least."
    m "Bien. Cela rend les choses plus faciles au moins."

# game/events_mc.rpy:292
translate french dinnerDay2_73cae700:

    # n "You continue to chatter with [m] while [e] occasionally makes faces at you."
    n "Vous continuez à bavarder avec [m] tandis que [e] vous fait occasionnellement des grimaces."

# game/events_mc.rpy:297
translate french stuffArrivesDay3_c27f600a:

    # n "You hear the doorbell ring."
    n "Vous entendez la sonnerie de la porte."

# game/events_mc.rpy:299
translate french stuffArrivesDay3_d4803e34:

    # pc "Uhhh..."
    pc "Uhhh..."

# game/events_mc.rpy:301
translate french stuffArrivesDay3_180d7c36:

    # pc "YAAAAWN..."
    pc "YAAAAWN..."

# game/events_mc.rpy:303
translate french stuffArrivesDay3_c87da698:

    # pcthink "Looks like [e] is up already..."
    pcthink "On dirait que [e] est déjà en place..."

# game/events_mc.rpy:304
translate french stuffArrivesDay3_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:305
translate french stuffArrivesDay3_869ad9b7:

    # pcthink "Wha... shit, the door. It must be my stuff being delivered!"
    pcthink "Wha... merde, la porte. Ca doit être mes affaires qui sont livrées !"

# game/events_mc.rpy:307
translate french stuffArrivesDay3_74e8e915:

    # n "You get up quickly, throw some clothes on and hurry to the front door."
    n "Vous vous levez rapidement, mettez quelques vêtements et vous vous précipitez vers la porte d'entrée."

# game/events_mc.rpy:312
translate french getEfromSchool1_719102df:

    # n "A bit later" with dissolve
    n "Un peu plus tard" with dissolve

# game/events_mc.rpy:315
translate french getEfromSchool1_88129d62:

    # pcthink "Alright, there is the school..."
    pcthink "D'accord, il y a l'école..."

# game/events_mc.rpy:316
translate french getEfromSchool1_f8e84e65:

    # n "Suddenly you hear a voice from behind you."
    n "Soudain, vous entendez une voix derrière vous."

# game/events_mc.rpy:317
translate french getEfromSchool1_9a3323ff:

    # Voice" "Hey! Stop right there!"
    "Voix" "Hé ! Arrêtez tout de suite !"

# game/events_mc.rpy:318
translate french getEfromSchool1_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh ?"

# game/events_mc.rpy:321
translate french getEfromSchool1_efbb62e5:

    # pcthink "Oh no... not this guy again..."
    pcthink "Oh non... pas encore ce type..."

# game/events_mc.rpy:323
translate french getEfromSchool1_f21e3a4e:

    # Cop" "Hey... *pant* ...I said... *pant* ... stop right there!"
    "Flic" "Hé... *halètement* ...j'ai dit... *halètement* ... arrêtez-vous là !"

# game/events_mc.rpy:324
translate french getEfromSchool1_5a35ce3d:

    # pc "I'm just standing here Officer..."
    pc "Je suis juste debout ici Officier..."

# game/events_mc.rpy:325
translate french getEfromSchool1_c7dd96ed:

    # Cop" "You... *pant* ... shit..."
    "Flic" "Vous... *halètement* ... merde..."

# game/events_mc.rpy:326
translate french getEfromSchool1_38aa9b62:

    # pc "A little out of shape Officer?"
    pc "Un officier un peu hors de forme ?"

# game/events_mc.rpy:328
translate french getEfromSchool1_b5c7bd7a:

    # Cop" "You better shut it... *pant* ...smart ass."
    "Flic" "Tu ferais mieux de la fermer... *halètement* ...petit malin."

# game/events_mc.rpy:330
translate french getEfromSchool1_04e1c72c:

    # Cop" "Just so you know... I know about you... I have seen your files!"
    "Policier" "Juste pour vous... Je sais sur vous... J'ai vu vos fichiers!"

# game/events_mc.rpy:331
translate french getEfromSchool1_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:333
translate french getEfromSchool1_4071f137:

    # Cop" "What?! Surprised?"
    "Policier" "Quoi?! Surpris?"

# game/events_mc.rpy:335
translate french getEfromSchool1_5bd96603:

    # Cop" "It wasn't easy to get them *gasp* and my colleagues even told me to let it go, but I'm not a lazy bum like they are."
    "Policier" "Il n'a pas été facile de les obtenir *gasp* et mes collègues m'ont même dit de laisser tomber, mais je ne suis pas un fainéant comme eux."

# game/events_mc.rpy:336
translate french getEfromSchool1_e91b54e7:

    # Cop" "The case might be closed, but I know you have skeletons in your closet!"
    "Policier" "L'affaire est peut-être classée, mais je sais que vous avez des squelettes dans votre placard !"

# game/events_mc.rpy:337
translate french getEfromSchool1_a08e5c45:

    # pcthink "Shit!"
    pcthink "Merde !"

# game/events_mc.rpy:339
translate french getEfromSchool1_b385900b:

    # Cop" "I'll be keeping an eye on you!"
    "Policier" "Je garderai un œil sur vous !"

# game/events_mc.rpy:340
translate french getEfromSchool1_22c9823e:

    # pc "Well, I hope you have fun with that. I have things to do..."
    pc "Eh bien, j'espère que vous vous amuserez avec ça. J'ai des choses à faire..."

# game/events_mc.rpy:341
translate french getEfromSchool1_9d86ab89:

    # Cop" "I'm watching you!"
    "Policier" "Je vous surveille !"

# game/events_mc.rpy:343
translate french getEfromSchool1_1f323051:

    # pc "Yeah... bye..."
    pc "Oui ... au revoir ..."

# game/events_mc.rpy:344
translate french getEfromSchool1_4d295fc8:

    # pcthink "Damn, this guy is annoying... I hope he doesn't become a problem..."
    pcthink "Merde, ce gars est ennuyeux... J'espère qu'il ne deviendra pas un problème..."

# game/events_mc.rpy:347
translate french getEfromSchool1_fc5cd1ac:

    # pcthink "Hum... the school didn't change a bit. It looks exactly like when I left..."
    pcthink "Hum... l'école n'a pas changé du tout. C'est exactement comme quand je suis parti..."

# game/events_mc.rpy:350
translate french getEfromSchool1_7ec6f8c8:

    # j "{size=-10}...and you really should have told me how hot [heshe] is{/size}."
    j "{size=-10}...et tu aurais vraiment dû me dire à quel point [heshe] est sexy{/size}."

# game/events_mc.rpy:351
translate french getEfromSchool1_b5156e92:

    # pcthink "Isn't that [j]?"
    pcthink "N'est-ce pas [j]?"

# game/events_mc.rpy:354
translate french getEfromSchool1_4ff91ccc:

    # j "Speaking of the devil, look who's here!"
    j "En parlant du diable, regardez qui est là !"

# game/events_mc.rpy:356
translate french getEfromSchool1_d593d4b0:

    # e "Eh?"
    e "Eh ?"

# game/events_mc.rpy:357
translate french getEfromSchool1_dc0ee556:

    # pc "Hey girls."
    pc "Salut les filles."

# game/events_mc.rpy:359
translate french getEfromSchool1_2e48f33f:

    # j "{b}[e]!{/b}"
    j "{b}[e]!{/b}"

# game/events_mc.rpy:361
translate french getEfromSchool1_3e82ab9a:

    # j "Can't you at least wait until you're home before you hump your big [bs]?"
    j "Ne peux-tu pas au moins attendre d'être à la maison avant de baiser ton gros [bs]?"

# game/events_mc.rpy:362
translate french getEfromSchool1_e262c0dd:

    # pcthink "What the?! Hump me?"
    pcthink "Qu'est-ce que?! Saute moi?"

# game/events_mc.rpy:366
translate french getEfromSchool1_85279c1b:

    # j "So... what's up [pc]? Did you miss your {i}cute little sister{/i} soo much that you have to come visit her?"
    j "Alors... quoi de neuf [pc]? Ta {i}mignonne petite sœur{/i} t'a tellement manqué que tu dois venir lui rendre visite ?"

# game/events_mc.rpy:367
translate french getEfromSchool1_b55bf2a3:

    # pcthink "What the fuck?"
    pcthink "C'est quoi ce bordel ?"

# game/events_mc.rpy:368
translate french getEfromSchool1_f20dc1e2:

    # j "Or... did you {i}cum{/i} for me?"
    j "Ou... as-tu du {i}sperme{/i} pour moi ?"

# game/events_mc.rpy:370
translate french getEfromSchool1_ac669b20:

    # e "Hnnn!!!"
    e "Hnnn !!!"

# game/events_mc.rpy:371
translate french getEfromSchool1_74b093ad:

    # j "Hehe, calm down [e], I'm just teasing."
    j "Héhé, calme-toi [e], je te taquine juste."

# game/events_mc.rpy:372
translate french getEfromSchool1_5f16d7e5:

    # pcthink "Damn, this girl is such a minx!"
    pcthink "Merde, cette fille est une vraie coquine !"

# game/events_mc.rpy:374
translate french getEfromSchool1_c6ae69db:

    # pc "Actually, [m] called me to pick up [e]. She is stuck in traffic and can't make it."
    pc "En fait, [m] m'a appelé pour aller chercher [e]. Elle est coincée dans les embouteillages et ne peut pas venir."

# game/events_mc.rpy:375
translate french getEfromSchool1_fbaa51cd:

    # pc "She tried to call you by the way."
    pc "Elle a essayé de vous appeler au fait."

# game/events_mc.rpy:377
translate french getEfromSchool1_50a16fe0:

    # j "Did she?"
    j "L'a-t-elle fait ?"

# game/events_mc.rpy:379
translate french getEfromSchool1_18cc7152:

    # j "I didn't notice. Oh but my phone is on vibrate, hehe."
    j "Je n'ai pas remarqué. Oh mais mon téléphone est sur vibreur, héhé."

# game/events_mc.rpy:380
translate french getEfromSchool1_9cdf1147:

    # pc "Uh-huh..."
    pc "Uh-huh..."

# game/events_mc.rpy:382
translate french getEfromSchool1_b37ee738:

    # j "Well, shall we go then?"
    j "Bon, on y va alors ?"

# game/events_mc.rpy:383
translate french getEfromSchool1_f6e48b56:

    # pc "You're coming with us?"
    pc "Tu viens avec nous ?"

# game/events_mc.rpy:384
translate french getEfromSchool1_5f925933:

    # j "Of course!"
    j "Bien sûr !"

# game/events_mc.rpy:386
translate french getEfromSchool1_43c5c0e8:

    # j "Someone has to make sure that a specific young lady doesn't get herself into trouble."
    j "Quelqu'un doit s'assurer que telle ou telle jeune femme ne s'attire pas d'ennuis."

# game/events_mc.rpy:387
translate french getEfromSchool1_9b114ec6:

    # pc "Eh... Not that I mind you coming with us, but I'm sure I can handle that."
    pc "Eh... Pas que ça me dérange que tu viennes avec nous, mais je suis sûr que je peux gérer ça."

# game/events_mc.rpy:388
translate french getEfromSchool1_9157e01d:

    # j "No you can't."
    j "Non tu ne peux pas."

# game/events_mc.rpy:389
translate french getEfromSchool1_5887b362:

    # pc "What? Why is that?"
    pc "Quoi ? Pourquoi ça ?"

# game/events_mc.rpy:391
translate french getEfromSchool1_465c7403:

    # j "Because you're the source of the problem, silly."
    j "Parce que tu es la source du problème, idiot."

# game/events_mc.rpy:392
translate french getEfromSchool1_7bb57f9a:

    # pc "Err... okay?"
    pc "Err... ok ?"

# game/events_mc.rpy:393
translate french getEfromSchool1_7cb675a2:

    # j "Well, come on. Let's go!"
    j "Eh bien, aller. Allons-y !"

# game/events_mc.rpy:394
translate french getEfromSchool1_94a92282:

    # e "Mhm!"
    e "Mhm !"

# game/events_mc.rpy:395
translate french getEfromSchool1_fef852ff:

    # pc "Yeah..."
    pc "Ouais..."

# game/events_mc.rpy:397
translate french getEfromSchool1_1bcb5e23:

    # n "You take the two back home."
    n "Vous ramenez les deux chez vous."

# game/events_mc.rpy:406
translate french afterGetEFromSchool_0070cec2:

    # n "After your arrival you let them do their thing and you go to your room."
    n "Après votre arrivée, vous les laissez faire leur travail et vous allez dans votre chambre."

# game/events_mc.rpy:409
translate french afterGetEFromSchool_b4779b58:

    # pcthink "Damn these girls are crazy..."
    pcthink "Bon sang, ces filles sont folles..."

# game/events_mc.rpy:410
translate french afterGetEFromSchool_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:411
translate french afterGetEFromSchool_012903ad:

    # pcthink "Alright, I guess it's time to unpack the rest of my stuff."
    pcthink "Très bien, je suppose qu'il est temps de déballer le reste de mes affaires."

# game/events_mc.rpy:413
translate french afterGetEFromSchool_cd88fbb2:

    # j "Hey!"
    j "Hé !"

# game/events_mc.rpy:415
translate french afterGetEFromSchool_57148086:

    # pc "Uh, hey [j], what's up?"
    pc "Uh, hé [j], quoi de neuf ?"

# game/events_mc.rpy:416
translate french afterGetEFromSchool_7fa1bbfe:

    # j "Do you need help?"
    j "As-tu besoin d'aide ?"

# game/events_mc.rpy:417
translate french afterGetEFromSchool_a1b83240:

    # pc "Err... no, I..."
    pc "Err... non, je..."

# game/events_mc.rpy:419
translate french afterGetEFromSchool_17e9c83e:

    # j "Fuuuuck, you have a VR headset too?"
    j "Meeerrrrrdddee, tu as aussi un casque VR ?"

# game/events_mc.rpy:421
translate french afterGetEFromSchool_ca5a95bf:

    # pc "...yeah..."
    pc "...ouais..."

# game/events_mc.rpy:423
translate french afterGetEFromSchool_b0fa534a:

    # j "Awesome!"
    j "Génial !"

# game/events_mc.rpy:425
translate french afterGetEFromSchool_f9754191:

    # j "Oh hey [e], let's help [pc] \"unpack\", hehe."
    j "Oh hé [e], aidons [pc] à 'déballer', héhé."

# game/events_mc.rpy:426
translate french afterGetEFromSchool_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:427
translate french afterGetEFromSchool_5c58ed99:

    # n "[e] nods."
    n "[e] hoche la tête."

# game/events_mc.rpy:428
translate french afterGetEFromSchool_581b1a35:

    # pcthink "Damn [j]. Luckily [e] didn't get the joke..."
    pcthink "Merde [j]. Heureusement que [e] n'a pas compris la blague..."

# game/events_mc.rpy:430
translate french afterGetEFromSchool_d7a797d0:

    # j "Okay, let's start with the boxes over here."
    j "Ok, commençons par les boîtes ici."

# game/events_mc.rpy:431
translate french afterGetEFromSchool_a7c89ecd:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:432
translate french afterGetEFromSchool_1e2add12:

    # pcthink "Well I guess I have no say in this..."
    pcthink "Eh bien, je suppose que je n'ai pas mon mot à dire dans cette..."

# game/events_mc.rpy:434
translate french afterGetEFromSchool_a3af40ea:

    # n "A few minutes later." with dissolve
    n "Quelques minutes plus tard." with dissolve

# game/events_mc.rpy:440
translate french afterGetEFromSchool_7a3851e5:

    # pcthink "Damn, [j] is stretching her ass at me all the time..."
    pcthink "Merde, [j] tend son cul vers moi tout le temps..."

# game/events_mc.rpy:442
translate french afterGetEFromSchool_1bbe003b:

    # pcthink "...this can't be coincidence."
    pcthink "...ça ne peut pas être une coïncidence."

# game/events_mc.rpy:444
translate french afterGetEFromSchool_75846895:

    # pc "Huh?{w=1}{nw}"
    pc "Hein?{w=1}{nw}"

# game/events_mc.rpy:447
translate french afterGetEFromSchool_ecc419db:

    # j "Whaa! Naughty..."
    j "Whaa ! Vilain..."

# game/events_mc.rpy:449
translate french afterGetEFromSchool_0ab94a3c:

    # j "[e]?"
    j "[e]?"

# game/events_mc.rpy:451
translate french afterGetEFromSchool_f151517c:

    # pc "Uh..."
    pc "Euh..."

# game/events_mc.rpy:453
translate french afterGetEFromSchool_8956243a:

    # j "Oh come on [e], don't be so selfish. I'm not going to take him away from you!"
    j "Oh allez [e], ne sois pas si égoïste. Je ne vais pas te l'enlever !"

# game/events_mc.rpy:455
translate french afterGetEFromSchool_7a959c1a:

    # pcthink "Take me away from her?"
    pcthink "M'éloigner d'elle ?"

# game/events_mc.rpy:457
translate french afterGetEFromSchool_ae3b81a3:

    # j "...but he's still a man and men like to get a little peek from time to time, right [pc]?"
    j "...mais il reste un homme et les hommes aiment avoir un petit coup d'œil de temps en temps, n'est-ce pas [pc]?"

# game/events_mc.rpy:459
translate french afterGetEFromSchool_a7c89ecd_1:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:463
translate french afterGetEFromSchool_11ff6de2:

    # j "Oh wow, look at this [e]."
    j "Oh wow, regarde ça [e]."

# game/events_mc.rpy:464
translate french afterGetEFromSchool_6e3b6836:

    # e "Hehe."
    e "Héhé."

# game/events_mc.rpy:465
translate french afterGetEFromSchool_ca581621:

    # pcthink "What the..."
    pcthink "Qu'est-ce que..."

# game/events_mc.rpy:466
translate french afterGetEFromSchool_21f579ff:

    # pc "Hey what are you doing with my underwear?"
    pc "Hé, qu'est-ce que tu fais avec mes sous-vêtements ?"

# game/events_mc.rpy:468
translate french afterGetEFromSchool_05320ff2:

    # j "Oh come on, what's the matter? It's sexy. I bet you look hot in it."
    j "Oh aller, quel est le problème ? C'est sexy. Je parie que tu es sexy dedans."

# game/events_mc.rpy:470
translate french afterGetEFromSchool_62fbd682:

    # j "Where is the other piece? I wanna try it on!"
    j "Où est l'autre pièce ? Je veux l'essayer !"

# game/events_mc.rpy:471
translate french afterGetEFromSchool_76308ec7:

    # pcthink "Jeez, that girl..."
    pcthink "Bon sang, cette fille..."

# game/events_mc.rpy:472
translate french afterGetEFromSchool_f8c6ed9a:

    # pc "It won't fit anyway, it's obviously not your size."
    pc "Il ne t'ira pas de toute façon, ce n'est manifestement pas ta taille."

# game/events_mc.rpy:474
translate french afterGetEFromSchool_65cf133e:

    # j "Whoops, yeah. I almost forgot that I don't have such a purrfect and sexy body..."
    j "Oups, oui. J'ai presque oublié que je n'ai pas un corps aussi parfait et sexy..."

# game/events_mc.rpy:476
translate french afterGetEFromSchool_47cb4908:

    # j "Too bad, I guess {b}you{/b} have to show us how it looks then, hehe."
    j "Dommage, je suppose que {b}tu{/b} dois nous montrer ce que ça donne alors, héhé."

# game/events_mc.rpy:477
translate french afterGetEFromSchool_7a0cceb2:

    # pcthink "Damn girl, was this your plan?"
    pcthink "Maudite fille, c'était ça ton plan ?"

# game/events_mc.rpy:479
translate french afterGetEFromSchool_fec28338:

    # j "Come on [pc], show us your sexy curves! I'm sure [e] wants to see them too."
    j "Allez [pc], montre-nous tes courbes sexy ! Je suis sûr que [e] veut les voir aussi."

# game/events_mc.rpy:480
translate french afterGetEFromSchool_fa6c08fd:

    # pcthink "Yeah I'm sure about that, judging by what happened last night..."
    pcthink "Oui, j'en suis sûr, à en juger par ce qui s'est passé hier soir..."

# game/events_mc.rpy:482
translate french afterGetEFromSchool_4909967a:

    # j "Come on, come on, don't be shy."
    j "Aller, aller, ne sois pas timide."

# game/events_mc.rpy:484
translate french afterGetEFromSchool_252f4d06:

    # j "Here I'll show you my titties too!"
    j "Ici, je vais te montrer mes nichons aussi !"

# game/events_mc.rpy:486
translate french afterGetEFromSchool_9623bf2a:

    # e "HNNGH!"
    e "HNNGH !"

# game/events_mc.rpy:487
translate french afterGetEFromSchool_ca581621_1:

    # pcthink "What the..."
    pcthink "Qu'est-ce que..."

# game/events_mc.rpy:488
translate french afterGetEFromSchool_6c63a394:

    # j "Come on [e], you've seen them a lot too, don't act so prudishly..."
    j "Aller [e], tu en as vu beaucoup aussi, ne fais pas ta pudibonde..."

# game/events_mc.rpy:489
translate french afterGetEFromSchool_296dd5ba:

    # pcthink "A lot?"
    pcthink "Beaucoup ?"

# game/events_mc.rpy:491
translate french afterGetEFromSchool_825836ed:

    # j "Ooooh I get it, it's because mine are bigger than yours, right? Don't worry, I bet she will like yours too."
    j "Ooooh j'ai compris, c'est parce que les miens sont plus grands que les tiens, non ? Ne t'inquiète pas, je parie qu'elle aimera les tiens aussi."

# game/events_mc.rpy:492
translate french afterGetEFromSchool_6e73906c:

    # e "HNN!!!"
    e "HNN !!!"

# game/events_mc.rpy:493
translate french afterGetEFromSchool_9285360e:

    # j "Jeez... calm down [e]! It's not like I'm going to take her away from you..."
    j "Bon sang... calmes-toi [e]! Ce n'est pas comme si j'allais te l'enlever..."

# game/events_mc.rpy:498
translate french haremChoice_21fa08ac:

    # j "...and by the way, didn't we agree to always share everything?"
    j "...et d'ailleurs, n'avons-nous pas convenu de toujours tout partager ?"

# game/events_mc.rpy:499
translate french haremChoice_f6a70ae5:

    # pcthink "Shit... this girl is so fucking naughty..."
    pcthink "Merde... cette fille est tellement salope..."

# game/events_mc.rpy:501
translate french haremChoice_f42382f7:

    # j "I bet [heshe] wouldn't mind if we did share, hehe... right [pc]?"
    j "Je parie que [heshe] ne verrait pas d'inconvénient à ce que nous partagions, héhé... n'est-ce pas [pc]?"

# game/events_mc.rpy:502
translate french haremChoice_b9d784ee:

    # pc "Eh...?"
    pc "Eh... ?"

# game/events_mc.rpy:503
translate french haremChoice_4ce753c9:

    # pcthink "Did she just imply a threesome? WTF! How am I supposed to answer to that?"
    pcthink "Est-ce qu'elle vient d'insinuer un plan à trois ? WTF ! Comment suis-je censé répondre à ça ?"

# game/events_mc.rpy:504
translate french haremChoice_a11119d3:

    # pcthink "Damn, if this would be one these adult games, this would probably be the harem choice..."
    pcthink "Mince, si c'était un de ces jeux pour adultes, ce serait probablement le choix du harem..."

# game/events_mc.rpy:508
translate french haremChoice_7d764bd3:

    # pc "Err... well I guess I wouldn't mind..."
    pc "Err... bien je suppose que ça ne me dérangerait pas..."

# game/events_mc.rpy:510
translate french haremChoice_6fd70fd4:

    # j "See [e]?"
    j "Voir [e]?"

# game/events_mc.rpy:512
translate french haremChoice_fa0b61d7:

    # pc "Hell no! I'm not going to get shared with anyone!"
    pc "Bon sang non ! Je ne vais pas être partagé avec qui que ce soit !"

# game/events_mc.rpy:513
translate french haremChoice_8be8da66:

    # j "Oh... I expected a different answer..."
    j "Oh... Je m'attendais à une réponse différente..."

# game/events_mc.rpy:515
translate french haremChoice_472c2ff4:

    # n "Suddenly the door to your room opens."
    n "Soudain, la porte de votre chambre s'ouvre."

# game/events_mc.rpy:517
translate french haremChoice_1e452f8f:

    # m "Hey Hun, I'm sorry for the hassle. I really didn't expect to be so late."
    m "Hé Hun, je suis désolé pour le dérangement. Je ne m'attendais vraiment pas à être si en retard."

# game/events_mc.rpy:519
translate french haremChoice_5e75a89f:

    # pc "Yeah, don't worry about it."
    pc "Ouais, ne t'inquiète pas pour ça."

# game/events_mc.rpy:520
translate french haremChoice_2ac46b68:

    # j "Hey [m]!"
    j "Hé [m]!"

# game/events_mc.rpy:522
translate french haremChoice_5c81e00b:

    # m "Oh hey [j], I thought I heard your voice. I didn't expect you today."
    m "Oh hé [j], je pensais avoir entendu ta voix. Je ne t'attendais pas aujourd'hui."

# game/events_mc.rpy:524
translate french haremChoice_77725ba0:

    # j "Yeah, we have a... school project to work on..."
    j "Ouais, nous avons un... projet d'école sur lequel travailler..."

# game/events_mc.rpy:526
translate french haremChoice_295a6916:

    # m "I see, well then, it's better you start working on it, don't you think?"
    m "Je vois, bien alors, c'est mieux que vous commenciez à travailler dessus, vous ne pensez pas ?"

# game/events_mc.rpy:528
translate french haremChoice_29fcb00c:

    # j "Yeah, sure."
    j "Oui, bien sûr."

# game/events_mc.rpy:530
translate french haremChoice_789ca657:

    # j "Come on [e], let's go!"
    j "Aller [e], on y va !"

# game/events_mc.rpy:531
translate french haremChoice_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:533
translate french haremChoice_e477e04e:

    # j "Oh I love that shirt by the way [m]. It looks really hot on you!"
    j "Oh j'adore cette chemise d'ailleurs [m]. Elle est vraiment sexy sur toi !"

# game/events_mc.rpy:534
translate french haremChoice_2a77b7f1:

    # m "Why thank you!"
    m "Pourquoi merci !"

# game/events_mc.rpy:536
translate french haremChoice_2adb6a63:

    # j "I wonder if you're wearing it for someone special, hehe."
    j "Je me demande si tu la portes pour quelqu'un de spécial, héhé."

# game/events_mc.rpy:537
translate french haremChoice_cd851e02:

    # m "Wh... well don't you two have something to do?"
    m "Qu... bien tu n'as pas quelque chose à faire tous les deux ?"

# game/events_mc.rpy:538
translate french haremChoice_d4fc2ce6:

    # j "Of course... {size=-10}Run [e]!{/size}"
    j "Bien sûr... {size=-10}Fais-le [e]!{/size}"

# game/events_mc.rpy:539
translate french haremChoice_01b39e2c:

    # e "Khehehe..."
    e "Khéhéhéhé..."

# game/events_mc.rpy:547
translate french haremChoice_1659dccf:

    # pcthink "Damn, did [j] just stare at [m]s tits?!"
    pcthink "Merde, est-ce que [j] vient de regarder les seins de [m]?!"

# game/events_mc.rpy:550
translate french haremChoice_812f0b3d:

    # m "Humph..."
    m "Humph..."

# game/events_mc.rpy:551
translate french haremChoice_f477bc32:

    # pcthink "Why is she closing the door?"
    pcthink "Pourquoi ferme-t-elle la porte ?"

# game/events_mc.rpy:553
translate french haremChoice_12a1c413:

    # m "I'm sorry, I didn't know about [j]. I hope she didn't cause you any trouble?"
    m "Je suis désolé, je ne savais pas pour [j]. J'espère qu'elle ne t'a pas causé d'ennuis ?"

# game/events_mc.rpy:554
translate french haremChoice_9ec943e6:

    # pc "Not at all..."
    pc "Pas du tout..."

# game/events_mc.rpy:556
translate french haremChoice_d823b615:

    # m "Oh that's good, I know she can be a bit ...feisty."
    m "Oh c'est bien, je sais qu'elle peut être un peu ... fougueuse."

# game/events_mc.rpy:557
translate french haremChoice_a5975337:

    # pcthink "...what an understatement..."
    pcthink "...quel euphémisme..."

# game/events_mc.rpy:559
translate french haremChoice_6365ec0f:

    # m "Anyway, are you hungry? I'll make dinner if you want?"
    m "Bref, tu as faim ? Je peux préparer le dîner si tu veux ?"

# game/events_mc.rpy:560
translate french haremChoice_a9d9509c:

    # pc "No, I'm fine, thanks."
    pc "Non, ça va, merci."

# game/events_mc.rpy:562
translate french haremChoice_6f236419:

    # m "Okay... Oh by the way..."
    m "Ok... Oh a plus tard..."

# game/events_mc.rpy:566
translate french hallPCtalktoEllie_004808c6:

    # n "You go back to your room."
    n "Vous retournez dans votre chambre."

# game/events_mc.rpy:569
translate french hallPCtalktoEllie_5b15a7d6:

    # pcthink "Okay, I guess it's time for some VR porn, hehe..."
    pcthink "Ok, je suppose qu'il est temps pour un peu de porno VR, héhé..."

# game/events_mc.rpy:570
translate french hallPCtalktoEllie_fec86c60:

    # pcthink "Err, Shit. I wanted to talk to [e] too..."
    pcthink "Err, Merde. Je voulais parler à [e] aussi..."

# game/events_mc.rpy:571
translate french hallPCtalktoEllie_fd689d22:

    # pcthink "...but I'm actually quite a bit horny now..."
    pcthink "...mais je suis en fait un peu excité maintenant..."

# game/events_mc.rpy:575
translate french hallPCtalktoEllie_dea0400a:

    # pcthink "Yeah, it's better to clear my mind before she comes over again tonight."
    pcthink "Oui, c'est mieux de me vider l'esprit avant qu'elle ne vienne à nouveau ce soir."

# game/events_mc.rpy:577
translate french hallPCtalktoEllie_f78c2d8f:

    # pc "Hey Ladies, did you miss me?"
    pc "Hé Mesdames, je vous ai manqué ?"

# game/events_mc.rpy:581
translate french hallPCtalktoEllie_ab8c79c4:

    # Girls" "Heeey, elder unrelated roommate..."
    "Les filles... Hé, l'aîné des colocataires sans lien de parenté..."

# game/events_mc.rpy:583
translate french hallPCtalktoEllie_e564218e:

    # pcthink "Err what the..."
    pcthink "Err ce que le..."

# game/events_mc.rpy:589
translate french hallPCtalktoEllie_d6ec470a:

    # pcthink "Yeah, I really shouldn't wait any longer. I haven't talked to her alone once, since I've been back..."
    pcthink "Ouais, je ne devrais vraiment pas attendre plus longtemps. Je ne lui ai pas parlé seul une seule fois, depuis que je suis revenu..."

# game/events_mc.rpy:591
translate french hallPCtalktoEllie_7c37fbb0:

    # n "You go to her room."
    n "Vous allez dans sa chambre."

# game/events_mc.rpy:595
translate french pcroomnight3_421360dd:

    # n "Nothing special happens for the rest of the day."
    n "Rien de spécial ne se passe pour le reste de la journée."

# game/events_mc.rpy:597
translate french pcroomnight3_987cdc4b:

    # n "Later that Night." with dissolve
    n "Plus tard dans la nuit." with dissolve

# game/events_mc.rpy:601
translate french pcroomnight3_b97d710a:

    # pc "Yaawn..."
    pc "Yaawn..."

# game/events_mc.rpy:602
translate french pcroomnight3_7818a9aa:

    # pcthink "It's late... I guess [e] should be coming over soon..."
    pcthink "Il est tard... Je suppose que [e] devrait bientôt arriver..."

# game/events_mc.rpy:603
translate french pcroomnight3_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:604
translate french pcroomnight3_b9b155fe:

    # extend "."
    extend "."

# game/events_mc.rpy:605
translate french pcroomnight3_b9b155fe_1:

    # extend "."
    extend "."

# game/events_mc.rpy:607
translate french pcroomnight3_1a88aff7:

    # m "{size=-10}[e]? What are you doing up so late? Trying to sneak into your [bs]'s bed again?{/size}"
    m "{taille=-10}[e]? Que fais-tu debout si tard ? Tu essaies encore de te faufiler dans le lit de ton [bs]? {/size}"

# game/events_mc.rpy:608
translate french pcroomnight3_962b8eaa:

    # pcthink "Huh? [m] is still awake?"
    pcthink "Huh ? [m] est toujours réveillé ?"

# game/events_mc.rpy:609
translate french pcroomnight3_a643ba94:

    # m "{size=-10}You should really let [himher] sleep [e]...{/size}"
    m "{size=-10}Tu devrais vraiment laisser [himher] dormir [e]...{/size}"

# game/events_mc.rpy:610
translate french pcroomnight3_2f115d4d:

    # e "{size=-10}...{/size}"
    e "{size=-10}...{/size}"

# game/events_mc.rpy:611
translate french pcroomnight3_142bf015:

    # m "{size=-10}Listen [e], I know you're really happy that [pc] is back, but [heshe] just moved in and we both don't know if [heshe] intends to stay...{/size}"
    m "{size=-10}Écoute [e], je sais que tu es vraiment heureuse que [pc] soit de retour, mais [heshe] vient d'emménager et nous ne savons pas si [heshe] a l'intention de rester...{/size}"

# game/events_mc.rpy:612
translate french pcroomnight3_0e370ad3:

    # m "{size=-10}...also, [heshe]'s been away for a long time, we basically don't know anything about [himher]. Plus, do you even know if [heshe] is okay with you sleeping in [hisher] bed?{/size}"
    m "{size=-10}...aussi, [heshe] est parti depuis longtemps, on ne sait rien de [himher]. En plus, est-ce que tu sais si [heshe] est d'accord pour que tu dormes dans le lit de [hisher] ? {/size}"

# game/events_mc.rpy:613
translate french pcroomnight3_7e05c46a:

    # pcthink "Damn, are you serious [m]?"
    pcthink "Merde, tu es sérieuse [m]?"

# game/events_mc.rpy:614
translate french pcroomnight3_9689d40d:

    # m "{size=-10}Come on [e], go back to your room. If [heshe]'s going to stay, you'll have enough opportunities to cuddle with [himher] in the future...{/size}"
    m "{size=-10}Aller [e], retourne dans ta chambre. Si [heshe] va rester, tu auras assez d'occasions de faire des câlins à [himher] dans le futur...{/size}"

# game/events_mc.rpy:615
translate french pcroomnight3_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:616
translate french pcroomnight3_a3d517f0:

    # pcthink "Well... I guess I'm going to sleep alone tonight..."
    pcthink "Eh bien... Je suppose que je vais dormir seul ce soir..."

# game/events_mc.rpy:621
translate french pcroomnight3_b297f259:

    # m "[bpc], WHAT THE HELL?!"
    m "[bpc], QU'EST-CE QUE C'EST CE BORDEL?!"

# game/events_mc.rpy:622
translate french pcroomnight3_25e7f477:

    # m "Don't tell me you got yourself into a fight AGAIN?!"
    m "Ne me dis pas que tu t'es ENCORE battu ?!"

# game/events_mc.rpy:623
translate french pcroomnight3_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:624
translate french pcroomnight3_a2d15ea3:

    # m "Just what the hell is wrong with you? Why must you constantly embarrass me?"
    m "Mais qu'est-ce qui ne va pas chez toi ? Pourquoi dois-tu constamment m'embarrasser ?"

# game/events_mc.rpy:625
translate french pcroomnight3_eca096dd:

    # m "Can't you be a normal [boygirl] once in your life?"
    m "Ne peux-tu pas être un [boygirl] normal une fois dans ta vie ?"

# game/events_mc.rpy:626
translate french pcroomnight3_a69377ef_2:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:627
translate french pcroomnight3_6095f49c:

    # m "Oh I guess that's a bit too much to ask with a Dad like yours..."
    m "Oh je suppose que c'est un peu trop demander avec un père comme le tien...."

# game/events_mc.rpy:628
translate french pcroomnight3_92df5d5f:

    # m "I swear, if I get a call from your school or the police again, you'll regret ever being born!"
    m "Je te jure, si je reçois encore un appel de ton école ou de la police, tu regretteras d'être né !"

# game/events_mc.rpy:629
translate french pcroomnight3_a69377ef_3:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:630
translate french pcroomnight3_151099dc:

    # m "DON'T JUST STAND THERE LIKE THE FOOL YOU ARE, YOU'RE DRIPPING BLOOD ALL OVER THE FLOOR, GO TO THE BATHROOM AND WASH YOUR FACE!"
    m "NE RESTE PAS LÀ COMME L'IDIOT QUE TU ES, TU VERSES DU SANG SUR TOUT LE SOL, Va dans la salle de bain et lave-toi le visage !"

# game/events_mc.rpy:631
translate french pcroomnight3_a69377ef_4:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:632
translate french pcroomnight3_28aa1485:

    # m "...and don't forget to wipe the blood off the floor when you're done..."
    m "...et n'oublie pas d'essuyer le sang sur le sol quand tu auras fini..."

# game/events_mc.rpy:633
translate french pcroomnight3_a69377ef_5:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:642
translate french wakeupDay3_fe60ed33:

    # pcthink "Uhh... not again..."
    pcthink "Uhh... pas encore..."

# game/events_mc.rpy:644
translate french wakeupDay3_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:645
translate french wakeupDay3_1eec5249:

    # pcthink "...she didn't even care I was hurt..."
    pcthink "...elle ne se souciait même pas que je sois blessé..."

# game/events_mc.rpy:646
translate french wakeupDay3_0754dd99:

    # pcthink "Fuck you [m], I'm not going to forgive you so easily!"
    pcthink "Va te faire foutre [m], je ne vais pas te pardonner si facilement !"

# game/events_mc.rpy:647
translate french wakeupDay3_f0cac90d_1:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:648
translate french wakeupDay3_27d07940:

    # pcthink "At least [e] was worried about me..."
    pcthink "Au moins [e] s'inquiétait pour moi..."

# game/events_mc.rpy:649
translate french wakeupDay3_c7edfe55:

    # pcthink "I can't even count how many times she stitched me together after I got into a fight."
    pcthink "Je ne peux même pas compter combien de fois elle m'a recousu après que je me sois battu."

# game/events_mc.rpy:650
translate french wakeupDay3_5fb8e78b:

    # pcthink "She was still so young, but she tried to cheer me up so many times..."
    pcthink "Elle était encore si jeune, mais elle a essayé de me remonter le moral tellement de fois..."

# game/events_mc.rpy:651
translate french wakeupDay3_a2d1e6e6:

    # pcthink "I think she knew that I was just angry..."
    pcthink "Je pense qu'elle savait que j'étais juste en colère..."

# game/events_mc.rpy:652
translate french wakeupDay3_f730a80c:

    # pcthink "Angry at [m], angry at the old man, angry at everything and everyone..."
    pcthink "En colère contre [m], en colère contre le vieux, en colère contre tout et tout le monde..."

# game/events_mc.rpy:653
translate french wakeupDay3_f0cac90d_2:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:654
translate french wakeupDay3_d1861303:

    # pcthink "Well I better find the perfect present for her birthday. I have a lot to make up for and only a few hours time."
    pcthink "Je ferais mieux de trouver le cadeau parfait pour son anniversaire. J'ai beaucoup à rattraper et seulement quelques heures."

# game/events_mc.rpy:656
translate french wakeupDay3_cfa20fe0:

    # n "You take a quick shower and make yourself ready to go into the city."
    n "Vous prenez une douche rapide et vous vous préparez à aller en ville."

# game/events_mc.rpy:659
translate french wakeupDay3_e9fe1850:

    # pcthink "Alright, time to go."
    pcthink "Très bien, il est temps de partir."

# game/events_mc.rpy:667
translate french pcCityDay3_28848481:

    # pcthink "Okay, where should I start..."
    pcthink "Ok, où dois-je commencer..."

# game/events_mc.rpy:668
translate french pcCityDay3_3199d732:

    # pcthink "Uhh... shit, I don't even know what [e] likes... I should have asked [m] or [j]..."
    pcthink "Uhh... merde, je ne sais même pas ce que [e] aime... J'aurais dû demander à [m] ou [j]..."

# game/events_mc.rpy:669
translate french pcCityDay3_c1686997:

    # pcthink "Hum..."
    pcthink "Hum..."

# game/events_mc.rpy:670
translate french pcCityDay3_ed44c29b:

    # pcthink "Wasn't there a cafe somewhere here? I can't think straight without a good coffee in the morning..."
    pcthink "N'y avait-il pas un café quelque part ici ? Je ne peux pas penser correctement sans un bon café le matin..."

# game/events_mc.rpy:672
translate french pcCityDay3_37827316:

    # n "You look around for the cafe until you find it."
    n "Vous cherchez le café jusqu'à ce que vous le trouviez."

# game/events_mc.rpy:674
translate french pcCityDay3_d9794f2e:

    # Dude" "Yeah?"
    "Mec" "Ouais ?"

# game/events_mc.rpy:675
translate french pcCityDay3_9ba53076:

    # pcthink "Wasn't there a girl working here last time?"
    pcthink "Il n'y avait pas une fille qui travaillait ici la dernière fois ?"

# game/events_mc.rpy:676
translate french pcCityDay3_3ec4a123:

    # pc "The biggest cup of the strongest coffee you have."
    pc "La plus grande tasse du café le plus fort que vous ayez."

# game/events_mc.rpy:677
translate french pcCityDay3_30b2640a:

    # Dude" "Sure..."
    "Mec" "Bien sûr..."

# game/events_mc.rpy:678
translate french pcCityDay3_ec5691e0:

    # pcthink "This guy looks kind of funny in the uniform..."
    pcthink "Ce type a l'air drôle dans cette uniforme..."

# game/events_mc.rpy:680
translate french pcCityDay3_37d2b588:

    # Dude" "Here you go..."
    "Mec" "voilà..."

# game/events_mc.rpy:681
translate french pcCityDay3_1376d297:

    # pc "Thanks, keep the change."
    pc "Merci, gardez la monaie."

# game/events_mc.rpy:682
translate french pcCityDay3_404de4ef:

    # Dude" "Thanks."
    "Mec" "Merci."

# game/events_mc.rpy:684
translate french pcCityDay3_2d94c374:

    # Girl" "{size=-10}Yeah, bye!{/size}"
    "Fille" "{size=-10}Oui, au revoir !{/size}"

# game/events_mc.rpy:685
translate french pcCityDay3_df2388d3:

    # pcthink "That's the barista from last time."
    pcthink "C'est la barista de la dernière fois."

# game/events_mc.rpy:687
translate french pcCityDay3_f6920ce9:

    # Girl" "Oh, you."
    "Fille" "Oh, toi."

# game/events_mc.rpy:688
translate french pcCityDay3_9f72f07c:

    # pc "Me?"
    pc "Moi ?"

# game/events_mc.rpy:689
translate french pcCityDay3_deb02910:

    # pcthink "Looks like she's remembering me as well."
    pcthink "On dirait qu'elle se souvient aussi de moi."

# game/events_mc.rpy:691
translate french pcCityDay3_d16d473d:

    # Girl" "Ah sorry, haha. You looked really tired last time, that's why I remember you, haha."
    "Fille" "Ah, désolé, haha. Tu avais l'air vraiment fatigué la dernière fois, c'est pour ça que je me souviens de toi, haha."

# game/events_mc.rpy:692
translate french pcCityDay3_528a0c0a:

    # pc "Yeah that sounds like me... I hope I didn't look too scary?"
    pc "Ouais, ça me ressemble... J'espère que je n'ai pas eu l'air trop effrayant ?"

# game/events_mc.rpy:693
translate french pcCityDay3_d7aed506:

    # Girl" "Haha no, not at all. Err... how is your friend?"
    "Fille" "Haha non, pas du tout. Err... comment va votre amie ?"

# game/events_mc.rpy:694
translate french pcCityDay3_0ea4f12e:

    # pc "Friend?"
    pc "Amie ?"

# game/events_mc.rpy:695
translate french pcCityDay3_14a251d1:

    # Girl" "The older woman..."
    "Fille" "La femme plus âgée..."

# game/events_mc.rpy:696
translate french pcCityDay3_26a6d4b5:

    # Girl" "Oh that sounds mean. I didn't mean that she looks old or something, haha. She's a regular."
    "Fille" "Oh, ça semble méchant. Je ne voulais pas dire qu'elle a l'air vieille ou autre, haha. C'est une habituée."

# game/events_mc.rpy:697
translate french pcCityDay3_2643ac94:

    # Girl" "...but she looked really sad when she left the other day. I hope she's alright?"
    "Fille" "...mais elle avait l'air vraiment triste quand elle est partie l'autre jour. J'espère qu'elle va bien ?"

# game/events_mc.rpy:698
translate french pcCityDay3_b3701ceb:

    # pc "I guess you mean [m]... yeah she's fine."
    pc "Je suppose que vous voulez dire [m]... oui elle va bien."

# game/events_mc.rpy:699
translate french pcCityDay3_1bd56328:

    # Girl" "Did you break up with her?"
    "Fille" "Tu as rompu avec elle ?"

# game/events_mc.rpy:700
translate french pcCityDay3_c251847b:

    # pc "Break up? Hell no! She's not my girlfriend."
    pc "Rompu? Du tout! Ce n'est pas ma petite amie."

# game/events_mc.rpy:701
translate french pcCityDay3_bf799a98:

    # Dude" "Hey [h], stop chattering, I want to go home!"
    "Mec" "Hé [h], arrête de bavarder, je veux rentrer à la maison !"

# game/events_mc.rpy:703
translate french pcCityDay3_897e803a:

    # Girl" "Ugh... Looks like I have to start my shift."
    "Fille" "Ugh... On dirait que je dois commencer mon service."

# game/events_mc.rpy:704
translate french pcCityDay3_0bea89ce:

    # pc "Yeah, too bad."
    pc "Ouais, dommage."

# game/events_mc.rpy:706
translate french pcCityDay3_6be367f3:

    # Girl" "I'm [h] by the way."
    "Fille" "Je suis [h] au fait."

# game/events_mc.rpy:710
translate french pcCityDay3_1ca71d76:

    # pc "[pc], nice to meet you."
    pc "[pc], heureux de vous rencontrer."

# game/events_mc.rpy:711
translate french pcCityDay3_bdcb3934:

    # h "Nice to meet you too. I hope you visit us more often from now on, we have the best coffee in town and the cutest barista too, haha."
    h "Ravi de vous rencontrer aussi. J'espère que vous nous rendrez visite plus souvent à partir de maintenant, nous avons le meilleur café de la ville et la barista la plus mignonne aussi, haha."

# game/events_mc.rpy:712
translate french pcCityDay3_6f8d404b:

    # pc "That's two really good reasons to come back. Especially the last one."
    pc "Ce sont deux très bonnes raisons de revenir. Surtout la dernière."

# game/events_mc.rpy:713
translate french pcCityDay3_f0e25423:

    # h "Hehe, thanks."
    h "Héhé, merci."

# game/events_mc.rpy:714
translate french pcCityDay3_607614bd:

    # Dude" "[h]!"
    "Mec" "[h]!"

# game/events_mc.rpy:716
translate french pcCityDay3_1799137c:

    # h "Yes, stop crying baby boy!"
    h "Oui, arrête de pleurer mon petit garçon !"

# game/events_mc.rpy:718
translate french pcCityDay3_742f2057:

    # h "Well, I have to go. See you soon~"
    h "Bien, je dois y aller. A bientôt."

# game/events_mc.rpy:719
translate french pcCityDay3_188cbca6:

    # pc "Yeah, seeya."
    pc "Yeah, à plus."

# game/events_mc.rpy:721
translate french pcCityDay3_1cc9788e:

    # pcthink "Wow, she seemed kind of nervous at first... because of me?"
    pcthink "Wow, elle semblait un peu nerveuse au début... à cause de moi ?"

# game/events_mc.rpy:722
translate french pcCityDay3_f423bdd0:

    # pcthink "Cute..."
    pcthink "Mignonne..."

# game/events_mc.rpy:725
translate french pcCityDay3_b3f60102:

    # pc "Yeah, well I have to go now. Bye..."
    pc "Ouais, eh bien je dois y aller maintenant. Bye..."

# game/events_mc.rpy:727
translate french pcCityDay3_47a11eb7:

    # n "You leave the cafe and walk down the street, starting to think about what kind of present [e] would really like to have."
    n "Vous quittez le café et descendez la rue, en commençant à réfléchir au type de cadeau pour [e] qu'elle aimerait vraiment avoir."

# game/events_mc.rpy:728
translate french pcCityDay3_7b2c5e7c:

    # n "You wander from window to window, enter some shops, look at jewelry, plushies, clothes and several other things."
    n "Vous vous promenez de fenêtre en fenêtre, entrez dans quelques boutiques, regardez les bijoux, les peluches, les vêtements et plusieurs autres choses."

# game/events_mc.rpy:729
translate french pcCityDay3_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:730
translate french pcCityDay3_878f0222:

    # pcthink "Hum... looked better from far away."
    pcthink "Hum... c'était mieux vu de loin."

# game/events_mc.rpy:731
translate french pcCityDay3_f0cac90d_1:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:732
translate french pcCityDay3_36bb9d3d:

    # pcthink "No... this won't do as well..."
    pcthink "Non... cela ne fera pas aussi bien..."

# game/events_mc.rpy:733
translate french pcCityDay3_f0cac90d_2:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:734
translate french pcCityDay3_038399c8:

    # pcthink "No..."
    pcthink "Non..."

# game/events_mc.rpy:735
translate french pcCityDay3_ac7ad650:

    # pcthink "...no..."
    pcthink "...non..."

# game/events_mc.rpy:736
translate french pcCityDay3_9677f25c:

    # pcthink "Nope, this looks cheap..."
    pcthink "Non, cela semble bon marché..."

# game/events_mc.rpy:737
translate french pcCityDay3_f0cac90d_3:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:738
translate french pcCityDay3_a0c75c22:

    # pcthink "Ugh... who buys this shit?"
    pcthink "Ugh... qui achète cette merde ?"

# game/events_mc.rpy:739
translate french pcCityDay3_f0cac90d_4:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:740
translate french pcCityDay3_7fc028a8:

    # n "Several shops and after what feels like hours you start to get exhausted."
    n "Plusieurs magasins et après ce qui semble être des heures, vous commencez à être épuisé."

# game/events_mc.rpy:741
translate french pcCityDay3_b15dee0c:

    # pcthink "Damn! Why is it so hard to find a good present..."
    pcthink "Merde ! Pourquoi c'est si difficile de trouver un bon cadeau..."

# game/events_mc.rpy:742
translate french pcCityDay3_78f01fe2:

    # n "You decide to enter one last shop and take a break after that. You enter the shop without looking at what they actually sell."
    n "Vous décidez d'entrer dans une dernière boutique et de faire une pause après cela. Vous entrez dans la boutique sans regarder ce qu'ils vendent réellement."

# game/events_mc.rpy:744
translate french pcCityDay3_011ebdf7:

    # pcthink "A phone shop?"
    pcthink "Un magasin de téléphone ?"

# game/events_mc.rpy:745
translate french pcCityDay3_d71efe78:

    # pcthink "Well I won't find anything here..."
    pcthink "Eh bien, je ne trouverai rien ici..."

# game/events_mc.rpy:746
translate french pcCityDay3_39735d16:

    # pcthink "Or...?"
    pcthink "Ou... ?"

# game/events_mc.rpy:747
translate french pcCityDay3_da46b3f1:

    # pcthink "Hum... [m] said [e] wouldn't know how to use it, but I highly doubt that."
    pcthink "Hum... [m] a dit que [e] ne saurait pas s'en servir, mais j'en doute fortement."

# game/events_mc.rpy:748
translate french pcCityDay3_6b328552:

    # pcthink "I don't know any girl her age who doesn't want to have a mobile phone."
    pcthink "Je ne connais aucune fille de son âge qui ne veuille pas avoir un téléphone portable."

# game/events_mc.rpy:749
translate french pcCityDay3_05e16121:

    # pcthink "Plus I don't think she has the money to buy a good one herself..."
    pcthink "De plus, je ne pense pas qu'elle ait l'argent pour en acheter un bon elle-même..."

# game/events_mc.rpy:750
translate french pcCityDay3_20fbc89e:

    # pcthink "Yeah, I bet she would be happy to have one, even if she's only using it to send messages and listen to music."
    pcthink "Oui, je parie qu'elle serait heureuse d'en avoir un, même si elle ne l'utilise que pour envoyer des messages et écouter de la musique."

# game/events_mc.rpy:751
translate french pcCityDay3_53244e6a:

    # pcthink "But I can't buy just {b}some{/b} phone, it has to be a good one. Maybe a Lie-Phone, or the new Moogle phone?"
    pcthink "Mais je ne peux pas acheter juste {b}un{/b} téléphone quelconque, il faut que ce soit un bon. Peut-être un Lie-Phone, ou le nouveau téléphone Moogle ?"

# game/events_mc.rpy:752
translate french pcCityDay3_92f58eb1:

    # pcthink "...and some cute accessories too. Girls her age love this kind of stuff... I think..."
    pcthink "...et quelques accessoires mignons aussi. Les filles de son âge aiment ce genre de choses... Je pense..."

# game/events_mc.rpy:754
translate french pcCityDay3_6c247d4a:

    # n "You buy a stupidly expensive mobile phone and some cute accessories.{p}Then you make your way back home."
    n "Vous achetez un téléphone portable stupidement cher et quelques accessoires mignons.{p}Puis vous rentrez chez vous."

# game/events_mc.rpy:757
translate french pcCityDay3_f27bff4b:

    # pc "I'm back..."
    pc "Je suis de retour..."

# game/events_mc.rpy:758
translate french pcCityDay3_2819ad57:

    # pcthink "...looks like [m] and [e] aren't back yet."
    pcthink "...on dirait que [m] et [e] ne sont pas encore revenues."

# game/events_mc.rpy:760
translate french pcCityDay3_97439a5b:

    # n "You hide the mobile in your room and after a while waiting for [m] and [e] to return you decide to spend some time watching TV."
    n "Vous cachez le portable dans votre chambre et après avoir attendu un moment que [m] et [e] reviennent, vous décidez de passer un peu de temps devant la télévision."

# game/events_mc.rpy:762
translate french pcCityDay3_2d31c06d:

    # Reporter" "...building that burned down a few days ago."
    "Journaliste" "...immeuble qui a brûlé il y a quelques jours."

# game/events_mc.rpy:763
translate french pcCityDay3_a2f0a046:

    # Reporter" "The man responsible for the fire stated that he wanted to protect his wife from becoming corrupted by another man."
    "Journaliste" "L'homme responsable de l'incendie a déclaré qu'il voulait empêcher sa femme d'être corrompue par un autre homme."

# game/events_mc.rpy:764
translate french pcCityDay3_58185e0f:

    # Reporter" "Police told us that the man believes that some unspecified person is using a magical serum to bend his wife's and other women's will."
    "Journaliste" "La police nous a dit que l'homme croit qu'une personne non spécifiée utilise un sérum magique pour faire plier la volonté de sa femme et des autres femmes."

# game/events_mc.rpy:766
translate french pcCityDay3_3fc5d9e7:

    # Reporter" "He was sent to a mental hospital..."
    "Journaliste" "Il a été envoyé dans un hôpital psychiatrique..."

# game/events_mc.rpy:767
translate french pcCityDay3_5300c54f:

    # pcthink "Jeez, this guy must have played too many porn games..."
    pcthink "Bon sang, ce type a dû jouer à trop de jeux pornos..."

# game/events_mc.rpy:768
translate french pcCityDay3_4e2d0012:

    # pcthink "Anyway I... huh?"
    pcthink "Bref, je... hein ?"

# game/events_mc.rpy:769
translate french pcCityDay3_8893cd1a:

    # n "*Mobile phone rings*"
    n "*Sonnerie de téléphone portable*"

# game/events_mc.rpy:771
translate french pcCityDay3_6509f6ba:

    # pcthink "[m]... what does she want now?"
    pcthink "[m]... que veut-elle maintenant ?"

# game/events_mc.rpy:773
translate french pcCityDay3_3b5608b8:

    # pc "Yes..."
    pc "Oui..."

# game/events_mc.rpy:774
translate french pcCityDay3_3886e06f:

    # m "Hey [pc], it's [m]."
    m "Hé [pc], c'est [m]."

# game/events_mc.rpy:775
translate french pcCityDay3_95d7fa4d:

    # pc "Yeah, I know."
    pc "Ouais, je sais."

# game/events_mc.rpy:776
translate french pcCityDay3_2917b6c2:

    # m "I'm sorry, we are running a bit late, if [t] shows up, could you tell her that we will be home soon?"
    m "Je suis désolé, nous sommes un peu en retard, si [t] se présente, pourrais-tu lui dire que nous serons bientôt à la maison ?"

# game/events_mc.rpy:777
translate french pcCityDay3_36c3c199:

    # pc "The therapist? Did you seriously invite her to [e]'s birthday?"
    pc "La thérapeute ? Tu l'as sérieusement invitée à l'anniversaire de [e] ?"

# game/events_mc.rpy:778
translate french pcCityDay3_91a23443:

    # m "No it's..."
    m "Non c'est..."

# game/events_mc.rpy:779
translate french pcCityDay3_37d7e05b:

    # m "It's... a session."
    m "C'est... une session."

# game/events_mc.rpy:780
translate french pcCityDay3_f9ff8ea8:

    # pc "What? That's even worse!"
    pc "Quoi ? C'est encore pire !"

# game/events_mc.rpy:781
translate french pcCityDay3_de7c93ca:

    # m "[pc], please. I know you don't like her, but these sessions are important for [e] and [t] doesn't have much time, so we have to take what we can get."
    m "[pc], s'il te plaît. Je sais que tu ne l'aimes pas, mais ces sessions sont importantes pour [e] et [t] n'a pas beaucoup de temps, donc nous devons prendre ce que nous pouvons obtenir."

# game/events_mc.rpy:782
translate french pcCityDay3_f0293ec3:

    # pc "Geez, [m] this is awful..."
    pc "Bon sang, [m] c'est horrible..."

# game/events_mc.rpy:783
translate french pcCityDay3_2e167f81:

    # m "I know, I'm sorry..."
    m "Je sais, je suis désolé..."

# game/events_mc.rpy:784
translate french pcCityDay3_3e339463:

    # pc "You should be sorry for [e], it's her birthday!"
    pc "Tu devrais être désolé pour [e], c'est son anniversaire !"

# game/events_mc.rpy:785
translate french pcCityDay3_51304096:

    # m "Will you tell her [pc]? Please?"
    m "Veux-tu lui dire [pc]? S'il te plaît ?"

# game/events_mc.rpy:786
translate french pcCityDay3_f25f4dc9:

    # pc "Humph... yeah fine..."
    pc "Humph... ouais bien..."

# game/events_mc.rpy:787
translate french pcCityDay3_c26c5d17:

    # m "Thank you, Hun. We'll be home soon."
    m "Merci, Hun. Nous serons bientôt à la maison."

# game/events_mc.rpy:789
translate french pcCityDay3_54b7ec6f:

    # pcthink "I can't believe it, she's ruining [e]'s birthday..."
    pcthink "Je ne peux pas le croire, elle gâche l'anniversaire de [e]..."

# game/events_mc.rpy:792
translate french pcCityDay3_a7b99095:

    # n "*Doorbell rings*"
    n "*Sonneries de la porte*"

# game/events_mc.rpy:793
translate french pcCityDay3_3f1118c4:

    # pcthink "Ugh... speak of the devil..."
    pcthink "Ugh... en parlant du diable..."

# game/events_mc.rpy:795
translate french pcCityDay3_35ff8356:

    # n "You get up and open the door."
    n "Vous vous levez et ouvrez la porte."

# game/events_mc.rpy:799
translate french ebdPreps_80b6e591:

    # m "This, and that... oh could you take the plate, Hun?"
    m "Ceci, et cela... oh tu peux prendre le plateau, Hun ?"

# game/events_mc.rpy:800
translate french ebdPreps_79572587:

    # pc "Sure..."
    pc "Bien sûr..."

# game/events_mc.rpy:802
translate french ebdPreps_845ed577:

    # m "Alright, that's it, everything is ready. Now we only need to wait for [e]."
    m "Très bien, ça y est, tout est prêt. Maintenant, il ne nous reste plus qu'à attendre [e]."

# game/events_mc.rpy:803
translate french ebdPreps_d4eacc2f:

    # pc "Is no one else coming?"
    pc "Personne d'autre ne vient ?"

# game/events_mc.rpy:805
translate french ebdPreps_d3107ef6:

    # m "Well..."
    m "Eh bien..."

# game/events_mc.rpy:807
translate french ebdPreps_e8f66559:

    # m "[e] doesn't have many friends because of... how she is."
    m "[e] n'a pas beaucoup d'amis à cause de... comment elle est."

# game/events_mc.rpy:808
translate french ebdPreps_5848f0a7:

    # m "...and aunt Jenna died a few years back..."
    m "...et tante Jenna est morte il y a quelques années..."

# game/events_mc.rpy:809
translate french ebdPreps_8d4e0c02:

    # pc "Jenna is dead?"
    pc "Jenna est morte ?"

# game/events_mc.rpy:811
translate french ebdPreps_53fdfe12:

    # m "Yes... that's one of the things I wrote you in the letter I sent you..."
    m "Oui... c'est une des choses que je t'ai écrites dans la lettre que je t'ai envoyée..."

# game/events_mc.rpy:812
translate french ebdPreps_322c09c2:

    # m "I'm sorry, Hun."
    m "Je suis désolée, Hun."

# game/events_mc.rpy:813
translate french ebdPreps_a222d23b:

    # pc "Uhh... yeah, forget it..."
    pc "Uhh... ouais, oublie ça..."

# game/events_mc.rpy:814
translate french ebdPreps_d15f5801:

    # pcthink "Damn, I liked aunt Jenna..."
    pcthink "Mince, j'aimais bien tante Jenna..."

# game/events_mc.rpy:816
translate french ebdPreps_57548985:

    # m "Anyway, why don't you sit down and I'll go get the punchbowl?"
    m "Bref, pourquoi ne pas t'asseoir et je vais chercher le bol de punch ?"

# game/events_mc.rpy:818
translate french ebdPreps_f38ba041:

    # j "I've got it already."
    j "Je l'ai déjà."

# game/events_mc.rpy:820
translate french ebdPreps_debf6129:

    # m "Oh, thanks [j]."
    m "Oh, merci [j]."

# game/events_mc.rpy:821
translate french ebdPreps_7addc437:

    # j "Anytime~"
    j "A ton service~"

# game/events_mc.rpy:823
translate french ebdPreps_4deecf0d:

    # pcthink "Geez..."
    pcthink "Bon sang..."

# game/events_mc.rpy:825
translate french ebdPreps_fdd4bb46:

    # m "Oh [t], did you finish for today?"
    m "Oh [t], as-tu terminé pour aujourd'hui ?"

# game/events_mc.rpy:827
translate french ebdPreps_7b962f4b:

    # t "Yes, obviously."
    t "Oui, évidemment."

# game/events_mc.rpy:828
translate french ebdPreps_9eed8506:

    # m "Nice, why don't you come and sit down with us for a while?"
    m "Sympa, pourquoi ne viendrais-tu pas t'asseoir avec nous un moment ?"

# game/events_mc.rpy:830
translate french ebdPreps_679e61d6:

    # pcthink "No.fucking.way!"
    pcthink "Pas question, putain !"

# game/events_mc.rpy:831
translate french ebdPreps_c441437e:

    # pcthink "Looks like [j] thinks the same."
    pcthink "On dirait que [j] pense la même chose."

# game/events_mc.rpy:832
translate french ebdPreps_3a7f8227:

    # m "You should try my punch."
    m "Tu devrais essayer mon punch."

# game/events_mc.rpy:834
translate french ebdPreps_3e298e1a:

    # t "Sorry, I don't have time and I still have to drive."
    t "Désolé, je n'ai pas le temps et je dois encore conduire."

# game/events_mc.rpy:835
translate french ebdPreps_66ad1b6c:

    # m "Oh, don't worry, there is no alcohol in it."
    m "Oh, ne t'inquiètes pas, il n'y a pas d'alcool dedans."

# game/events_mc.rpy:836
translate french ebdPreps_b5fc3376:

    # t "No I..."
    t "Non je..."

# game/events_mc.rpy:838
translate french ebdPreps_09a94aa2:

    # j "Oh you HAVE to try it, I'm sure you've never had punch as good as [m]'s! Hehe."
    j "Oh vous DEVEZ essayer, je suis sûr que vous n'avez jamais eu de punch aussi bon que celui de [m] ! Héhé."

# game/events_mc.rpy:839
translate french ebdPreps_d3b0e5fb:

    # pcthink "What the fuck [j]?"
    pcthink "C'est quoi ce bordel [j]?"

# game/events_mc.rpy:841
translate french ebdPreps_c9e9538c:

    # m "Why thank you [j]. See [t]? Come on, I'm sure your work can wait for a few minutes."
    m "Pourquoi merci [j]. Vous voyez [t]? Aller, je suis sûr que votre travail peut attendre quelques minutes."

# game/events_mc.rpy:843
translate french ebdPreps_1bb01731:

    # t "..."
    new "[t]..."

# game/events_mc.rpy:844
translate french ebdPreps_5d1b27ce:

    # t "Humph... fine, but just one glass."
    t "Humph... bien, mais juste un verre."

# game/events_mc.rpy:851
translate french ebd_81b263e7:

    # m "Oh [pc], do you know what got into [t]?"
    m "Oh [pc], sais-tu ce qui est arrivé à [t]?"

# game/events_mc.rpy:852
translate french ebd_adb4fc90:

    # pc "Err... no, why? What happened?"
    pc "Err... non, pourquoi ? Que s'est-il passé ?"

# game/events_mc.rpy:853
translate french ebd_08c180df:

    # m "She came into the room, blushing bright red like a tomato, grabbed her bag and left..."
    m "Elle est entrée dans la pièce, rougissant comme une tomate, a pris son sac et est partie..."

# game/events_mc.rpy:855
translate french ebd_483d7c75:

    # j "Yeah, all she said was \"[pc], the perv molested me while I was in the bathroom.\" Hehe."
    j "Ouais, tout ce qu'elle a dit c'est '[pc], le pervers m'a molesté pendant que j'étais dans la salle de bain.' Héhé."

# game/events_mc.rpy:856
translate french ebd_310f9987:

    # pc "W-WHAT?!"
    pc "Q-QUOI?!"

# game/events_mc.rpy:857
translate french ebd_dd73012c:

    # m "[j]! That's not funny."
    m "[j]! Ce n'est pas drôle."

# game/events_mc.rpy:858
translate french ebd_f5e3fc18:

    # j "Hehe, sorry."
    j "Héhé, désolé."

# game/events_mc.rpy:860
translate french ebd_f22fccd7:

    # pcthink "Gawd! She was just joking... damn you [j], I almost died."
    pcthink "Bon sang! Elle plaisantait... Bon sang, [j], j'ai failli mourir."

# game/events_mc.rpy:862
translate french ebd_de9a065c:

    # m "All [t] said, was that she has to leave and before I could say anything she was gone..."
    m "Tout ce que [t] a dit, c'est qu'elle devait partir et avant que je puisse dire quoi que ce soit, elle était partie..."

# game/events_mc.rpy:863
translate french ebd_36d4c18b:

    # pc "Strange..."
    pc "Étrange..."

# game/events_mc.rpy:864
translate french ebd_00670cd8:

    # m "Right?"
    m "N'est-ce pas?"

# game/events_mc.rpy:865
translate french ebd_582afdce:

    # m "Anyway, what's up with [e]? Will she be ready soon?"
    m "Quoi qu'il en soit, que se passe-t-il avec [e]? Sera-t-elle bientôt prête ?"

# game/events_mc.rpy:866
translate french ebd_1242be4e:

    # pc "Yeah, she's changing right now."
    pc "Oui, elle est en train de se changer en ce moment."

# game/events_mc.rpy:868
translate french ebd_a75714bf:

    # j "Oh, so you molested her instead?"
    j "Oh, donc tu l'as agressée à la place ?"

# game/events_mc.rpy:870
translate french ebd_04348ff1:

    # m "[j]!"
    m "[j]!"

# game/events_mc.rpy:872
translate french ebd_bb0ed4ca:

    # j "Yeah, sorry [m], you're right, I shouldn't say things like that..."
    j "Ouais, désolé [m], tu as raison, je ne devrais pas dire des choses comme ça...."

# game/events_mc.rpy:874
translate french ebd_054ce595:

    # j "...It's not like [pc] {b}{i}needs{/i}{/b} to molest her anyway, hehe."
    j "...C'est pas comme si [pc] {b}{i}avait besoin{/i}{/b} de la molester de toute façon, héhé."

# game/events_mc.rpy:876
translate french ebd_c350f789:

    # m "Ugh... you're hopeless [j]."
    m "Ugh... tu es sans espoir [j]."

# game/events_mc.rpy:877
translate french ebd_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:879
translate french ebd_b7bc8b83:

    # n "A little bit later." with dissolve
    n "Un peu plus tard." with dissolve

# game/events_mc.rpy:883
translate french ebd_4ca3b84a:

    # j "...and then you saw the burning apartment building?"
    j "...et ensuite vous avez vu l'immeuble en feu ?"

# game/events_mc.rpy:885
translate french ebd_c417e350:

    # pc "Yeah."
    pc "Ouais."

# game/events_mc.rpy:886
translate french ebd_f5b6ff60:

    # j "Crazy..."
    j "Fou..."

# game/events_mc.rpy:888
translate french ebd_be56fe30:

    # m "I'm so sorry, Hun. But at least you are here with us now. *hic*"
    m "Je suis tellement désolé, Hun. Mais au moins tu es ici avec nous maintenant. *hic*"

# game/events_mc.rpy:890
translate french ebd_e8dd4ad9:

    # j "Yeah, without the fire we would've never met, so cheers to the crazy dude who..."
    j "Oui, sans le feu, nous ne nous serions jamais rencontrés, alors bravo au fou qui..."

# game/events_mc.rpy:892
translate french ebd_9097c040:

    # m "There you are, we've been waiting for you [e]... Oh you look beautiful!"
    m "Te voilà, nous t'attendions [e]... Oh, tu es magnifique !"

# game/events_mc.rpy:894
translate french ebd_92ca9df6:

    # j "Wow, [e], you look luscious!"
    j "Wow, [e], tu es succulente !"

# game/events_mc.rpy:896
translate french ebd_cf30de3b:

    # m "I guess now we know what took her so long."
    m "Je suppose que maintenant nous savons ce qui lui a pris si longtemps."

# game/events_mc.rpy:897
translate french ebd_fba7b537:

    # j "Hehe."
    j "Héhé."

# game/events_mc.rpy:898
translate french ebd_72a33094:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:899
translate french ebd_e612037c:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:900
translate french ebd_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:902
translate french ebd_f0a677d2:

    # j "Hey, say something [pc]!"
    j "Hé, dis quelque chose [pc]!"

# game/events_mc.rpy:903
translate french ebd_a7c89ecd:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:904
translate french ebd_4abc8c9c:

    # pc "Sorry, I had to collect my jaw from the floor first, you look stunning [e]."
    pc "Désolé, je devais d'abord ramasser ma mâchoire du sol, tu es éblouissante [e]."

# game/events_mc.rpy:906
translate french ebd_72a33094_1:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:907
translate french ebd_bb3c8ee6:

    # m "Well then, why don't you come over here and sit down with us, [e]..."
    m "Eh bien, pourquoi ne pas venir ici et t'asseoir avec nous, [e]..."

# game/events_mc.rpy:909
translate french ebd_628d2bbd:

    # j "...WHAHAHA, [heshe] really fell out of the bed?"
    j "...WHAHAHA, [heshe] vraiment tombé du lit ?"

# game/events_mc.rpy:911
translate french ebd_a702c4e1:

    # m "Yes, it was really funny *hic*"
    m "Oui, c'était vraiment drôle *hic*."

# game/events_mc.rpy:912
translate french ebd_adb99edb:

    # m "And guess what, [heshe] was sleeping completely naked, haha."
    m "Et devines quoi, [heshe] dormait complètement nu, haha."

# game/events_mc.rpy:913
translate french ebd_258a582b:

    # j "NO WAY!"
    j "PAS QUESTION !"

# game/events_mc.rpy:914
translate french ebd_a8dfa9c8:

    # m "It's true, right [pc]?"
    m "C'est vrai, n'est-ce pas [pc]?"

# game/events_mc.rpy:916
translate french ebd_5db57f75:

    # j "Aww, I would've loved to see that! Hehe."
    j "Aww, j'aurais aimé voir ça ! Héhé."

# game/events_mc.rpy:918
translate french ebd_eecf66f6:

    # m "Oh by the way, don't you want to give [e] your presents?"
    m "Oh au fait, vous ne voulez pas donner à [e] vos cadeaux ?"

# game/events_mc.rpy:920
translate french ebd_9546c353:

    # j "I already gave her mine yesterday."
    j "Je lui ai déjà donné le mien hier."

# game/events_mc.rpy:922
translate french ebd_15ca938c:

    # m "Really, what is it?"
    m "Vraiment, qu'est-ce que c'est ?"

# game/events_mc.rpy:924
translate french ebd_d0106f24:

    # j "Just some nice clothes and... a secret, hehe."
    j "Juste quelques beaux vêtements et... un secret, héhé."

# game/events_mc.rpy:926
translate french ebd_ed817fb8:

    # m "A shecret eh? *hic*"
    m "Une shecret eh ? *hic*"

# game/events_mc.rpy:928
translate french ebd_fd3af669:

    # j "What did you get her [pc]?"
    j "Qu'est-ce que tu lui as donné [pc]?"

# game/events_mc.rpy:929
translate french ebd_55fc82ce:

    # pc "Oh I have a..."
    pc "Oh j'ai un..."

# game/events_mc.rpy:930
translate french ebd_36080486:

    # pcthink "Shit, I can't give it to her in front of [m]. She still thinks [e] has no idea how electronics work..."
    pcthink "Merde, je ne peux pas le lui donner devant [m]. Elle pense toujours que [e] n'a aucune idée du fonctionnement de l'électronique..."

# game/events_mc.rpy:932
translate french ebd_7581e536:

    # m "Well?"
    m "Alors ?"

# game/events_mc.rpy:933
translate french ebd_9879879d:

    # pc "Err... well, haha, it's a secret too..."
    pc "Err... bien, haha, c'est un secret aussi..."

# game/events_mc.rpy:934
translate french ebd_448719c3:

    # m "Huh? Why do you all have secret presentsh... did I miss the joke here? *hic*"
    m "Huh ? Pourquoi vous avez tous des cadeaux secrets... J'ai raté la blague ? *hic*"

# game/events_mc.rpy:936
translate french ebd_105c0c39:

    # j "Naaw, don't worry, we just don't want to destroy the surprise, hehe."
    j "Naan, ne t'inquiètes pas, nous ne voulons pas gâcher la surprise, héhé."

# game/events_mc.rpy:938
translate french ebd_4e935e33:

    # m "Oh... okay... *hic*"
    m "Oh... ok... *hic*"

# game/events_mc.rpy:940
translate french ebd_47c298c2:

    # m "...and then I deshided to *hic... shtop drinking and shange my life... it wash the besht deshision I've ever made..."
    m "...et puis j'ai décidé de *hic... arrêter de boire et changer de vie... ça a été la meilleure décision que j'ai jamais prise..."

# game/events_mc.rpy:942
translate french ebd_c9a195dc:

    # m "Oh nooo, the besht deshision wash to get [pc] back... *hic*"
    m "Oh nooon, la meilleure prise de deshision pour récupérer [pc] ... *hic*"

# game/events_mc.rpy:943
translate french ebd_1af29da9:

    # j "Yeah, that was a great decision, hehe."
    j "Ouais, c'était une grande décision, héhé."

# game/events_mc.rpy:944
translate french ebd_a951b22b:

    # m "Ugh..."
    m "Ugh..."

# game/events_mc.rpy:946
translate french ebd_cc241df9:

    # m "I shink I need to pee..."
    m "Je crois que j'ai besoin de faire pipi..."

# game/events_mc.rpy:948
translate french ebd_06a6d364:

    # pc "Whoa!"
    pc "Whoua !"

# game/events_mc.rpy:949
translate french ebd_0c127202:

    # m "Whups..."
    m "Whups..."

# game/events_mc.rpy:950
translate french ebd_6e3b6836:

    # e "Hehe."
    e "Héhé."

# game/events_mc.rpy:951
translate french ebd_fe5aa5c4:

    # pc "Are you okay [m]?"
    pc "Ça va [m]?"

# game/events_mc.rpy:952
translate french ebd_3001f0b2:

    # m "S-shorry, [pc]..."
    m "D-Déchsolé [pc]..."

# game/events_mc.rpy:953
translate french ebd_a35e971a:

    # m "Ugh... I don't feel sho well..."
    m "Ugh... Je ne me sens pas très bien..."

# game/events_mc.rpy:954
translate french ebd_3d64b039:

    # pcthink "Shit, she definitely had enough punch for today..."
    pcthink "Merde, elle a définitivement assez de punch pour aujourd'hui..."

# game/events_mc.rpy:955
translate french ebd_d9543283:

    # pc "Maybe you should lie down for a while."
    pc "Tu devrais peut-être t'allonger un moment."

# game/events_mc.rpy:956
translate french ebd_c0175e8c:

    # m "...but it's [e]'sh birthday... *hic*"
    m "...mais c'est l'anniversaire de [e]... *hic*"

# game/events_mc.rpy:957
translate french ebd_2c10d7d0:

    # pc "Doesn't help if you feel bad [m]."
    pc "Cela n'aide pas si tu te sens mal [m]."

# game/events_mc.rpy:958
translate french ebd_e442a3ed:

    # m "Uhh... everything is shpinning..."
    m "Uhh... tout se met en place..."

# game/events_mc.rpy:960
translate french ebd_35fc80fb:

    # j "You should help her to the bedroom, [pc]. Hehe."
    j "Tu devrais l'aider à aller dans la chambre, [pc]. Héhé."

# game/events_mc.rpy:962
translate french ebd_8c916250:

    # m "But I don't want to go to bed yet... *hic*"
    m "Mais je ne veux pas encore aller me coucher... *hic*"

# game/events_mc.rpy:963
translate french ebd_6a46278c:

    # pc "Come on [m], I'll help you..."
    pc "Viens [m], je vais t'aider..."

# game/events_mc.rpy:964
translate french ebd_64543edd:

    # m "Ugh... okay... i-if I must..."
    m "Ugh... ok... si je dois..."

# game/events_mc.rpy:970
translate french ebd2_307d7ef6:

    # j "Wow, I've never seen [m] {b}this{/b} drunk before!"
    j "Wow, je n'ai jamais vu [m] {b}si{/b} ivre avant !"

# game/events_mc.rpy:971
translate french ebd2_d8890d8b:

    # e "Heh..."
    e "Heh..."

# game/events_mc.rpy:973
translate french ebd2_4705691f:

    # pc "Well I did... several times..."
    pc "Eh bien je l'ai fait... plusieurs fois..."

# game/events_mc.rpy:974
translate french ebd2_91eb52ce:

    # pcthink "...and she was completely different this time..."
    pcthink "...et elle était complètement différente cette fois-ci..."

# game/events_mc.rpy:975
translate french ebd2_e8aaaaae:

    # j "She will be alright... right?"
    j "Elle va s'en sortir... n'est-ce pas ?"

# game/events_mc.rpy:976
translate french ebd2_fef852ff:

    # pc "Yeah..."
    pc "Ouais..."

# game/events_mc.rpy:977
translate french ebd2_d4ffef5b:

    # j "Great! Now let's start the {i}real{/i} party, right [e]? Hehe..."
    j "Super ! Maintenant, commençons la {i}vraie{/i} fête, n'est-ce pas [e]? Héhé..."

# game/events_mc.rpy:979
translate french ebd2_72a33094:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:980
translate french ebd2_0ab94a3c:

    # j "[e]?"
    j "[e]?"

# game/events_mc.rpy:981
translate french ebd2_c11599d9:

    # e "...ehh..."
    e "...ehh..."

# game/events_mc.rpy:984
translate french ebd2_72a33094_1:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:985
translate french ebd2_e23650ca:

    # j "Awww damn it..."
    j "Awww bon sang..."

# game/events_mc.rpy:986
translate french ebd2_5a4fe1da:

    # pc "Looks like the party is over before it even started..."
    pc "On dirait que la fête est terminée avant même d'avoir commencé..."

# game/events_mc.rpy:987
translate french ebd2_c77a48de:

    # j "AAAAaaaawwww... stupid [j]!"
    j "AAAAaaaawwww... stupide [j]!"

# game/events_mc.rpy:988
translate french ebd2_b4fa3fc0:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:989
translate french ebd2_54326bbc:

    # j "I'm sorry..."
    j "Je suis désolée..."

# game/events_mc.rpy:990
translate french ebd2_03240a86:

    # j "I wanted this to be something special for [e] and now everything is fucked..."
    j "Je voulais que ce soit quelque chose de spécial pour [e] et maintenant tout est foutu..."

# game/events_mc.rpy:991
translate french ebd2_2955da2c:

    # pc "Well, you tried... and I guess this is a birthday she will remember..."
    pc "Eh bien, tu as essayé... et je suppose que c'est un anniversaire dont elle se souviendra...."

# game/events_mc.rpy:992
translate french ebd2_d1ed6d17:

    # j "If she remembers anything at all..."
    j "Si elle se souvient de quoi que ce soit..."

# game/events_mc.rpy:993
translate french ebd2_b4fa3fc0_1:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:994
translate french ebd2_84db82c7:

    # pc "I'm going to take her to bed..."
    pc "Je vais l'emmener au lit..."

# game/events_mc.rpy:995
translate french ebd2_87d3c8fc:

    # j "Yeah..."
    j "Ouais..."

# game/events_mc.rpy:997
translate french ebd2_2d4a91d0:

    # pcthink "I guess I was right..."
    pcthink "Je suppose que j'avais raison..."

# game/events_mc.rpy:998
translate french ebd2_5009a718:

    # pcthink "...it didn't turn out well..."
    pcthink "...ça ne s'est pas bien passé..."

# game/events_mc.rpy:1002
translate french ebd2_65ee5a5c:

    # pc "Good night [e]."
    pc "Bonne nuit [e]."

# game/events_mc.rpy:1003
translate french ebd2_5916f987:

    # e "Hnn..."
    e "Hnn..."

# game/events_mc.rpy:1010
translate french ebdEnd_017fb70c:

    # pcthink "Well, back home..."
    pcthink "Eh bien, de retour à la maison..."

# game/events_mc.rpy:1012
translate french ebdEnd_692b88a9:

    # pcthink "Home..."
    pcthink "Maisom..."

# game/events_mc.rpy:1013
translate french ebdEnd_229bcf50:

    # pcthink "I've never felt at home in this house..."
    pcthink "Je ne me suis jamais senti chez moi dans cette maison..."

# game/events_mc.rpy:1015
translate french ebdEnd_eff3a57b:

    # pcthink "Hum... I guess [m] isn't going to clear the table anytime soon..."
    pcthink "Hum... Je suppose que [m] ne va pas vider la table de sitôt..."

# game/events_mc.rpy:1016
translate french ebdEnd_fd163d05:

    # pcthink "At least there's no huge pile of junk to clean up like after a normal party..."
    pcthink "Au moins il n'y a pas d'énorme tas de déchets à nettoyer comme après une fête normale...."

# game/events_mc.rpy:1018
translate french ebdEnd_4e202750:

    # n "You clear the table and decide to go to bed."
    n "Vous débarrassez la table et décidez d'aller vous coucher."

# game/events_mc.rpy:1020
translate french ebdEnd_49c5868b:

    # pcthink "What a \"party\"..."
    pcthink "Quelle 'fête'..."

# game/events_mc.rpy:1021
translate french ebdEnd_41ca071c:

    # pcthink "That must have been the worst birthday party ever..."
    pcthink "Ça a dû être la pire fête d'anniversaire de tous les temps..."

# game/events_mc.rpy:1022
translate french ebdEnd_ca21d5ae:

    # pcthink "Poor [e], I hope she{w=1.0}{nw}"
    pcthink "Pauvre [e], j'espère qu'elle{w=1.0}{nw}"

# game/events_mc.rpy:1025
translate french ebdEnd_4e0bec6e:

    # pc "Whoa [e]!"
    pc "Woua [e]!"

# game/events_mc.rpy:1027
translate french ebdEnd_e792b68b:

    # e "Uhhh..."
    e "Uhhh..."

# game/events_mc.rpy:1029
translate french ebdEnd_442ac6e9:

    # pcthink "Wow she's really drunk..."
    pcthink "Wow elle est vraiment bourrée..."

# game/events_mc.rpy:1032
translate french ebdEnd_ddf68bfe:

    # pc "[e]?"
    pc "[e]?"

# game/events_mc.rpy:1033
translate french ebdEnd_0e681e16:

    # e "*snore*"
    e "*ronflement*"

# game/events_mc.rpy:1034
translate french ebdEnd_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1035
translate french ebdEnd_c52114ef:

    # pc "Oh well... good night [e]..."
    pc "Ah bon... bonne nuit [e]..."

# game/events_mc.rpy:1047
translate french afterEBD_a479e7bc:

    # pc "*Yaaawn*"
    pc "*Yaaawn*"

# game/events_mc.rpy:1049
translate french afterEBD_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1052
translate french afterEBD_f12901fd:

    # pcthink "[e]..."
    pcthink "[e]..."

# game/events_mc.rpy:1053
translate french afterEBD_c8badc00:

    # pcthink "When did she become so attached to me?"
    pcthink "Quand est-elle devenue si attachée à moi ?"

# game/events_mc.rpy:1054
translate french afterEBD_4979b763:

    # pcthink "She's was always a bit clingy when we were younger, but this is... different..."
    pcthink "Elle a toujours été un peu collante quand on était plus jeunes, mais là c'est... différent..."

# game/events_mc.rpy:1055
translate french afterEBD_be5aaa98:

    # pcthink "Could she really be in love with me, like [j] said?"
    pcthink "Pourrait-elle vraiment être amoureuse de moi, comme [j] l'a dit ?"

# game/events_mc.rpy:1056
translate french afterEBD_a0d3b905:

    # pcthink "...with her own [bs]?"
    pcthink "...avec son propre [bs]?"

# game/events_mc.rpy:1057
translate french afterEBD_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1058
translate french afterEBD_7e43320d:

    # pcthink "...and why doesn't she talk?"
    pcthink "...et pourquoi elle ne parle pas ?"

# game/events_mc.rpy:1059
translate french afterEBD_580a09c1:

    # pcthink "Or does she?"
    pcthink "Ou le fait-elle ?"

# game/events_mc.rpy:1060
translate french afterEBD_6911a2a4:

    # pcthink "Could she be talking to [j]?"
    pcthink "Pourrait-elle parler à [j]?"

# game/events_mc.rpy:1061
translate french afterEBD_00702135:

    # pcthink "I'd understand it if she doesn't want to talk to [m]... or that therabitch..."
    pcthink "Je comprendrais qu'elle ne veuille pas parler à [m]... ou à cette salopeute..."

# game/events_mc.rpy:1062
translate french afterEBD_ec023f64:

    # pcthink "...but why wouldn't she want to talk to me?"
    pcthink "...mais pourquoi ne voudrait-elle pas me parler ?"

# game/events_mc.rpy:1063
translate french afterEBD_b23e951c:

    # pcthink "What the hell happened after I moved out?!"
    pcthink "Que s'est-il passé après que j'ai déménagé ?!"

# game/events_mc.rpy:1066
translate french afterEBD_7027498e:

    # e "Hnnn..."
    e "Hnnn..."

# game/events_mc.rpy:1067
translate french afterEBD_0e7a8064:

    # pcthink "Damn..."
    pcthink "Merde..."

# game/events_mc.rpy:1069
translate french afterEBD_b90183ff:

    # pcthink "Well, time to get up. I need to use the bathroom."
    pcthink "Eh bien, il est temps de se lever. Je dois aller aux toilettes."

# game/events_mc.rpy:1071
translate french afterEBD_8dbc43ac:

    # pcthink "I guess I'll let her sleep..."
    pcthink "Je suppose que je vais la laisser dormir..."

# game/events_mc.rpy:1073
translate french afterEBD_38a7142f:

    # pcthink "I hope [m] isn't up already..."
    pcthink "J'espère que [m] n'est pas déjà en place..."

# game/events_mc.rpy:1079
translate french officeFirst_658f3f89:

    # n "On your way to the office it starts to rain."
    n "Sur le chemin du bureau, il commence à pleuvoir."

# game/events_mc.rpy:1080
translate french officeFirst_8e6b2543:

    # pcthink "Shit, lucky me!"
    pcthink "Merde, j'ai de la chance !"

# game/events_mc.rpy:1081
translate french officeFirst_bc2ee6f5:

    # n "You can see a woman running towards the entrance of the building in which you suspect the office to be located."
    n "Vous pouvez voir une femme courir vers l'entrée du bâtiment dans lequel vous soupçonnez que le bureau se trouve."

# game/events_mc.rpy:1083
translate french officeFirst_7af06681:

    # pc "Hey! Hey you!"
    pc "Hé ! Hé vous !"

# game/events_mc.rpy:1085
translate french officeFirst_4f3b0408:

    # Woman" "Y-yes?!"
    "Femme" "O-oui ?!"

# game/events_mc.rpy:1086
translate french officeFirst_51a6f7e0:

    # pc "Is this Atomic Aeronautics?"
    pc "Est-ce que c'est Atomic Aeronautics ?"

# game/events_mc.rpy:1087
translate french officeFirst_eeaceb23:

    # Woman" "Y-yes!"
    "Femme" "O-oui !"

# game/events_mc.rpy:1088
translate french officeFirst_b6ac243e:

    # pc "I'm [pc]... [pc] [pcsure]."
    pc "Je suis [pc]... [pc] [pcsure]..."

# game/events_mc.rpy:1089
translate french officeFirst_ba58eaa8:

    # Woman" "pc "[pcsure]?"
    "Femme" "[pc] [pcsure]?"

# game/events_mc.rpy:1090
translate french officeFirst_5cf9f3a1:

    # pc "Yes, I'm starting work here tomorrow!"
    pc "Oui, je commence à travailler ici demain !"

# game/events_mc.rpy:1091
translate french officeFirst_c9aca35f:

    # Woman" "O-oh, that's you... please, let's go inside!"
    "Femme" "O-oh, c'est vous... S'il vous plaît, allons à l'intérieur !"

# game/events_mc.rpy:1093
translate french officeFirst_d97311e5:

    # You follow her."
    "Vous la suivez."

# game/events_mc.rpy:1095
translate french officeFirst_e9edfff6:

    # pc "Finally out of the rain..."
    pc "Enfin à l'abri..."

# game/events_mc.rpy:1096
translate french officeFirst_ea7019d0:

    # Woman" "Why are you here?"
    "Femme" "Pourquoi êtes-vous là ?"

# game/events_mc.rpy:1097
translate french officeFirst_225fadf5:

    # pc "Huh?"
    pc "Huh ?"

# game/events_mc.rpy:1098
translate french officeFirst_2f424692:

    # Woman" "I-I mean, you're starting tomorrow, why are you here today?"
    "Femme" "Je veux dire, vous commencez demain, pourquoi êtes-vous là aujourd'hui ?"

# game/events_mc.rpy:1099
translate french officeFirst_10609a33:

    # pc "Well... it's been a while since I left town, and I don't know this area, so I thought it might be a good idea to have a look before I start."
    pc "Bon... ça fait un moment que j'ai quitté la ville, et je ne connais pas ce quartier, alors j'ai pensé que ce serait une bonne idée de jeter un coup d'œil avant de commencer."

# game/events_mc.rpy:1100
translate french officeFirst_0651871c:

    # Woman" "I see."
    "Femme" "Je vois."

# game/events_mc.rpy:1101
translate french officeFirst_0368c379:

    # pc "I could ask you the same question by the way. It's Sunday, what are you doing here?"
    pc "Je pourrais vous poser la même question d'ailleurs. C'est dimanche, que faites-vous ici ?"

# game/events_mc.rpy:1102
translate french officeFirst_f2fbbb51:

    # Woman" "I... C.J... the boss asked me to bring in some papers."
    "Femme" "Je... C.J... le patron m'a demandé d'apporter des papiers."

# game/events_mc.rpy:1103
translate french officeFirst_c7965c80:

    # pc "He's here, too?"
    pc "Il est là aussi ?"

# game/events_mc.rpy:1104
translate french officeFirst_225cc38a:

    # Woman" "Yes, like almost every Sunday."
    "Femme" "Oui, comme presque tous les dimanches."

# game/events_mc.rpy:1105
translate french officeFirst_cb272a23:

    # pcthink "WTF, he must be a workaholic..."
    pcthink "WTF, il doit être un bourreau de travail..."

# game/events_mc.rpy:1106
translate french officeFirst_332ec2ad:

    # pc "Every Sunday?"
    pc "Tous les dimanches ?"

# game/events_mc.rpy:1108
translate french officeFirst_202424ff:

    # Woman" "Yes... I can tell him that you are here."
    "Femme" "Oui... Je peux lui dire que vous êtes ici."

# game/events_mc.rpy:1109
translate french officeFirst_c3f1eb0b:

    # pc "Yeah, why not. Can I have a look around?"
    pc "Ouais, pourquoi pas. Je peux jeter un coup d'œil ?"

# game/events_mc.rpy:1110
translate french officeFirst_04e2f270:

    # Woman" "Uhm... I can show you a bit, why don't you follow me."
    "Femme" "Euh... Je peux vous montrer un peu, pourquoi ne pas me suivre."

# game/events_mc.rpy:1111
translate french officeFirst_d688a87b:

    # pc "Yeah, sure. What's your name by the way?"
    pc "Oui, bien sûr. Au fait, quel est votre nom ?"

# game/events_mc.rpy:1112
translate french officeFirst_dbc67763:

    # Woman" "Stephanie..."
    "Femme" "Stéphanie..."

# game/events_mc.rpy:1113
translate french officeFirst_f7e59657:

    # pc "Well, nice to meet you, Stephanie."
    pc "Eh bien, enchanté de vous rencontrer, Stéphanie."

# game/events_mc.rpy:1117
translate french officeFirst_d84438d3:

    # st "This is the fabrication and testing hall."
    st "C'est le hall de fabrication et d'essai."

# game/events_mc.rpy:1119
translate french officeFirst_73c10355:

    # pc "Whoa, I didn't know that you're actually building stuff here..."
    pc "Woua, je ne savais pas que vous étiez en train de construire des trucs ici..."

# game/events_mc.rpy:1120
translate french officeFirst_57cdd18c:

    # st "This is the company headquarters, all our prototypes are built and tested here."
    st "C'est le siège de l'entreprise, tous nos prototypes sont construits et testés ici."

# game/events_mc.rpy:1121
translate french officeFirst_96746b87:

    # pc "That's really impressive."
    pc "C'est vraiment impressionnant."

# game/events_mc.rpy:1125
translate french officeFirst_9e5a74fe:

    # pc "Damn, this building is larger than I'd expected it to be..."
    pc "Bon sang, ce bâtiment est plus grand que ce à quoi je m'attendais..."

# game/events_mc.rpy:1128
translate french officeFirst_8502ad06:

    # st "C.J, I have the papers you asked for."
    st "C.J, j'ai les papiers que vous avez demandés."

# game/events_mc.rpy:1129
translate french officeFirst_8fd26fd2:

    # boss "Oh good, thanks, [st]."
    boss "Oh bien, merci, [st]."

# game/events_mc.rpy:1130
translate french officeFirst_95047118:

    # st "There is something else..."
    st "Il y a autre chose..."

# game/events_mc.rpy:1132
translate french officeFirst_9b1a9288:

    # st "Our new employee..."
    st "Notre nouvel employé..."

# game/events_mc.rpy:1134
translate french officeFirst_b04d1381:

    # boss "Oh, I see, you must be [pc]. I didn't expect you today."
    boss "Oh, je vois, vous devez être [pc]. Je ne vous attendais pas aujourd'hui."

# game/events_mc.rpy:1135
translate french officeFirst_e671bf7c:

    # pc "Err... yeah, I actually just wanted to have a look at the office, but then it started to rain and Stephanie was nice enough to let me in."
    pc "Err... oui, en fait je voulais juste jeter un coup d'œil au bureau, mais il a commencé à pleuvoir et Stéphanie a été assez gentille pour me laisser entrer."

# game/events_mc.rpy:1136
translate french officeFirst_e38743f9:

    # boss "Anyway, nice to meet you."
    boss "Bref, ravi de vous rencontrer."

# game/events_mc.rpy:1138
translate french officeFirst_749df471:

    # boss "[st], why don't you go and get yourself and [pc] a towel to get dry with."
    boss "[st], pourquoi ne vas-tu pas vous chercher à toi et [pc] une serviette pour vous sécher."

# game/events_mc.rpy:1139
translate french officeFirst_5dc56ca1:

    # st "Yes, [boss]."
    st "Oui, [boss]."

# game/events_mc.rpy:1141
translate french officeFirst_8261ab61:

    # boss "You should have called me, I'd have let you in."
    boss "Vous auriez dû m'appeler, je vous aurais fait entrer."

# game/events_mc.rpy:1142
translate french officeFirst_7dd1a646:

    # pc "I didn't know you're here."
    pc "Je ne savais pas que vous étiez là."

# game/events_mc.rpy:1144
translate french officeFirst_befea34a:

    # boss "Ha! I'm always here. My wife is happy when I leave her alone and I'm happy when I don't have to see her... A drink?"
    boss "Ha ! Je suis toujours là. Ma femme est heureuse quand je la laisse tranquille et je suis heureux quand je n'ai pas à la voir... Un verre ?"

# game/events_mc.rpy:1145
translate french officeFirst_4be4d14b:

    # pc "Err... no, thanks."
    pc "Err... non, merci."

# game/events_mc.rpy:1146
translate french officeFirst_56c830b3:

    # boss "Well I'll take one."
    boss "Eh bien je vais en prendre un."

# game/events_mc.rpy:1148
translate french officeFirst_1f2241b6:

    # boss "Actually, it's good that you're here today, so we can go through all the details while it's quiet."
    boss "En fait, c'est bien que vous soyez là aujourd'hui, comme ça nous pourrons passer en revue tous les détails pendant que c'est calme."

# game/events_mc.rpy:1149
translate french officeFirst_667747fd:

    # boss "I guess [st] showed you around a bit?"
    boss "Je suppose que [st] vous a fait visiter un peu ?"

# game/events_mc.rpy:1150
translate french officeFirst_29ffe3c1:

    # pc "Yeah, it's quite impressive."
    pc "Oui, c'est assez impressionnant."

# game/events_mc.rpy:1151
translate french officeFirst_9491da86:

    # boss "Ha! That's nothing. We're just doing the rough tasks here, but I've seen your work, now {b}that's{/b} impressive. You're a true artist."
    boss "Ha ! Ce n'est rien. On fait juste les tâches grossières ici, mais j'ai vu votre travail, maintenant {b}ça c'est{/b} impressionnant. Vous êtes un vrai artiste."

# game/events_mc.rpy:1152
translate french officeFirst_dc740ee7:

    # pc "Thanks, I do my best."
    pc "Merci, je fais de mon mieux."

# game/events_mc.rpy:1154
translate french officeFirst_9ccf7cb1:

    # boss "That's why I hired you. Now let's sit down, there are a few things I need from you asap!"
    boss "C'est pourquoi je vous ai engagé. Maintenant, asseyons-nous, j'ai besoin de quelques trucs de votre part dès que possible !"

# game/events_mc.rpy:1156
translate french officeFirst_aee865c1:

    # n "After a quick introduction he tells you about a new project he's planning, and your role in it. It turns out that there is not much time left, you only have one week from start to finish for your part of the project."
    n "Après une rapide introduction, il vous parle d'un nouveau projet qu'il prévoit, et de votre rôle dans celui-ci. Il s'avère qu'il ne reste plus beaucoup de temps, vous n'avez qu'une semaine du début à la fin pour votre partie du projet."

# game/events_mc.rpy:1157
translate french officeFirst_3fad21d5:

    # n "A while later."
    n "Un peu plus tard."

# game/events_mc.rpy:1159
translate french officeFirst_ab03fde0:

    # boss "...and if you need anything, be sure to call or just come by. Even if I'm not here, [st] is always there to help."
    boss "...et si vous avez besoin de quelque chose, n'hésitez pas à appeler ou à passer. Même si je ne suis pas là, [st] est toujours là pour aider."

# game/events_mc.rpy:1160
translate french officeFirst_4047cc0a:

    # pc "Sure. By the way, is she your secretary?"
    pc "Bien sûr. Au fait, c'est votre secrétaire ?"

# game/events_mc.rpy:1161
translate french officeFirst_25aa3f3b:

    # boss "Yes and no. She's actually more than just a secretary. In fact, she could handle the company on her own if she'd just be a bit more confident, but that woman is way too shy, I tell ya..."
    boss "Oui et non. Elle est en fait plus qu'une simple secrétaire. En fait, elle pourrait gérer l'entreprise toute seule si elle était un peu plus confiante, mais cette femme est bien trop timide, je vous le dis..."

# game/events_mc.rpy:1162
translate french officeFirst_6510b358:

    # pc "Yeah, I've noticed that."
    pc "Oui, j'ai remarqué ça."

# game/events_mc.rpy:1163
translate french officeFirst_9726a64d:

    # boss "Don't worry, she'll open up to you over time. That's just the way she is."
    boss "Ne t'inquiète pas, elle s'ouvrira à toi avec le temps. C'est juste comme ça qu'elle est."

# game/events_mc.rpy:1164
translate french officeFirst_0fb9e488:

    # pc "I hope so... errm... can I ask you a personal question?"
    pc "J'espère que oui... errm... puis-je vous poser une question personnelle ?"

# game/events_mc.rpy:1165
translate french officeFirst_a9b94d3f:

    # boss "There is nothing going on between us, so if you're interested in her, just go for it."
    boss "Il n'y a rien entre nous, donc si vous êtes intéressé par elle, foncez."

# game/events_mc.rpy:1166
translate french officeFirst_df0e83ab:

    # pc "Eh... haha, actually I was wondering what C.J. stands for?"
    pc "Eh... haha, en fait je me demandais ce que signifie C.J. ?"

# game/events_mc.rpy:1167
translate french officeFirst_8ae5dbe3:

    # boss "My name?"
    boss "Mon nom ?"

# game/events_mc.rpy:1168
translate french officeFirst_86a016ab:

    # pc "Yeah, it's always just C.J. in every paper I've seen so far."
    pc "Ouais, c'est toujours juste C.J. dans tous les papiers que j'ai vu jusqu'à présent."

# game/events_mc.rpy:1169
translate french officeFirst_e52b1fe5:

    # boss "Colin-James Olafsson is the name, and yes, I have some Viking ancestry. One of them even sailed with the children of Eirik the Red."
    boss "Colin-James Olafsson est le nom, et oui, j'ai quelques ancêtres vikings. L'un d'eux a même navigué avec les enfants d'Eirik le Rouge."

# game/events_mc.rpy:1170
translate french officeFirst_30438830:

    # pc "Wow, that's interesting!"
    pc "Wow, c'est intéressant !"

# game/events_mc.rpy:1171
translate french officeFirst_11a7017c:

    # boss "Anyway, I won't keep you any longer. We both have a lot of work to do. See ya next week."
    boss "De toute façon, je ne vous retiens pas plus longtemps. Nous avons tous les deux beaucoup de travail à faire. A la semaine prochaine."

# game/events_mc.rpy:1172
translate french officeFirst_cb70ec2a:

    # pc "Yeah, goodbye."
    pc "Oui, au revoir."

# game/events_mc.rpy:1174
translate french officeFirst_3bde9f79:

    # n "On your way back home."
    n "Sur le chemin du retour."

# game/events_mc.rpy:1175
translate french officeFirst_a1967dee:

    # *Ding*"
    "*Ding*"

# game/events_mc.rpy:1177
translate french officeFirst_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh ?"

# game/events_mc.rpy:1178
translate french officeFirst_88dffbc9:

    # pcthink "A message from [e]? HA! I knew it!"
    pcthink "Un message de [e]? HA ! Je le savais !"

# game/events_mc.rpy:1180
translate french officeFirst_62ddcc97:

    # pcthink "Heh... cute dress..."
    pcthink "Heh... jolie robe..."

# game/events_mc.rpy:1181
translate french officeFirst_e81b5770:

    # pcthink "Is that her tongue sticking out?"
    pcthink "C'est sa langue qui sort ?"

# game/events_mc.rpy:1183
translate french officeFirst_195d3dfd:

    # pcthink "Yep, it is... damn she's so cute."
    pcthink "Oui, c'est ça... elle est si mignonne."

# game/events_mc.rpy:1187
translate french officeFirst_53981479:

    # Cop" "Ouuch!"
    "Policier" "Ouuch!"

# game/events_mc.rpy:1188
translate french officeFirst_26b08e4a:

    # pc "Fuck!"
    pc "Merde!"

# game/events_mc.rpy:1191
translate french officeFirst_228fb161:

    # Cop" "YOU!"
    "Policier" "VOUS !"

# game/events_mc.rpy:1192
translate french officeFirst_95710a31:

    # pc "Oh, no..."
    pc "Oh, non..."

# game/events_mc.rpy:1193
translate french officeFirst_17f6ad68:

    # pcthink "Not this guy again..."
    pcthink "Pas encore ce type..."

# game/events_mc.rpy:1195
translate french officeFirst_4acfd17a:

    # Cop" "So-so, not watching where we're going, are we?"
    "Policier" "Alors, on ne regarde pas où on va, hein ?"

# game/events_mc.rpy:1196
translate french officeFirst_25d64821:

    # pc "I doubt that {b}you{/b} were looking, idiot..."
    pc "Je doute que {b}vous{/b} regardiez, idiot..."

# game/events_mc.rpy:1197
translate french officeFirst_261aa3ea:

    # pcthink "Shit, I shouldn't have said that!"
    pcthink "Merde, je n'aurais pas dû dire ça !"

# game/events_mc.rpy:1199
translate french officeFirst_9a256965:

    # Cop" "What was that? Insulting an officer? W-wait... you ran into me, that was obviously an attack! Ha! Now I got you!"
    "Policier" "C'était quoi ça ? Insulter un officier ? A-attendez... vous m'êtes rentré dedans, c'était évidemment une attaque ! Ha ! Maintenant je vous tiens !"

# game/events_mc.rpy:1200
translate french officeFirst_2ea675f7:

    # pc "What? That's bullshit and you know it!"
    pc "Quoi ? C'est des conneries et vous le savez !"

# game/events_mc.rpy:1202
translate french officeFirst_f40d5809:

    # Cop" "Hands where I can see them, turn around, asshole!"
    "Policier" "Les mains où je peux les voir, tournez-vous, connard !"

# game/events_mc.rpy:1203
translate french officeFirst_5fdae6d7:

    # pcthink "Fuck... what an idiot..."
    pcthink "Putain... quel idiot..."

# game/events_mc.rpy:1208
translate french officeFirst_59a0c788:

    # pcthink "Damn... I had hoped to never see this cell again..."
    pcthink "Mince... J'avais espéré ne jamais revoir cette cellule..."

# game/events_mc.rpy:1209
translate french officeFirst_5e6ec546:

    # pcthink "Fuck that goddamn wannabe Sherlock! What the fuck is his problem anyway!"
    pcthink "J'emmerde ce putain d'aspirant Sherlock ! C'est quoi son putain de problème de toute façon !"

# game/events_mc.rpy:1211
translate french officeFirst_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1213
translate french officeFirst_53fca454:

    # pcthink "How many times have I seen this cell from the inside?"
    pcthink "Combien de fois ai-je vu cette cellule de l'intérieur ?"

# game/events_mc.rpy:1214
translate french officeFirst_2c00bbfc:

    # pcthink "Shit, I hope they let me go soon, I don't want to spend the night here..."
    pcthink "Merde, j'espère qu'ils vont me laisser partir rapidement, je ne veux pas passer la nuit ici..."

# game/events_mc.rpy:1216
translate french officeFirst_f0cac90d_1:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1218
translate french officeFirst_2d9183c8:

    # pcthink "Especially not in wet clothes..."
    pcthink "Surtout pas en vêtements mouillés..."

# game/events_mc.rpy:1220
translate french officeFirst_cb9aaae9:

    # pcthink "...what's up with her?"
    pcthink "...qu'est-ce qui se passe avec elle ?"

# game/events_mc.rpy:1222
translate french officeFirst_3e8305e8:

    # pcthink "Damn, I hope [m] and [e] don't find out I'm in here."
    pcthink "Merde, j'espère que [m] et [e] ne vont pas découvrir que je suis ici."

# game/events_mc.rpy:1225
translate french officeFirst_3307dfad:

    # Prostitute" "Hey there, handsome. I haven't seen you here before."
    "Prostituée" "Hé là, beau gosse. Je ne t'ai pas vu ici avant."

# game/events_mc.rpy:1227
translate french officeFirst_0429c5f5:

    # Prostitute" "Hey there, beautiful. I haven't seen you here before."
    "Prostituée" "Hé là, ma belle. Je ne t'ai jamais vue ici auparavant."

# game/events_mc.rpy:1229
translate french officeFirst_9f4df712:

    # pc "Ugh... yeah, you're a few years late..."
    pc "Ugh... ouais, tu as quelques années de retard..."

# game/events_mc.rpy:1230
translate french officeFirst_a4202cfa:

    # Prostitute" "Huh? What do you mean, honey?"
    "Prostituée" "Hein ? Qu'est-ce que tu veux dire, chéri ?"

# game/events_mc.rpy:1231
translate french officeFirst_2df6c4d3:

    # pc "...forget it."
    pc "...oublie ça."

# game/events_mc.rpy:1234
translate french officeFirst_996f0cca:

    # Prostitute" "But it's never too late to have a bit fun, don't you think, sexy?"
    "Prostituée" "Mais il n'est jamais trop tard pour s'amuser un peu, n'est-ce pas, sexy ?"

# game/events_mc.rpy:1236
translate french officeFirst_f9decee2:

    # pc "We are in a holding cell already, do you want to get in even more trouble?"
    pc "Nous sommes déjà dans une cellule de détention, voulez-vous avoir encore plus d'ennuis ?"

# game/events_mc.rpy:1237
translate french officeFirst_fa635850:

    # Prostitute" "No risk, no fun."
    "Prostituée" "Pas de risque, pas de plaisir."

# game/events_mc.rpy:1239
translate french officeFirst_82943c22:

    # Prostitute" "Come on, sexy, I'll make it worth your time!"
    "Prostituée" "Allez, sexy, je vais te faire gagner du temps !"

# game/events_mc.rpy:1240
translate french officeFirst_3151510c:

    # oc_ "Apart you two!"
    oc_ "À part vous deux!"

# game/events_mc.rpy:1242
translate french officeFirst_df4ac260:

    # Prostitute" "Ughh, shit!"
    "Prostituée" "Ughh, merde !"

# game/events_mc.rpy:1243
translate french officeFirst_bf282ccb:

    # oc_ "Sam, aren't you in enough trouble already?"
    oc_ "Sam, n'as-tu pas déjà assez d'ennuis ?"

# game/events_mc.rpy:1245
translate french officeFirst_086a64ed:

    # Prostitute" "Fuck off, old man, you're not my dad!"
    "Prostituée" "Va te faire foutre, le vieux, t'es pas mon père !"

# game/events_mc.rpy:1246
translate french officeFirst_90d23aed:

    # oc_ "And you're lucky that I'm not..."
    oc_ "Et vous avez de la chance que je ne le sois pas..."

# game/events_mc.rpy:1247
translate french officeFirst_9df8f015:

    # Prostitute" "Pfft..."
    "Prostituée" "Pfft..."

# game/events_mc.rpy:1248
translate french officeFirst_88d25c77:

    # oc_ "Wait a minute..."
    oc_ "Attendez une minute..."

# game/events_mc.rpy:1249
translate french officeFirst_e654f32d:

    # oc_ "[pc]?"
    oc_ "[pc]?"

# game/events_mc.rpy:1251
translate french officeFirst_225fadf5_1:

    # pc "Huh?"
    pc "Huh ?"

# game/events_mc.rpy:1252
translate french officeFirst_9d393639:

    # pcthink "Is that... Johnson?!"
    pcthink "Est-ce que c'est... Johnson ?!"

# game/events_mc.rpy:1254
translate french officeFirst_24da5bb9:

    # oc "What the hell are {b}you{/b} doing here? I thought I'd never see you again?!"
    oc "Mais qu'est-ce que {b}tu{/b} fais ici ? J'ai cru que je ne te reverrais plus jamais ?!"

# game/events_mc.rpy:1255
translate french officeFirst_cbbfc35c:

    # pc "Yeah, likewise..."
    pc "Ouais, de même..."

# game/events_mc.rpy:1256
translate french officeFirst_423514d7:

    # oc "Come on, you know what I mean. When did you come back?"
    oc "Allez, tu sais ce que je veux dire. Quand es-tu revenu ?"

# game/events_mc.rpy:1257
translate french officeFirst_3d6db029:

    # pc "About a week ago..."
    pc "Il y a environ une semaine..."

# game/events_mc.rpy:1258
translate french officeFirst_60faf072:

    # oc "A week?!"
    oc "Une semaine ?!"

# game/events_mc.rpy:1259
translate french officeFirst_caaed063:

    # oc "A fuckin' week and you're in a cell already?"
    oc "Une putain de semaine et tu es déjà dans une cellule ?"

# game/events_mc.rpy:1260
translate french officeFirst_55b427e9:

    # pc "Hey, it's not my fault this time. I've changed a lot since back then."
    pc "Hé, ce n'est pas ma faute cette fois. J'ai beaucoup changé depuis cette époque."

# game/events_mc.rpy:1261
translate french officeFirst_602ec11f:

    # pcthink "Wow, that sounds like [m]..."
    pcthink "Wow, ça ressemble à [m]..."

# game/events_mc.rpy:1262
translate french officeFirst_cd85b673:

    # oc "...and you think I believe that? Seriously, you should know better, [pc]."
    oc "...et tu penses que je crois ça ? Sérieusement, tu devrais mieux le savoir, [pc]."

# game/events_mc.rpy:1263
translate french officeFirst_272dc344:

    # pc "It's true. There is this idiot... shit, I mean a co-worker of yours who is on me constantly, for whatever weird reason..."
    pc "C'est vrai. Il y a cet idiot... merde, je veux dire un de tes collègues qui est constamment sur moi, pour une raison bizarre..."

# game/events_mc.rpy:1264
translate french officeFirst_63b4d994:

    # oc "Who?"
    oc "Qui ?"

# game/events_mc.rpy:1265
translate french officeFirst_c657bee8:

    # pc "I don't know his name. He wasn't here back then... He's quite short, clumsy, {i}out of shape{/i}... and he thinks he's the new white \"Shaft\" or something..."
    pc "Je ne connais pas son nom. Il n'était pas là à l'époque... Il est assez court, maladroit, {i}hors norme{/i}... et il pense qu'il est le nouveau 'Shaft' blanc ou quelque chose comme ça..."

# game/events_mc.rpy:1266
translate french officeFirst_fde66fd5:

    # oc "...[w]."
    oc "...[w]..."

# game/events_mc.rpy:1267
translate french officeFirst_e71f2f2e:

    # oc "So that's why he wanted to have access to the old files."
    oc "C'est donc pour ça qu'il voulait avoir accès aux anciens fichiers."

# game/events_mc.rpy:1268
translate french officeFirst_5ebb18b5:

    # pc "Seriously, what's his problem?"
    pc "Sérieusement, c'est quoi son problème ?"

# game/events_mc.rpy:1269
translate french officeFirst_dcf7b7fd:

    # oc "..."
    new "[oc]..."

# game/events_mc.rpy:1271
translate french officeFirst_c2974f5c:

    # oc "Come on, let's get you outta here."
    oc "Allez, on va te sortir de là."

# game/events_mc.rpy:1273
translate french officeFirst_ca88187e:

    # You follow him to his office where he quickly fills out all the necessary paperwork."
    "Vous le suivez dans son bureau où il remplit rapidement tous les documents nécessaires."

# game/events_mc.rpy:1275
translate french officeFirst_a3dfc68d:

    # oc "Alright, that's it, you're free to go. Make sure that's the last time I have to see you here!"
    oc "Très bien, c'est tout, tu es libre de partir. Assures-toi que c'est la dernière fois que je te vois ici !"

# game/events_mc.rpy:1277
translate french officeFirst_2af139c3:

    # pc "If it was up to me I wouldn't be here in the first place... but seriously, thanks."
    pc "Si cela ne tenait qu'à moi, je ne serais pas ici en premier lieu... mais sérieusement, merci."

# game/events_mc.rpy:1279
translate french officeFirst_57276ab8:

    # w "Oi! What the..."
    w "Oi ! Qu'est-ce que..."

# game/events_mc.rpy:1280
translate french officeFirst_102c0d27:

    # oc "Ugh... bad timing..."
    oc "Ugh... mauvais timing..."

# game/events_mc.rpy:1281
translate french officeFirst_b568e141:

    # w "Johnson, what's going on here? What are you doing with my prisoner?"
    w "Johnson, qu'est-ce qui se passe ici ? Que faites-vous avec mon prisonnier ?"

# game/events_mc.rpy:1283
translate french officeFirst_9c60277f:

    # oc "First of all, [heshe] is not {b}your{/b} prisoner..."
    oc "Tout d'abord, [heshe] n'est pas {b}votre prisonnier{/b}..."

# game/events_mc.rpy:1284
translate french officeFirst_91fb484b:

    # oc "Secondly, you know as much as I do, that your accusations won't stand a chance in court. You should be happy that I'm doing the paperwork."
    oc "Deuxièmement, vous savez autant que moi que vos accusations n'auront aucune chance au tribunal. Vous devriez être heureux que je m'occupe de la paperasse."

# game/events_mc.rpy:1286
translate french officeFirst_d4377c33:

    # w "T-that's... that's harassment! Y-you only want to bully me!"
    w "C-c'est... c'est du harcèlement ! V-vous voulez seulement m'intimider !"

# game/events_mc.rpy:1287
translate french officeFirst_d581dd55:

    # oc "Sure, [w]... why don't you go back on the streets and watch the traffic..."
    oc "Bien sûr, [w]... pourquoi ne pas retourner dans les rues et regarder le trafic..."

# game/events_mc.rpy:1288
translate french officeFirst_c97e1600:

    # w "{b}You{/b}... I know that you're in cahoots over something... That one closed case, it's obvious that you're hiding something and I'm going to prove it!"
    w "{b}Vous{/b}... Je sais que vous êtes de mèche pour quelque chose... Cette affaire classée, il est évident que vous cachez quelque chose et je vais le prouver !"

# game/events_mc.rpy:1289
translate french officeFirst_5ee56c6d:

    # w "Say goodbye to your pension, Johnson!"
    w "Dites adieu à votre pension, Johnson !"

# game/events_mc.rpy:1291
translate french officeFirst_e61485cd:

    # oc "Jeez..."
    oc "Bon sang..."

# game/events_mc.rpy:1292
translate french officeFirst_c46df379:

    # pc "That doesn't sound good... this guy has some serious issues..."
    pc "Cela ne semble pas bon... ce type a de sérieux problèmes..."

# game/events_mc.rpy:1294
translate french officeFirst_57259f13:

    # oc "Don't worry about him, nobody here takes him seriously, and the case was closed a long time ago."
    oc "Ne t'inquiètes pas pour lui, personne ici ne le prend au sérieux, et l'affaire a été classée il y a longtemps."

# game/events_mc.rpy:1295
translate french officeFirst_3b866ef1:

    # pc "I don't want you to get into trouble because of me."
    pc "Je ne veux pas que tu aies des problèmes à cause de moi."

# game/events_mc.rpy:1296
translate french officeFirst_a68577fe:

    # oc "Better worry about yourself. Make sure that I don't have to see you again before my retirement."
    oc "Mieux vaut s'occuper de soi. Fais en sorte que je n'aie pas à te revoir avant ma retraite."

# game/events_mc.rpy:1297
translate french officeFirst_b121344f:

    # pc "Heh, can't be long, old man."
    pc "Hé, ça ne peut pas durer longtemps, mon vieux."

# game/events_mc.rpy:1298
translate french officeFirst_dc1eedda:

    # oc "Ha...{w=1} Ha...{w=1} Now move ya ass outta here!"
    oc "Ha...{w=1} Ha...{w=1} Maintenant bouge ton cul hors d'ici !"

# game/events_mc.rpy:1299
translate french officeFirst_59114b30:

    # pc "I'm gone already... thanks, Johnson..."
    pc "Je suis déjà parti... merci, Johnson..."

# game/events_mc.rpy:1301
translate french officeFirst_647852c1:

    # n "Your way back home is quite unspectacular, but you can't help but to think about how many times [oc] saved your ass when you were younger..."
    n "Votre retour à la maison est assez peu spectaculaire, mais vous ne pouvez vous empêcher de penser au nombre de fois où [oc] vous a sauvé les fesses quand vous étiez plus jeune...."

# game/events_mc.rpy:1302
translate french officeFirst_34b397f1:

    # n "A bit later."
    n "Un peu plus tard."

# game/events_mc.rpy:1308
translate french dinnerDay4Home_235607d8:

    # m "So, how was your day, Hun? You've been away longer than I thought you would be."
    m "Alors, comment s'est passée ta journée, Hun ? Tu as été absent plus longtemps que je ne le pensais."

# game/events_mc.rpy:1309
translate french dinnerDay4Home_b8ee6247:

    # pc "Uh, yeah nothing special... I met my new boss and we had a long conversation about work..."
    pc "Euh, oui rien de spécial... J'ai rencontré mon nouveau patron et nous avons eu une longue conversation sur le travail..."

# game/events_mc.rpy:1311
translate french dinnerDay4Home_3697c268:

    # m "Oh really? He was there on a Sunday?"
    m "Oh vraiment ? Il était là un dimanche ?"

# game/events_mc.rpy:1312
translate french dinnerDay4Home_85b8611b:

    # pc "Yeah, I was surprised as well."
    pc "Oui, j'ai été surpris aussi."

# game/events_mc.rpy:1313
translate french dinnerDay4Home_9b505c74:

    # m "Now I understand why you took so long."
    m "Maintenant je comprends pourquoi tu as mis si longtemps."

# game/events_mc.rpy:1314
translate french dinnerDay4Home_a6b055a9:

    # pcthink "Why would she be interested in how long I'm away? I didn't even mention how long I'd be gone..."
    pcthink "Pourquoi serait-elle intéressée par la durée de mon absence ? Je n'ai même pas mentionné combien de temps je serais parti..."

# game/events_mc.rpy:1316
translate french dinnerDay4Home_c819dd93:

    # m "Oh but, he doesn't expect you to work on weekends, too, does he?"
    m "Oh mais, il ne s'attend pas à ce que tu travailles aussi le week-end, n'est-ce pas ?"

# game/events_mc.rpy:1317
translate french dinnerDay4Home_8ffc4c76:

    # pc "No... as I said, I'm only expected to be there a few days per week."
    pc "Non... comme je l'ai dit, je ne suis censé être là que quelques jours par semaine."

# game/events_mc.rpy:1318
translate french dinnerDay4Home_afe32509:

    # pc "In fact, since we settled everything today, I don't even need to go there next week, but will start working from here tomorrow."
    pc "En fait, comme nous avons tout réglé aujourd'hui, je n'ai même pas besoin d'y aller la semaine prochaine, mais je commencerai à travailler d'ici demain."

# game/events_mc.rpy:1320
translate french dinnerDay4Home_63078123:

    # m "Oh, that's nice. But don't forget to take enough breaks in between."
    m "Oh, c'est bien. Mais n'oublie pas de faire suffisamment de pauses entre les deux."

# game/events_mc.rpy:1321
translate french dinnerDay4Home_7961594a:

    # pc "Yeah, sure, I'm not a workaholic, [m]..."
    pc "Oui, bien sûr, je ne suis pas un bourreau de travail, [m]..."

# game/events_mc.rpy:1323
translate french dinnerDay4Home_d408d503:

    # m "That's good to hear."
    m "C'est bon à entendre."

# game/events_mc.rpy:1324
translate french dinnerDay4Home_cb688308:

    # pcthink "Jeez, is she just acting or does she really want me to stay home..."
    pcthink "Bon sang, elle joue la comédie ou elle veut vraiment que je reste à la maison..."

# game/events_mc.rpy:1326
translate french dinnerDay4Home_d9a23300:

    # pcthink "...and why would she want me to be here?"
    pcthink "...et pourquoi voudrait-elle que je sois là ?"

# game/events_mc.rpy:1328
translate french dinnerDay4Home_72a33094:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:1329
translate french dinnerDay4Home_f423bdd0:

    # pcthink "Cute..."
    pcthink "Mignonne..."

# game/events_mc.rpy:1334
translate french homeday4night_8cf29fc6:

    # n "At night..."
    n "La nuit..."

# game/events_mc.rpy:1336
translate french homeday4night_88d5a7a4:

    # pcthink "Shit, maybe I should have used the opportunity to ask [m] some more questions when she was under the influence of the pills..."
    pcthink "Merde, j'aurais peut-être dû en profiter pour poser à [m] d'autres questions quand elle était sous l'emprise des pilules..."

# game/events_mc.rpy:1337
translate french homeday4night_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1338
translate french homeday4night_815350c3:

    # pcthink "...maybe tomorrow..."
    pcthink "...peut-être demain..."

# game/events_mc.rpy:1339
translate french homeday4night_f0cac90d_1:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1340
translate french homeday4night_9c027654:

    # pcthink "Hum... it's really late..."
    pcthink "Hum... il est vraiment tard..."

# game/events_mc.rpy:1341
translate french homeday4night_9ffe0b3c:

    # pcthink "e "should be here by now..."
    pcthink "e "devrait être là maintenant..."

# game/events_mc.rpy:1342
translate french homeday4night_f0cac90d_2:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1344
translate french homeday4night_eb300fc6:

    # pcthink "I wonder if she's embarrassed about what happened in her room earlier today..."
    pcthink "Je me demande si elle est gênée de ce qui s'est passé dans sa chambre plus tôt dans la journée...."

# game/events_mc.rpy:1346
translate french homeday4night_8a587bc9:

    # pcthink "Nah... I don't think so..."
    pcthink "Nan... Je ne pense pas..."

# game/events_mc.rpy:1347
translate french homeday4night_98679eca:

    # pcthink "She wasn't really shy about it, and she {b}did{/b} return the kiss after all..."
    pcthink "Elle n'était pas vraiment timide à ce sujet, et elle {b}a{/b} rendu le baiser après tout..."

# game/events_mc.rpy:1348
translate french homeday4night_45b128ac:

    # pcthink "It's probably [m] who told her to stay in her room again, even though... I haven't heard her talking to [e]..."
    pcthink "C'est probablement [m] qui lui a dit de rester dans sa chambre à nouveau, même si... je ne l'ai pas entendue parler à [e]..."

# game/events_mc.rpy:1350
translate french homeday4night_f0cac90d_3:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1352
translate french homeday4night_5b73bbbd:

    # pcthink "Anyway, I better get some sleep..."
    pcthink "Bref, je ferais mieux d'aller dormir..."

# game/events_mc.rpy:1357
translate french homeday4night_956ae696:

    # m "[bpc]! WHAT THE HELL DID YOU DO THIS TIME?" with hpunch
    m "[bpc]! QU'AS-TU FAIT CETTE FOIS-CI ?" with hpunch

# game/events_mc.rpy:1358
translate french homeday4night_5bd3bf2f:

    # oc "Calm down, [m]. It doesn't help to yell at [himher]."
    oc "Calme-toi, [m]. Ça ne sert à rien de crier sur [himher]."

# game/events_mc.rpy:1359
translate french homeday4night_8aee846a:

    # m "WHAT DO YOU KNOW? YOU DON'T HAVE TO LIVE WITH THIS..."
    m "QU'EST-CE QUE TU SAIS? TU N'AS PAS À VIVRE AVEC ÇA..."

# game/events_mc.rpy:1360
translate french homeday4night_97824f31:

    # oc "CALM DOWN!"
    oc "DU CALME!"

# game/events_mc.rpy:1361
translate french homeday4night_42a34ab2:

    # m "Humph... fine..."
    m "Humph... bien..."

# game/events_mc.rpy:1362
translate french homeday4night_7feca2c9:

    # m "So what did [heshe] do this time?"
    m "Alors, qu'a fait [heshe] cette fois-ci ?"

# game/events_mc.rpy:1363
translate french homeday4night_3c2af74d:

    # oc "[m], you're not making this any better! Sit down and think about who you're talking to!"
    oc "[m], tu n'arranges pas les choses ! Assieds-toi et pense de qui tu parles !"

# game/events_mc.rpy:1364
translate french homeday4night_e612037c:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:1365
translate french homeday4night_99f84bee:

    # m "I'm sorry, [oc]... [heshe]'s just..."
    m "Je suis désolé, [oc]... [heshe] est juste..."

# game/events_mc.rpy:1366
translate french homeday4night_149911e6:

    # m "I just can't stand [himher] any longer..."
    m "Je ne supporte plus [himher] ..."

# game/events_mc.rpy:1375
translate french d5morning_03946767:

    # pc "Yaaawn..."
    pc "Yaaawn..."

# game/events_mc.rpy:1376
translate french d5morning_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1377
translate french d5morning_ebe882ba:

    # pcthink "Damn dreams..."
    pcthink "Maudits rêves..."

# game/events_mc.rpy:1378
translate french d5morning_8f182d39:

    # pcthink "...I think that day was the last time I got into a fight with those two assholes from school again..."
    pcthink "...Je crois que ce jour-là, c'est la dernière fois que je me suis battu avec ces deux connards de l'école...."

# game/events_mc.rpy:1379
translate french d5morning_2179c941:

    # pcthink "I broke the nose of one of them and hit the other one with a cane."
    pcthink "J'ai cassé le nez de l'un d'eux et frappé l'autre avec une canne."

# game/events_mc.rpy:1380
translate french d5morning_117c625f:

    # pcthink "...I can't say I regret it, but it was really dumb..."
    pcthink "...Je ne peux pas dire que je le regrette, mais c'était vraiment stupide..."

# game/events_mc.rpy:1381
translate french d5morning_247fdc1b:

    # pcthink "Without [oc] I would've ended up in jail..."
    pcthink "Sans [oc] j'aurais fini en prison..."

# game/events_mc.rpy:1383
translate french d5morning_b0477cd7:

    # pcthink "I guess I wasn't the nicest guy around that time..."
    pcthink "Je suppose que je n'étais pas le gars le plus gentil à cette époque..."

# game/events_mc.rpy:1385
translate french d5morning_6ddfdaa0:

    # pcthink "I guess I wasn't the nicest girl around that time..."
    pcthink "Je suppose que je n'étais pas la plus gentille des filles à cette époque..."

# game/events_mc.rpy:1386
translate french d5morning_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1388
translate french d5morning_9f541c54:

    # pcthink "e "didn't come over..."
    pcthink "e "n'est pas venu..."

# game/events_mc.rpy:1389
translate french d5morning_c1686997:

    # pcthink "Hum..."
    pcthink "Hum..."

# game/events_mc.rpy:1391
translate french d5morning_453b4052:

    # n "You get up and make yourself ready for breakfast."
    n "Vous vous levez et vous vous préparez pour le petit-déjeuner."

# game/events_mc.rpy:1393
translate french d5morning_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1394
translate french d5morning_e4a69cb1:

    # pcthink "e "didn't look at me once yet..."
    pcthink "e "ne m'a pas encore regardé une seule fois..."

# game/events_mc.rpy:1395
translate french d5morning_72a33094:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:1396
translate french d5morning_367e6ba5:

    # pc "Is everything alright, [e]?"
    pc "Tout va bien, [e]?"

# game/events_mc.rpy:1399
translate french d5morning_52d6b73f:

    # e "Hn..."
    e "Hn..."

# game/events_mc.rpy:1404
translate french d5morning_744f56ce:

    # n "A bit later..."
    n "Un peu plus tard..."

# game/events_mc.rpy:1406
translate french d5morning_4a9406ec:

    # m "We are leaving, I've made you some extra food for the day, it's in the fridge."
    m "Nous partons, je t'ai préparé de la nourriture supplémentaire pour la journée, elle est dans le réfrigérateur."

# game/events_mc.rpy:1407
translate french d5morning_24aeb086:

    # pc "Thanks, [m]."
    pc "Merci, [m]."

# game/events_mc.rpy:1408
translate french d5morning_952a7e5d:

    # m "If you need something else, whatever it might be, don't hesitate and give me a call, okay, Hun?"
    m "Si tu as besoin d'autre chose, quoi que ce soit, n'hésites pas et appelles-moi, d'accord, Hun ?"

# game/events_mc.rpy:1409
translate french d5morning_61f77a4c:

    # pc "Sure."
    pc "Bien sûr."

# game/events_mc.rpy:1411
translate french d5morning_ec1a6290:

    # pcthink "Why is she so nice to me today? Huh? ...what's up with [e]?!"
    pcthink "Pourquoi est-elle si gentille avec moi aujourd'hui ? Huh ? ...qu'est-ce qui se passe avec [e]?!"

# game/events_mc.rpy:1413
translate french d5morning_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh ?"

# game/events_mc.rpy:1415
translate french d5morning_06a6d364:

    # pc "Whoa!"
    pc "Whoua !"

# game/events_mc.rpy:1418
translate french d5morning_58feae78:

    # pcthink "Wow, [e]..."
    pcthink "Wow, [e]..."

# game/events_mc.rpy:1420
translate french d5morning_72a33094_1:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:1421
translate french d5morning_4cd516c4:

    # pc "Heh... you should probably go, or you'll be late... We can talk later, okay?"
    pc "Hé... tu devrais probablement y aller, ou tu vas être en retard... Nous pouvons parler plus tard, d'accord ?"

# game/events_mc.rpy:1422
translate french d5morning_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:1424
translate french d5morning_5ab369aa:

    # pcthink "Wow, that came out of nowhere..."
    pcthink "Wow, c'est sorti de nulle part..."

# game/events_mc.rpy:1426
translate french d5morning_20ad7ae7:

    # pcthink "Err... Looks like I'm not the only one who's surprised."
    pcthink "Err... On dirait que je ne suis pas le seul à être surpris."

# game/events_mc.rpy:1427
translate french d5morning_e612037c:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:1429
translate french d5morning_4f0f31e7:

    # pcthink "Phew... [m] didn't say a word, but that look on her face... I wonder what she was thinking."
    pcthink "Ouf... [m] n'a pas dit un mot, mais ce regard sur son visage... Je me demande à quoi elle pensait."

# game/events_mc.rpy:1435
translate french d5morning_f5618bbd:

    # pcthink "Anyway, I should probably start working on the project for [boss], I have a lot to do and I don't have that much time..."
    pcthink "De toute façon, je devrais probablement commencer à travailler sur le projet de [boss], j'ai beaucoup à faire et je n'ai pas beaucoup de temps..."

# game/events_mc.rpy:1437
translate french d5morning_a76f8e5c:

    # n "You spend the rest of the morning and afternoon working on the project, only taking short breaks to go to the bathroom or get some snacks from the kitchen."
    n "Vous passez le reste de la matinée et de l'après-midi à travailler sur le projet, ne prenant que de courtes pauses pour aller aux toilettes ou prendre des encas dans la cuisine."

# game/events_mc.rpy:1439
translate french d5morning_8f21880a:

    # pcthink "Ugh, this won't work either..."
    pcthink "Ugh, ça ne marchera pas non plus..."

# game/events_mc.rpy:1440
translate french d5morning_43080636:

    # pcthink "Damn, where is the error?"
    pcthink "Mince, où est l'erreur ?"

# game/events_mc.rpy:1441
translate french d5morning_2ab2f07a:

    # pcthink "...gaaawd, I need a break..."
    pcthink "...booonnn saaannnggg, j'ai besoin d'une pause..."

# game/events_mc.rpy:1443
translate french d5morning_4dc0d086:

    # pcthink "I think I've heard [e] and [m] coming back a while ago and it sounded like there was someone else as well..."
    pcthink "Je crois avoir entendu [e] et [m] revenir il y a un moment et il semblait qu'il y avait aussi quelqu'un d'autre..."

# game/events_mc.rpy:1445
translate french d5morning_c9c7cbd9:

    # pcthink "Maybe I should just stop for today..."
    pcthink "Je devrais peut-être arrêter pour aujourd'hui..."

# game/events_mc.rpy:1449
translate french porn_vr_4_f834cc31:

    # n "A while later..."
    n "Un peu plus tard..."

# game/events_mc.rpy:1451
translate french porn_vr_4_7cd59846:

    # Voice" "Mphh... mphhh..."
    "Voix" "Mphh... mphhh..."

# game/events_mc.rpy:1456
translate french porn_vr_4_594132e1:

    # pcthink "Gawd, I love these girls!"
    pcthink "Bon sang, j'aime ces filles !"

# game/events_mc.rpy:1458
translate french porn_vr_4_010b482c:

    # pcthink "Too bad there's no girl on girl action..."
    pcthink "Dommage qu'il n'y ait pas d'action entre filles..."

# game/events_mc.rpy:1460
translate french porn_vr_4_e499de78:

    # Girl 2" "Are you about to cum babe?"
    "Fille 2" "Tu es sur le point de jouir, bébé ?"

# game/events_mc.rpy:1461
translate french porn_vr_4_794a11fa:

    # pcthink "Hehe, yeah spray that load all over them!"
    pcthink "Héhé, ouais pulvériser cette charge partout sur elles !"

# game/events_mc.rpy:1465
translate french porn_vr_4_029af3e9:

    # pc "What!? Are you fucking serious? That's even worse than in porn games..."
    pc "Quoi !? Vous êtes sérieuses, putain ? C'est encore pire que dans les jeux pornos..."

# game/events_mc.rpy:1467
translate french porn_vr_4_90a099af:

    # pcthink "Fuck..."
    pcthink "Putain..."

# game/events_mc.rpy:1468
translate french porn_vr_4_a14752ab:

    # pcthink "...that was... unsatisfying..."
    pcthink "...c'était... insatisfaisant..."

# game/events_mc.rpy:1470
translate french porn_vr_4_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1471
translate french porn_vr_4_4bcfd45d:

    # pcthink "Hum... shit, isn't it dinner time already?"
    pcthink "Hum... merde, ce n'est pas déjà l'heure du dîner ?"

# game/events_mc.rpy:1472
translate french porn_vr_4_bd478faf:

    # pcthink "I guess I'll have a look..."
    pcthink "Je suppose que je vais jeter un coup d'œil..."

# game/events_mc.rpy:1474
translate french porn_vr_4_91617c2e:

    # pcthink "It's funny how quick I got used to [m] making dinner every evening... or breakfast..."
    pcthink "C'est drôle comme je me suis vite habituée à [m] qui fait le dîner tous les soirs... ou le petit déjeuner..."

# game/events_mc.rpy:1477
translate french porn_vr_4_1fd0abf0:

    # pcthink "Huh? Isn't [m] making dinner today?"
    pcthink "Huh ? Est-ce que [m] ne fait pas le dîner aujourd'hui ?"

# game/events_mc.rpy:1479
translate french porn_vr_4_43ebd0bd:

    # pcthink "[e]?"
    pcthink "[e]?"

# game/events_mc.rpy:1484
translate french d6morning_180d7c36:

    # pc "YAAAAWN..."
    pc "YAAAAWN..."

# game/events_mc.rpy:1487
translate french d6morning_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1488
translate french d6morning_ca66ede7:

    # pcthink "e "is gone again..."
    pcthink "e "a encore disparu..."

# game/events_mc.rpy:1490
translate french d6morning_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1492
translate french d6morning_2edac4fd:

    # pc "Wait!"
    pc "Attends !"

# game/events_mc.rpy:1493
translate french d6morning_04f5c868:

    # pcthink "Did that really happen last night?"
    pcthink "C'est vraiment arrivé la nuit dernière ?"

# game/events_mc.rpy:1494
translate french d6morning_71a5f304:

    # pcthink "...did we really...?"
    pcthink "...avons-nous vraiment... ?"

# game/events_mc.rpy:1495
translate french d6morning_a69377ef_2:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1496
translate french d6morning_4cc63f49:

    # pcthink "T-that's impossible, she's my little sister..."
    pcthink "C-c'est impossible, c'est ma petite sœur..."

# game/events_mc.rpy:1497
translate french d6morning_b4c72b6d:

    # pcthink "Err, I mean, she's not, but still..."
    pcthink "Err, je veux dire, elle ne l'est pas, mais quand même..."

# game/events_mc.rpy:1499
translate french d6morning_2eece04e:

    # pcthink "...and wouldn't she be here if we..."
    pcthink "...et ne serait-elle pas là si nous..."

# game/events_mc.rpy:1500
translate french d6morning_d022dadb:

    # pcthink "Wait, what time is it?"
    pcthink "Attends, quelle heure est-il ?"

# game/events_mc.rpy:1503
translate french d6morning_e072fbd9:

    # pcthink "Ugh, shit, I overslept... dammit!"
    pcthink "Ugh, merde, j'ai trop dormi... merde !"

# game/events_mc.rpy:1504
translate french d6morning_0e85c611:

    # pcthink "I better get up, I still have a lot of work to do..."
    pcthink "Je ferais mieux de me lever, j'ai encore beaucoup de travail à faire..."

# game/events_mc.rpy:1506
translate french d6morning_690b379a:

    # n "You get up, make yourself ready and start working on the project again for a while."
    n "Vous vous levez, vous vous préparez et recommencez à travailler sur le projet pendant un certain temps."

# game/events_mc.rpy:1508
translate french d6morning_9ef2adad:

    # pcthink "There we go... this should work nicely..."
    pcthink "Voilà... cela devrait bien fonctionner..."

# game/events_mc.rpy:1509
translate french d6morning_263d2d8e:

    # pc "Ugh...yaaawn..."
    pc "Ugh...yaaawn..."

# game/events_mc.rpy:1510
translate french d6morning_4cdbbc35:

    # pcthink "I could use a break..."
    pcthink "Je pourrais faire une pause..."

# game/events_mc.rpy:1511
translate french d6morning_8581051f:

    # pcthink "Maybe I should take a walk... get some fresh air... get a coffee..."
    pcthink "Je devrais peut-être faire une promenade... prendre l'air... prendre un café..."

# game/events_mc.rpy:1513
translate french d6morning_56f01959:

    # pcthink "...coffee... I could visit the girl... [h] at the cafe."
    pcthink "...café... Je pourrais rendre visite à la fille... [h] au café."

# game/events_mc.rpy:1515
translate french d6morning_7bc8bca3:

    # pcthink "Oh right, she gave me her number, I totally forgot about that!"
    pcthink "Ah oui, elle m'a donné son numéro, j'avais complètement oublié ça !"

# game/events_mc.rpy:1517
translate french d6morning_88320126:

    # pcthink "Hum... should I call her?"
    pcthink "Hum... Je dois l'appeler ?"

# game/events_mc.rpy:1522
translate french d6morning_be1e64d5:

    # pcthink "Yeah, she's cute... and..."
    pcthink "Oui, elle est mignonne... et..."

# game/events_mc.rpy:1523
translate french d6morning_fa2a0c40:

    # pcthink "... she seems to be the only normal girl I've met since I'm back home..."
    pcthink "... elle semble être la seule fille normale que j'ai rencontré depuis que je suis de retour à la maison..."

# game/events_mc.rpy:1525
translate french d6morning_19b50a19:

    # pcthink "I'm going to call her, let's see where this is going."
    pcthink "Je vais l'appeler, on va voir où ça va nous mener."

# game/events_mc.rpy:1527
translate french d6morning_a69377ef_3:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1529
translate french d6morning_227eb657:

    # h "Yes?"
    h "Oui ?"

# game/events_mc.rpy:1530
translate french d6morning_be61a613:

    # pc "Hey, it's me, [pc], the one from the..."
    pc "Hé, c'est moi, [pc], celui du..."

# game/events_mc.rpy:1531
translate french d6morning_7e9d795c:

    # h "Oh, hey, [pc]! Hi! How are you?! How have you been?"
    h "Oh, hé, [pc]! Salut ! Comment vas-tu ?! Comment allez-vous ?"

# game/events_mc.rpy:1532
translate french d6morning_f67a852d:

    # pc "Err, I'm good, how are you?"
    pc "Err, je vais bien, et toi ?"

# game/events_mc.rpy:1533
translate french d6morning_7fa6ffc2:

    # h "Good, I'm really good, I've been waiting for... for you to show up at the cafe again, haha."
    h "Bien, je vais vraiment bien, j'attendais que... que tu te montres à nouveau au café, haha."

# game/events_mc.rpy:1534
translate french d6morning_02b345a1:

    # pc "Well I was about to go for a walk and get a coffee, are you there?"
    pc "Eh bien, j'étais sur le point de faire une promenade et de prendre un café, es-tu là ?"

# game/events_mc.rpy:1535
translate french d6morning_63ebac69:

    # h "Yes! Of course! Oh you mean at the cafe, right? Haha."
    h "Oui ! Bien sûr ! Oh, tu veux dire au café, non ? Haha."

# game/events_mc.rpy:1536
translate french d6morning_42feb7d0:

    # pc "Yeah. Aren't you working today?"
    pc "Ouais. Tu ne travailles pas aujourd'hui ?"

# game/events_mc.rpy:1537
translate french d6morning_7746410b:

    # h "I-I am, haha. Of course, I'll be there, just come over whenever you want!"
    h "Je-je le suis, haha. Bien sûr, je serai là, viens quand tu veux !"

# game/events_mc.rpy:1538
translate french d6morning_812a8bad:

    # pc "Whenever I want?"
    pc "Quand je veux ?"

# game/events_mc.rpy:1539
translate french d6morning_30ed2430:

    # h "Yes, sure! Ehm... I mean, during working hours of course, haha... I mean... yes working hours... haha."
    h "Oui, bien sûr ! Ehm... Je veux dire, pendant les heures de travail bien sûr, haha... Je veux dire... oui les heures de travail... haha."

# game/events_mc.rpy:1540
translate french d6morning_37bc2c3c:

    # pc "Heh, alright, I'll be there soon, reserve a table for me."
    pc "Heu, d'accord, j'arrive bientôt, réserves-moi une table."

# game/events_mc.rpy:1542
translate french d6morning_d3c3bf69:

    # h "Of course, sir! Haha."
    h "Bien sûr, monsieur ! Haha."

# game/events_mc.rpy:1544
translate french d6morning_bad8f7ec:

    # h "Of course, ma'am! Haha."
    h "Bien sûr, madame ! Haha."

# game/events_mc.rpy:1545
translate french d6morning_7683043e:

    # pc "Haha, later, [h]."
    pc "Haha, à plus tard, [h]."

# game/events_mc.rpy:1546
translate french d6morning_c047c425:

    # h "Later, [pc]."
    h "A plus tard, [pc]."

# game/events_mc.rpy:1548
translate french d6morning_5329c3c2:

    # n "You wait for a second before hanging up, and you're sure that she didn't hang up before you."
    n "Vous attendez une seconde avant de raccrocher, et vous êtes sûr qu'elle n'a pas raccroché avant vous."

# game/events_mc.rpy:1550
translate french d6morning_b4ec99ba:

    # pcthink "Damn, she's cute."
    pcthink "Merde, elle est mignonne."

# game/events_mc.rpy:1553
translate french d6morning_9cbcc245:

    # pcthink "Nah, I just want to get a coffee..."
    pcthink "Nan, je veux juste prendre un café..."

# game/events_mc.rpy:1554
translate french d6morning_731706d8:

    # pcthink "Alright, let's go!"
    pcthink "Très bien, allons-y !"

# game/events_mc.rpy:1563
translate french d6_backhome_87c6d927:

    # pcthink "Alright..."
    pcthink "Très bien..."

# game/events_mc.rpy:1565
translate french d6_backhome_d7e55587:

    # pcthink "Time to get back to work... well, at least for a bit, we don't want to overdo it."
    pcthink "Il est temps de se remettre au travail... enfin, du moins pour un peu, nous ne voulons pas en faire trop."

# game/events_mc.rpy:1568
translate french d6_backhome_33b1335e:

    # pcthink "Huh? Why is my computer on?"
    pcthink "Huh ? Pourquoi mon ordinateur est-il allumé ?"

# game/events_mc.rpy:1569
translate french d6_backhome_1e9727dd:

    # pcthink "Didn't I shut it down?"
    pcthink "Ne l'ai-je pas éteint ?"

# game/events_mc.rpy:1571
translate french d6_backhome_ca581621:

    # pcthink "What the..."
    pcthink "Qu'est-ce que..."

# game/events_mc.rpy:1572
translate french d6_backhome_0e861919:

    # pcthink "Did I...?"
    pcthink "Est-ce que j'ai... ?"

# game/events_mc.rpy:1573
translate french d6_backhome_128634ca:

    # pcthink "No I didn't search for this!"
    pcthink "Non, je n'ai pas cherché cela !"

# game/events_mc.rpy:1574
translate french d6_backhome_d4c69f41:

    # pcthink "Did [e] use my computer again?!"
    pcthink "Est-ce que [e] a encore utilisé mon ordinateur ?!"

# game/events_mc.rpy:1575
translate french d6_backhome_ba2f9118:

    # pcthink "Incest, huh?"
    pcthink "Inceste, hein ?"

# game/events_mc.rpy:1576
translate french d6_backhome_81c2eddc:

    # pcthink "Interesting..."
    pcthink "Intéressant..."

# game/events_mc.rpy:1577
translate french d6_backhome_2624f60c:

    # pcthink "A little bit late now though..."
    pcthink "Un peu tard maintenant mais..."

# game/events_mc.rpy:1578
translate french d6_backhome_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1594
translate french d7_wakeup_8bf6dc88:

    # n "*Knock* *Knock*"
    n "*Toc* *Toc*"

# game/events_mc.rpy:1597
translate french d7_wakeup_f151517c:

    # pc "Uh..."
    pc "Euh..."

# game/events_mc.rpy:1599
translate french d7_wakeup_910fd392:

    # j "Unn..."
    j "Unn..."

# game/events_mc.rpy:1601
translate french d7_wakeup_6376492b:

    # m "{size=-10}[e], are you there? Come on, we need to get going soon!{/size}"
    m "{size=-10}[e], tu es là ? Aller, on doit y aller bientôt!{/size}"

# game/events_mc.rpy:1602
translate french d7_wakeup_8d4b0c8b:

    # j "Oh shit!"
    j "Oh merde !"

# game/events_mc.rpy:1604
translate french d7_wakeup_dabeb1e2:

    # n "*Knock* *Knock* *Knock*"
    n "*Toc* *Toc* *Toc*"

# game/events_mc.rpy:1606
translate french d7_wakeup_1824a53c:

    # j "We totally overslept."
    j "Nous avons totalement dormi trop longtemps."

# game/events_mc.rpy:1607
translate french d7_wakeup_4865e636:

    # pc "Yeah, damn it!"
    pc "Ouais, bon sang !"

# game/events_mc.rpy:1608
translate french d7_wakeup_2c2639dc:

    # m "{size=-10}[pc]?{/size}"
    m "{size=-10}[pc]?{/size}"

# game/events_mc.rpy:1610
translate french d7_wakeup_ab857d04:

    # n " *Knock* *Knock*"
    n " *Toc* *Toc*"

# game/events_mc.rpy:1612
translate french d7_wakeup_dc75fae1:

    # m "{size=-10}I'm coming in!{/size}"
    m "{size=-10}J'arrive!{/size}"

# game/events_mc.rpy:1613
translate french d7_wakeup_62dd52c5:

    # j "Now we have a problem..."
    j "Maintenant nous avons un problème..."

# game/events_mc.rpy:1614
translate french d7_wakeup_7a9c7117:

    # e "Unn..."
    e "Unn..."

# game/events_mc.rpy:1615
translate french d7_wakeup_225fadf5:

    # pc "Huh?"
    pc "Huh ?"

# game/events_mc.rpy:1617
translate french d7_wakeup_cb44f5d6:

    # pc "What the?! [e]?" with hpunch
    pc "Qu'est-ce que c'est ?! [e]?" with hpunch

# game/events_mc.rpy:1618
translate french d7_wakeup_267417d2:

    # pcthink "Shit, shit, shit, this is bad! I need to do something!"
    pcthink "Merde, merde, merde, c'est mauvais ! Je dois faire quelque chose !"

# game/events_mc.rpy:1628
translate french d7_wakeup_9c8aa1a8:

    # j "WHAA!"
    j "WHAA !"

# game/events_mc.rpy:1631
translate french d7_wakeup_611f0752:

    # m "What the!?"
    m "Qu'est-ce que c'est que ça ?"

# game/events_mc.rpy:1632
translate french d7_wakeup_4ef843b7:

    # j "Auu..."
    j "Auu..."

# game/events_mc.rpy:1633
translate french d7_wakeup_9b3d3223:

    # m "[j]?!"
    m "[j]?!"

# game/events_mc.rpy:1635
translate french d7_wakeup_a9ccd390:

    # e "NN!"
    e "NN !"

# game/events_mc.rpy:1637
translate french d7_wakeup_dcb808e1:

    # m "[e]?!"
    m "[e]?!"

# game/events_mc.rpy:1638
translate french d7_wakeup_fc74700e:

    # pcthink "Oh shit, that was stupid!"
    pcthink "Oh merde, c'était stupide !"

# game/events_mc.rpy:1643
translate french d7_wakeup_c39c25e8:

    # pcthink "Right, I can't hide both of em!"
    pcthink "Bon, je ne peux pas les cacher tous les deux !"

# game/events_mc.rpy:1644
translate french d7_wakeup_c55aed56:

    # e "Ehh?!"
    e "Ehh ?!"

# game/events_mc.rpy:1645
translate french d7_wakeup_1bb9fdc8:

    # pc "{size=-10}Hush, [e]!{/size}"
    pc "{size=-10}Chut, [e]!{/size}"

# game/events_mc.rpy:1646
translate french d7_wakeup_8bee63b5:

    # j "WHAAA!"
    j "WOUAAA !"

# game/events_mc.rpy:1649
translate french d7_wakeup_4d1fb2a5:

    # pcthink "Oh shit, I forgot that [j] is naked."
    pcthink "Oh merde, j'ai oublié que [j] est nue."

# game/events_mc.rpy:1651
translate french d7_wakeup_9b3d3223_1:

    # m "[j]?!"
    m "[j]?!"

# game/events_mc.rpy:1652
translate french d7_wakeup_15afcc2b:

    # j "Uhh... morning [m]... hehe..."
    j "Uhh... matin [m]... héhé..."

# game/events_mc.rpy:1653
translate french d7_wakeup_70c45725:

    # m "What on earth is going on here?"
    m "Que diable se passe-t-il ici ?"

# game/events_mc.rpy:1654
translate french d7_wakeup_9e0cbd4c:

    # pc "Err... this is not what it's... uhh..."
    pc "Err... ce n'est pas ce que c'est... uhh..."

# game/events_mc.rpy:1655
translate french d7_wakeup_a08e5c45:

    # pcthink "Shit!"
    pcthink "Merde !"

# game/events_mc.rpy:1656
translate french d7_wakeup_745d2ec1:

    # pc "Well it {b}{i}is{/i}{/b} what it looks like..."
    pc "Eh bien il {b}{i}c'est{/i}{/b} ce que ça donne..."

# game/events_mc.rpy:1657
translate french d7_wakeup_9e1b2e8a:

    # m "Why didn't you tell me?!"
    m "Pourquoi tu ne m'as rien dit ?!"

# game/events_mc.rpy:1658
translate french d7_wakeup_24359880:

    # j "Sorry, that's my fault..."
    j "Désolé, c'est ma faute..."

# game/events_mc.rpy:1659
translate french d7_wakeup_38fa5992:

    # m "Uhm..."
    m "Uhm..."

# game/events_mc.rpy:1660
translate french d7_wakeup_c630047d:

    # pcthink "Shit, I hope she didn't see [e]!"
    pcthink "Merde, j'espère qu'elle n'a pas vue [e]!"

# game/events_mc.rpy:1661
translate french d7_wakeup_e13f5324:

    # m "You... you haven't seen [e], have you?"
    m "Vous... vous n'avez pas vu [e], n'est-ce pas ?"

# game/events_mc.rpy:1662
translate french d7_wakeup_3aa1a87d:

    # pcthink "Phew..."
    pcthink "Ouf..."

# game/events_mc.rpy:1663
translate french d7_wakeup_fda5f912:

    # pc "Err... no... no I haven't..."
    pc "Err... non... non je n'ai pas..."

# game/events_mc.rpy:1665
translate french d7_wakeup_76a7757e:

    # j "I think I've heard her opening her door a while ago."
    j "Je crois l'avoir entendue ouvrir sa porte il y a un moment."

# game/events_mc.rpy:1667
translate french d7_wakeup_10b9afd3:

    # m "Umm... okay..."
    m "Umm... ok..."

# game/events_mc.rpy:1668
translate french d7_wakeup_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1669
translate french d7_wakeup_7e3552b4:

    # j "Hehe..."
    j "Héhé..."

# game/events_mc.rpy:1670
translate french d7_wakeup_e7673a6d:

    # m "Does she... know about..."
    m "Est-elle... au courant de..."

# game/events_mc.rpy:1671
translate french d7_wakeup_ea4be5c8:

    # m "...this?"
    m "...ceci ?"

# game/events_mc.rpy:1672
translate french d7_wakeup_bdcd7a62:

    # j "Umm... yeah, I told her, hehe."
    j "Umm... ouais, je lui ai dit, héhé."

# game/events_mc.rpy:1673
translate french d7_wakeup_58714e34:

    # m "...okay..."
    m "...ok..."

# game/events_mc.rpy:1674
translate french d7_wakeup_b4fa3fc0:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:1675
translate french d7_wakeup_2637c315:

    # m "Uhh... right, well I... I'll make some breakfast..."
    m "Uhh... d'accord, bien je... Je vais faire un petit déjeuner..."

# game/events_mc.rpy:1680
translate french d7_wakeup_ca33a6dd:

    # j "[e], you better hurry!"
    j "[e], tu ferais mieux de te dépêcher !"

# game/events_mc.rpy:1682
translate french d7_wakeup_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:1684
translate french d7_wakeup_79c731b2:

    # n "[e] jumps up and quickly runs out the balcony."
    n "[e] saute et s'enfuit rapidement par le balcon."

# game/events_mc.rpy:1686
translate french d7_wakeup_7fe431b0:

    # pc "Huh? [e], wait!"
    pc "Huh ? [e], attends !"

# game/events_mc.rpy:1688
translate french d7_wakeup_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1689
translate french d7_wakeup_fdacea98:

    # pc "Damn, I hope [m] doesn't see her like this..."
    pc "Merde, j'espère que [m] ne la voit pas comme ça..."

# game/events_mc.rpy:1691
translate french d7_wakeup_b4fa3fc0_1:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:1694
translate french d7_wakeup_f78b1475:

    # m "Are you awake, Hun?"
    m "Tu es réveillé, Hun ?"

# game/events_mc.rpy:1695
translate french d7_wakeup_6804df5a:

    # pc "...ugh... Yeah, I'm awake..."
    pc "...ugh... Ouais, je suis réveillé..."

# game/events_mc.rpy:1697
translate french d7_wakeup_f422d8e0:

    # m "Good morning, Hun."
    m "Bonjour, Hun."

# game/events_mc.rpy:1698
translate french d7_wakeup_f17dbea8:

    # pc "...what's up, [m]?"
    pc "...quoi de neuf, [m]?"

# game/events_mc.rpy:1700
translate french d7_wakeup_90c961dc:

    # m "Oh, I thought I might ask if you want some breakfast? I hope that's okay? Did I wake you up, Hun?"
    m "Oh, j'ai pensé que je pourrais demander si tu veux un petit déjeuner ? J'espère que c'est bon ? Je t'ai réveillé, Hun ?"

# game/events_mc.rpy:1702
translate french d7_wakeup_6e81f568:

    # m "Well, breakfast is ready, and you have some work to do, right?"
    m "Eh bien, le petit déjeuner est prêt, et tu as du travail à faire, non ?"

# game/events_mc.rpy:1703
translate french d7_wakeup_2ff46816:

    # pc "Uhn... I'm coming..."
    pc "Uhn... J'arrive..."

# game/events_mc.rpy:1704
translate french d7_wakeup_672f5c8a:

    # m "Alright~"
    m "Très bien~"

# game/events_mc.rpy:1706
translate french d7_wakeup_e308e089:

    # pcthink "Damn... she's almost acting as if she's my real mother..."
    pcthink "Merde... elle agit presque comme si elle était ma vraie mère..."

# game/events_mc.rpy:1716
translate french d7_breakfast_1a15478a:

    # pcthink "Looks like [j] is gone already..."
    pcthink "On dirait que [j] est déjà partie..."

# game/events_mc.rpy:1717
translate french d7_breakfast_58626add:

    # m "So, what are your plans today, Hun?"
    m "Alors, quels sont tes plans aujourd'hui, Hun ?"

# game/events_mc.rpy:1718
translate french d7_breakfast_2c634384:

    # pc "Err... work, I guess..."
    pc "Err... travail, je suppose..."

# game/events_mc.rpy:1719
translate french d7_breakfast_2cbba7f5:

    # m "Oh, I bet you're making good progress?"
    m "Oh, je parie que tu fais de bons progrès ?"

# game/events_mc.rpy:1720
translate french d7_breakfast_7816c0f8:

    # pc "Yeah, kind of."
    pc "Oui, en quelque sorte."

# game/events_mc.rpy:1721
translate french d7_breakfast_d408d503:

    # m "That's good to hear."
    m "C'est bon à entendre."

# game/events_mc.rpy:1723
translate french d7_breakfast_d8728d98:

    # n "You continue to make small talk for a bit until [m] and [e] get ready to leave..."
    n "Vous continuez à faire la conversation pendant un moment jusqu'à ce que [m] et [e] se préparent à partir..."

# game/events_mc.rpy:1726
translate french d7_breakfast_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1727
translate french d7_breakfast_e612037c:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:1728
translate french d7_breakfast_b4fa3fc0:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:1730
translate french d7_breakfast_7b551003:

    # pcthink "Shit, nobody is saying anything..."
    pcthink "Merde, personne ne dit rien..."

# game/events_mc.rpy:1731
translate french d7_breakfast_a29c1aed:

    # pcthink "Looks like I really fucked up..."
    pcthink "On dirait que j'ai vraiment merdé..."

# game/events_mc.rpy:1732
translate french d7_breakfast_52d0cc61:

    # pcthink "Everybody seems to be mad at me..."
    pcthink "Tout le monde semble être en colère contre moi..."

# game/events_mc.rpy:1733
translate french d7_breakfast_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1734
translate french d7_breakfast_bb8fff34:

    # pc "So, uh..."
    pc "Donc, euh..."

# game/events_mc.rpy:1739
translate french d7_breakfast_801242f5:

    # pc "...about what happened..."
    pc "...à propos de ce qui s'est passé..."

# game/events_mc.rpy:1742
translate french d7_breakfast_971f3699:

    # pc "Looks like it's going to be a nice day today..."
    pc "On dirait qu'il va faire beau aujourd'hui..."

# game/events_mc.rpy:1745
translate french d7_breakfast_594f9961:

    # m "Humph... don't you think you should apologize to someone?"
    m "Humph... tu ne penses pas que tu devrais t'excuser auprès de quelqu'un ?"

# game/events_mc.rpy:1748
translate french d7_breakfast_91375c36:

    # j "Pff, is that all you have to say?"
    j "Pff, c'est tout ce que tu as à dire ?"

# game/events_mc.rpy:1750
translate french d7_breakfast_58a1eaba:

    # m "That was not nice, [pc]."
    m "Ce n'était pas gentil, [pc]."

# game/events_mc.rpy:1752
translate french d7_breakfast_b1fed97d:

    # j "Yeah, that really hurt."
    j "Ouais, ça fait vraiment mal."

# game/events_mc.rpy:1753
translate french d7_breakfast_9b940997:

    # pc "I'm sorry... I was surprised and just reacted..."
    pc "Je suis désolé... J'ai été surpris et j'ai juste réagi..."

# game/events_mc.rpy:1754
translate french d7_breakfast_87c73e4e:

    # j "...humpf... so it was \"just\" a reaction, huh?"
    j "...humpf... donc c'était 'juste' une réaction, hein ?"

# game/events_mc.rpy:1755
translate french d7_breakfast_5959fb60:

    # pc "Yes, it wasn't on purpose!"
    pc "Oui, ce n'était pas volontaire !"

# game/events_mc.rpy:1756
translate french d7_breakfast_e7062f61:

    # j "Humph..."
    j "Humph..."

# game/events_mc.rpy:1759
translate french d7_breakfast_9f9cc93e:

    # j "So uhh... I hope you don't mind me having spent the night here, [m]."
    j "Donc euh... J'espère que ça ne te dérange pas que j'ai passé la nuit ici, [m]."

# game/events_mc.rpy:1760
translate french d7_breakfast_e612037c_1:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:1762
translate french d7_breakfast_294a0cae:

    # m "Well... you should have asked me first."
    m "Eh bien... vous auriez dû me demander d'abord."

# game/events_mc.rpy:1764
translate french d7_breakfast_15208453:

    # j "I know, but... it was kinda unexpected."
    j "Je sais, mais... c'était un peu inattendu."

# game/events_mc.rpy:1766
translate french d7_breakfast_0dc14203:

    # m "Unexpected?"
    m "Inattendu ?"

# game/events_mc.rpy:1767
translate french d7_breakfast_176cf8c8:

    # j "Yeah, hehe."
    j "Ouais, héhé."

# game/events_mc.rpy:1768
translate french d7_breakfast_e612037c_2:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:1770
translate french d7_breakfast_941a283f:

    # pcthink "Hum... looks like [j] doesn't want to tell her what really happened..."
    pcthink "Hum... on dirait que [j] ne veut pas lui dire ce qui s'est réellement passé..."

# game/events_mc.rpy:1772
translate french d7_breakfast_f8ed8d86:

    # j "But I'll stay at home tonight, so [pc]'s bed will be free, hehe."
    j "Mais je reste chez moi ce soir, donc le lit de [pc] sera libre, héhé."

# game/events_mc.rpy:1773
translate french d7_breakfast_c648d533:

    # pcthink "Uh-Oh..."
    pcthink "Uh-Oh..."

# game/events_mc.rpy:1775
translate french d7_breakfast_c946a8f3:

    # j "[e] can have her big [bs] all for herself again, or maybe even..."
    j "[e] peut à nouveau avoir son grand [bs] pour elle toute seule, ou peut-être même..."

# game/events_mc.rpy:1777
translate french d7_breakfast_04348ff1:

    # m "[j]!"
    m "[j]!"

# game/events_mc.rpy:1778
translate french d7_breakfast_aeeb5622:

    # m "That's is not funny!"
    m "C'est pas drôle !"

# game/events_mc.rpy:1779
translate french d7_breakfast_4686d521:

    # j "Sorry..."
    j "Désolé..."

# game/events_mc.rpy:1780
translate french d7_breakfast_54a75af6:

    # pcthink "I knew it..."
    pcthink "Je le savais..."

# game/events_mc.rpy:1782
translate french d7_breakfast_03d4e094:

    # n "A little bit later."
    n "Un peu plus tard."

# game/events_mc.rpy:1785
translate french d7_breakfast_cf0b512e:

    # pcthink "Well... everybody is gone..."
    pcthink "Eh bien... tout le monde est parti..."

# game/events_mc.rpy:1786
translate french d7_breakfast_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:1787
translate french d7_breakfast_324d7900:

    # pcthink "Hum, I guess I should get some work done..."
    pcthink "Hum, je suppose que je devrais travailler un peu..."

# game/events_mc.rpy:1789
translate french d7_breakfast_5f3edcc8:

    # n "An hour later."
    n "Une heure plus tard."

# game/events_mc.rpy:1791
translate french d7_breakfast_a69377ef_2:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1792
translate french d7_breakfast_b9b155fe:

    # extend "."
    extend "."

# game/events_mc.rpy:1793
translate french d7_breakfast_b9b155fe_1:

    # extend "."
    extend "."

# game/events_mc.rpy:1794
translate french d7_breakfast_b9b155fe_2:

    # extend "."
    extend "."

# game/events_mc.rpy:1795
translate french d7_breakfast_6cbcbb85:

    # pcthink "Naaaaaaaaw... I can't think... not a single line of code done..."
    pcthink "Naaaaaaaannn... Je n'arrive pas à réfléchir... pas une seule ligne de code de faite..."

# game/events_mc.rpy:1796
translate french d7_breakfast_7923630c:

    # pcthink "I'm tired, I'm bored... I can't work like this."
    pcthink "Je suis fatigué, je m'ennuie... Je ne peux pas travailler comme ça."

# game/events_mc.rpy:1797
translate french d7_breakfast_01f6d6fa:

    # pcthink "I need to do something else..."
    pcthink "Je dois faire autre chose..."

# game/events_mc.rpy:1798
translate french d7_breakfast_a69377ef_3:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1799
translate french d7_breakfast_dadf699b:

    # pcthink "I'll go for a walk..."
    pcthink "Je vais faire un tour..."

# game/events_mc.rpy:1805
translate french d7_evening_5838427b:

    # pcthink "Damn, that was embarrassing... everyone was staring at me..."
    pcthink "Merde, c'était embarrassant... tout le monde me fixait..."

# game/events_mc.rpy:1806
translate french d7_evening_3e9379b4:

    # pcthink "I don't get it, why would you ever need so much toilet paper?"
    pcthink "Je ne comprends pas, pourquoi aurait-on besoin d'autant de papier toilette ?"

# game/events_mc.rpy:1807
translate french d7_evening_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1808
translate french d7_evening_6ea063df:

    # pcthink "Anyway, I better get some work done now..."
    pcthink "Bref, je ferais mieux de travailler maintenant..."

# game/events_mc.rpy:1812
translate french d7_home_f68a2112:

    # n "Later..."
    n "A plus tard..."

# game/events_mc.rpy:1814
translate french d7_home_efeb7a86:

    # pcthink "Alright, that's better. At least {b}some{/b} progress..."
    pcthink "Très bien, c'est mieux. Au moins {b}quelques{/b} progrès..."

# game/events_mc.rpy:1817
translate french d7_home_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh ?"

# game/events_mc.rpy:1826
translate french d7_home_02970987:

    # pcthink "Oh shit! It's the boss!"
    pcthink "Oh merde ! C'est le patron !"

# game/events_mc.rpy:1845
translate french d7_home_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1846
translate french d7_home_b9b155fe:

    # extend "."
    extend "."

# game/events_mc.rpy:1847
translate french d7_home_b9b155fe_1:

    # extend "."
    extend "."

# game/events_mc.rpy:1848
translate french d7_home_b9b155fe_2:

    # extend "."
    extend "."

# game/events_mc.rpy:1908
translate french d7_home_eabfa31f:

    # pcthink "So [st] told him about my visit... I wonder why they were talking about me."
    pcthink "Donc [st] lui a parlé de ma visite... Je me demande pourquoi ils parlaient de moi."

# game/events_mc.rpy:1913
translate french d8_morning_8bf6dc88:

    # n "*Knock* *Knock*"
    n "*Toc* *Toc*"

# game/events_mc.rpy:1915
translate french d8_morning_e2ee86ba:

    # pc "Uhh..."
    pc "Uhh..."

# game/events_mc.rpy:1917
translate french d8_morning_a2c50bf9:

    # m "Time to get up, [e]..."
    m "Il est temps de se lever, [e]..."

# game/events_mc.rpy:1919
translate french d8_morning_c0150f59:

    # e "Uhnn..."
    e "Uhnn..."

# game/events_mc.rpy:1921
translate french d8_morning_ea7b0f4c:

    # pcthink "...of course she snuck over again..."
    pcthink "...bien sûr, elle s'est encore faufilée..."

# game/events_mc.rpy:1924
translate french d8_morning_81b740a4:

    # m "... and [pc] as well."
    m "... et [pc] également."

# game/events_mc.rpy:1926
translate french d8_morning_2ed3e3d9:

    # m "... do you want to have breakfast with us, [pc]?"
    m "... veux-tu prendre le petit déjeuner avec nous, [pc]?"

# game/events_mc.rpy:1927
translate french d8_morning_6faca216:

    # pc "*YAAAWN*... yeah..."
    pc "*YAAAWN*... ouais..."

# game/events_mc.rpy:1928
translate french d8_morning_2f1ef192:

    # pcthink "... [m] really {b}has{/b} changed..."
    pcthink "... [m] vraiment {b}a{/b} changée..."

# game/events_mc.rpy:1931
translate french d8_morning_23fe4ea7:

    # n "You and [m] engage in some small talk again, you can't help but wonder if this is how a normal family would have their breakfast..."
    n "Vous et [m] engagez à nouveau une petite conversation, vous ne pouvez vous empêcher de vous demander si c'est ainsi qu'une famille normale prendrait son petit déjeuner...."

# game/events_mc.rpy:1933
translate french d8_morning_848f5afd:

    # m "See you later, Hun."
    m "A plus tard, Hun."

# game/events_mc.rpy:1934
translate french d8_morning_19392efe:

    # pc "Later..."
    pc "Plus tard..."

# game/events_mc.rpy:1935
translate french d8_morning_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1937
translate french d8_morning_f2c88496:

    # pcthink "Shit... I'm almost starting to enjoy the time here..."
    pcthink "Merde... Je commence presque à apprécier le temps passé ici..."

# game/events_mc.rpy:1938
translate french d8_morning_6cafa954:

    # pcthink "m "really changed, but I'm sure she's still hiding something from me."
    pcthink "m "vraiment changé, mais je suis sûr qu'elle me cache encore quelque chose."

# game/events_mc.rpy:1939
translate french d8_morning_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:1940
translate french d8_morning_0eb791b9:

    # pcthink "I wonder why she didn't mention anything about [e] sleeping in my bed again..."
    pcthink "Je me demande pourquoi elle n'a rien dit à propos de [e] dormant dans mon lit à nouveau..."

# game/events_mc.rpy:1941
translate french d8_morning_61a4ffe9:

    # pcthink "Maybe she's starting to accept it?"
    pcthink "Peut-être qu'elle commence à l'accepter ?"

# game/events_mc.rpy:1942
translate french d8_morning_4e48822d:

    # pcthink "...or she just gave up trying..."
    pcthink "...ou elle a juste abandonné..."

# game/events_mc.rpy:1944
translate french d8_morning_10f6ec45:

    # pcthink "Well, whatever, I have work to do..."
    pcthink "Bon, peu importe, j'ai du travail à faire..."

# game/events_mc.rpy:1946
translate french d8_morning_e6f0cf97:

    # n "A few hours later..."
    n "Quelques heures plus tard..."

# game/events_mc.rpy:1952
translate french d8_eveningWJada_ae86d761:

    # pcthink "Hum, I just hope [m] remembers when the pills wear off..."
    pcthink "Hum, j'espère juste que [m] se souvient quand les pilules ne font plus effet..."

# game/events_mc.rpy:1953
translate french d8_eveningWJada_a0bd8abc:

    # pcthink "Maybe I should keep everyone together until the pills wear off. She {b}{i}has{/i}{/b} to remember when I don't give her the chance to think about anything else..."
    pcthink "Peut-être que je devrais garder tout le monde ensemble jusqu'à ce que les pilules fassent effet. Elle {b}{i}doit{/i}{/b} se souvenir quand je ne lui laisse pas la chance de penser à autre chose..."

# game/events_mc.rpy:1955
translate french d8_eveningWJada_fdc42a74:

    # pc "Hey, how about we celebrate this with a... err... with something, haha."
    pc "Hé, que dirriez-vous de fêter ça avec un... euh... avec quelque chose, haha."

# game/events_mc.rpy:1957
translate french d8_eveningWJada_66934b11:

    # j "Yeah, why not, hehe."
    j "Ouais, pourquoi pas, héhé."

# game/events_mc.rpy:1959
translate french d8_eveningWJada_e4c2420d:

    # m "Oh, that's a great idea, I'll make us some drinks!"
    m "Oh, c'est une excellente idée, je vais nous préparer des boissons !"

# game/events_mc.rpy:1961
translate french d8_eveningWJada_a3d1076c:

    # j "Uhh..."
    j "Uhh..."

# game/events_mc.rpy:1963
translate french d8_eveningWJada_537ee46a:

    # j "...but better nothing alcoholic... hehe...heh..."
    j "...mais mieux vaut rien d'alcoolique... héhé...heh..."

# game/events_mc.rpy:1965
translate french d8_eveningWJada_99771c90:

    # m "Huh? Of course not, you know that we won't have alcohol in this house, honey."
    m "Huh ? Bien sûr que non, tu sais que nous n'avons pas d'alcool dans cette maison, chérie."

# game/events_mc.rpy:1967
translate french d8_eveningWJada_6ab4742f:

    # j "Uhh... yeah, hehe..."
    j "Uhh... ouais, héhé..."

# game/events_mc.rpy:1969
translate french d8_eveningWJada_084f5294:

    # j "{size=-10}Did she just call me honey?{/size}"
    j "{size=-10}Est-ce qu'elle vient de m'appeler chérie ? {/size}"

# game/events_mc.rpy:1970
translate french d8_eveningWJada_dbc63618:

    # pc "{size=-10}Yeah, it sounded like it...{/size}"
    pc "{size=-10}Ouais, ça y ressemblait...{/size}"

# game/events_mc.rpy:1971
translate french d8_eveningWJada_4796f34c:

    # j "Weird..."
    j "Bizarre..."

# game/events_mc.rpy:1974
translate french d8_eveningWJada_7c3855bb:

    # j "...and then she was like \"What the hell was that?!\", haha."
    j "...et ensuite elle était comme 'Qu'est-ce que c'était que ça ?!', haha."

# game/events_mc.rpy:1976
translate french d8_eveningWJada_ef50611b:

    # pc "Damn, that must have looked funny."
    pc "Mince, ça devait avoir l'air drôle."

# game/events_mc.rpy:1978
translate french d8_eveningWJada_ba0e7088:

    # j "Yeah it did! Haha."
    j "Oui, c'est vrai ! Haha."

# game/events_mc.rpy:1980
translate french d8_eveningWJada_20c435f6:

    # pc "You better not do that here though, as funny as it sounds..."
    pc "Tu ferais mieux de ne pas faire ça ici, aussi drôle que cela puisse paraître..."

# game/events_mc.rpy:1982
translate french d8_eveningWJada_585d60ca:

    # j "Don't worry, I promise I won't cause you any trouble."
    j "Ne t'inquiètes pas, je te promets que je ne te causerai aucun problème."

# game/events_mc.rpy:1984
translate french d8_eveningWJada_95be680b:

    # pc "Good, otherwise I'd have to send you back to that creepy old perv."
    pc "Bien, sinon je devrais te renvoyer chez ce vieux pervers."

# game/events_mc.rpy:1986
translate french d8_eveningWJada_8e79494f:

    # j "...and we don't want that, hehe."
    j "...et nous ne voulons pas cela, héhé."

# game/events_mc.rpy:1988
translate french d8_eveningWJada_072dc244:

    # j "Thanks again by the way, [m]. It's really nice of you to let me stay here..."
    j "Merci encore au fait, [m]. C'est vraiment gentil de ta part de me laisser rester ici..."

# game/events_mc.rpy:1990
translate french d8_eveningWJada_e612037c:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:1991
translate french d8_eveningWJada_a7ecf1ab:

    # pc "[m]?!"
    pc "[m]?!"

# game/events_mc.rpy:1993
translate french d8_eveningWJada_487a1850:

    # m "Hm?"
    m "Hm ?"

# game/events_mc.rpy:1994
translate french d8_eveningWJada_83ad7b2f:

    # pcthink "Shit, looks like the pills are going to wear off..."
    pcthink "Merde, on dirait que les pilules vont plus faire effet..."

# game/events_mc.rpy:1995
translate french d8_eveningWJada_52f9ab04:

    # pc "[j] said thank you for letting her stay."
    pc "[j] a dit merci de la laisser rester."

# game/events_mc.rpy:1997
translate french d8_eveningWJada_2302705b:

    # m "Letting her..."
    m "La laisser..."

# game/events_mc.rpy:1999
translate french d8_eveningWJada_e8055ccc:

    # j "Yeah, thanks, [m]. You're a life saver!"
    j "Ouais, merci, [m]. Tu es une sauveuse de vie !"

# game/events_mc.rpy:2000
translate french d8_eveningWJada_0c86cf15:

    # pcthink "I better try to remind her about the talk we had..."
    pcthink "Je ferais mieux d'essayer de lui rappeler la discussion que nous avons eue..."

# game/events_mc.rpy:2002
translate french d8_eveningWJada_bb068418:

    # pc "Yeah, now [e] has her BFF here, and err... we \"have her close\" ..."
    pc "Ouais, maintenant [e] a sa meilleure amie ici, et euh... nous 'l'avons proche'..."

# game/events_mc.rpy:2003
translate french d8_eveningWJada_a0667ccf:

    # pc "Err...to keep an eye on her!"
    pc "Err...pour garder un œil sur elle !"

# game/events_mc.rpy:2005
translate french d8_eveningWJada_1de1e8bb:

    # j "Hey, I'm sitting right here you know."
    j "Hé, je suis assis juste ici tu sais."

# game/events_mc.rpy:2006
translate french d8_eveningWJada_8372f8a0:

    # pc "Of course I know."
    pc "Bien sûr que je le sais."

# game/events_mc.rpy:2007
translate french d8_eveningWJada_3fa6b1c9:

    # e "Khehehe."
    e "Khéhéhéhé."

# game/events_mc.rpy:2009
translate french d8_eveningWJada_9c7e54c5:

    # j "Aww, stop laughing [e], they will probably monitor us 24/7!"
    j "Aww, arrête de rire [e], ils vont probablement nous surveiller 24/24 et 7/7 !"

# game/events_mc.rpy:2010
translate french d8_eveningWJada_bf277c84:

    # pc "Haha, every second."
    pc "Haha, chaque seconde."

# game/events_mc.rpy:2013
translate french d8_eveningWJada_91a2ea44:

    # j "Dick!"
    j "Connard !"

# game/events_mc.rpy:2015
translate french d8_eveningWJada_f45321f9:

    # j "Bitch!"
    j "Salope !"

# game/events_mc.rpy:2016
translate french d8_eveningWJada_ab0407ce:

    # pc "Haha."
    pc "Haha."

# game/events_mc.rpy:2018
translate french d8_eveningWJada_f87d08f5:

    # m "[j], I don't want to hear any swear words in this house, so if you want to stay here for a while, you better stop that!"
    m "[j], je ne veux pas entendre de gros mots dans cette maison, alors si tu veux rester ici un moment, tu ferais mieux d'arrêter ça !"

# game/events_mc.rpy:2020
translate french d8_eveningWJada_b8cd6cc7:

    # j "Uhh... sorry, no swear words anymore..."
    j "Uhh... désolé, plus de gros mots..."

# game/events_mc.rpy:2021
translate french d8_eveningWJada_e659ac4d:

    # pcthink "Phew, looks like it worked..."
    pcthink "Ouf, on dirait que ça a marché..."

# game/events_mc.rpy:2023
translate french d8_eveningWJada_f68a2112:

    # n "Later..."
    n "A plus tard..."

# game/events_mc.rpy:2024
translate french d8_eveningWJada_9a87cbd7:

    # e "HNNN!"
    e "HNNN !"

# game/events_mc.rpy:2025
translate french d8_eveningWJada_cddc6e66:

    # pc "Right! Go right!"
    pc "D'accord! Allons-y !"

# game/events_mc.rpy:2026
translate french d8_eveningWJada_f8f4d1e2:

    # j "I know!"
    j "Je sais !"

# game/events_mc.rpy:2028
translate french d8_eveningWJada_a9ccd390:

    # e "NN!"
    e "NN !"

# game/events_mc.rpy:2029
translate french d8_eveningWJada_99551eb4:

    # pc "Watch out, behind you!"
    pc "Attention, derrière toi !"

# game/events_mc.rpy:2031
translate french d8_eveningWJada_b8fed04a:

    # j "Fuuuuuck!"
    j "Meeerrrdddeee !"

# game/events_mc.rpy:2032
translate french d8_eveningWJada_a4fe781c:

    # j "Aww damn it, I almost got her..."
    j "Bon sang, j'ai failli l'avoir..."

# game/events_mc.rpy:2033
translate french d8_eveningWJada_49f7755e:

    # pc "Welp..."
    pc "Eh biennn..."

# game/events_mc.rpy:2036
translate french d8_eveningWJada_8bf6dc88:

    # n "*Knock* *Knock*"
    n "*Toc* *Toc*"

# game/events_mc.rpy:2039
translate french d8_eveningWJada_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:2040
translate french d8_eveningWJada_20b8f121:

    # pc "Yes?"
    pc "Oui ?"

# game/events_mc.rpy:2042
translate french d8_eveningWJada_96772528:

    # m "Hey, are you having fun?"
    m "Hé, tu t'amuses ?"

# game/events_mc.rpy:2044
translate french d8_eveningWJada_3863cbbf:

    # j "Naaaw... I just lost..."
    j "Naaan... Je viens de perdre..."

# game/events_mc.rpy:2045
translate french d8_eveningWJada_42fb089f:

    # m "That's... well..."
    m "C'est... eh bien..."

# game/events_mc.rpy:2047
translate french d8_eveningWJada_9603b05a:

    # m "Anyway, [pc], I'm going to bed and I just wanted to make sure that..."
    m "Bref, [pc], je vais me coucher et je voulais juste m'assurer que..."

# game/events_mc.rpy:2049
translate french d8_eveningWJada_a177bee0:

    # m "...well, I'm sure it isn't necessary, but I wanted to remind you that [e] has her own room and I expect [j] to sleep in [e]'s room, too."
    m "...bien, je suis sûr que ce n'est pas nécessaire, mais je voulais vous rappeler que [e] a sa propre chambre et que je m'attends à ce que [j] dorme dans la chambre de [e] aussi."

# game/events_mc.rpy:2051
translate french d8_eveningWJada_3f91d725:

    # m "Umm... well, I... I just wanted to remind you that this is {b}{i}your{/i}{/b} room, [e] has her own room and bed and [j] is her best friend so..."
    m "Umm... bien, je... Je voulais juste te rappeler que c'est {b}{i}ta{/i}{/b} chambre, [e] a sa propre chambre et son propre lit et [j] est sa meilleure amie donc..."

# game/events_mc.rpy:2052
translate french d8_eveningWJada_3c8e055f:

    # m "I..."
    m "Je..."

# game/events_mc.rpy:2053
translate french d8_eveningWJada_4d699985:

    # m "I just wanted to make sure that you all remember that."
    m "Je voulais juste m'assurer que vous vous en souveniez tous."

# game/events_mc.rpy:2054
translate french d8_eveningWJada_8095c2c4:

    # pcthink "Jeez, is she serious now?"
    pcthink "Bon sang, elle est sérieuse maintenant ?"

# game/events_mc.rpy:2055
translate french d8_eveningWJada_b3c9e471:

    # j "Oh don't worry [m]..."
    j "Oh ne t'inquiète pas [m]..."

# game/events_mc.rpy:2057
translate french d8_eveningWJada_ca366517:

    # j "Of course we remember that..."
    j "Bien sûr, on s'en souvient..."

# game/events_mc.rpy:2059
translate french d8_eveningWJada_c3c54b71:

    # j "[e] and I will of course sleep in [e]'s room."
    j "[e] et moi allons bien sûr dormir dans la chambre de [e]."

# game/events_mc.rpy:2060
translate french d8_eveningWJada_225fadf5:

    # pc "Huh?"
    pc "Huh ?"

# game/events_mc.rpy:2061
translate french d8_eveningWJada_d593d4b0:

    # e "Eh?"
    e "Eh ?"

# game/events_mc.rpy:2062
translate french d8_eveningWJada_a4b0f5dd:

    # m "Oh really?!"
    m "Oh vraiment ?!"

# game/events_mc.rpy:2064
translate french d8_eveningWJada_58163bb5:

    # j "Of course, and I'll make sure that everyone stays in their respective beds."
    j "Bien sûr, et je veillerai à ce que chacun reste dans son lit respectif."

# game/events_mc.rpy:2066
translate french d8_eveningWJada_01fc1be6:

    # m "That's, umm... well thanks [j]..."
    m "C'est, humm... bien merci [j]..."

# game/events_mc.rpy:2067
translate french d8_eveningWJada_38c7e193:

    # pcthink "Wow, [j] really wants to make a good impression..."
    pcthink "Wow, [j] veut vraiment faire bonne impression..."

# game/events_mc.rpy:2068
translate french d8_eveningWJada_943ce365:

    # m "Okay then... have a good night everyone..."
    m "Ok alors... bonne nuit à tous..."

# game/events_mc.rpy:2069
translate french d8_eveningWJada_9c35c852:

    # j "Good night, [m]."
    j "Bonne nuit, [m]."

# game/events_mc.rpy:2071
translate french d8_eveningWJada_ac74160d:

    # j "Well, I guess that means we should go to bed, huh, [e]?"
    j "Eh bien, je suppose que cela signifie que nous devrions aller au lit, hein, [e]?"

# game/events_mc.rpy:2073
translate french d8_eveningWJada_1da7971d:

    # e "Unh..."
    e "Unh..."

# game/events_mc.rpy:2075
translate french d8_eveningWJada_66aa8c20:

    # j "Oh come on [e], I don't want to get kicked out again!"
    j "Oh allez [e], je ne veux pas me faire virer à nouveau !"

# game/events_mc.rpy:2077
translate french d8_eveningWJada_8ba6ae53:

    # pc "Well, I guess she's right, let's play along, okay?"
    pc "Bon, je suppose qu'elle a raison, jouons le jeu, ok ?"

# game/events_mc.rpy:2082
translate french d8_eveningWJada_755b8a4e:

    # n "You expect a simple goodnight kiss, but right when your lips meet, you can feel [e]'s tongue sliding into your mouth..."
    n "Vous vous attendez à un simple baiser de bonne nuit, mais au moment où vos lèvres se rencontrent, vous pouvez sentir la langue de [e] glisser dans votre bouche..."

# game/events_mc.rpy:2087
translate french d8_eveningWJada_7828051b:

    # j "Hehe, cute!"
    j "Héhé, mignon !"

# game/events_mc.rpy:2089
translate french d8_eveningWJada_e63cf01d:

    # j "Good night, [pc]."
    j "Bonne nuit, [pc]."

# game/events_mc.rpy:2090
translate french d8_eveningWJada_c0d50c6d:

    # pc "Night, [j], night, [e]."
    pc "Bonne nuit, [j], bonne nuit, [e]."

# game/events_mc.rpy:2093
translate french d8_eveningWJada_7d716a37:

    # j "Night..."
    j "Bonne nuit..."

# game/events_mc.rpy:2095
translate french d8_eveningWJada_b9e1b26a:

    # j "{size=-15}[pcmd]{/size}."
    j "{size=-15}[pcmd]{/size}."

# game/events_mc.rpy:2096
translate french d8_eveningWJada_77b0399a:

    # n "*Smooch*"
    n "*Bisous*"

# game/events_mc.rpy:2098
translate french d8_eveningWJada_6e5441be:

    # pc "Night, [j]..."
    pc "Bonne nuit, [j]..."

# game/events_mc.rpy:2100
translate french d8_eveningWJada_e8a48adf:

    # pc "Heh, night, sweetie."
    pc "Heu, bonne nuit, chérie."

# game/events_mc.rpy:2102
translate french d8_eveningWJada_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:2105
translate french d8_eveningWJada_f2ead045:

    # j "Good night, [pcmd]..."
    j "Bonne nuit, [pcmd]..."

# game/events_mc.rpy:2110
translate french d8_eveningWJada_2d849fbb:

    # pc "Heh... good night, babygirl."
    pc "Heu... bonne nuit, petite fille."

# game/events_mc.rpy:2112
translate french d8_eveningWJada_2f6d0e57:

    # pc "Night, sweetie."
    pc "Bonne nuit, ma chérie."

# game/events_mc.rpy:2113
translate french d8_eveningWJada_8de614b1:

    # e "Khehe!"
    e "Khéhéhe !"

# game/events_mc.rpy:2115
translate french d8_eveningWJada_744f56ce:

    # n "A bit later..."
    n "Un peu plus tard..."

# game/events_mc.rpy:2117
translate french d8_eveningWJada_e6315939:

    # j "{size=-10}Hahaha... yes!{/size}"
    j "{size=-10}Hahaha... oui!{/size}"

# game/events_mc.rpy:2118
translate french d8_eveningWJada_7ae9f4d0:

    # n "You can hear [j]'s voice through the wall."
    n "Vous pouvez entendre la voix de [j] à travers le mur."

# game/events_mc.rpy:2119
translate french d8_eveningWJada_ab504d42:

    # j "{size=-10}Do you think...{/size}"
    j "{size=-10}Penses-tu que...{/size}"

# game/events_mc.rpy:2120
translate french d8_eveningWJada_08da4a08:

    # pcthink "Looks like [e] and [j] are still awake..."
    pcthink "On dirait que [e] et [j] sont toujours réveillés..."

# game/events_mc.rpy:2121
translate french d8_eveningWJada_4de159dd:

    # j "{size=-10}Haha, nooo, I'll help you, but that's too...{/size}"
    j "{size=-10}Haha, nooon, je vais t'aider, mais c'est trop...{/size}"

# game/events_mc.rpy:2122
translate french d8_eveningWJada_305b9374:

    # j "{size=-10}...yes!{/size}"
    j "{size=-10}...oui!{/size}"

# game/events_mc.rpy:2123
translate french d8_eveningWJada_cfbbd79b:

    # j "{size=-10}...really like [himher]!{/size}"
    j "{size=-10}...aime vraiment [himher]!{/size}"

# game/events_mc.rpy:2124
translate french d8_eveningWJada_75fc793d:

    # pcthink "Damn, I can't understand all of it."
    pcthink "Merde, je ne peux pas tout comprendre."

# game/events_mc.rpy:2125
translate french d8_eveningWJada_9f00adb1:

    # j "{size=-10}...really sure about it?{/size}"
    j "{size=-10}...vraiment sûr de lui ? {/size}"

# game/events_mc.rpy:2126
translate french d8_eveningWJada_19b7b455:

    # n "From this point on you can only hear some whispering, but can't understand a word."
    n "A partir de ce moment, vous n'entendez que quelques chuchotements, mais ne comprenez pas un mot."

# game/events_mc.rpy:2127
translate french d8_eveningWJada_66e2dfd5:

    # pcthink "Hum... I wonder what they were talking about..."
    pcthink "Hum... Je me demande de quoi elles parlaient..."

# game/events_mc.rpy:2135
translate french d9_wakeup_0b3abc60:

    # n "*Sniff*"
    n "*Renifle*"

# game/events_mc.rpy:2136
translate french d9_wakeup_ba0ce6f0:

    # n "*Wimper*"
    n "*Mauviette*"

# game/events_mc.rpy:2137
translate french d9_wakeup_e78beedd:

    # pc "Huh? What's wrong, [m]?"
    pc "Huh ? Qu'est-ce qui ne va pas, [m]?"

# game/events_mc.rpy:2138
translate french d9_wakeup_62545805:

    # m "*Sniff* ...oh..."
    m "*Renifle* ...Oh..."

# game/events_mc.rpy:2139
translate french d9_wakeup_a398b817:

    # m "I'm sorry, [pc], it's nothing..."
    m "Je suis désolée, [pc], ce n'est rien..."

# game/events_mc.rpy:2140
translate french d9_wakeup_4c4f5296:

    # pc "Where's Dad?"
    pc "Où est papa ?"

# game/events_mc.rpy:2141
translate french d9_wakeup_fbcdc9b6:

    # m "*Sniff* ...he didn't tell you?"
    m "* Renifle* ...il ne te l'a pas dit?"

# game/events_mc.rpy:2142
translate french d9_wakeup_37638e4d:

    # pc "Uh-uh."
    pc "Uh-uh."

# game/events_mc.rpy:2143
translate french d9_wakeup_8083b61b:

    # m "He got suspended and is attending the hearing... [oc] just picked him up..."
    m "Il a été suspendu et assiste à l'audience... [oc] vient de le récupérer..."

# game/events_mc.rpy:2144
translate french d9_wakeup_5a07187a:

    # pc "Why did he get suspended?"
    pc "Pourquoi a-t-il été suspendu ?"

# game/events_mc.rpy:2145
translate french d9_wakeup_507892dd:

    # m "*Sniff* ...I don't know, he didn't tell me... he was probably drunk again..."
    m "*Renifle* ...Je ne sais pas, il ne me l'a pas dit... il était probablement encore ivre..."

# game/events_mc.rpy:2146
translate french d9_wakeup_f3c083a8:

    # pc "Does that mean he will be home more often from now on?"
    pc "Cela signifie-t-il qu'il sera plus souvent à la maison à partir de maintenant ?"

# game/events_mc.rpy:2147
translate french d9_wakeup_02e42931:

    # m "*Sniff* ...yes..."
    m "*Renifle* ...Oui..."

# game/events_mc.rpy:2148
translate french d9_wakeup_930497c3:

    # m "[oc] said that your Dad will probably stay home for a while from now on..."
    m "[oc] dit que ton père va probablement rester à la maison pendant un certain temps à partir de maintenant ..."

# game/events_mc.rpy:2149
translate french d9_wakeup_dd2d8f0d:

    # pc "Did [oc] get suspended too?"
    pc "Est-ce que [oc] a aussi été suspendu ?"

# game/events_mc.rpy:2150
translate french d9_wakeup_e5fc8ba9:

    # m "No, but he has to attend the hearing too."
    m "Non, mais il doit aussi assister à l'audience."

# game/events_mc.rpy:2161
translate french d9_breakfast_f136f8bf:

    # pc "Morning everyone."
    pc "Bonjour à tous."

# game/events_mc.rpy:2162
translate french d9_breakfast_c32695b8:

    # m "Good morning, hun."
    m "Bonjour, hun."

# game/events_mc.rpy:2164
translate french d9_breakfast_c7687275:

    # m "Nice shirt."
    m "Belle chemise."

# game/events_mc.rpy:2165
translate french d9_breakfast_4cb533b6:

    # pc "Thanks..."
    pc "Merci..."

# game/events_mc.rpy:2167
translate french d9_breakfast_ea9caa26:

    # m "Oh by the way [j], I didn't see you bring any of your stuff yesterday?"
    m "Oh au fait [j], je ne t'ai pas vu apporter tes affaires hier ?"

# game/events_mc.rpy:2169
translate french d9_breakfast_ffd273b8:

    # j "Yeah, most of it is still in the apartment..."
    j "Oui, la plupart sont encore dans l'appartement..."

# game/events_mc.rpy:2171
translate french d9_breakfast_4d1e2a17:

    # m "I see, you should get it here soon if you plan to stay for a bit longer..."
    m "Je vois, tu devrais l'avoir bientôt si tu comptes rester un peu plus longtemps..."

# game/events_mc.rpy:2173
translate french d9_breakfast_87d3c8fc:

    # j "Yeah..."
    j "Ouais..."

# game/events_mc.rpy:2175
translate french d9_breakfast_dbf3eae9:

    # m "It will surely help to make you feel at home."
    m "Il t'aidera sûrement à te sentir chez toi."

# game/events_mc.rpy:2177
translate french d9_breakfast_80206a6d:

    # j "Oh thanks, I feel at home already, I mean, the only place I've ever had a breakfast like this is here."
    j "Oh merci, je me sens déjà chez moi, je veux dire, le seul endroit où j'ai jamais eu un petit déjeuner comme ça, c'est ici."

# game/events_mc.rpy:2179
translate french d9_breakfast_5ac27665:

    # m "Really? You don't eat breakfast together with your mom?"
    m "Vraiment ? Tu ne prends pas le petit-déjeuner avec ta mère ?"

# game/events_mc.rpy:2181
translate french d9_breakfast_aa83e1d2:

    # j "No, but I always liked it when I was staying here overnight. It feels like... having a real family."
    j "Non, mais j'ai toujours aimé ça quand je passais la nuit ici. C'est comme... avoir une vraie famille."

# game/events_mc.rpy:2183
translate french d9_breakfast_9b154c9e:

    # m "I didn't know that it meant so much to you..."
    m "Je ne savais pas que ça comptait autant pour toi..."

# game/events_mc.rpy:2185
translate french d9_breakfast_9a1d349c:

    # j "It kinda makes me feel like having a real family..."
    j "Ça me donne l'impression d'avoir une vraie famille..."

# game/events_mc.rpy:2186
translate french d9_breakfast_0a005ee3:

    # pcthink "Damn, I know how she feels..."
    pcthink "Merde, je sais ce qu'elle ressent..."

# game/events_mc.rpy:2188
translate french d9_breakfast_d8ea254e:

    # j "By the way, [pc], did you ever call [m] Mom?"
    j "Au fait, [pc], as-tu déjà appelé [m] maman ?"

# game/events_mc.rpy:2189
translate french d9_breakfast_fcf41317:

    # pc "Eh?"
    pc "Eh ?"

# game/events_mc.rpy:2191
translate french d9_breakfast_472b2611:

    # m "[j] you know that I'm not [hisher] mom."
    m "[j] tu sais que je ne suis pas [hisher] maman."

# game/events_mc.rpy:2192
translate french d9_breakfast_3fa6b1c9:

    # e "Khehehe."
    e "Khéhéhéhé."

# game/events_mc.rpy:2194
translate french d9_breakfast_d9c4138e:

    # m "...I mean... I wouldn't mind..."
    m "...Je veux dire... ça ne me dérangerait pas..."

# game/events_mc.rpy:2195
translate french d9_breakfast_a7c89ecd:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:2200
translate french d9_breakfast_30efd40d:

    # pc "Well, I guess I could..."
    pc "Eh bien, je suppose que je pourrais..."

# game/events_mc.rpy:2202
translate french d9_breakfast_d93364d6:

    # m "Oh, but you don't need to if you feel uncomfortable about it."
    m "Oh, mais tu n'as pas besoin de le faire si cela te met mal à l'aise."

# game/events_mc.rpy:2203
translate french d9_breakfast_803c4437:

    # pc "Nah, it's okay..."
    pc "Nan, c'est bon..."

# game/events_mc.rpy:2204
translate french d9_breakfast_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:2205
translate french d9_breakfast_c8768c83:

    # pc "...[mmom]."
    pc "...[mmom]..."

# game/events_mc.rpy:2207
translate french d9_breakfast_e612037c:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:2209
translate french d9_breakfast_86d648c6:

    # m "Umm..."
    m "Umm..."

# game/events_mc.rpy:2211
translate french d9_breakfast_beb41611:

    # m "A-Anyway... [j]. Me and [e] will go shopping and pick up some groceries after breakfast, why don't you come with us so we can get your stuff on our way back?"
    m "D-De toute façon... [j]. Moi et [e] on va aller faire des courses après le petit déjeuner, pourquoi tu ne viendrais pas avec nous pour qu'on puisse prendre tes affaires au retour ?"

# game/events_mc.rpy:2213
translate french d9_breakfast_4de61af5:

    # j "Yeah, why not."
    j "Oui, pourquoi pas."

# game/events_mc.rpy:2216
translate french d9_breakfast_63bae69e:

    # pc "...well about your stuff in the apartment, [j]..."
    pc "...bien à propos de tes affaires dans l'appartement, [j]..."

# game/events_mc.rpy:2217
translate french d9_breakfast_227f3a09:

    # j "Hm?"
    j "Hm ?"

# game/events_mc.rpy:2219
translate french d9_breakfast_99a47862:

    # m "Right, you can come and help me and [e] with the groceries after breakfast, we can get your stuff on our way back."
    m "Bien, tu peux venir m'aider et [e] avec les courses après le petit déjeuner, on pourra prendre tes affaires sur le chemin du retour."

# game/events_mc.rpy:2221
translate french d9_breakfast_a3d1076c:

    # j "Uhh..."
    j "Uhh..."

# game/events_mc.rpy:2223
translate french d9_breakfast_402d4708:

    # j "...well, okay."
    j "...bien, ok."

# game/events_mc.rpy:2224
translate french d9_breakfast_82b74462:

    # pcthink "Hmm... I have the feeling that she had some other plans..."
    pcthink "Hmm... J'ai le sentiment qu'elle avait d'autres plans..."

# game/events_mc.rpy:2226
translate french d9_breakfast_ff62b093:

    # n "After everyone is done eating you're surprised to see [j] helping with the dishes instantly."
    n "Lorsque tout le monde a fini de manger, vous êtes surpris de voir [j] aider à la vaisselle instantanément."

# game/events_mc.rpy:2227
translate french d9_breakfast_d64ac769:

    # pcthink "Hum... she's full of surprises..."
    pcthink "Hum... elle est pleine de surprises..."

# game/events_mc.rpy:2228
translate french d9_breakfast_7f827474:

    # n "Since there's nothing left to do, you decide go back to your room and do some work..."
    n "Comme il n'y a plus rien à faire, vous décidez de retourner dans votre chambre et de travailler un peu..."

# game/events_mc.rpy:2232
translate french d9_breakfast_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:2233
translate french d9_breakfast_b9b155fe:

    # extend "."
    extend "."

# game/events_mc.rpy:2234
translate french d9_breakfast_b9b155fe_1:

    # extend "."
    extend "."

# game/events_mc.rpy:2235
translate french d9_breakfast_b9b155fe_2:

    # extend "."
    extend "."

# game/events_mc.rpy:2236
translate french d9_breakfast_5531c1c8:

    # pcthink "Hum... that's weird..."
    pcthink "Hum... c'est bizarre..."

# game/events_mc.rpy:2237
translate french d9_breakfast_0b7d0499:

    # pcthink "...something doesn't fit..."
    pcthink "...quelque chose ne colle pas..."

# game/events_mc.rpy:2238
translate french d9_breakfast_4a57ea09:

    # m "[pc]?"
    m "[pc]?"

# game/events_mc.rpy:2240
translate french d9_breakfast_9455801f:

    # pc "Hm?"
    pc "Hm ?"

# game/events_mc.rpy:2242
translate french d9_breakfast_924d2dc2:

    # m "Didn't you hear me knocking?"
    m "Tu ne m'as pas entendu frapper ?"

# game/events_mc.rpy:2244
translate french d9_breakfast_395b9705:

    # pc "Uh... no, what's up?"
    pc "Euh... non, qu'est-ce qu'il y a ?"

# game/events_mc.rpy:2246
translate french d9_breakfast_bb3b7b86:

    # m "We're leaving, do you need anything from the city?"
    m "Nous partons, as-tu besoin de quelque chose en ville ?"

# game/events_mc.rpy:2248
translate french d9_breakfast_7940353d:

    # pc "Hm? Ah, no... nothing."
    pc "Hm ? Ah, non... rien."

# game/events_mc.rpy:2250
translate french d9_breakfast_10781970:

    # m "Okay, see you later then."
    m "Ok, à plus tard alors."

# game/events_mc.rpy:2252
translate french d9_breakfast_df3a4c48:

    # pc "Yeah, later..."
    pc "Ouais, à plus tard..."

# game/events_mc.rpy:2253
translate french d9_breakfast_ca388340:

    # pc "[mmom]."
    pc "[mmom]."

# game/events_mc.rpy:2261
translate french d9_breakfast_c660d414:

    # pcthink "Ugh... calling her mom feels really weird..."
    pcthink "Ugh... l'appeler ma mère est vraiment bizarre..."

# game/events_mc.rpy:2262
translate french d9_breakfast_c7677361:

    # pcthink "Why am I even doing this... I don't trust her god dammit..."
    pcthink "Pourquoi est-ce que je fais ça... Je n'ai pas confiance en elle, bon sang..."

# game/events_mc.rpy:2264
translate french d9_breakfast_930a282a:

    # pcthink "Shit... better get back to work... I don't want to think about stuff like that..."
    pcthink "Merde... je ferais mieux de me remettre au travail... Je ne veux pas penser à ce genre de choses..."

# game/events_mc.rpy:2266
translate french d9_breakfast_a8c6cc8a:

    # pc "Hum..."
    pc "Hum..."

# game/events_mc.rpy:2267
translate french d9_breakfast_186419d4:

    # pcthink "...there seems to be something missing..."
    pcthink "...il semble qu'il manque quelque chose..."

# game/events_mc.rpy:2268
translate french d9_breakfast_c1686997:

    # pcthink "Hum..."
    pcthink "Hum..."

# game/events_mc.rpy:2269
translate french d9_breakfast_b9b155fe_3:

    # extend "."
    extend "."

# game/events_mc.rpy:2270
translate french d9_breakfast_b9b155fe_4:

    # extend "."
    extend "."

# game/events_mc.rpy:2271
translate french d9_breakfast_b9b155fe_5:

    # extend "."
    extend "."

# game/events_mc.rpy:2277
translate french d9_breakfast_a69377ef_2:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:2278
translate french d9_breakfast_af934448:

    # st "...yes?"
    st "...oui ?"

# game/events_mc.rpy:2279
translate french d9_breakfast_c7479b17:

    # pcthink "As I thought, she's working at the weekend again..."
    pcthink "Comme je le pensais, elle travaille encore le week-end..."

# game/events_mc.rpy:2280
translate french d9_breakfast_01a67768:

    # pc "Hey, [st], I'm just working on the project, but I think I'm missing some files, could you check the files you sent me?"
    pc "Hé, [st], je suis en train de travailler sur le projet, mais je pense qu'il me manque des fichiers, pouvez-vous vérifier les fichiers que vous m'avez envoyés ?"

# game/events_mc.rpy:2281
translate french d9_breakfast_3994b364:

    # st "Uhm... yes, give me a minute..."
    st "Uhm... oui, donnez-moi une minute..."

# game/events_mc.rpy:2282
translate french d9_breakfast_38710895:

    # pc "Okay, thanks."
    pc "Ok, merci."

# game/events_mc.rpy:2283
translate french d9_breakfast_a69377ef_3:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:2284
translate french d9_breakfast_b9b155fe_6:

    # extend "."
    extend "."

# game/events_mc.rpy:2285
translate french d9_breakfast_b9b155fe_7:

    # extend "."
    extend "."

# game/events_mc.rpy:2286
translate french d9_breakfast_b9b155fe_8:

    # extend "."
    extend "."

# game/events_mc.rpy:2287
translate french d9_breakfast_e8152094:

    # st "This... might be a problem..."
    st "Ceci... pourrait être un problème..."

# game/events_mc.rpy:2288
translate french d9_breakfast_a2a6fb65:

    # pc "What do you mean?"
    pc "Que veux-tu dire ?"

# game/events_mc.rpy:2289
translate french d9_breakfast_21f4c37c:

    # st "It looks like [boss] forgot to include the latest data, but I'll have to check some older files to make sure, there's definitely something missing though."
    st "Il semble que [boss] ait oublié d'inclure les dernières données, mais je vais devoir vérifier certains fichiers plus anciens pour m'en assurer, il y a certainement quelque chose qui manque cependant."

# game/events_mc.rpy:2290
translate french d9_breakfast_9ac1eb9c:

    # pc "Okay... sounds like it'll take a while?"
    pc "Ok... on dirait que ça va prendre un moment ?"

# game/events_mc.rpy:2291
translate french d9_breakfast_dbed7976:

    # st "Yes..."
    st "Oui..."

# game/events_mc.rpy:2292
translate french d9_breakfast_01ed9b97:

    # pc "Hum... okay, well it looks like I'll have an actual weekend then."
    pc "Hum... ok, on dirait que je vais avoir un vrai week-end alors."

# game/events_mc.rpy:2293
translate french d9_breakfast_d9eaa6bf:

    # st "Sorry... I'll send them to you as soon as I can."
    st "Désolé... Je vous les enverrai dès que possible."

# game/events_mc.rpy:2294
translate french d9_breakfast_0e17cfdd:

    # pc "Yeah no sweat, it's the weekend we shouldn't be working in the first place..."
    pc "Ouais pas de soucis, c'est le week-end où on ne devrait pas travailler en premier lieu...."

# game/events_mc.rpy:2295
translate french d9_breakfast_2384fd4b:

    # st "..."
    new "[st]..."

# game/events_mc.rpy:2296
translate french d9_breakfast_203650b4:

    # pc "Thinking about it, I don't have much else to do, so maybe I'll come over later."
    pc "En y réfléchissant, je n'ai pas grand chose d'autre à faire, alors peut-être que je passerai plus tard."

# game/events_mc.rpy:2297
translate french d9_breakfast_2384fd4b_1:

    # st "..."
    new "[st]..."

# game/events_mc.rpy:2298
translate french d9_breakfast_bcbd1c8f:

    # st "Okay."
    st "Ok."

# game/events_mc.rpy:2299
translate french d9_breakfast_64b6a4cc:

    # pc "Well... maybe later then."
    pc "Eh bien... peut-être plus tard alors."

# game/events_mc.rpy:2300
translate french d9_breakfast_32c6a094:

    # st "L-later..."
    st "A à plus tard..."

# game/events_mc.rpy:2306
translate french d9_breakfast_77ddecae:

    # pcthink "Hum... I don't get that woman..."
    pcthink "Hum... Je ne comprends pas cette femme..."

# game/events_mc.rpy:2307
translate french d9_breakfast_54475726:

    # pcthink "Anyway, what am I going to do now?"
    pcthink "Bref, qu'est-ce que je vais faire maintenant ?"

# game/events_mc.rpy:2308
translate french d9_breakfast_e28e8ec7:

    # n "*BZZZZZ*"
    n "*BZZZZZ*"

# game/events_mc.rpy:2310
translate french d9_breakfast_4ab6197a:

    # pcthink "Huh? Did she forget anything?"
    pcthink "Huh ? A-t-elle oublié quelque chose ?"

# game/events_mc.rpy:2312
translate french d9_breakfast_f0cb153b:

    # pc "What's up [st], did you forget anything?"
    pc "Quoi de neuf [st], avez-vous oublié quelque chose ?"

# game/events_mc.rpy:2313
translate french d9_breakfast_54d61c82:

    # ma "Hello, this is [ma] Shelter."
    ma "Bonjour, c'est [ma] Shelter."

# game/events_mc.rpy:2314
translate french d9_breakfast_62612790:

    # pc "Ma..."
    pc "Ma..."

# game/events_mc.rpy:2315
translate french d9_breakfast_87dc5ca4:

    # pcthink "Oh right, the office secretary from the rental company."
    pcthink "Oh oui, la secrétaire de bureau de la société de location."

# game/events_mc.rpy:2316
translate french d9_breakfast_6d96d7b7:

    # pc "Sorry I thought you were my co-worker, I just talked to her."
    pc "Désolé j'ai cru que vous étiez ma collègue, je viens de lui parler."

# game/events_mc.rpy:2317
translate french d9_breakfast_26dfc1f2:

    # ma "No problem, do you want me to call again another time?"
    ma "Pas de problème, voulez-vous que je rappelle une autre fois ?"

# game/events_mc.rpy:2318
translate french d9_breakfast_4145ad23:

    # pc "No, it's okay, [ma], what do you need?"
    pc "Non, c'est bon, [ma], de quoi avez-vous besoin ?"

# game/events_mc.rpy:2319
translate french d9_breakfast_2717beb3:

    # ma "I have some good and some bad news for you, which one would you like to hear first?"
    ma "J'ai une bonne et une mauvaise nouvelle pour vous, laquelle voulez-vous entendre en premier ?"

# game/events_mc.rpy:2322
translate french d9_breakfast_f1f284ff:

    # ma "We are ready to refund your deposit for the apartment, all I need is your signature on a few documents."
    ma "Nous sommes prêts à rembourser votre dépôt pour l'appartement, tout ce dont j'ai besoin est votre signature sur quelques documents."

# game/events_mc.rpy:2323
translate french d9_breakfast_62b8cef0:

    # pc "Okay, so what's the bad news then?"
    pc "Ok, alors quelle est la mauvaise nouvelle ?"

# game/events_mc.rpy:2324
translate french d9_breakfast_e124b86c:

    # ma "I still don't have any news on another apartment."
    ma "Je n'ai toujours pas de nouvelles pour un autre appartement."

# game/events_mc.rpy:2325
translate french d9_breakfast_4b3a0d7e:

    # pc "Well, that's no big problem for now, I have somewhere to stay for a while."
    pc "Bon, ce n'est pas un gros problème pour le moment, j'ai un endroit où rester pour un moment."

# game/events_mc.rpy:2326
translate french d9_breakfast_f0d78308:

    # ma "Good to hear."
    ma "Bon à savoir."

# game/events_mc.rpy:2327
translate french d9_breakfast_1eca72ab:

    # ma "Do you want me to send you the documents via email?"
    ma "Voulez-vous que je vous envoie les documents par e-mail ?"

# game/events_mc.rpy:2330
translate french d9_breakfast_e124b86c_1:

    # ma "I still don't have any news on another apartment."
    ma "Je n'ai toujours pas de nouvelles pour un autre appartement."

# game/events_mc.rpy:2331
translate french d9_breakfast_4b3a0d7e_1:

    # pc "Well, that's no big problem for now, I have somewhere to stay for a while."
    pc "Bon, ce n'est pas un gros problème pour le moment, j'ai un endroit où rester pour un moment."

# game/events_mc.rpy:2332
translate french d9_breakfast_bb1c37cf:

    # ma "Oh, that's good to hear."
    ma "Oh, c'est bon à entendre."

# game/events_mc.rpy:2333
translate french d9_breakfast_2ad7efbe:

    # pc "So what's the good news?"
    pc "Alors, quelle est la bonne nouvelle ?"

# game/events_mc.rpy:2334
translate french d9_breakfast_f1f284ff_1:

    # ma "We are ready to refund your deposit for the apartment, all I need is your signature on a few documents."
    ma "Nous sommes prêts à rembourser votre dépôt pour l'appartement, tout ce dont j'ai besoin est votre signature sur quelques documents."

# game/events_mc.rpy:2335
translate french d9_breakfast_16838e2a:

    # pc "Good, can you send them over?"
    pc "Bien, pouvez-vous les envoyer ?"

# game/events_mc.rpy:2336
translate french d9_breakfast_bd7e709d:

    # ma "Sure."
    ma "Bien sûr."

# game/events_mc.rpy:2337
translate french d9_breakfast_843e8754:

    # ma "Do you want me to send them via email?"
    ma "Voulez-vous que je les envoie par e-mail ?"

# game/events_mc.rpy:2341
translate french d9_leavehome_0b3883cc:

    # pc "I sadly don't have a printer here..."
    pc "Je n'ai malheureusement pas d'imprimante ici..."

# game/events_mc.rpy:2342
translate french d9_leavehome_6f8c5996:

    # ma "Oh, well I can send them by post, but you could also come over during business hours, our office is in the same city."
    ma "Oh, eh bien je peux les envoyer par la poste, mais vous pourriez aussi venir pendant les heures de bureau, notre bureau est dans la même ville."

# game/events_mc.rpy:2343
translate french d9_leavehome_25c8e98b:

    # pc "Okay, I could come over Monday, what's the address?"
    pc "Ok, je pourrais passer lundi, quelle est l'adresse ?"

# game/events_mc.rpy:2344
translate french d9_leavehome_2cbd6059:

    # ma "It's... well, actually..."
    ma "C'est ça... eh bien, en fait..."

# game/events_mc.rpy:2345
translate french d9_leavehome_e35ac279:

    # pc "Hmm?"
    pc "Hmm ?"

# game/events_mc.rpy:2346
translate french d9_leavehome_069ecdd9:

    # ma "We usually don't accept customers on sunday, but I wanted to do some overtime anyway, so you could come over after closing time if you want."
    ma "D'habitude, nous n'acceptons pas de clients le dimanche, mais je voulais quand même faire quelques heures supplémentaires, donc vous pouvez venir après la fermeture si vous voulez."

# game/events_mc.rpy:2347
translate french d9_leavehome_9dbc81eb:

    # pc "Hmm, yeah sounds good, I was thinking about going to the city anyway."
    pc "Hmm, ouais ça me semble bien, je pensais à aller en ville de toute façon."

# game/events_mc.rpy:2348
translate french d9_leavehome_490058b6:

    # ma "Okay, the address is..."
    ma "Ok, l'adresse est..."

# game/events_mc.rpy:2350
translate french d9_leavehome_08d9627c:

    # n "You note the address and even though it's still a lot of time until closing time, you get ready to leave..."
    n "Vous notez l'adresse et même s'il reste encore beaucoup de temps avant la fermeture, vous vous préparez à partir...."

# game/events_mc.rpy:2355
translate french d9_backhome_pcroom_f834cc31:

    # n "A while later..."
    n "Un peu plus tard..."

# game/events_mc.rpy:2359
translate french d9_backhome_pcroom_e4ab4b10:

    # j "The movie is better than I thought, if only the screen was bigger..."
    j "Le film est meilleur que je ne le pensais, si seulement l'écran était plus grand...."

# game/events_mc.rpy:2361
translate french d9_backhome_pcroom_7ccf276c:

    # j "How come you don't have a TV?"
    j "Comment se fait-il que tu n'aies pas de télévision ?"

# game/events_mc.rpy:2363
translate french d9_backhome_pcroom_a289ebbc:

    # pc "I had one, but it broke when I was packing my stuff before moving."
    pc "J'en avais une, mais elle s'est cassé quand j'emballais mes affaires avant de déménager."

# game/events_mc.rpy:2365
translate french d9_backhome_pcroom_42ce01d2:

    # j "Oh shit, that sucks, you really should get a new one."
    j "Oh merde, ça craint, tu devrais vraiment en acheter une nouvelle."

# game/events_mc.rpy:2367
translate french d9_backhome_pcroom_e0db8102:

    # pc "Yeah, you're right..."
    pc "Oui, tu as raison..."

# game/events_mc.rpy:2369
translate french d9_backhome_pcroom_46f8b8fb:

    # j "Hey, looks like he's going to bang the princess, hehe."
    j "Hé, on dirait qu'il va se taper la princesse, héhé."

# game/events_mc.rpy:2370
translate french d9_backhome_pcroom_9c76afe8:

    # pcthink "[j]... always naughty..."
    pcthink "[j]... toujours vilaine fille..."

# game/events_mc.rpy:2372
translate french d9_backhome_pcroom_63365631:

    # j "Night, [pcmd]."
    j "Bonne nuit, [pcmd]."

# game/events_mc.rpy:2373
translate french d9_backhome_pcroom_4376661d:

    # pc "Night you two."
    pc "Bonne nuit vous deux."

# game/events_mc.rpy:2375
translate french d9_backhome_pcroom_8664ba0b:

    # pcthink "Well, I guess I'm going to bed too..."
    pcthink "Bon, je crois que je vais aller me coucher aussi..."

# game/events_mc.rpy:2377
translate french d9_backhome_pcroom_215e1221:

    # pcthink "I better file the papers somewhere before I forget about it..."
    pcthink "Je ferais mieux de classer les papiers quelque part avant de les oublier..."

# game/events_mc.rpy:2379
translate french d9_backhome_pcroom_787daedf:

    # pcthink "What's this? That's not my name..."
    pcthink "Qu'est-ce que c'est ? Ce n'est pas mon nom..."

# game/events_mc.rpy:2380
translate french d9_backhome_pcroom_8c991f22:

    # pcthink "Did she hand me the wrong papers?"
    pcthink "M'a-t-elle remis les mauvais papiers?"

# game/events_mc.rpy:2381
translate french d9_backhome_pcroom_071734fa:

    # pcthink "Shit, I better call her tomorrow..."
    pcthink "Merde, je ferais mieux de l'appeler demain..."

# game/events_mc.rpy:2382
translate french d9_backhome_pcroom_ac7ec890:

    # pcthink "Oh damn, I don't have her number."
    pcthink "Oh mince, je n'ai pas son numéro."

# game/events_mc.rpy:2385
translate french d9_backhome_pcroom_905de5b0:

    # pcthink "Huh? [h] tried to call me?!"
    pcthink "Huh ? [h] a essayé de m'appeler ?!"

# game/events_mc.rpy:2386
translate french d9_backhome_pcroom_d7f47e07:

    # pcthink "Oh shit, I wanted to text her..."
    pcthink "Oh merde, je voulais lui envoyer un texto..."

# game/events_mc.rpy:2387
translate french d9_backhome_pcroom_328d0f51:

    # pcthink "Wait, tomorrow is the date!"
    pcthink "Attends, c'est demain!"

# game/events_mc.rpy:2388
translate french d9_backhome_pcroom_379d93ec:

    # pcthink "Dammit, I totally forgot about that."
    pcthink "Merde, j'avais complètement oublié ça."

# game/events_mc.rpy:2390
translate french d9_backhome_pcroom_b15122a3:

    # pcthink "Alright, I'll call her, I hope she's still awake..."
    pcthink "D'accord, je vais l'appeler, j'espère qu'elle est toujours réveillée..."

# game/events_mc.rpy:2393
translate french d9_backhome_pcroom_4eeeb2c1:

    # pcthink "Maybe I should send her a message instead..."
    pcthink "Peut-être que je devrais plutôt lui envoyer un message..."

# game/events_mc.rpy:2395
translate french d9_backhome_pcroom_10363c10:

    # h "Yes, yes, I'm awake! Ah... I mean... hi [pc]!"
    h "Oui, oui, je suis réveillée ! Ah... je veux dire... bonjour [pc]!"

# game/events_mc.rpy:2396
translate french d9_backhome_pcroom_87896395:

    # pc "Oh shit, did I wake you up? I'm sorry..."
    pc "Oh merde, je t'ai réveillé ? Je suis désolé..."

# game/events_mc.rpy:2397
translate french d9_backhome_pcroom_71f0a118:

    # h "Nono, I'm totally awake, it's fine, really!"
    h "Non non, je suis totalement réveillée, c'est bon, vraiment !"

# game/events_mc.rpy:2398
translate french d9_backhome_pcroom_b4ec99ba:

    # pcthink "Damn, she's cute."
    pcthink "Merde, elle est mignonne."

# game/events_mc.rpy:2400
translate french d9_backhome_pcroom_32a88a20:

    # pc "I just saw that you tried to call me and thought I'd call back."
    pc "Je viens de voir que vous avez essayé de m'appeler et j'ai pensé que je devais vous rappeler."

# game/events_mc.rpy:2401
translate french d9_backhome_pcroom_7abd5452:

    # h "Oh, haha, I was just... uhm... since you didn't text me..."
    h "Oh, haha, j'étais juste... uhm... puisque vous ne m'avais pas envoyé de message..."

# game/events_mc.rpy:2403
translate french d9_backhome_pcroom_baec15ef:

    # pc "Yeah, sorry about that, it was a really busy week. Lots of stuff happened."
    pc "Ouais, désolé pour ça, c'était une semaine très chargée. Beaucoup de choses se sont passées."

# game/events_mc.rpy:2405
translate french d9_backhome_pcroom_7bbac3ba:

    # h "It did for sure!"
    h "C'est sûr !"

# game/events_mc.rpy:2406
translate french d9_backhome_pcroom_f560ac96:

    # h "So do you still want to go on a date tomorrow?"
    h "Alors vous voulez toujours aller à un rendez-vous demain ?"

# game/events_mc.rpy:2407
translate french d9_backhome_pcroom_7e06d964:

    # pc "Yes, of course!"
    pc "Oui, bien sûr !"

# game/events_mc.rpy:2408
translate french d9_backhome_pcroom_fb659fe9:

    # h "{size=-20}Yes!{/size}... ahem... good, but uhm... the lock down basically disrupted all my plans..."
    h "{size=-20}Oui!{/size}... ahem... bien, mais uhm... la fermeture a fondamentalement perturbé tous mes plans..."

# game/events_mc.rpy:2409
translate french d9_backhome_pcroom_9bebaa59:

    # pc "Lock down?"
    pc "La fermeture ?"

# game/events_mc.rpy:2410
translate french d9_backhome_pcroom_7c618874:

    # h "You didn't get the news yet? All public areas like restaurants, bars and so on are closed from tomorrow on."
    h "Vous n'avez pas encore eu la nouvelle ? Tous les lieux publics comme les restaurants, les bars, etc. sont fermés à partir de demain."

# game/events_mc.rpy:2411
translate french d9_backhome_pcroom_a7483d27:

    # pc "What?! Why?"
    pc "Quoi ?! Pourquoi ?"

# game/events_mc.rpy:2412
translate french d9_backhome_pcroom_e570a7e4:

    # h "Uhh... you don't watch TV?"
    h "Euh... vous ne regardez pas la télé ?"

# game/events_mc.rpy:2413
translate french d9_backhome_pcroom_52dde854:

    # pc "Not much lately..."
    pc "Pas grand-chose ces derniers temps..."

# game/events_mc.rpy:2414
translate french d9_backhome_pcroom_641e39b8:

    # h "You've really been busy, haven't you?"
    h "Vous avez été très occupé, n'est-ce pas ?"

# game/events_mc.rpy:2415
translate french d9_backhome_pcroom_429b9a1d:

    # pc "Yeah... sorry."
    pc "Ouais... désolé."

# game/events_mc.rpy:2416
translate french d9_backhome_pcroom_3f39ef32:

    # h "It's okay, it's nice that you didn't forget about me."
    h "C'est bon, c'est bien que vous ne m'ayez pas oublié."

# game/events_mc.rpy:2418
translate french d9_backhome_pcroom_35c9fc02:

    # pc "How could I forget such a cute girl."
    pc "Comment aurais-je pu oublier une fille aussi mignonne."

# game/events_mc.rpy:2419
translate french d9_backhome_pcroom_d4e6c9d6:

    # h "Haha, thanks!"
    h "Haha, merci !"

# game/events_mc.rpy:2420
translate french d9_backhome_pcroom_8ddf7669:

    # pc "So where do you want to go tomorrow if everything's closed?"
    pc "Alors où voulez-vous aller demain si tout est fermé ?"

# game/events_mc.rpy:2421
translate french d9_backhome_pcroom_a39d5d94:

    # h "Oh I have an idea already, but I won't tell, hehe."
    h "Oh j'ai déjà une idée, mais je ne le dirai pas, héhé."

# game/events_mc.rpy:2422
translate french d9_backhome_pcroom_84edfdb0:

    # pc "Oh come on, that's mean."
    pc "Oh allez, c'est méchant."

# game/events_mc.rpy:2423
translate french d9_backhome_pcroom_121e0175:

    # h "Hehe... well, there is a nice place I'd like to go."
    h "Héhé... bien, il y a un bel endroit où j'aimerais aller."

# game/events_mc.rpy:2424
translate french d9_backhome_pcroom_227113d8:

    # pc "Where is it?"
    pc "Où est-il ?"

# game/events_mc.rpy:2425
translate french d9_backhome_pcroom_f6f5d842:

    # h "Hmm... I'll tell you this much: It's a quiet and beautiful place."
    h "Hmm... Je vais vous dire ceci : C'est un endroit calme et magnifique."

# game/events_mc.rpy:2426
translate french d9_backhome_pcroom_40891c8f:

    # pc "Quiet and beautiful, huh?"
    pc "Calme et magnifique, hein ?"

# game/events_mc.rpy:2427
translate french d9_backhome_pcroom_ba1b1223:

    # h "Yes, I think you'll like it."
    h "Oui, je pense que vous l'aimerez."

# game/events_mc.rpy:2428
translate french d9_backhome_pcroom_70cd033b:

    # pc "I bet!"
    pc "Je parie !"

# game/events_mc.rpy:2430
translate french d9_backhome_pcroom_e2a2f85a:

    # pc "Do I need a coat and tie?"
    pc "Ai-je besoin d'un manteau et d'une cravate ?"

# game/events_mc.rpy:2432
translate french d9_backhome_pcroom_b4c4f045:

    # pc "Do I need a fancy dress?"
    pc "Ai-je besoin d'un déguisement ?"

# game/events_mc.rpy:2433
translate french d9_backhome_pcroom_b39a23b4:

    # h "Haha, no! You'll look good anyway..."
    h "Haha, non ! Vous serez bien comme ça..."

# game/events_mc.rpy:2434
translate french d9_backhome_pcroom_723709d4:

    # pc "Now you're making me blush."
    pc "Maintenant vous me faites rougir."

# game/events_mc.rpy:2435
translate french d9_backhome_pcroom_84e2e925:

    # h "Hehe... {size=-20}*yaaaaawn*{/size}"
    h "Héhé... {size=-20}*yaaaaawn*{/size}"

# game/events_mc.rpy:2436
translate french d9_backhome_pcroom_feedf2db:

    # pc "Well, I can't wait, but I better let you sleep now."
    pc "Eh bien, je ne peux pas attendre, mais je ferais mieux de vous laisser dormir maintenant."

# game/events_mc.rpy:2437
translate french d9_backhome_pcroom_cd6fd29c:

    # h "Oh I'm not tired, really!"
    h "Oh je ne suis pas fatiguée, vraiment !"

# game/events_mc.rpy:2438
translate french d9_backhome_pcroom_034c3085:

    # pc "Heh, we'll have a lot of time to talk tomorrow, and you know what?"
    pc "Heu, nous aurons beaucoup de temps pour parler demain, et vous savez quoi ?"

# game/events_mc.rpy:2439
translate french d9_backhome_pcroom_ff932e82:

    # h "What?"
    h "Quoi ?"

# game/events_mc.rpy:2440
translate french d9_backhome_pcroom_54362f51:

    # pc "It will be even better, because we can see each other."
    pc "Ce sera encore mieux, car nous pourrons nous voir."

# game/events_mc.rpy:2441
translate french d9_backhome_pcroom_419d2aa8:

    # h "Haha, right! And maybe even..."
    h "Haha, c'est vrai ! Et peut-être même..."

# game/events_mc.rpy:2442
translate french d9_backhome_pcroom_182bc93e:

    # h "Touch... haha!"
    h "Se toucher... haha !"

# game/events_mc.rpy:2443
translate french d9_backhome_pcroom_09f0e787:

    # pc "Haha, you're cute! Good night and sweet dreams, [h]!"
    pc "Haha, vous êtes mignonne ! Bonne nuit et beaux rêves, [h]!"

# game/events_mc.rpy:2444
translate french d9_backhome_pcroom_79363e4c:

    # h "You too! Sweet dreams, [pc]! See you tomorrow!"
    h "Vous aussi ! Faites de beaux rêves, [pc]! A demain !"

# game/events_mc.rpy:2445
translate french d9_backhome_pcroom_806f8391:

    # pc "See you tomorrow, [h]!"
    pc "A demain, [h]!"

# game/events_mc.rpy:2447
translate french d9_backhome_pcroom_a84cf61e:

    # pcthink "Wow, she's so cute..."
    pcthink "Wow, elle est si mignonne..."

# game/events_mc.rpy:2449
translate french d9_backhome_pcroom_276756bd:

    # pcthink "I'm curious where she wants to go tomorrow..."
    pcthink "Je suis curieux de savoir où elle veut aller demain..."

# game/events_mc.rpy:2451
translate french d9_backhome_pcroom_8cbec36f:

    # pcthink "I guess I'll have to go to her office again..."
    pcthink "Je suppose que je vais devoir retourner à son bureau..."

# game/events_mc.rpy:2452
translate french d9_backhome_pcroom_2b1354a9:

    # pcthink "Anyway, time to sleep."
    pcthink "Bref, il est temps de dormir."

# game/events_mc.rpy:2460
translate french d9dream_99c802ff:

    # Voice" "*Sniff*"
    "Voix" "*Renifler*"

# game/events_mc.rpy:2461
translate french d9dream_8f56933b:

    # n "You can hear a weeping noise."
    n "Vous pouvez entendre un bruit de pleurs."

# game/events_mc.rpy:2462
translate french d9dream_e701d572:

    # pc "Dad?"
    pc "Papa ?"

# game/events_mc.rpy:2463
translate french d9dream_9aef7dc5:

    # Dad" "Oh hey, [pc]... *sniff*"
    "Papa" "Oh hé, [pc]... *renifle*"

# game/events_mc.rpy:2464
translate french d9dream_e6e5c075:

    # n "He wipes tears from his eyes. You can see him holding a photo of your mom in his hands."
    n "Il essuie les larmes de ses yeux. Vous pouvez le voir tenir une photo de votre mère dans ses mains."

# game/events_mc.rpy:2465
translate french d9dream_84c70355:

    # Dad" "You're back early..."
    "Papa" "Tu rentres tôt..."

# game/events_mc.rpy:2466
translate french d9dream_96535a88:

    # pc "Are you crying, Dad?"
    pc "Tu pleures, papa ?"

# game/events_mc.rpy:2467
translate french d9dream_e24b33bd:

    # Dad" "No... *sniff* ...must've gotten something in my eye..."
    "Papa" "Non... *renifle* ...j'ai dû me mettre quelque chose dans l'œil..."

# game/events_mc.rpy:2468
translate french d9dream_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:2469
translate french d9dream_da7061f8:

    # Dad" "Do you miss your mom, [pc]?"
    "Papa" "Ta mère te manque-t-elle, [pc]?"

# game/events_mc.rpy:2470
translate french d9dream_3b5608b8:

    # pc "Yes..."
    pc "Oui..."

# game/events_mc.rpy:2471
translate french d9dream_234a4777:

    # Dad" "Hm... you should forget about her..."
    "Papa" "Hm... tu devrais l'oublier..."

# game/events_mc.rpy:2472
translate french d9dream_0628eca5:

    # pc "I don't want to forget her!"
    pc "Je ne veux pas l'oublier !"

# game/events_mc.rpy:2473
translate french d9dream_d262fbf2:

    # Dad" "But it's better for you... for both of us..."
    "Papa" "Mais c'est mieux pour toi... pour nous deux..."

# game/events_mc.rpy:2474
translate french d9dream_85c0867b:

    # Dad" "...and do yourself a favor and never fall in love..."
    "Papa" "...et fais-toi une faveur, ne tombe jamais amoureux..."

# game/events_mc.rpy:2475
translate french d9dream_a69377ef_1:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:2476
translate french d9dream_785d229f:

    # n "He puts the photo of your mom into a drawer."
    n "Il met la photo de votre mère dans un tiroir."

# game/events_mc.rpy:2477
translate french d9dream_f363bd8e:

    # pc "Don't you love [m], Dad?"
    pc "Tu n'aimes pas [m], papa?"

# game/events_mc.rpy:2478
translate french d9dream_51070d0f:

    # Dad" "[m]?! Pff..."
    "Papa" "[m]?! Pff..."

# game/events_mc.rpy:2479
translate french d9dream_14cfced5:

    # Dad" "That b... she doesn't even know how to cook a proper meal, let alone how to suck a..."
    "Papa" "Cette b... elle ne sait même pas comment préparer un bon repas, encore moins comment sucer un..."

# game/events_mc.rpy:2480
translate french d9dream_8854a3cf:

    # Dad" "Uch... damn it..."
    "Papa" "Uch... merde..."

# game/events_mc.rpy:2481
translate french d9dream_d582bfa7:

    # Dad" "Just always remember that she's not your mom, even if she tries to be... she's useless and she'll never be your mom!"
    "Papa" "Rappelle-toi toujours qu'elle n'est pas ta mère, même si elle essaie de l'être... elle est inutile et elle ne sera jamais ta mère !"

# game/events_mc.rpy:2482
translate french d9dream_5d7119e2:

    # pc "...okay..."
    pc "...ok..."

# game/events_mc.rpy:2483
translate french d9dream_155a3712:

    # Dad" "Now go back to your room or go out and play or whatever you do..."
    "Papa" "Retourne dans ta chambre ou va jouer dehors, comme tu veux..."

# game/events_mc.rpy:2484
translate french d9dream_880b4aa8:

    # pc "Okay..."
    pc "Ok..."

# game/events_mc.rpy:2485
translate french d9dream_465f1434:

    # n "You're about to leave the room..."
    n "Vous êtes sur le point de quitter la pièce..."

# game/events_mc.rpy:2486
translate french d9dream_14e7ad76:

    # Dad" "[pc]?"
    "Papa" "[pc]?"

# game/events_mc.rpy:2487
translate french d9dream_bbe33639:

    # pc "Yes, Dad?"
    pc "Oui, papa ?"

# game/events_mc.rpy:2488
translate french d9dream_0b01384d:

    # Dad" "I'm sorry that I..."
    "Papa" "je suis désolé de..."

# game/events_mc.rpy:2489
translate french d9dream_503d5266:

    # Dad" "...sometimes..."
    "Papa" "...parfois..."

# game/events_mc.rpy:2490
translate french d9dream_cde1b3be:

    # Dad" "..."
    "Papa" "..."

# game/events_mc.rpy:2491
translate french d9dream_b283b4e3:

    # Dad" "Never mind, just go..."
    "Papa" "Peu importe, va..."

# game/events_mc.rpy:2497
translate french d10wakeup_e2ee86ba:

    # pc "Uhh..."
    pc "Uhh..."

# game/events_mc.rpy:2498
translate french d10wakeup_bfc6ca53:

    # pc "{cps=10}...{/cps}"
    pc "{cps=10}...{/cps}"

# game/events_mc.rpy:2499
translate french d10wakeup_debe3bfd:

    # pcthink "These dreams..."
    pcthink "Ces rêves..."

# game/events_mc.rpy:2500
translate french d10wakeup_1c3a6ac3:

    # pcthink "...I hate them..."
    pcthink "...je les déteste..."

# game/events_mc.rpy:2501
translate french d10wakeup_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:2502
translate french d10wakeup_015a74bc:

    # pcthink "I think that was the only time ever I saw the old man crying..."
    pcthink "Je pense que c'est la seule fois que j'ai vu le vieil homme pleurer..."

# game/events_mc.rpy:2503
translate french d10wakeup_8fd5c2f4:

    # pcthink "He didn't even cry at Mom's funeral..."
    pcthink "Il n'a même pas pleuré à l'enterrement de maman..."

# game/events_mc.rpy:2504
translate french d10wakeup_f0cac90d_1:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:2505
translate french d10wakeup_853f810c:

    # pcthink "Shit, time to get up."
    pcthink "Merde, il est temps de se lever."

# game/events_mc.rpy:2507
translate french d10wakeup_550133db:

    # n "You get up and take a shower."
    n "Vous vous levez et prenez une douche."

# game/events_mc.rpy:2511
translate french d10pcroom_11ffdab3:

    # pcthink "Alright, what to do now... hmm..."
    pcthink "Très bien, que faire maintenant... hmm..."

# game/events_mc.rpy:2514
translate french d10pcroom_82548f9b:

    # j "Morning!"
    j "Bonjour !"

# game/events_mc.rpy:2515
translate french d10pcroom_c3dd90d4:

    # pc "Mornin' [j], what's up?"
    pc "Jour' [j], quoi de neuf ?"

# game/events_mc.rpy:2516
translate french d10pcroom_ba46d8da:

    # j "Follow me, I want to show you something!"
    j "Suis-moi, je veux te montrer quelque chose !"

# game/events_mc.rpy:2517
translate french d10pcroom_9c6c5efc:

    # pc "Huh? What is it?"
    pc "Huh ? Qu'est-ce que c'est ?"

# game/events_mc.rpy:2519
translate french d10pcroom_e2c8352b:

    # j "Just come, it's a surprise!"
    j "Viens, c'est une surprise !"

# game/events_mc.rpy:2520
translate french d10pcroom_76c0b06f:

    # pc "Eh... fine..."
    pc "Eh... bien..."

# game/events_mc.rpy:2522
translate french d10pcroom_2aea1f34:

    # n "You follow her to [e]'s room."
    n "Vous la suivez dans la chambre de [e]."

# game/events_mc.rpy:2529
translate french d10johnson_225fadf5:

    # pc "Huh?"
    pc "Huh ?"

# game/events_mc.rpy:2531
translate french d10johnson_c97cde28:

    # pc "What's going on here? [oc]?!"
    pc "Qu'est-ce qui se passe ici ? [oc]?!"

# game/events_mc.rpy:2533
translate french d10johnson_30d49bc7:

    # m "Oh [pc], there was this policeman here while you were gone..."
    m "Oh [pc], il y avait ce policier ici pendant que tu étais parti..."

# game/events_mc.rpy:2535
translate french d10johnson_3f113d39:

    # oc "Looks like [w] still doesn't want to give up."
    oc "On dirait que [w] ne veut toujours pas abandonner."

# game/events_mc.rpy:2537
translate french d10johnson_89238e9a:

    # pc "[w]? Oh no, not that guy again."
    pc "[w]? Oh non, pas encore ce type."

# game/events_mc.rpy:2539
translate french d10johnson_cf23b743:

    # j "You know that guy?"
    j "Vous connaissez ce type ?"

# game/events_mc.rpy:2540
translate french d10johnson_72964e10:

    # pc "Sadly, yes. What happened, what did he want?"
    pc "Malheureusement, oui. Que s'est-il passé, que voulait-il ?"

# game/events_mc.rpy:2541
translate french d10johnson_32b17776:

    # j "He was asking weird questions and acted like we're all gangsters..."
    j "Il posait des questions bizarres et agissait comme si nous étions tous des gangsters..."

# game/events_mc.rpy:2543
translate french d10johnson_9f733bc4:

    # oc "m "called me after he was gone."
    oc "m "m'a appelé après qu'il soit parti."

# game/events_mc.rpy:2545
translate french d10johnson_a731af6d:

    # m "I didn't know what to do, I was confused and scared."
    m "Je ne savais pas quoi faire, j'étais confuse et effrayée."

# game/events_mc.rpy:2547
translate french d10johnson_b6f21aca:

    # oc "It's okay, [m], he can't do anything."
    oc "C'est bon, [m], il ne peut rien faire."

# game/events_mc.rpy:2549
translate french d10johnson_a57c8079:

    # pc "Jeez, [oc], isn't there anything you can do to stop him? Do we have to get an injunction?"
    pc "Bon sang, [oc], il n'y a rien que tu puisses faire pour l'arrêter ? Faut-il obtenir une injonction ?"

# game/events_mc.rpy:2551
translate french d10johnson_3bf3dfc8:

    # oc "It's not that easy, especially not with all the shit you've done in the past."
    oc "Ce n'est pas si facile, surtout pas avec toute la merde que tu as faite dans le passé."

# game/events_mc.rpy:2553
translate french d10johnson_c8912b9c:

    # pc "Oh come on, [oc], it's been years now."
    pc "Oh allez, [oc], ça fait des années maintenant."

# game/events_mc.rpy:2555
translate french d10johnson_0c42f081:

    # oc "Yeah, but not long enough to be removed from your file, if there's anything he can link to you, any old case, even if it's just a suspicion, he has all the rights to go after it."
    oc "Ouais, mais pas assez longtemps pour être retiré de ton dossier, s'il y a quelque chose qu'il peut relier à toi, n'importe quelle vieille affaire, même si c'est juste un soupçon, il a tous les droits de s'en prendre à elle."

# game/events_mc.rpy:2557
translate french d10johnson_a01f7be6:

    # pc "Shit! So we can't do anything?"
    pc "Merde ! Alors on ne peut rien faire ?"

# game/events_mc.rpy:2559
translate french d10johnson_5af31d80:

    # oc "Nobody really takes him seriously and I told the chief about your case, so as long as [w] doesn't find any hard evidence for something, he won't be able to do much more than asking questions and be as annoying as always."
    oc "Personne ne le prend vraiment au sérieux et j'ai parlé de ton cas au chef, donc tant que [w] ne trouve pas de preuves tangibles pour quelque chose, il ne pourra pas faire beaucoup plus que poser des questions et être aussi ennuyeux que d'habitude."

# game/events_mc.rpy:2561
translate french d10johnson_d9907ef1:

    # m "Will he come back?"
    m "Va-t-il revenir ?"

# game/events_mc.rpy:2563
translate french d10johnson_cd8a6e2e:

    # oc "Possibly, but you can safely ignore him. He knows he can't do anything without getting himself into trouble."
    oc "Possible, mais vous pouvez l'ignorer sans risque. Il sait qu'il ne peut rien faire sans s'attirer des ennuis."

# game/events_mc.rpy:2565
translate french d10johnson_c33ac23e:

    # oc "Just don't do anything he can use as pretence."
    oc "Ne faites rien qu'il puisse utiliser comme prétexte."

# game/events_mc.rpy:2567
translate french d10johnson_6f057b0a:

    # m "Of course not!"
    m "Bien sûr que non !"

# game/events_mc.rpy:2569
translate french d10johnson_dd58edec:

    # oc "That was more or less directed at [pc]..."
    oc "Cela s'adressait plus ou moins à [pc]..."

# game/events_mc.rpy:2571
translate french d10johnson_c9a0e6ec:

    # oc "...and [j]."
    oc "...et [j]."

# game/events_mc.rpy:2573
translate french d10johnson_d1743dfa:

    # pc "Wait, you know each other?"
    pc "Attends, vous vous connaissez ?"

# game/events_mc.rpy:2575
translate french d10johnson_4042c389:

    # j "Eh...haha..."
    j "Eh...haha..."

# game/events_mc.rpy:2577
translate french d10johnson_e9043f27:

    # oc "Let's just say I wasn't surprised to find her here..."
    oc "Disons que je n'étais pas surpris de la trouver ici..."

# game/events_mc.rpy:2579
translate french d10johnson_aaf54574:

    # m "[j], I didn't know you had trouble with the law?"
    m "[j], je ne savais pas que tu avais des problèmes avec la loi ?"

# game/events_mc.rpy:2581
translate french d10johnson_5122a028:

    # j "It was an accident, I swear!"
    j "C'était un accident, je le jure !"

# game/events_mc.rpy:2583
translate french d10johnson_df701526:

    # oc "Don't worry about it, [m], she's just a little trouble maker, I'm sure [pc] can handle her."
    oc "Ne t'inquiète pas, [m], elle est juste un petit fauteur de trouble, je suis sûr que [pc] peut la gérer."

# game/events_mc.rpy:2584
translate french d10johnson_9fbca821:

    # j "Huh?"
    j "Huh ?"

# game/events_mc.rpy:2586
translate french d10johnson_15060167:

    # oc "Anyway, my wife is waiting, so I better take my leave. Call me if there's any trouble again."
    oc "De toute façon, ma femme attend, alors je ferais mieux de prendre congé. Appelez-moi s'il y a encore des problèmes."

# game/events_mc.rpy:2588
translate french d10johnson_0b42741a:

    # m "Thanks, [oc], give her my regards."
    m "Merci, [oc], transmets-lui mes salutations."

# game/events_mc.rpy:2590
translate french d10johnson_449844e5:

    # oc "Sure."
    oc "Bien sûr."

# game/events_mc.rpy:2592
translate french d10johnson_c9095921:

    # oc "...and you stay clear of trouble."
    oc "...et vous évitez les problèmes."

# game/events_mc.rpy:2593
translate french d10johnson_3639171b:

    # pc "Eh? Sure, I'll try my best."
    pc "Eh ? Bien sûr, je vais faire de mon mieux."

# game/events_mc.rpy:2595
translate french d10johnson_522fc3ca:

    # oc "{size=-10}...and take good care of [j], she needs someone like you to look after her.{/size}"
    oc "{size=-10}...et prends bien soin de [j], elle a besoin de quelqu'un comme toi pour s'occuper d'elle. {/size}"

# game/events_mc.rpy:2597
translate french d10johnson_36bb3adf:

    # pc "I know, thanks, [oc]."
    pc "Je sais, merci, [oc]."

# game/events_mc.rpy:2600
translate french d10johnson_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:2602
translate french d10johnson_f57abf22:

    # m "So, I think there's something we should talk about, [j]... but first..."
    m "Donc, je pense qu'il y a quelque chose dont nous devrions parler, [j]... mais d'abord..."

# game/events_mc.rpy:2604
translate french d10johnson_f6a949de:

    # m "...who is that [w] guy, [pc]?"
    m "...qui est ce [w] de gars, [pc]?"

# game/events_mc.rpy:2606
translate french d10johnson_1299eaec:

    # n "You explain the situation, how you met [w] on your very first day at the burning apartment and all the other occasions when you bumped into each other. How he suspected you to be a criminal for no reason..."
    n "Vous expliquez la situation, comment vous avez rencontré [w] lors de votre tout premier jour à l'appartement en feu et toutes les autres occasions où vous vous êtes croisés. Comment il vous a soupçonné d'être un criminel sans raison..."

# game/events_mc.rpy:2608
translate french d10johnson_113d07fe:

    # m "My god, I hope he doesn't dig too deep..."
    m "Mon dieu, j'espère qu'il ne creuse pas trop profond..."

# game/events_mc.rpy:2610
translate french d10johnson_640bba00:

    # e "Nh!"
    e "Nh !"

# game/events_mc.rpy:2612
translate french d10johnson_0bc33d60:

    # pc "As [oc] said, he can't do anything as long as we don't give him a reason."
    pc "Comme [oc] l'a dit, il ne peut rien faire tant qu'on ne lui donne pas de raison."

# game/events_mc.rpy:2614
translate french d10johnson_09fa3a44:

    # j "What an annoying guy."
    j "Quel type ennuyeux."

# game/events_mc.rpy:2616
translate french d10johnson_78fa2646:

    # m "Oh right, isn't there something you wanted to tell us, [j]? How did you and [oc] come to know each other?"
    m "Ah oui, il n'y a pas quelque chose que tu voulais nous dire, [j]? Comment as-tu fait connaissance avec [oc]?"

# game/events_mc.rpy:2618
translate french d10johnson_3773ac01:

    # j "Eh... it's nothing, really!"
    j "Eh... c'est rien, vraiment !"

# game/events_mc.rpy:2620
translate french d10johnson_3225628c:

    # m "[j], if you want to live with us, then you should at least be honest !"
    m "[j], si tu veux vivre avec nous, alors tu devrais au moins être honnête !"

# game/events_mc.rpy:2623
translate french d10johnson_22eff034:

    # m "I mean..."
    m "Je veux dire..."

# game/events_mc.rpy:2625
translate french d10johnson_ac734b6c:

    # m "Don't you agree, [pc]?"
    m "N'es-tu pas d'accord, [pc]?"

# game/events_mc.rpy:2626
translate french d10johnson_36106d45:

    # pc "Hm? Well, yeah, she's right, [j]."
    pc "Hm ? Eh bien, oui, elle a raison, [j]."

# game/events_mc.rpy:2628
translate french d10johnson_26f32268:

    # j "Hmph... fine..."
    j "Hmph... bien..."

# game/events_mc.rpy:2629
translate french d10johnson_2a972d71:

    # pcthink "This is probably going to be about the bike she took from her mom's ex..."
    pcthink "Cela va probablement être à propos de la moto qu'elle a pris à l'ex de sa mère..."

# game/events_mc.rpy:2631
translate french d10johnson_5d7b2792:

    # n "As expected, [j] talks about the motorcycle, but then she goes on about the argument she had with her Mom, explaining that it wasn't the first time, then about what happened in school and even bits and pieces about things that happened in the past."
    n "Comme prévu, [j] parle de la moto, mais elle enchaîne sur la dispute qu'elle a eue avec sa maman en expliquant que ce n'était pas la première fois, puis sur ce qui s'est passé à l'école et même des bribes de choses qui se sont passées dans le passé."

# game/events_mc.rpy:2632
translate french d10johnson_fb0503dc:

    # n "All in all it's nothing too dramatic, but it seems like she just needed a little push to go all out. She talks about her childhood, how she's never seen her real Dad, how most of her Mom's boyfriends were assholes or didn't care about her..."
    n "Dans l'ensemble, ce n'est pas trop dramatique, mais il semble qu'elle avait juste besoin d'un petit coup de pouce pour se lancer. Elle parle de son enfance, du fait qu'elle n'a jamais vu son vrai père, que la plupart des petits amis de sa mère étaient des connards ou ne s'intéressaient pas à elle..."

# game/events_mc.rpy:2633
translate french d10johnson_365476fa:

    # n "After roughly an hour..."
    n "Après environ une heure..."

# game/events_mc.rpy:2635
translate french d10johnson_b4fa3fc0:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:2636
translate french d10johnson_e612037c:

    # m "..."
    new "[m]..."

# game/events_mc.rpy:2638
translate french d10johnson_72a33094:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:2639
translate french d10johnson_a7c89ecd:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:2641
translate french d10johnson_88ec1494:

    # pc "Well, now that you're here with us, you don't have to worry about that anymore."
    pc "Eh bien, maintenant que tu es ici avec nous, tu n'as plus à t'inquiéter de ça."

# game/events_mc.rpy:2643
translate french d10johnson_ee170f72:

    # m "Absolutely, let the past be the past and focus on the future."
    m "Absolument, laisses le passé au passé et concentres-toi sur l'avenir."

# game/events_mc.rpy:2645
translate french d10johnson_9810575c:

    # j "Eh, sorry, I didn't mean to tell my whole life story!"
    j "Eh, désolé, je ne voulais pas raconter toute l'histoire de ma vie !"

# game/events_mc.rpy:2646
translate french d10johnson_43e4e875:

    # pc "Sometimes things have to be said."
    pc "Parfois, les choses doivent être dites."

# game/events_mc.rpy:2648
translate french d10johnson_e0a84304:

    # j "Maybe, but it feels like I've been talking all evening..."
    j "Peut-être, mais j'ai l'impression d'avoir parlé toute la soirée..."

# game/events_mc.rpy:2650
translate french d10johnson_55dd0a42:

    # m "Oh, right. I should make dinner."
    m "Oh, c'est vrai. Je devrais faire le dîner."

# game/events_mc.rpy:2652
translate french d10johnson_e59f988c:

    # j "I-I'll help!"
    j "Je-je vais t'aider !"

# game/events_mc.rpy:2653
translate french d10johnson_97a38a54:

    # m "You don't need to, [j]."
    m "Tu n'en as pas besoin, [j]."

# game/events_mc.rpy:2654
translate french d10johnson_7509b543:

    # j "But I want to..."
    j "Mais je le veux..."

# game/events_mc.rpy:2656
translate french d10johnson_5f7c3303:

    # pc "*Smirk*"
    pc "*Sourire en coin*"

# game/events_mc.rpy:2659
translate french d10johnson_39681a47:

    # pc "Haha, the new Housekeeper update is funny!"
    pc "Haha, la mise à jour de la nouvelle gouvernante est drôle !"

# game/events_mc.rpy:2661
translate french d10johnson_2d2fab39:

    # pcthink "Too bad it's male MC only..."
    pcthink "Dommage que ce ne soit que des MC masculins..."

# game/events_mc.rpy:2663
translate french d10johnson_f11f8b6b:

    # pc "Hey [e], you should have a look at this!"
    pc "Hé [e], tu devrais jeter un coup d'œil à ceci !"

# game/events_mc.rpy:2664
translate french d10johnson_72a33094_1:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:2666
translate french d10johnson_a9ffb6ca:

    # pc "...or just stay there and look cute."
    pc "...ou juste rester là et avoir l'air mignonne."

# game/events_mc.rpy:2668
translate french d10johnson_ff1cafc9:

    # pc "...or just stay there looking sexy."
    pc "...ou juste rester là en étant sexy."

# game/events_mc.rpy:2670
translate french d10johnson_3fa6b1c9:

    # e "Khehehe."
    e "Khéhéhéhé."

# game/events_mc.rpy:2671
translate french d10johnson_affa0beb:

    # pc "...hmm..."
    pc "...hmm..."

# game/events_mc.rpy:2672
translate french d10johnson_bf217bfc:

    # pc "You know, I think [j] is right, I really should get a new TV."
    pc "Tu sais, je pense que [j] a raison, je devrais vraiment acheter une nouvelle télé."

# game/events_mc.rpy:2674
translate french d10johnson_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:2675
translate french d10johnson_84395206:

    # pc "By the way, why don't you have your own TV?"
    pc "Au fait, pourquoi n'as-tu pas ta propre télévision ?"

# game/events_mc.rpy:2676
translate french d10johnson_4e286571:

    # e "Hm..."
    e "Hm..."

# game/events_mc.rpy:2677
translate french d10johnson_36c9a7bf:

    # pc "...and why does [m] think you're bad with electronics?"
    pc "...et pourquoi [m] pense-t-elle que tu es mauvaise en électronique ?"

# game/events_mc.rpy:2679
translate french d10johnson_a0a4da04:

    # e "Uhh..."
    e "Uhh..."

# game/events_mc.rpy:2683
translate french d10johnson_00b9e6c9:

    # j "Time to stop drooling over each other, dinner is readyyyyy~!"
    j "Il est temps d'arrêter de baver sur l'autre, le dîner est prêttttt~ !"

# game/events_mc.rpy:2685
translate french d10johnson_9a8ef00c:

    # n "You go have dinner and it tastes surprisingly well, even though you half expected it to be a disaster since [j] helped with it." with dissolve
    n "Vous allez dîner et le goût est étonnamment bon, même si vous vous attendiez à moitié à ce que ce soit un désastre puisque [j] a aidé à le préparer." with dissolve

# game/events_mc.rpy:2686
translate french d10johnson_3855fdcb:

    # n "It's quite late already, so everyone quickly goes to bed after dinner, you decide do the same."
    n "Il est déjà assez tard, alors tout le monde va rapidement se coucher après le dîner, vous décidez de faire de même."

# game/events_mc.rpy:2701
translate french d10nightsex_38cd2de2:

    # Voice" "Wakey wakey." with slowdissolve
    "Voix" "Wakey wakey." with slowdissolve

# game/events_mc.rpy:2702
translate french d10nightsex_9f4f5a57:

    # pc "Uhh... is it morning already?"
    pc "Uhh... c'est déjà le matin ?"

# game/events_mc.rpy:2703
translate french d10nightsex_9bdc0426:

    # Voice" "No, Silly, it's still night!"
    "Voix" "Non, idiot, c'est encore la nuit !"

# game/events_mc.rpy:2704
translate french d10nightsex_109cf5a7:

    # Voice" "Khehehe."
    "Voix" "Khéhéhéhé."

# game/events_mc.rpy:2706
translate french d10nightsex_61e4f5ac:

    # pc "Huh?!"
    pc "Huh ?!"

# game/events_mc.rpy:2708
translate french d10nightsex_2391c076:

    # pc "[j]!?"
    pc "[j]!?"

# game/events_mc.rpy:2710
translate french d10nightsex_6df89eaf:

    # pc "[e], what are you..."
    pc "[e], que fais-tu..."

# game/events_mc.rpy:2711
translate french d10nightsex_3af9c0d7:

    # pc "Aren't you supposed to sleep in your bed?"
    pc "N'es-tu pas censé dormir dans ton lit ?"

# game/events_mc.rpy:2713
translate french d10nightsex_72a33094:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:2714
translate french d10nightsex_b74916c3:

    # j "Well, we're not sleeping..."
    j "Eh bien, nous ne dormons pas..."

# game/events_mc.rpy:2716
translate french d10nightsex_7a3f2b13:

    # j "We're just here to say good night, hehe."
    j "Nous sommes juste ici pour dire bonne nuit, héhé."

# game/events_mc.rpy:2717
translate french d10nightsex_de5fe506:

    # pc "I see..."
    pc "Je vois..."

# game/events_mc.rpy:2723
translate french d10nightsex_aadd0180:

    # e "*Smootch*"
    e "*Bises*"

# game/events_mc.rpy:2725
translate french d10nightsex_bae972de:

    # pc "Oh, I think I like where this is going!"
    pc "Oh, je pense que j'aime où cela va !"

# game/events_mc.rpy:2727
translate french d10nightsex_6c679a4b:

    # j "So, are we allowed to stay a little longer, [pcmd]?"
    j "Alors, avons-nous le droit de rester un peu plus longtemps, [pcmd]?"

# game/events_mc.rpy:2728
translate french d10nightsex_41979139:

    # pc "How could I possibly say no."
    pc "Comment pourrais-je dire non."

# game/events_mc.rpy:2729
translate french d10nightsex_53154678:

    # j "Hmm, thank you, [pcmd]."
    j "Hmm, merci, [pcmd]."

# game/events_mc.rpy:2731
translate french d10nightsex_8bcba63c:

    # j "*Smootch*"
    j "*Biiisssous*"

# game/events_mc.rpy:2733
translate french d10nightsex_8bcba63c_1:

    # j "*Smootch*"
    j "*Biiisssous*"

# game/events_mc.rpy:2734
translate french d10nightsex_aadd0180_1:

    # e "*Smootch*"
    e "*Bises*"

# game/events_mc.rpy:2736
translate french d10nightsex_08a84e50:

    # pcthink "Damn, this is so hot!"
    pcthink "Merde, c'est tellement chaud !"

# game/events_mc.rpy:2749
translate french d10nightsex_86321039:

    # j "You or me?"
    j "Toi ou moi ?"

# game/events_mc.rpy:2751
translate french d10nightsex_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:2753
translate french d10nightsex_7ad69330:

    # j "You decide."
    j "Tu as décidé."

# game/events_mc.rpy:2755
translate french d10nightsex_bfd83a51:

    # pc "Decide what?"
    pc "Décider quoi ?"

# game/events_mc.rpy:2757
translate french d10nightsex_54c5f8f0:

    # j "Whose pussy you want to lick, hehe."
    j "La chatte de qui tu veux lécher, héhé."

# game/events_mc.rpy:2758
translate french d10nightsex_96184459:

    # pc "Oh, I see."
    pc "Oh, je vois."

# game/events_mc.rpy:2760
translate french d10nightsex_d8629ddd:

    # pc "Why not both?"
    pc "Pourquoi pas les deux ?"

# game/events_mc.rpy:2762
translate french d10nightsex_46ae205e:

    # j "You can't do us both at the same time."
    j "Tu ne peux pas faire les deux en même temps."

# game/events_mc.rpy:2764
translate french d10nightsex_ea206375:

    # pc "Ha! Are you sure about that?"
    pc "Ha ! Tu es sûr de ça ?"

# game/events_mc.rpy:2767
translate french d10nightsex_042cf2c5:

    # j "Haha, okay, who do you want to taste first then?"
    j "Haha, ok, qui veux-tu goûter en premier alors ?"

# game/events_mc.rpy:2769
translate french d10nightsex_f8a565cd:

    # j "Haha, but who gets your dick first?"
    j "Haha, mais qui aura ta bite en premier ?"

# game/events_mc.rpy:2771
translate french d10nightsex_66c3f992:

    # pc "Tough question."
    pc "Question difficile."

# game/events_mc.rpy:2775
translate french d10nightfun_0fcb1868:

    # j "Awww..."
    j "Awww..."

# game/events_mc.rpy:2778
translate french d10nightfun_fd2b49ba:

    # e "Mmm..."
    e "Mmm..."

# game/events_mc.rpy:2780
translate french d10nightfun_ba55f128:

    # e "...nnnaah."
    e "...nnnaan."

# game/events_mc.rpy:2789
translate french d10nightfun_3b76887f:

    # j "Yay!"
    j "Ouais !"

# game/events_mc.rpy:2794
translate french d10nightfun_03dc513d:

    # j "But leave me some pussy, okay, [e]?"
    j "Mais laisse-moi un peu de chatte, ok, [e]?"

# game/events_mc.rpy:2795
translate french d10nightfun_36eeb504:

    # e "Kehehe."
    e "Kehéhéhe."

# game/events_mc.rpy:2798
translate french d10nle_d2992ced:

    # pc "Come here, [e]."
    pc "Viens ici, [e]."

# game/events_mc.rpy:2800
translate french d10nle_9d52e2e8:

    # j "I'll take that yummy pussy then."
    j "Je vais prendre cette délicieuse chatte alors."

# game/events_mc.rpy:2810
translate french d10nle_b43efd55:

    # j "Mhhh... I love how you taste, [pcmd]!"
    j "Mhhh... J'aime ton goût, [pcmd]!"

# game/events_mc.rpy:2811
translate french d10nle_2d99ee61:

    # pc "Nhhh, and I love how you work your tongue!"
    pc "Nhhh, et j'aime comment tu travailles avec ta langue !"

# game/events_mc.rpy:2812
translate french d10nle_380637e2:

    # j "Just don't come too early, you two!"
    j "Ne viens pas trop tôt, nous sommes deux !"

# game/events_mc.rpy:2813
translate french d10nle_7f627bb0:

    # e "Hnnn... hn!"
    e "Hnnn... hn !"

# game/events_mc.rpy:2814
translate french d10nle_86446217:

    # pc "I can't guarantee for anything if you go on like this, [j]."
    pc "Je ne peux rien garantir si tu continues comme ça, [j]."

# game/events_mc.rpy:2815
translate french d10nle_02ea632c:

    # j "Hehe... mphh... I love... mhh... your pussy, [pcmd]."
    j "Héhé... mphh... j'adore... mhh... ta chatte, [pcmd]."

# game/events_mc.rpy:2816
translate french d10nle_8829d4f3:

    # j "I just... mhh... can't hold back!"
    j "J'ai juste... mhh... je ne peux pas me retenir !"

# game/events_mc.rpy:2820
translate french md10nle_3b7ae288:

    # pc "[j] come over here, I want to taste you as well!"
    pc "[j] viens par ici, je veux te goûter aussi !"

# game/events_mc.rpy:2821
translate french md10nle_3b76887f:

    # j "Yay!"
    j "Ouais !"

# game/events_mc.rpy:2825
translate french md10nle_e45fc757:

    # j "Here I cum, [pcmd]!"
    j "Ici je jouis, [pcmd]!"

# game/events_mc.rpy:2826
translate french md10nle_76ae973b:

    # pc "Heh, not yet!"
    pc "Heu, pas encore !"

# game/events_mc.rpy:2829
translate french md10nle_256cb5d1:

    # e "Nhh... nhhh..."
    e "Nhh... nhhh..."

# game/events_mc.rpy:2830
translate french md10nle_04eac808:

    # j "[e]... mhh... hold on... mphh."
    j "[e]... mhh... attends... mphh."

# game/events_mc.rpy:2831
translate french md10nle_c3d61fdd:

    # e "Nhh... haaa..."
    e "Nhh... haaa..."

# game/events_mc.rpy:2832
translate french md10nle_539e527d:

    # j "Hold on, [e], hold on..."
    j "Attends, [e], attends..."

# game/events_mc.rpy:2836
translate french md10nle_ace490b7:

    # e "Haaaa!"
    e "Haaaa !"

# game/events_mc.rpy:2838
translate french md10nle_ed6b3306:

    # e "Haaa..."
    e "Haaa..."

# game/events_mc.rpy:2839
translate french md10nle_6ac5ba71:

    # j "Dammit, [e]."
    j "Merde, [e]."

# game/events_mc.rpy:2845
translate french d10nlj_b4fa3fc0:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:2853
translate french d10nlj_6db3b035:

    # j "Hnnn... I just... gnnnh... love your tongue... haaa...!"
    j "Hnnn... J'adore... gnnnh... ta langue... haaa... !"

# game/events_mc.rpy:2854
translate french d10nlj_743809c6:

    # j "Do you... hnn... like how I taste, [pcmd]?"
    j "Est-ce que tu... hnn... aimes mon goût, [pcmd]?"

# game/events_mc.rpy:2855
translate french d10nlj_82adeed6:

    # pc "I love it!"
    pc "J'adore ça !"

# game/events_mc.rpy:2856
translate french d10nlj_e9278008:

    # j "Hnn... yes, [pcmd], right there."
    j "Hnn... oui, [pcmd], juste là."

# game/events_mc.rpy:2863
translate french md10nlj_556b7777:

    # j "Your turn, [e]!"
    j "Ton tour, [e]!"

# game/events_mc.rpy:2864
translate french md10nlj_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:2867
translate french md10nlj_a8361128:

    # e "Mhhh..."
    e "Mhhh..."

# game/events_mc.rpy:2868
translate french md10nlj_59cd70f8:

    # j "Gaawd... [pcmd]... nhh... I love it... hnnn."
    j "Bon sang... [pcmd]... nhh... J'aime ça... hnnn."

# game/events_mc.rpy:2869
translate french md10nlj_367bea97:

    # j "Hold on, hold on... fuck!"
    j "Attends, attends... putain !"

# game/events_mc.rpy:2873
translate french md10nlj_4f2be3b6:

    # j "Haaaa!"
    j "Haaaa !"

# game/events_mc.rpy:2875
translate french md10nlj_ec25329c:

    # j "Haaa... your tongue feels amazing, [pcmd]."
    j "Haaa... ta langue est incroyable, [pcmd]."

# game/events_mc.rpy:2881
translate french d10nfe_5916f987:

    # e "Hnn..."
    e "Hnn..."

# game/events_mc.rpy:2883
translate french d10nfe_57867369:

    # e "Nhhhaa..."
    e "Nhhhaa..."

# game/events_mc.rpy:2886
translate french d10nfe_b4fa3fc0:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:2890
translate french d10nfe_fee914df:

    # j "Go on, [e]."
    j "Continue, [e]."

# game/events_mc.rpy:2899
translate french d10nfe_a4b12779:

    # e "Haa... haaa..."
    e "Haa... haaa..."

# game/events_mc.rpy:2900
translate french d10nfe_31c6bd65:

    # j "Damn, [e], don't go too fast yet!"
    j "Merde, [e], ne va pas encore trop vite !"

# game/events_mc.rpy:2902
translate french d10nfe_8c2d61de:

    # j "I want some fun, too..."
    j "Je veux aussi m'amuser..."

# game/events_mc.rpy:2904
translate french d10nfe_2b66b452:

    # j "If you don't mind, [pcmd]."
    j "Si tu le veux bien, [pcmd]."

# game/events_mc.rpy:2905
translate french d10nfe_651eb3f3:

    # pc "Get get over here, Babygirl!"
    pc "Viens par ici, petite fille !"

# game/events_mc.rpy:2906
translate french d10nfe_3b76887f:

    # j "Yay!"
    j "Ouais !"

# game/events_mc.rpy:2908
translate french d10nfe_c2622bd7:

    # j "I hope you don't mind, [e]."
    j "J'espère que cela ne te dérange pas, [e]."

# game/events_mc.rpy:2910
translate french d10nfe_72a33094:

    # e "..."
    new "[e]..."

# game/events_mc.rpy:2918
translate french d10nfe_4d63df1b:

    # j "Hnn... how are you holding up, [e]?"
    j "Hnn... comment tu tiens le coup, [e]?"

# game/events_mc.rpy:2919
translate french d10nfe_5983978b:

    # e "Nhhh!...hnnn..."
    e "Nhhhh !...hnnn..."

# game/events_mc.rpy:2921
translate french d10nfe_15da9523:

    # j "Ohhh, damn, [pcmd]... this feels great!"
    j "Ohhh, bon sang, [pcmd]... ça fait du bien !"

# game/events_mc.rpy:2922
translate french d10nfe_805a228c:

    # e "Hnn... nnhhh..."
    e "Hnn... nnhhh..."

# game/events_mc.rpy:2923
translate french d10nfe_1e48254c:

    # j "Nhhh... yes, there... haaaa!"
    j "Nhhh... oui, là... haaaa !"

# game/events_mc.rpy:2924
translate french d10nfe_b8b3efba:

    # e "Haaa... nnhhh..."
    e "Haaa... nnhhh..."

# game/events_mc.rpy:2925
translate french d10nfe_9ce693ec:

    # pcthink "Shit, [e] is tightening up..."
    pcthink "Merde, [e] se resserre..."

# game/events_mc.rpy:2932
translate french md10nfe_f4d25661:

    # j "Now's my turn, [e]."
    j "C'est maintenant mon tour, [e]."

# game/events_mc.rpy:2933
translate french md10nfe_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:2936
translate french md10nfe_256cb5d1:

    # e "Nhh... nhhh..."
    e "Nhh... nhhh..."

# game/events_mc.rpy:2937
translate french md10nfe_aeff8489:

    # j "Gaawd... [e]... nhh... hold on... hnnn."
    j "bon sang... [e]... nhh... attends... hnnn."

# game/events_mc.rpy:2938
translate french md10nfe_c3d61fdd:

    # e "Nhh... haaa..."
    e "Nhh... haaa..."

# game/events_mc.rpy:2939
translate french md10nfe_367bea97:

    # j "Hold on, hold on... fuck!"
    j "Attends, attends... putain !"

# game/events_mc.rpy:2943
translate french md10nfe_ace490b7:

    # e "Haaaa!"
    e "Haaaa !"

# game/events_mc.rpy:2945
translate french md10nfe_ed6b3306:

    # e "Haaa..."
    e "Haaa..."

# game/events_mc.rpy:2951
translate french d10nfj_5ff0b171:

    # j "Right there."
    j "Juste là."

# game/events_mc.rpy:2953
translate french d10nfj_08681c4a:

    # j "Haa... yes!"
    j "Haa... oui !"

# game/events_mc.rpy:2954
translate french d10nfj_bf503bc9:

    # pc "[e] come here."
    pc "[e] viens ici."

# game/events_mc.rpy:2956
translate french d10nfj_52d6b73f:

    # e "Hn..."
    e "Hn..."

# game/events_mc.rpy:2966
translate french d10nfj_2e7b609d:

    # e "Hnnn... nhnnn!"
    e "Hnnn... nhnnn !"

# game/events_mc.rpy:2967
translate french d10nfj_b26a3110:

    # j "Ohh gaaaawd, [pcmd], I love your dick!"
    j "Ohh booonnn sssaannngg, [pcmd], j'aime ta bite !"

# game/events_mc.rpy:2968
translate french d10nfj_dec085be:

    # j "Shit, hnn... [pcmd]."
    j "Merde, hnn... [pcmd]."

# game/events_mc.rpy:2969
translate french d10nfj_c5f2357d:

    # e "Nhhh..."
    e "Nhhhh..."

# game/events_mc.rpy:2970
translate french d10nfj_2381c1d1:

    # j "[pcmd], [pcmd]!"
    j "[pcmd], [pcmd]!"

# game/events_mc.rpy:2978
translate french md10nfj_256cb5d1:

    # e "Nhh... nhhh..."
    e "Nhh... nhhh..."

# game/events_mc.rpy:2979
translate french md10nfj_aeff8489:

    # j "Gaawd... [e]... nhh... hold on... hnnn."
    j "bon sang... [e]... nhh... attends... hnnn."

# game/events_mc.rpy:2980
translate french md10nfj_c3d61fdd:

    # e "Nhh... haaa..."
    e "Nhh... haaa..."

# game/events_mc.rpy:2981
translate french md10nfj_367bea97:

    # j "Hold on, hold on... fuck!"
    j "Attends, attends... putain !"

# game/events_mc.rpy:2985
translate french md10nfj_ace490b7:

    # e "Haaaa!"
    e "Haaaa !"

# game/events_mc.rpy:2987
translate french md10nfj_ed6b3306:

    # e "Haaa..."
    e "Haaa..."

# game/events_mc.rpy:2992
translate french d10nend_93cccb90:

    # pc "I'm getting close!"
    pc "J'y suis presque !"

# game/events_mc.rpy:2996
translate french d10nend_06ff9bda:

    # j "[e], I think it's time for the grand finale."
    j "[e], je pense qu'il est temps pour le grand final."

# game/events_mc.rpy:2997
translate french d10nend_9bee5006:

    # e "Hn!"
    e "Hn !"

# game/events_mc.rpy:2998
translate french d10nend_38132b9b:

    # pc "Hey, why did you stop?"
    pc "Hé, pourquoi tu t'es arrêtée ?"

# game/events_mc.rpy:3000
translate french d10nend_7ec0328a:

    # j "Like we agreed?"
    j "Comme nous l'avons convenu ?"

# game/events_mc.rpy:3001
translate french d10nend_61bb715b:

    # e "Mh-hm!"
    e "Mh-hm !"

# game/events_mc.rpy:3003
translate french d10nend_9942717c:

    # j "Come on, get up, [pcmd]."
    j "Allez, lève-toi, [pcmd]."

# game/events_mc.rpy:3004
translate french d10nend_225fadf5:

    # pc "Huh?"
    pc "Huh ?"

# game/events_mc.rpy:3006
translate french d10nend_ad9342e9:

    # pc "What are you up to?"
    pc "Qu'est-ce que tu fais ?"

# game/events_mc.rpy:3007
translate french d10nend_405f0f47:

    # j "The grant finale! You on top of [e]."
    j "La subvention finale ! Toi au sommet de [e]."

# game/events_mc.rpy:3008
translate french d10nend_fd5d6251:

    # pc "Hmm... sounds good."
    pc "Hmm... ça sonne bien."

# game/events_mc.rpy:3010
translate french d10nend_3ea2c48b:

    # pc "Let's try this."
    pc "Essayons ceci."

# game/events_mc.rpy:3011
translate french d10nend_9a273c4d:

    # j "Mmh... looks sexy! I'll watch!"
    j "Mmh... ça a l'air sexy ! Je vais regarder !"

# game/events_mc.rpy:3016
translate french d10nend_0d66a17d:

    # j "Damn, this looks so hot!"
    j "Bon sang, ça a l'air si chaud !"

# game/events_mc.rpy:3018
translate french d10nend_d3597349:

    # j "You like it, [e]?"
    j "Tu aimes ça, [e]?"

# game/events_mc.rpy:3019
translate french d10nend_4563a8a3:

    # e "Hn!... nnhh..."
    e "Hn !... nnhh..."

# game/events_mc.rpy:3020
translate french d10nend_a4e732ed:

    # j "Haha, obvious."
    j "Haha, évident."

# game/events_mc.rpy:3025
translate french d10nend_9455801f:

    # pc "Hm?"
    pc "Hm ?"

# game/events_mc.rpy:3027
translate french d10nend_b4fa3fc0:

    # j "..."
    new "[j]..."

# game/events_mc.rpy:3028
translate french d10nend_19e9c6c1:

    # pc "Heh, come closer..."
    pc "Heu, viens plus près..."

# game/events_mc.rpy:3033
translate french d10nend_cd975a4b:

    # j "Hnn, yes, [pcmd]!"
    j "Hnn, oui, [pcmd]!"

# game/events_mc.rpy:3038
translate french d10nend_a2d0a918:

    # pc "Damn, I'm close, [e], are you ready?"
    pc "Merde, je suis proche, [e], es-tu prête ?"

# game/events_mc.rpy:3039
translate french d10nend_7348351a:

    # e "Hnn!"
    e "Hnn !"

# game/events_mc.rpy:3040
translate french d10nend_7fc31305:

    # j "Oh, [pcmd], I'm close, too!"
    j "Oh, [pcmd], je suis proche, aussi !"

# game/events_mc.rpy:3041
translate french d10nend_6ba83f13:

    # e "Hnnn... hnnnn..."
    e "Hnnn... hnnnn..."

# game/events_mc.rpy:3043
translate french d10nend_463e8cb9:

    # pc "Shit!"
    pc "Merde !"

# game/events_mc.rpy:3044
translate french d10nend_c3b6f930:

    # j "Nhhh... fill her, [pcmd]! Fill her with your hot, juicy cum!"
    j "Nhhh... remplis la, [pcmd]! Remplis-la de ton sperme chaud et juteux !"

# game/events_mc.rpy:3045
translate french d10nend_d4f1256f:

    # e "Hnnn... nhaaa..." with flash
    e "Hnnn... nhaaa..." with flash

# game/events_mc.rpy:3046
translate french d10nend_b52f4e4b:

    # j "Fill your little sister's baby room with your seed, [pcmd]!"
    j "Remplis la chambre de bébé de ta petite sœur avec tes graines, [pcmd]!"

# game/events_mc.rpy:3047
translate french d10nend_0cf7346a:

    # pc "Holy shit!" with flash
    pc "Putain de merde !" with flash

# game/events_mc.rpy:3049
translate french d10nend_f2a00302:

    # e "Haaaaa!"
    e "Haaaaa !"

# game/events_mc.rpy:3052
translate french d10nend_aeed5ca7:

    # pc "Gawd!"
    pc "Bon sang !"

# game/events_mc.rpy:3055
translate french d10nend_9ec09061:

    # pc "Damn..."
    pc "Merde..."

# game/events_mc.rpy:3058
translate french d10nend_94e1dbb5:

    # pc "...phew..."
    pc "...ouf..."

# game/events_mc.rpy:3059
translate french d10nend_ff62eb80:

    # j "Mmmh, [pcmd] that was awesome!"
    j "Mmmh, [pcmd] c'était génial !"

# game/events_mc.rpy:3063
translate french d10nend_ad9342e9_1:

    # pc "What are you up to?"
    pc "Qu'est-ce que tu fais ?"

# game/events_mc.rpy:3065
translate french d10nend_1bba5b0e:

    # j "You'll love it, promise!"
    j "Tu vas l'adorer, promis !"

# game/events_mc.rpy:3067
translate french d10nend_e4bf7a13:

    # pc "Now I'm curious."
    pc "Maintenant je suis curieux."

# game/events_mc.rpy:3069
translate french d10nend_3f66b558:

    # pc "Oh, I think I know..."
    pc "Oh, je pense que je sais..."

# game/events_mc.rpy:3075
translate french d10nend_e93f2588:

    # j "Do you like it, [pcmd]?"
    j "Tu aimes ça, [pcmd]?"

# game/events_mc.rpy:3087
translate french d10nend_e7217a9b:

    # pc "Hmmm... suck on my clit, [e]."
    pc "Hmmm... suce mon clito, [e]."

# game/events_mc.rpy:3088
translate french d10nend_a8361128:

    # e "Mhhh..."
    e "Mhhh..."

# game/events_mc.rpy:3089
translate french d10nend_0c5d6f71:

    # pc "How could I not like it, with two beautiful, hot girls..."
    pc "Comment pourrais-je ne pas aimer ça, avec deux belles filles sexy..."

# game/events_mc.rpy:3091
translate french d10nend_103333d8:

    # j "Mhh... do you like...mphh... getting eaten out by your... mhhh... little girls, [pcmd]?"
    j "Mhh... tu aimes...mphh... te faire dévorer par tes... mhhh... petites filles, [pcmd]?"

# game/events_mc.rpy:3092
translate french d10nend_a4dee7a9:

    # pc "It feels great, girls!"
    pc "Ça fait du bien, les filles !"

# game/events_mc.rpy:3093
translate french d10nend_ca813d77:

    # j "Mhhh... your skin is so smooth..."
    j "Mhhh... ta peau est si lisse..."

# game/events_mc.rpy:3094
translate french d10nend_e4c5adeb:

    # e "Mnhh..."
    e "Mnhh..."

# game/events_mc.rpy:3095
translate french d10nend_18b80caa:

    # j "Do you want to cum into your little sister's mouth, [pcmd]!"
    j "Tu veux jouir dans la bouche de ta petite sœur, [pcmd]!"

# game/events_mc.rpy:3096
translate french d10nend_9ec09061_1:

    # pc "Damn..."
    pc "Merde..."

# game/events_mc.rpy:3097
translate french d10nend_29c12796:

    # j "Look at her...mhhh... right between your legs... mmph... tasting your juicy, wet pussy..."
    j "Regarde-la...mhhh... juste entre tes jambes... mmph... goûtant ta chatte juteuse et humide..."

# game/events_mc.rpy:3098
translate french d10nend_82da1106:

    # pc "Damn, you look so sexy down there..."
    pc "Bon sang, tu es si sexy là-dessous..."

# game/events_mc.rpy:3099
translate french d10nend_d9173080:

    # j "Let us taste all your yummy pussy juice... mmphh..."
    j "Laisse-nous goûter tout ton délicieux jus de chatte... mmphh..."

# game/events_mc.rpy:3100
translate french d10nend_a24b49bc:

    # pc "Shit..." with flash
    pc "Merde..." with flash

# game/events_mc.rpy:3101
translate french d10nend_fe9c36e3:

    # j "Cum for us, [pcmd], mhh... cum for us!"
    j "Jouis pour nous, [pcmd], mhh... jouis pour nous !"

# game/events_mc.rpy:3102
translate french d10nend_a1c8f658:

    # pc "Oh fuck! Don't stop, don't stop!" with flash
    pc "Oh putain ! Ne t'arrête pas, ne t'arrête pas !" with flash

# game/events_mc.rpy:3103
translate french d10nend_808c8846:

    # j "Yes, [pcmd], cum for us!"
    j "Oui, [pcmd], gicle pour nous !"

# game/events_mc.rpy:3107
translate french d10nend_dbd30717:

    # pc "FUCK!"
    pc "PUTAIN!"

# game/events_mc.rpy:3109
translate french d10nend_cad09cce:

    # e "*Lick*"
    e "*Lécher*"

# game/events_mc.rpy:3111
translate french d10nend_98805c19:

    # e "*Smooch*"
    e "*Bisous*"

# game/events_mc.rpy:3113
translate french d10nend_fa9126f2:

    # j "Gaaawd, we need to do this again some day."
    j "Bon saang, nous devons refaire ça un jour."

# game/events_mc.rpy:3114
translate french d10nend_c417e350:

    # pc "Yeah."
    pc "Ouais."

# game/events_mc.rpy:3116
translate french d10nend_49eceec2:

    # j "How about tomorrow night?"
    j "Pourquoi pas demain soir ?"

# game/events_mc.rpy:3117
translate french d10nend_472fb1cf:

    # pc "Haha, sure."
    pc "Haha, bien sûr."

# game/events_mc.rpy:3118
translate french d10nend_51daeb63:

    # j "And the night after tomorrow?"
    j "Et après-demain soir ?"

# game/events_mc.rpy:3119
translate french d10nend_3fa6b1c9:

    # e "Khehehe."
    e "Khéhéhéhé."

# game/events_mc.rpy:3120
translate french d10nend_29ebc440:

    # pc "How about every night?"
    pc "Pourquoi pas tous les soirs ?"

# game/events_mc.rpy:3121
translate french d10nend_02818a2e:

    # j "Hehe, oh [pcmd], you're so naughty..."
    j "Héhé, oh [pcmd], tu es si vilain..."

# game/events_mc.rpy:3122
translate french d10nend_c176eb1f:

    # pc "Hey, it's you two who came over to keep me from sleeping."
    pc "Hé, c'est vous deux qui êtes venus m'empêcher de dormir."

# game/events_mc.rpy:3123
translate french d10nend_00309318:

    # j "You didn't seem to mind it much."
    j "Ça n'avait pas l'air de te déranger beaucoup."

# game/events_mc.rpy:3124
translate french d10nend_611ff6e2:

    # pc "Speaking of which, maybe you two should go back to your room, [e] needs to get up early for school, right [e]?"
    pc "En parlant de ça, vous devriez peut-être retourner toutes les deux dans votre chambre, [e] doit se lever tôt pour l'école, n'est-ce pas [e]?"

# game/events_mc.rpy:3126
translate french d10nend_95918891:

    # e "*Snooze*"
    e "*Snooze*"

# game/events_mc.rpy:3127
translate french d10nend_9ec09061_2:

    # pc "Damn..."
    pc "Merde..."

# game/events_mc.rpy:3129
translate french d10nend_2c0c9e9d:

    # j "I guess we're going to sleep here tonight."
    j "Je suppose que nous allons dormir ici ce soir."

# game/events_mc.rpy:3130
translate french d10nend_f312f230:

    # pc "Well, let's hope [m] doesn't notice anything."
    pc "Eh bien, espérons que [m] ne remarque rien."

# game/events_mc.rpy:3132
translate french d10nend_d92e8f46:

    # j "We really need to make her change her mind, so she's okay with us sleeping here instead of [e]'s room."
    j "Nous devons vraiment la faire changer d'avis, pour qu'elle soit d'accord pour que nous dormions ici au lieu de la chambre de [e]."

# game/events_mc.rpy:3133
translate french d10nend_f9f62fc1:

    # pc "Maybe she'll change her mind in the future."
    pc "Peut-être qu'elle changera d'avis dans le futur."

# game/events_mc.rpy:3135
translate french d10nend_48207abb:

    # j "I'm sure you can convince her, hehe."
    j "Je suis sûr que tu peux la convaincre, héhé."

# game/events_mc.rpy:3136
translate french d10nend_462cc5bd:

    # pc "Maybe... wait, was that some kind of suggestion again?"
    pc "Peut-être... attends, c'était encore une sorte de suggestion ?"

# game/events_mc.rpy:3138
translate french d10nend_1e448d60:

    # j "Hehe, good night, [pcmd]."
    j "Héhé, bonne nuit, [pcmd]."

# game/events_mc.rpy:3139
translate french d10nend_510163bd:

    # pc "Oh well, whatever... Night, [j]..."
    pc "Oh bien, peu importe... Bonne nuit, [j]..."

# game/events_mc.rpy:3149
translate french d11wakeup_a68105ea:

    # m "[pc], do you want to eat breakfast with us?"
    m "[pc], tu veux prendre le petit déjeuner avec nous ?"

# game/events_mc.rpy:3151
translate french d11wakeup_8e3eb455:

    # pc "YAAAAAaaaawn..."
    pc "YAAAAAaaaawn..."

# game/events_mc.rpy:3154
translate french d11wakeup_d9fd0b2d:

    # pcthink "Looks like they got up in time..."
    pcthink "On dirait qu'ils se sont levés à temps..."

# game/events_mc.rpy:3156
translate french d11wakeup_4f79c49f:

    # pcthink "Looks like [e] didn't come over again..."
    pcthink "On dirait que [e] n'est pas revenue..."

# game/events_mc.rpy:3158
translate french d11wakeup_4a57ea09:

    # m "[pc]?"
    m "[pc]?"

# game/events_mc.rpy:3160
translate french d11wakeup_3538051c:

    # pc "Yeah, I'm coming..."
    pc "Oui, j'arrive..."

# game/events_mc.rpy:3162
translate french d11wakeup_b4ec8360:

    # n "You get up, take a quick shower and then join the others for breakfast. [j] and [m] seem to have been engaged in a conversation for a while..."
    n "Vous vous levez, prenez une douche rapide puis rejoignez les autres pour le petit-déjeuner. [j] et [m] semblent être en conversation depuis un moment..."

# game/events_mc.rpy:3165
translate french d11wakeup_2add307d:

    # j "...and then it all went down from there..."
    j "...et tout s'est écroulé à partir de là..."

# game/events_mc.rpy:3167
translate french d11wakeup_8b73d057:

    # m "I see... say, [j], for how long are you suspended from school again?"
    m "Je vois... disons, [j], pendant combien de temps es-tu à nouveau suspendu de l'école ?"

# game/events_mc.rpy:3169
translate french d11wakeup_1a481b63:

    # j "This week still."
    j "Cette semaine encore."

# game/events_mc.rpy:3171
translate french d11wakeup_f0f7d53a:

    # m "I see, any plans for today?"
    m "Je vois, des plans pour aujourd'hui ?"

# game/events_mc.rpy:3173
translate french d11wakeup_abc21079:

    # j "Uhh... no, not really."
    j "Uhh... non, pas vraiment."

# game/events_mc.rpy:3175
translate french d11wakeup_c177f7ea:

    # m "Do you have any plans for the future? Maybe you could look for a job, it's not that long anymore until school is out."
    m "As-tu des projets pour l'avenir ? Tu pourrais peut-être chercher un emploi, il ne reste plus beaucoup de temps avant la fin de l'école."

# game/events_mc.rpy:3177
translate french d11wakeup_70f903af:

    # j "I... don't know. I will probably end up as a waitress anyway..."
    j "Je... ne sais pas. Je finirai probablement comme serveuse de toute façon..."

# game/events_mc.rpy:3179
translate french d11wakeup_8c91b24d:

    # m "You don't have any plans for the future?"
    m "Tu n'as pas de projets pour l'avenir ?"

# game/events_mc.rpy:3181
translate french d11wakeup_6d0ab809:

    # j "It's not like I have much to chose from."
    j "Ce n'est pas comme si j'avais beaucoup de choix."

# game/events_mc.rpy:3182
translate french d11wakeup_844c942c:

    # pc "I didn't know what to do either."
    pc "Je ne savais pas non plus quoi faire."

# game/events_mc.rpy:3184
translate french d11wakeup_227f3a09:

    # j "Hm?"
    j "Hm ?"

# game/events_mc.rpy:3185
translate french d11wakeup_8151da85:

    # pc "After school... I didn't have a degree, I had no idea what to do, nor did I care at the time... and look at me now, I'm basically self-employed and do what I like."
    pc "Après l'école... Je n'avais pas de diplôme, je n'avais aucune idée de ce que je devais faire, et je m'en fichais à l'époque... et regardez-moi maintenant, je suis pratiquement à mon compte et je fais ce que je veux."

# game/events_mc.rpy:3187
translate french d11wakeup_3aa4d591:

    # m "You don't have a degree?"
    m "Tu n'as pas de diplôme ?"

# game/events_mc.rpy:3189
translate french d11wakeup_c2b4ba68:

    # pc "I didn't, you know how I left... I got my degree a few years later, but until then I still had many different jobs."
    pc "Je ne l'ai pas fait, tu sais comment je suis parti... J'ai eu mon diplôme quelques années plus tard, mais jusqu'à ce moment-là, j'avais encore beaucoup d'emplois différents."

# game/events_mc.rpy:3190
translate french d11wakeup_add3d30b:

    # pc "I'm not saying that a degree doesn't matter, it does, but just because you don't have a degree doesn't mean you can't do anything. There are still a lot of opportunities in life."
    pc "Je ne dis pas qu'un diplôme ne compte pas, il compte, mais ce n'est pas parce que vous n'avez pas de diplôme que vous ne pouvez rien faire. Il y a encore beaucoup d'opportunités dans la vie."

# game/events_mc.rpy:3192
translate french d11wakeup_19c1e4e2:

    # m "I see..."
    m "Je vois..."

# game/events_mc.rpy:3193
translate french d11wakeup_a69377ef:

    # pc "..."
    new "[pc]..."

# game/events_mc.rpy:3195
translate french d11wakeup_cb5a7a19:

    # m "So, what are your plans for today?"
    m "Alors, quels sont tes plans pour aujourd'hui ?"

# game/events_mc.rpy:3197
translate french d11wakeup_577d4c6e:

    # pc "Today is the deadline for the project I was working on, so I'll have to present it at work today."
    pc "Aujourd'hui est la date limite pour le projet sur lequel je travaillais, je dois donc le présenter au travail aujourd'hui."

# game/events_mc.rpy:3199
translate french d11wakeup_38ab289d:

    # m "I see, sounds like it'll be a busy day for you."
    m "Je vois, on dirait que ce sera une journée chargée pour toi."

# game/events_mc.rpy:3201
translate french d11wakeup_92e2e8a0:

    # pc "Yeah, probably..."
    pc "Ouais, probablement..."

# game/events_mc.rpy:3203
translate french d11wakeup_d7508814:

    # n "After breakfast, [e] and [m] get ready to leave for the day. To everyone's surprise, [j] demands to take care of the dishes... you decide to look through the project one more time before you leave for town."
    n "Après le petit-déjeuner, [e] et [m] se préparent à partir pour la journée. À la surprise générale, [j] exige de s'occuper de la vaisselle... vous décidez de parcourir le projet une dernière fois avant de partir en ville."

# game/events_mc.rpy:3208
translate french d11bhpcroom_d6d25b7b:

    # pcthink "Hmm... what to do now..."
    pcthink "Hmm... que faire maintenant..."

# game/events_mc.rpy:3209
translate french d11bhpcroom_f0cac90d:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:3210
translate french d11bhpcroom_a2266564:

    # pcthink "Maybe I should look for a good TV to buy..."
    pcthink "Je devrais peut-être chercher une bonne télé à acheter..."

# game/events_mc.rpy:3211
translate french d11bhpcroom_f7367be0:

    # pcthink "I mean, it looks like I'll stay..."
    pcthink "Je veux dire, il semble que je vais rester..."

# game/events_mc.rpy:3212
translate french d11bhpcroom_35ad042e:

    # pcthink "Right?"
    pcthink "D'accord ?"

# game/events_mc.rpy:3213
translate french d11bhpcroom_f0cac90d_1:

    # pcthink "..."
    new "[pcthink]..."

# game/events_mc.rpy:3215
translate french d11bhpcroom_40a6633b:

    # pcthink "Damn, my life really took a 180 over the last two weeks..."
    pcthink "Merde, ma vie a vraiment pris un virage à 180° ces deux dernières semaines..."

# game/events_mc.rpy:3218
translate french d11bhpcroom_3dd86183:

    # j "Whatcha doing?"
    j "Qu'est-ce que tu fais ?"

# game/events_mc.rpy:3220
translate french d11bhpcroom_953fabce:

    # pc "I was just about to look for a TV online."
    pc "J'étais justement en train de chercher une télé en ligne."

# game/events_mc.rpy:3222
translate french d11bhpcroom_dc67dfba:

    # j "Finally! That's great! Can I help?"
    j "Enfin ! C'est génial ! Je peux t'aider ?"

# game/events_mc.rpy:3223
translate french d11bhpcroom_79572587:

    # pc "Sure..."
    pc "Bien sûr..."

# game/events_mc.rpy:3225
translate french d11bhpcroom_a7b99095:

    # n "*Doorbell rings*"
    n "*Sonneries de la porte*"

# game/events_mc.rpy:3227
translate french d11bhpcroom_9455801f:

    # pc "Hm?"
    pc "Hm ?"

# game/events_mc.rpy:3228
translate french d11bhpcroom_f7a2c366:

    # j "Are you waiting for somebody?"
    j "Tu attends quelqu'un ?"

# game/events_mc.rpy:3229
translate french d11bhpcroom_3cdf5875:

    # pc "No, not that I remember..."
    pc "Non, pas que je me souvienne..."

# game/events_mc.rpy:3231
translate french d11bhpcroom_0b75a0f5:

    # j "Oh no, I hope it's not that weird cop again..."
    j "Oh non, j'espère que ce n'est pas encore ce flic bizarre..."

translate french strings:

    # game/events_mc.rpy:249
    old "[mschoice] Watch TV in the living room"
    new "[mschoice] Regarder la TV dans le salon"

    # game/events_mc.rpy:505
    old "[mschoice] {i}(Damn, I love the idea...){/i}"
    new "[mschoice] {i}(Bon sang, j'adore l'idée...){/i}"

    # game/events_mc.rpy:505
    old "[mschoice] {i}(There is no way I would ever do that!){/i}"
    new "[mschoice] {i}(Il n'y a aucune chance que je fasse ça !){/i}"

    # game/events_mc.rpy:572
    old "[mschoice] She will come over tonight anyway... gimme porn!"
    new "[mschoice] Elle viendra ce soir de toute façon... Aller du porno !"

    # game/events_mc.rpy:572
    old "[mschoice] I should really talk to her now!"
    new "[mschoice] Je devrais vraiment lui parler maintenant !"

    # game/events_mc.rpy:707
    old "[mschoice] Introduce yourself."
    new "[mschoice] Se présenter."

    # game/events_mc.rpy:707
    old "[mschoice] Say good bye and leave."
    new "[mschoice] Dire au revoir et partir."

    # game/events_mc.rpy:1518
    old "[mschoice] Call her?"
    new "[mschoice] L'appeler ?"

    # game/events_mc.rpy:1518
    old "[mschoice] Call her!"
    new "[mschoice] Appelez-la !"

    # game/events_mc.rpy:1518
    old "[mschoice] Nah, not now."
    new "[mschoice] Nan, pas maintenant."

    # game/events_mc.rpy:1620
    old "[mschoice] Push [j] out of the bed."
    new "[mschoice] Pousser [j] hors du lit."

    # game/events_mc.rpy:1620
    old "[mschoice] Hide [e] under the blanket."
    new "[mschoice] Cacher [e] sous la couverture."

    # game/events_mc.rpy:1735
    old "[mschoice] Tell them you're sorry."
    new "[mschoice] Leur dire que vous êtes désolé."

    # game/events_mc.rpy:1735
    old "[mschoice] Talk about the weather."
    new "[mschoice] Parler de la météo."

    # game/events_mc.rpy:2197
    old "[mschoice] Call her Mom."
    new "[mschoice] Appeler sa mère."

    # game/events_mc.rpy:2197
    old "[mschoice] Change the topic."
    new "[mschoice] Changer de sujet."

    # game/events_mc.rpy:2320
    old "[mschoice] The good news."
    new "[mschoice] La bonne nouvelle."

    # game/events_mc.rpy:2320
    old "[mschoice] The bad news."
    new "[mschoice] La mauvaise nouvelle."

    # game/events_mc.rpy:2772
    old "[mschoice] [e]."
    new "[mschoice] [e]."

    # game/events_mc.rpy:2772
    old "[mschoice] [j]."
    new "[mschoice] [j]."

    # game/events_mc.rpy:2817
    old "[mschoice] Switch to [j]."
    new "[mschoice] Passer à [j]."

    # game/events_mc.rpy:2817
    old "[mschoice] Go on, [e]."
    new "[mschoice] Continuer avec [e]."

    # game/events_mc.rpy:2817
    old "[mschoice] Cum."
    new "[mschoice] Jouir."

    # game/events_mc.rpy:2857
    old "[mschoice] Switch to [e]."
    new "[mschoice] Passer à [e]."

    # game/events_mc.rpy:2857
    old "[mschoice] Go on, [j]."
    new "[mschoice] Continuer avec [j]."

    # game/events_mc.rpy:2926
    old "[mschoice] Switch to fuck [j]."
    new "[mschoice] Baiser [j]."

    # game/events_mc.rpy:2971
    old "[mschoice] Switch to fuck [e]."
    new "[mschoice] Baiser [e]."

