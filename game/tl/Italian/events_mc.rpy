#     old ""
#     ""
# "Plural-Forms: nplurals=2; plural=(n != 1);\n"
# "X-italian-Project: bad-memories\n"
# "X-italian-Project-ID: 507994\n"
# "X-italian-Language: it\n"
# "X-italian-File: /main/game/tl/italian/events_mc.po\n"
# "X-italian-File-ID: 3720\n"
# "Project-Id-Version: bad-memories\n"
# "Content-Type: text/plain; charset=UTF-8\n"
# "Language-Team: Italian\n"
# "Language: it_IT\n"
# "PO-Revision-Date: 2022-05-03 20:31\n"

# TODO: Translation updated at 2022-04-03 HH:HH

# game/events_mc.rpy:5
translate italian day0Morning_65a592f9:

    # pc "YAAAHHWN..."
    pc "YAAAHHWN..."

# game/events_mc.rpy:6
translate italian day0Morning_3b638262:

    # pcthink "Gawd, I needed that sleep..."
    pcthink "Gawd, avevo bisogno di quel sonno..."

# game/events_mc.rpy:8
translate italian day0Morning_fe495e49:

    # pcthink "Huh?! Who put the blanket on me?"
    pcthink "Huh?! Chi mi ha messo la coperta?"

# game/events_mc.rpy:9
translate italian day0Morning_1a156d9c:

    # pcthink "...and took my shoes off?"
    pcthink "...e mi tolse le scarpe?"

# game/events_mc.rpy:12
translate italian day0Morning_aff1c6b1:

    # pcthink "Uhh ouch, my back..."
    pcthink "Uhh ouch, la mia schiena..."

# game/events_mc.rpy:14
translate italian day0Morning_35a70f21:

    # pcthink "Argh... wasn't the best idea to fall asleep in a chair!"
    pcthink "Argh... non era l'idea migliore per addormentarsi su una sedia!"

# game/events_mc.rpy:16
translate italian day0Morning_7159aa77:

    # n "You struggle to get up from the chair, but eventually make it."
    n "Lottate per alzarvi dalla sedia, ma alla fine fatelo."

# game/events_mc.rpy:18
translate italian day0Morning_63d6be43:

    # pcthink "Ugh... So this is how old people feel."
    pcthink "Ugh... Quindi questo è quanto si sentono le persone anziane."

# game/events_mc.rpy:19
translate italian day0Morning_1f6ff3b9:

    # pcthink "I better try not to sit in that chair again when I'm tired..."
    pcthink "Meglio cercare di non sedermi di nuovo su quella sedia quando sono stanco..."

# game/events_mc.rpy:20
translate italian day0Morning_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:21
translate italian day0Morning_f92b90eb:

    # pcthink "...back home after all these years..."
    pcthink "...torna a casa dopo tutti questi anni..."

# game/events_mc.rpy:22
translate italian day0Morning_07ad280b:

    # pcthink "I'll have to have a look for [e] later..."
    pcthink "Dovrò cercare [e] più tardi..."

# game/events_mc.rpy:23
translate italian day0Morning_eed7ec06:

    # pcthink "But first things first. I really need to use the bathroom!"
    pcthink "Ma le prime cose prima. Ho davvero bisogno di usare il bagno!"

# game/events_mc.rpy:30
translate italian childRoomDay0_5f0b8470:

    # pc "Hey [e], [m] told me to..."
    pc "Ehi [e], [m] mi ha detto a..."

# game/events_mc.rpy:31
translate italian childRoomDay0_ad420ce1:

    # pcthink "Huh? She's not here?"
    pcthink "Huh? Non è qui?"

# game/events_mc.rpy:32
translate italian childRoomDay0_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:33
translate italian childRoomDay0_eac0ec9f:

    # pcthink "Maybe she is in the bathroom?"
    pcthink "Forse è in bagno?"

# game/events_mc.rpy:35
translate italian childRoomDay0_4c93f6e9:

    # pcthink "Hmm... the door is not locked and there is nothing to hear."
    pcthink "Hmm... la porta non è chiusa e non c'è niente da sentire."

# game/events_mc.rpy:37
translate italian childRoomDay0_70d4d8a9:

    # pcthink "No one here..."
    pcthink "Nessuno qui..."

# game/events_mc.rpy:39
translate italian childRoomDay0_b76c7bc0:

    # pcthink "Okay, that's weird... did I miss her?"
    pcthink "Ok, è strano... mi è mancata?"

# game/events_mc.rpy:40
translate italian childRoomDay0_8ad047f3:

    # pcthink "Maybe in the Livingroom?"
    pcthink "Forse nel Livingroom?"

# game/events_mc.rpy:42
translate italian childRoomDay0_50f4faf2:

    # pcthink "Nope, not here either... where is she?"
    pcthink "Nope, nemmeno quichar@@0 dov’è lei?"

# game/events_mc.rpy:51
translate italian kitchenBreakfastDay0_719102df:

    # n "A bit later" with dissolve
    n "Un po' più tardi" with dissolve

# game/events_mc.rpy:55
translate italian kitchenBreakfastDay0_024f1747:

    # m "Oh, so you only need to go to work every few days?"
    m "Oh, quindi devi solo andare al lavoro ogni pochi giorni?"

# game/events_mc.rpy:56
translate italian kitchenBreakfastDay0_d3fae95e:

    # pc "Yeah, you could say I'm an employed freelancer."
    pc "Sì, si potrebbe dire che io sono un freelancer."

# game/events_mc.rpy:57
translate italian kitchenBreakfastDay0_da8f7c38:

    # pcthink "What's up with [e]?"
    pcthink "Cosa c'è di più con [e]?"

# game/events_mc.rpy:58
translate italian kitchenBreakfastDay0_48b558b0:

    # m "Sounds interesting, but does this job really pay that well?"
    m "Suona interessante, ma questo lavoro paga davvero così bene?"

# game/events_mc.rpy:59
translate italian kitchenBreakfastDay0_4358ca4b:

    # pc "The pay is okay, I manage..."
    pc "La paga va bene, gestisco..."

# game/events_mc.rpy:60
translate italian kitchenBreakfastDay0_7433034f:

    # pcthink "...actually more than okay, but why would I tell her that..."
    pcthink "in realtà più che ok, ma perché le direi che..."

# game/events_mc.rpy:61
translate italian kitchenBreakfastDay0_44098f14:

    # m "Well, if you like your job, who am I to judge. But if you ever need something, just tell me, okay?"
    m "Beh, se ti piace il tuo lavoro, chi sono io a giudicare. Ma se hai mai bisogno di qualcosa, mi dica bene?"

# game/events_mc.rpy:62
translate italian kitchenBreakfastDay0_77b9401c:

    # pcthink "Not gonna happen..."
    pcthink "Non accadrà..."

# game/events_mc.rpy:63
translate italian kitchenBreakfastDay0_deb9672a:

    # pc "Don't worry, I won't need anything."
    pc "Non ti preoccupare, non avrò bisogno di nulla."

# game/events_mc.rpy:64
translate italian kitchenBreakfastDay0_473b0738:

    # m "Alright..."
    m "Bene..."

# game/events_mc.rpy:66
translate italian kitchenBreakfastDay0_ad73f7a1:

    # m "[e], don't you want to eat something?"
    m "[e], non vuoi mangiare qualcosa?"

# game/events_mc.rpy:68
translate italian kitchenBreakfastDay0_8fbec38d:

    # m "Alright, I'm going to make myself ready for work."
    m "Bene, mi renderò pronto per il lavoro."

# game/events_mc.rpy:70
translate italian kitchenBreakfastDay0_46a844ac:

    # m "[e] eat up, okay?"
    m "[e] mangiare su, ok?"

# game/events_mc.rpy:72
translate italian kitchenBreakfastDay0_b3d1ca42:

    # pcthink "Ugh... she's doing it again..."
    pcthink "Ugh... lo sta facendo di nuovo..."

# game/events_mc.rpy:74
translate italian kitchenBreakfastDay0_deadf7f8:

    # pc "Err... what's up [e]?"
    pc "Err... che cos'è [e]?"

# game/events_mc.rpy:75
translate italian kitchenBreakfastDay0_72a33094:

    # e "..."
    e "..."

# game/events_mc.rpy:76
translate italian kitchenBreakfastDay0_2ce091cd:

    # pcthink "Damn, she doesn't talk. I keep forgetting that..."
    pcthink "Dannazione, lei non parla. Continuo a dimenticarlo..."

# game/events_mc.rpy:77
translate italian kitchenBreakfastDay0_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:78
translate italian kitchenBreakfastDay0_72a33094_1:

    # e "..."
    e "..."

# game/events_mc.rpy:79
translate italian kitchenBreakfastDay0_8b857d1c:

    # pc "Uh, hey I'm sorry I didn't recognize you back in the city..."
    pc "Uh, hey Mi dispiace di non averti riconosciuto di nuovo in città..."

# game/events_mc.rpy:80
translate italian kitchenBreakfastDay0_72a33094_2:

    # e "..."
    e "..."

# game/events_mc.rpy:81
translate italian kitchenBreakfastDay0_8096f560:

    # pcthink "This is awkward..."
    pcthink "Questo è imbarazzante..."

# game/events_mc.rpy:82
translate italian kitchenBreakfastDay0_ec8a779d:

    # pcthink "Why is she staring at me like this?!"
    pcthink "Perché mi sta fissando così?!"

# game/events_mc.rpy:83
translate italian kitchenBreakfastDay0_0ecb835a:

    # pcthink "It's almost creepy..."
    pcthink "È quasi raccapricciante..."

# game/events_mc.rpy:85
translate italian kitchenBreakfastDay0_d4887870:

    # m "[e], get ready for school, we need to leave in five minutes!"
    m "[e], preparatevi per la scuola, abbiamo bisogno di partire in cinque minuti!"

# game/events_mc.rpy:88
translate italian kitchenBreakfastDay0_cf0767d9:

    # n "She jumps up immediately and leaves the room."
    n "Salta subito e lascia la stanza."

# game/events_mc.rpy:89
translate italian kitchenBreakfastDay0_142c6d46:

    # pcthink "Phew... thanks [m]..."
    pcthink "Phew... grazie [m]..."

# game/events_mc.rpy:90
translate italian kitchenBreakfastDay0_e8626d4a:

    # n "A few minutes later" with dissolve
    n "Alcuni minuti dopo" with dissolve

# game/events_mc.rpy:94
translate italian kitchenBreakfastDay0_71b5423a:

    # pcthink "Damn I don't know how to handle the situation with [e]..."
    pcthink "Dannazione Non so come gestire la situazione con [e]..."

# game/events_mc.rpy:95
translate italian kitchenBreakfastDay0_1784afc6:

    # pcthink "But at least she doesn't seem to be mad at me..."
    pcthink "Ma almeno lei non sembra essere arrabbiata con me..."

# game/events_mc.rpy:96
translate italian kitchenBreakfastDay0_3881cbd8:

    # pcthink "Maybe I shouldn't think too much about it."
    pcthink "Forse non dovrei pensarci troppo."

# game/events_mc.rpy:97
translate italian kitchenBreakfastDay0_e9849b15:

    # pcthink "I mean, she's just a normal girl...right?"
    pcthink "Voglio dire, lei è solo una ragazza normale...giusto?"

# game/events_mc.rpy:98
translate italian kitchenBreakfastDay0_4883c233:

    # pcthink "Aside from not talking..."
    pcthink "Oltre a non parlare..."

# game/events_mc.rpy:99
translate italian kitchenBreakfastDay0_8e2aeca0:

    # pcthink "...and staring at me like a creep..."
    pcthink "...e mi fisso come un strisciante..."

# game/events_mc.rpy:100
translate italian kitchenBreakfastDay0_88a665a7:

    # pcthink "...and sleeping in my bed..."
    pcthink "...e dormire nel mio letto..."

# game/events_mc.rpy:101
translate italian kitchenBreakfastDay0_f0aa75e3:

    # pcthink "Ugh... don't think about it, she's just a normal girl! No sweat!"
    pcthink "Ugh... non ci pensi, è solo una ragazza normale! Nessun sudore!"

# game/events_mc.rpy:103
translate italian kitchenBreakfastDay0_4f2c56a3:

    # n "*KNOCK* *KNOCK"
    n "*KNOCK* *KNOCK"

# game/events_mc.rpy:105
translate italian kitchenBreakfastDay0_c48e5fe0:

    # m "We are leaving, will you manage?"
    m "Partiamo, riuscirete?"

# game/events_mc.rpy:106
translate italian kitchenBreakfastDay0_39d36142:

    # pc "Yeah, sure."
    pc "Sì, certo."

# game/events_mc.rpy:107
translate italian kitchenBreakfastDay0_f03b848b:

    # pcthink "Damn, [e] looks like she's trying to seduce someone..."
    pcthink "Dannazione, [e] sembra che stia cercando di sedurre qualcuno..."

# game/events_mc.rpy:108
translate italian kitchenBreakfastDay0_592d8410:

    # m "Food is in the fridge, take whatever you want, okay?"
    m "Il cibo è in frigo, prendi quello che vuoi, va bene?"

# game/events_mc.rpy:109
translate italian kitchenBreakfastDay0_ac935b76:

    # pc "Yeah, okay. Thanks."
    pc "Sì, va bene. Grazie."

# game/events_mc.rpy:110
translate italian kitchenBreakfastDay0_848f5afd:

    # m "See you later, Hun."
    m "Ci vediamo più tardi, Hun."

# game/events_mc.rpy:111
translate italian kitchenBreakfastDay0_19392efe:

    # pc "Later..."
    pc "Più tardi..."

# game/events_mc.rpy:113
translate italian kitchenBreakfastDay0_15f55841:

    # pcthink "What's up with that \"Hun\" thing of hers? She never called me that back then..."
    pcthink "Che cos'è con quella cosa di lei \"Hun\"? Lei non mi ha mai chiamato quel poi..."

# game/events_mc.rpy:115
translate italian kitchenBreakfastDay0_00c77948:

    # pcthink "Anyway, time to relax..."
    pcthink "Comunque, tempo per rilassarsi..."

# game/events_mc.rpy:122
translate italian pcDream1_428d1889:

    # n "You lay down in your bed."
    n "Ti sei coricato nel letto."

# game/events_mc.rpy:124
translate italian pcDream1_ac247afb:

    # pcthink "It feels so weird to be back..."
    pcthink "Si sente così strano per essere tornati..."

# game/events_mc.rpy:125
translate italian pcDream1_e59fabcc:

    # pcthink "...seeing [e] and [m] again..."
    pcthink "...vedendo [e] e [m] di nuovo..."

# game/events_mc.rpy:126
translate italian pcDream1_f7c84076:

    # pcthink "[e] surely has grown up, but... actually I thought she should be a bit older by now."
    pcthink "[e] è sicuramente cresciuto, ma... in realtà pensavo che avrebbe dovuto essere un po' più anziana adesso."

# game/events_mc.rpy:127
translate italian pcDream1_825c8758:

    # pcthink "How old is she anyway?"
    pcthink "Quanti anni ha comunque?"

# game/events_mc.rpy:128
translate italian pcDream1_72087c33:

    # pcthink "Uhh..."
    pcthink "Uhh..."

# game/events_mc.rpy:129
translate italian pcDream1_f7cb6db2:

    # pcthink "Shit, I can't remember..."
    pcthink "Merda, non riesco a ricordare..."

# game/events_mc.rpy:130
translate italian pcDream1_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:131
translate italian pcDream1_b45b7ed4:

    # pcthink "...and [m]... did she really change?"
    pcthink "...e [m]... è cambiata davvero?"

# game/events_mc.rpy:132
translate italian pcDream1_d1c86b16:

    # pcthink "Something is bugging me about her... I don't trust her."
    pcthink "Qualcosa mi sta buggando su di lei... Non mi fido di lei."

# game/events_mc.rpy:133
translate italian pcDream1_0c94bab4:

    # pcthink "I bet she's... *yaaawn* ...hiding something..."
    pcthink "Scommetto lei... *yaaawn* ...nascondere qualcosa..."

# game/events_mc.rpy:136
translate italian pcDream1_8f03e192:

    # n "You close your eyes and fall asleep."
    n "Chiudete gli occhi e vi addormentate."

# game/events_mc.rpy:138
translate italian pcDream1_d0053a27:

    # n "In your dreams" with dissolve
    n "Nei tuoi sogni" with dissolve

# game/events_mc.rpy:141
translate italian pcDream1_6834e5d1:

    # m "Welcome home [e]."
    m "Casa di benvenuto [e]."

# game/events_mc.rpy:142
translate italian pcDream1_ddb0e6fa:

    # e "Hello, Mommy."
    e "Ciao, Mamma."

# game/events_mc.rpy:143
translate italian pcDream1_1812dc36:

    # m "Wait a minute, show me your face..."
    m "Aspetta un minuto, mostrami il tuo viso..."

# game/events_mc.rpy:144
translate italian pcDream1_2abbc6c7:

    # e "Um, it's nothing, Mommy. I just..."
    e "Um, non è niente, Mamma. Semplicemente..."

# game/events_mc.rpy:145
translate italian pcDream1_24dc9c21:

    # m "[bpc]! WHAT DID YOU DO TO HER?!"
    m "[bpc]! COSA FAI QUI?!"

# game/events_mc.rpy:146
translate italian pcDream1_d25692ca:

    # pc "Huh? I didn't do anything!"
    pc "Huh? Non ho fatto niente!"

# game/events_mc.rpy:147
translate italian pcDream1_25928455:

    # e "No. Mommy, it wasn't..."
    e "No. Mamma, non era..."

# game/events_mc.rpy:148
translate italian pcDream1_43ef6ea4:

    # m "Shut up [e]! [pc] what the hell?!"
    m "Chiudi [e]! [pc] che inferno?!"

# game/events_mc.rpy:149
translate italian pcDream1_71977e73:

    # pc "I-I didn't do anything, I swear! There were some guys bullying her and I..."
    pc "Io non ho fatto niente, giuro! C'erano alcuni ragazzi che la predevano e Io..."

# game/events_mc.rpy:150
translate italian pcDream1_86b1fd11:

    # m "SHUT UP!"
    m "SHUT UP!"

# game/events_mc.rpy:151
translate italian pcDream1_a19c3737:

    # m "I don't want to hear any excuses from you! You stupid [ds] of an asshole!"
    m "Non voglio sentire scuse da te! Sei stupido [ds!t] di un culo!"

# game/events_mc.rpy:152
translate italian pcDream1_10b5ce69:

    # e "Mommy, please, [heshe] helped me!"
    e "Mamma, per favore, [heshe!t] mi ha aiutato!"

# game/events_mc.rpy:153
translate italian pcDream1_c2cdcf05:

    # m "Go to your room [pc]! I don't want to see you leave your room until you go to school tomorrow!"
    m "Vai nella tua stanza [pc]! Non voglio vederti lasciare la tua stanza fino a quando domani andrai a scuola!"

# game/events_mc.rpy:154
translate italian pcDream1_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:157
translate italian pcDream1_e2ee86ba:

    # pc "Uhh..."
    pc "Uhh..."

# game/events_mc.rpy:158
translate italian pcDream1_f4d55998:

    # pcthink "Shit... the memories keep coming back..."
    pcthink "Mettete... i ricordi continuano a tornare..."

# game/events_mc.rpy:160
translate italian pcDream1_149222e3:

    # pcthink "I wish I could just keep them locked somewhere in the back of my brain."
    pcthink "Vorrei che potessi solo tenerli bloccati da qualche parte nella parte posteriore del mio cervello."

# game/events_mc.rpy:161
translate italian pcDream1_f0cac90d_1:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:162
translate italian pcDream1_e4520362:

    # pcthink "Damn, it really doesn't help to wallow in self-pity..."
    pcthink "Dannazione, in realtà non aiuta a sveglio in auto-pietà..."

# game/events_mc.rpy:163
translate italian pcDream1_f45797d4:

    # pcthink "I better get up and do something useful..."
    pcthink "Meglio alzarmi e fare qualcosa di utile..."

# game/events_mc.rpy:164
translate italian pcDream1_3d2a2575:

    # pcthink "...well, or whatever."
    pcthink "...beh, o altro."

# game/events_mc.rpy:174
translate italian wakeupDay1_8bf6dc88:

    # n "*Knock* *Knock*"
    n "*Knock* *Knock*"

# game/events_mc.rpy:175
translate italian wakeupDay1_73d106f4:

    # m "[pc], are you awake?"
    m "[pc], sei sveglio?"

# game/events_mc.rpy:176
translate italian wakeupDay1_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:178
translate italian wakeupDay1_dabeb1e2:

    # n "*Knock* *Knock* *Knock*"
    n "*Knock* *Knock* *Knock*"

# game/events_mc.rpy:179
translate italian wakeupDay1_4a57ea09:

    # m "[pc]?"
    m "[pc]?"

# game/events_mc.rpy:180
translate italian wakeupDay1_d4803e34:

    # pc "Uhhh..."
    pc "Uhhh..."

# game/events_mc.rpy:182
translate italian wakeupDay1_4300e10e:

    # pc "Uhnnn... yeah..."
    pc "Uhnnnn... sì..."

# game/events_mc.rpy:184
translate italian wakeupDay1_60a71439:

    # m "I can't find [e], do you..."
    m "Non riesco a trovare [e], fai..."

# game/events_mc.rpy:186
translate italian wakeupDay1_f6b14dde:

    # m "What the?! [pc]?"
    m "Che cosa?! [pc]?"

# game/events_mc.rpy:187
translate italian wakeupDay1_225fadf5:

    # pc "Huh?"
    pc "Huh?"

# game/events_mc.rpy:189
translate italian wakeupDay1_d1ae462b:

    # n "You turn around."
    n "Si gira intorno."

# game/events_mc.rpy:193
translate italian wakeupDay1_7d60a407:

    # pc "WHOA!!!"
    pc "QUI!!!"

# game/events_mc.rpy:195
translate italian wakeupDay1_820ffab4:

    # pc "What the hell are you doing here, [e]?!"
    pc "Che diavolo stai facendo qui, [e]?!"

# game/events_mc.rpy:196
translate italian wakeupDay1_990aec05:

    # m "Hahaha..."
    m "Hahaha..."

# game/events_mc.rpy:199
translate italian wakeupDay1_ffc63eee:

    # m "I guess you didn't know she was here either."
    m "Immagino che non sapesse che anche lei era qui."

# game/events_mc.rpy:200
translate italian wakeupDay1_4781350b:

    # m "Looks like someone wanted to cuddle a bit with her big [bs]."
    m "Sembra che qualcuno volesse coccolare un po 'con il suo grande [bs!t]."

# game/events_mc.rpy:202
translate italian wakeupDay1_251417f1:

    # m "But, [e], you really shouldn't sneak into your [bs]'s room like this."
    m "Ma, [e], non dovresti intrufolarti nella tua stanza di [bs!t] così."

# game/events_mc.rpy:208
translate italian wakeupDay1_fd57a2d8:

    # n "[e] jumps over you and leaves the room."
    n "[e] salta sopra di te e lascia la stanza."

# game/events_mc.rpy:211
translate italian wakeupDay1_5d70399b:

    # m "By the way, you should start wearing some pants in bed."
    m "A proposito, dovresti iniziare a indossare dei pantaloni a letto."

# game/events_mc.rpy:213
translate italian wakeupDay1_89d409bd:

    # m "By the way, you should start wearing pyjamas in bed."
    m "A proposito, dovresti iniziare a indossare pigiami a letto."

# game/events_mc.rpy:214
translate italian wakeupDay1_61e4f5ac:

    # pc "Huh?!"
    pc "Huh?!"

# game/events_mc.rpy:218
translate italian wakeupDay1_dbd30717:

    # pc "FUCK!"
    pc "FUCK!"

# game/events_mc.rpy:219
translate italian wakeupDay1_01fbb572:

    # m "Hahaha, don't worry, Hun. I have seen that before... even though, you've obviously become a [pcgender]..."
    m "Hahaha, non ti preoccupare, Hun. L'ho visto prima... anche se, ovviamente, sei diventato un [pcgender!t]..."

# game/events_mc.rpy:221
translate italian wakeupDay1_d82c55e9:

    # pc "Yeah, could you leave the room please!"
    pc "Sì, potresti lasciare la stanza, per favore!"

# game/events_mc.rpy:222
translate italian wakeupDay1_1d3c410e:

    # m "Hahaha, of course, I'm sorry honey."
    m "Hahaha, naturalmente, mi dispiace il miele."

# game/events_mc.rpy:224
translate italian wakeupDay1_0d7416ed:

    # pcthink "Shit! I was so surprised by [e] that I didn't think about sleeping naked..."
    pcthink "Merda! Sono rimasto così sorpreso da [e] che non ho pensato di dormire nudo..."

# game/events_mc.rpy:225
translate italian wakeupDay1_31a769fc:

    # pcthink "That was embarassing..."
    pcthink "Si stava imbarcando..."

# game/events_mc.rpy:227
translate italian wakeupDay1_6ba40074:

    # pcthink "Especially with that morning wood..."
    pcthink "Soprattutto con quel legno del mattino..."

# game/events_mc.rpy:228
translate italian wakeupDay1_8cd64871:

    # pcthink "Wait, did [e] see that too?"
    pcthink "Aspetta, [e] ha visto anche questo?"

# game/events_mc.rpy:229
translate italian wakeupDay1_f22ad3c6:

    # pcthink "Damn it..."
    pcthink "Dannazione..."

# game/events_mc.rpy:231
translate italian wakeupDay1_503241c0:

    # n "You get dressed and take a quick shower."
    n "Ti vesti e fai una doccia veloce."

# game/events_mc.rpy:232
translate italian wakeupDay1_42ed6300:

    # n "After breakfast, [m] and [e] leave for work and school.{p}You go back to your room."
    n "Dopo la prima colazione, [m] e [e] partono per lavoro e scuola.{p}Torna nella tua camera."

# game/events_mc.rpy:233
translate italian wakeupDay1_66b94188:

    # n "After a while" with dissolve
    n "Dopo un po'" with dissolve

# game/events_mc.rpy:237
translate italian wakeupDay1_341ebece:

    # pcthink "Damn it's so boring..."
    pcthink "Dannazione è così noiosa..."

# game/events_mc.rpy:238
translate italian wakeupDay1_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:239
translate italian wakeupDay1_c7fd02d6:

    # pcthink "Maybe I should look for an apartment again..."
    pcthink "Forse dovrei cercare di nuovo un appartamento..."

# game/events_mc.rpy:240
translate italian wakeupDay1_a69377ef_2:

    # pc "..."
    pc "..."

# game/events_mc.rpy:241
translate italian wakeupDay1_088ca266:

    # pcthink "It's nice to see [e] again, but I don't want to stay here with [m]..."
    pcthink "È bello vedere [e] di nuovo, ma non voglio rimanere qui con [m]..."

# game/events_mc.rpy:242
translate italian wakeupDay1_a69377ef_3:

    # pc "..."
    pc "..."

# game/events_mc.rpy:243
translate italian wakeupDay1_15ac3a2e:

    # pcthink "I can still visit them from time to time..."
    pcthink "Posso ancora visitarli di tanto in tanto..."

# game/events_mc.rpy:244
translate italian wakeupDay1_a69377ef_4:

    # pc "..."
    pc "..."

# game/events_mc.rpy:245
translate italian wakeupDay1_f22ad3c6_1:

    # pcthink "Damn it..."
    pcthink "Dannazione..."

# game/events_mc.rpy:246
translate italian wakeupDay1_3203a608:

    # pcthink "Tomorrow my stuff comes...maybe I'll wait until then, so I can at least use my computer instead of the phone for that..."
    pcthink "Domani arriva la mia roba... forse aspetterò fino ad allora, così posso almeno usare il mio computer al posto del telefono per quello..."

# game/events_mc.rpy:247
translate italian wakeupDay1_ae6d2505:

    # pcthink "...yeah I guess that's a good idea..."
    pcthink "...credo che questa sia una buona idea..."

# game/events_mc.rpy:248
translate italian wakeupDay1_a69377ef_5:

    # pc "..."
    pc "..."

# game/events_mc.rpy:252
translate italian wakeupDay1_c2505a61:

    # n "You go over to the living room and zap around the channels..."
    n "Si passa al soggiorno e zap intorno ai canali..."

# game/events_mc.rpy:255
translate italian wakeupDay1_612659db:

    # pcthink "Commercials."
    pcthink "Commerciali."

# game/events_mc.rpy:256
translate italian wakeupDay1_8225006e:

    # pcthink "Commercials..."
    pcthink "Commerciali..."

# game/events_mc.rpy:257
translate italian wakeupDay1_f4244b81:

    # pcthink "Yaawn..."
    pcthink "Yaawn..."

# game/events_mc.rpy:264
translate italian dinnerDay2_7e01f61f:

    # m "So... I know it's just your second day back home, but how do you feel about it? Have you settled in a bit already?"
    m "So... so che è solo il tuo secondo giorno di ritorno a casa, ma come ti senti a riguardo? Ti sei già sistemato in un po'?"

# game/events_mc.rpy:265
translate italian dinnerDay2_e3fd9646:

    # pc "Err... it's still a bit weird to be honest."
    pc "Err... è ancora un po' strano essere onesti."

# game/events_mc.rpy:267
translate italian dinnerDay2_a94c74f3:

    # m "Oh, I'm sure you will come to like it here after a while."
    m "Oh, sono sicuro che vi piacerà qui dopo un po'."

# game/events_mc.rpy:268
translate italian dinnerDay2_ef30c1d2:

    # pcthink "After a while? For how long does she expect me to stay?"
    pcthink "Dopo un po'? Per quanto tempo si aspetta che rimanga?"

# game/events_mc.rpy:270
translate italian dinnerDay2_3c8e055f:

    # m "I..."
    m "I..."

# game/events_mc.rpy:271
translate italian dinnerDay2_e612037c:

    # m "..."
    m "..."

# game/events_mc.rpy:273
translate italian dinnerDay2_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh?"

# game/events_mc.rpy:274
translate italian dinnerDay2_8746b622:

    # m "I know you don't have many good memories in this house..."
    m "So che non hai molti buoni ricordi in questa casa..."

# game/events_mc.rpy:276
translate italian dinnerDay2_7ce7e5c3:

    # m "But I'm sure that will change from now on!"
    m "Ma sono sicuro che da ora in poi cambierà!"

# game/events_mc.rpy:277
translate italian dinnerDay2_267d5d43:

    # pc "Eh... yeah... maybe..."
    pc "Eh... sì... forse..."

# game/events_mc.rpy:278
translate italian dinnerDay2_1568d1ae:

    # pcthink "Damn I think she really means it... I'm not sure that it's possible though."
    pcthink "Dannazione Penso che lei lo significhi veramente... non sono sicuro che sia possibile però."

# game/events_mc.rpy:280
translate italian dinnerDay2_40da31b4:

    # m "Anyway, it must be boring being alone here half of the day?"
    m "In ogni caso, deve essere noioso essere solo qui la metà del giorno?"

# game/events_mc.rpy:281
translate italian dinnerDay2_a367d625:

    # pc "Yeah, kinda."
    pc "Sì, gente."

# game/events_mc.rpy:283
translate italian dinnerDay2_79065927:

    # m "When did you say your stuff comes?"
    m "Quando hai detto che è arrivata la tua roba?"

# game/events_mc.rpy:285
translate italian dinnerDay2_b0155ac1:

    # pc "...should be delivered by tomorrow. I don't know the exact time though."
    pc "... dovrebbe essere consegnato entro domani. Non so il tempo esatto però."

# game/events_mc.rpy:287
translate italian dinnerDay2_cafd2cdd:

    # m "Have you been able to change your address?"
    m "Sei stato in grado di cambiare il tuo indirizzo?"

# game/events_mc.rpy:289
translate italian dinnerDay2_7c8a8431:

    # pc "Yeah, I sent them the new address via email."
    pc "Sì, ho inviato loro il nuovo indirizzo via email."

# game/events_mc.rpy:290
translate italian dinnerDay2_13506d52:

    # m "Good. That makes it easier at least."
    m "Bene. Questo rende almeno più facile."

# game/events_mc.rpy:292
translate italian dinnerDay2_73cae700:

    # n "You continue to chatter with [m] while [e] occasionally makes faces at you."
    n "Continui a chiacchierare con [m] mentre [e] occasionalmente ti fa volti."

# game/events_mc.rpy:297
translate italian stuffArrivesDay3_c27f600a:

    # n "You hear the doorbell ring."
    n "Senti l'anello del campanello."

# game/events_mc.rpy:299
translate italian stuffArrivesDay3_d4803e34:

    # pc "Uhhh..."
    pc "Uhhh..."

# game/events_mc.rpy:301
translate italian stuffArrivesDay3_180d7c36:

    # pc "YAAAAWN..."
    pc "YAAAAWN..."

# game/events_mc.rpy:303
translate italian stuffArrivesDay3_c87da698:

    # pcthink "Looks like [e] is up already..."
    pcthink "Sembra che [e] sia già finito..."

# game/events_mc.rpy:304
translate italian stuffArrivesDay3_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:305
translate italian stuffArrivesDay3_869ad9b7:

    # pcthink "Wha... shit, the door. It must be my stuff being delivered!"
    pcthink "Wh... merda, la porta. Deve essere la mia roba che viene consegnata!"

# game/events_mc.rpy:307
translate italian stuffArrivesDay3_74e8e915:

    # n "You get up quickly, throw some clothes on and hurry to the front door."
    n "Ti alzi rapidamente, getti dei vestiti e sbrigati alla porta d'ingresso."

# game/events_mc.rpy:312
translate italian getEfromSchool1_719102df:

    # n "A bit later" with dissolve
    n "Un po' più tardi" with dissolve

# game/events_mc.rpy:315
translate italian getEfromSchool1_88129d62:

    # pcthink "Alright, there is the school..."
    pcthink "Va bene, c'è la scuola..."

# game/events_mc.rpy:316
translate italian getEfromSchool1_f8e84e65:

    # n "Suddenly you hear a voice from behind you."
    n "Improvvisamente sentite una voce da dietro di voi."

# game/events_mc.rpy:317
translate italian getEfromSchool1_9a3323ff:

    # Voice" "Hey! Stop right there!"
    "Voce" "Hey! Fermatevi proprio lì!"

# game/events_mc.rpy:318
translate italian getEfromSchool1_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh?"

# game/events_mc.rpy:321
translate italian getEfromSchool1_efbb62e5:

    # pcthink "Oh no... not this guy again..."
    pcthink "Oh no... non questo ragazzo di nuovo..."

# game/events_mc.rpy:323
translate italian getEfromSchool1_f21e3a4e:

    # Cop" "Hey... *pant* ...I said... *pant* ... stop right there!"
    "Cop" "Hey... *pant* ...Ho detto... *pant* ... fermatevi proprio lì!"

# game/events_mc.rpy:324
translate italian getEfromSchool1_5a35ce3d:

    # pc "I'm just standing here Officer..."
    pc "Sono solo qui ufficiale..."

# game/events_mc.rpy:325
translate italian getEfromSchool1_c7dd96ed:

    # Cop" "You... *pant* ... shit..."
    "Cop" "You... *pant* ... merda..."

# game/events_mc.rpy:326
translate italian getEfromSchool1_38aa9b62:

    # pc "A little out of shape Officer?"
    pc "Un po 'fuori forma Ufficiale?"

# game/events_mc.rpy:328
translate italian getEfromSchool1_b5c7bd7a:

    # Cop" "You better shut it... *pant* ...smart ass."
    "Cop" "È meglio chiuderlo... *pant* ...culo intelligente."

# game/events_mc.rpy:330
translate italian getEfromSchool1_04e1c72c:

    # Cop" "Just so you know... I know about you... I have seen your files!"
    "Cop" "Solo così sapete... So di voi... Ho visto i vostri file!"

# game/events_mc.rpy:331
translate italian getEfromSchool1_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:333
translate italian getEfromSchool1_4071f137:

    # Cop" "What?! Surprised?"
    "Cop" "Cosa?! Sorpreso?"

# game/events_mc.rpy:335
translate italian getEfromSchool1_5bd96603:

    # Cop" "It wasn't easy to get them *gasp* and my colleagues even told me to let it go, but I'm not a lazy bum like they are."
    "Cop" "Non è stato facile ottenerli *gasp* e i miei colleghi mi hanno persino detto di lasciarlo andare, ma non sono un ragazzo pigro come loro."

# game/events_mc.rpy:336
translate italian getEfromSchool1_e91b54e7:

    # Cop" "The case might be closed, but I know you have skeletons in your closet!"
    "Cop" "Il caso potrebbe essere chiuso, ma so che hai scheletri nel tuo armadio!"

# game/events_mc.rpy:337
translate italian getEfromSchool1_a08e5c45:

    # pcthink "Shit!"
    pcthink "Merda!"

# game/events_mc.rpy:339
translate italian getEfromSchool1_b385900b:

    # Cop" "I'll be keeping an eye on you!"
    "Cop" "Sarò tenere d'occhio su di voi!"

# game/events_mc.rpy:340
translate italian getEfromSchool1_22c9823e:

    # pc "Well, I hope you have fun with that. I have things to do..."
    pc "Beh, spero che ti diverti con quello. Ho delle cose da fare..."

# game/events_mc.rpy:341
translate italian getEfromSchool1_9d86ab89:

    # Cop" "I'm watching you!"
    "Cop" "Ti sto guardando!"

# game/events_mc.rpy:343
translate italian getEfromSchool1_1f323051:

    # pc "Yeah... bye..."
    pc "Yeah... bye..."

# game/events_mc.rpy:344
translate italian getEfromSchool1_4d295fc8:

    # pcthink "Damn, this guy is annoying... I hope he doesn't become a problem..."
    pcthink "Dannazione, questo tipo è fastidioso... Spero che non diventi un problema..."

# game/events_mc.rpy:347
translate italian getEfromSchool1_fc5cd1ac:

    # pcthink "Hum... the school didn't change a bit. It looks exactly like when I left..."
    pcthink "Uomo... la scuola non è cambiata un po'. Sembra esattamente quando sono partita..."

# game/events_mc.rpy:350
translate italian getEfromSchool1_7ec6f8c8:

    # j "{size=-10}...and you really should have told me how hot [heshe] is{/size}."
    j "{size=-10}...e dovresti davvero dirmi quanto sia caldo [heshe!t] {/size}."

# game/events_mc.rpy:351
translate italian getEfromSchool1_b5156e92:

    # pcthink "Isn't that [j]?"
    pcthink "Non è [j]?"

# game/events_mc.rpy:354
translate italian getEfromSchool1_4ff91ccc:

    # j "Speaking of the devil, look who's here!"
    j "Parlando del diavolo, guardate chi è qui!"

# game/events_mc.rpy:356
translate italian getEfromSchool1_d593d4b0:

    # e "Eh?"
    e "Eh?"

# game/events_mc.rpy:357
translate italian getEfromSchool1_dc0ee556:

    # pc "Hey girls."
    pc "Ehi ragazze."

# game/events_mc.rpy:359
translate italian getEfromSchool1_2e48f33f:

    # j "{b}[e]!{/b}"
    j "{b}[e]!{/b}"

# game/events_mc.rpy:361
translate italian getEfromSchool1_3e82ab9a:

    # j "Can't you at least wait until you're home before you hump your big [bs]?"
    j "Non puoi almeno aspettare fino a quando sei a casa prima di gobbare il tuo grande [bs!t]?"

# game/events_mc.rpy:362
translate italian getEfromSchool1_e262c0dd:

    # pcthink "What the?! Hump me?"
    pcthink "Che cosa?! Hump me?"

# game/events_mc.rpy:366
translate italian getEfromSchool1_85279c1b:

    # j "So... what's up [pc]? Did you miss your {i}cute little sister{/i} soo much that you have to come visit her?"
    j "Cos'è [pc]? Ti sei perso la tua {i}sorellina carina {/i} tanto che devi venire a trovarla?"

# game/events_mc.rpy:367
translate italian getEfromSchool1_b55bf2a3:

    # pcthink "What the fuck?"
    pcthink "Che cazzo fa?"

# game/events_mc.rpy:368
translate italian getEfromSchool1_f20dc1e2:

    # j "Or... did you {i}cum{/i} for me?"
    j "Oppure... hai {i}cum{/i} per me?"

# game/events_mc.rpy:370
translate italian getEfromSchool1_ac669b20:

    # e "Hnnn!!!"
    e "Nnn!!!"

# game/events_mc.rpy:371
translate italian getEfromSchool1_74b093ad:

    # j "Hehe, calm down [e], I'm just teasing."
    j "Hehe, calma [e], sto solo prendendo in giro."

# game/events_mc.rpy:372
translate italian getEfromSchool1_5f16d7e5:

    # pcthink "Damn, this girl is such a minx!"
    pcthink "Dannazione, questa ragazza è una minx!"

# game/events_mc.rpy:374
translate italian getEfromSchool1_c6ae69db:

    # pc "Actually, [m] called me to pick up [e]. She is stuck in traffic and can't make it."
    pc "In realtà, [m] mi ha chiamato per raccogliere [e]. È bloccata nel traffico e non può farla."

# game/events_mc.rpy:375
translate italian getEfromSchool1_fbaa51cd:

    # pc "She tried to call you by the way."
    pc "Lei ha cercato di chiamarti a proposito."

# game/events_mc.rpy:377
translate italian getEfromSchool1_50a16fe0:

    # j "Did she?"
    j "Lei è stata?"

# game/events_mc.rpy:379
translate italian getEfromSchool1_18cc7152:

    # j "I didn't notice. Oh but my phone is on vibrate, hehe."
    j "Non ho notato. Oh, ma il mio telefono è in vibrazione, hehe."

# game/events_mc.rpy:380
translate italian getEfromSchool1_9cdf1147:

    # pc "Uh-huh..."
    pc "Uh-huh..."

# game/events_mc.rpy:382
translate italian getEfromSchool1_b37ee738:

    # j "Well, shall we go then?"
    j "Beh, allora andremo?"

# game/events_mc.rpy:383
translate italian getEfromSchool1_f6e48b56:

    # pc "You're coming with us?"
    pc "Stai venendo con noi?"

# game/events_mc.rpy:384
translate italian getEfromSchool1_5f925933:

    # j "Of course!"
    j "Certo!"

# game/events_mc.rpy:386
translate italian getEfromSchool1_43c5c0e8:

    # j "Someone has to make sure that a specific young lady doesn't get herself into trouble."
    j "Qualcuno deve assicurarsi che una giovane donna specifica non si faccia nei guai."

# game/events_mc.rpy:387
translate italian getEfromSchool1_9b114ec6:

    # pc "Eh... Not that I mind you coming with us, but I'm sure I can handle that."
    pc "Eh... Non mi dispiace che tu venga con noi, ma sono sicuro di poterlo gestire."

# game/events_mc.rpy:388
translate italian getEfromSchool1_9157e01d:

    # j "No you can't."
    j "No, non puoi."

# game/events_mc.rpy:389
translate italian getEfromSchool1_5887b362:

    # pc "What? Why is that?"
    pc "Cosa? Perché quello?"

# game/events_mc.rpy:391
translate italian getEfromSchool1_465c7403:

    # j "Because you're the source of the problem, silly."
    j "Perché sei la fonte del problema, sciocco."

# game/events_mc.rpy:392
translate italian getEfromSchool1_7bb57f9a:

    # pc "Err... okay?"
    pc "Errato... okay?"

# game/events_mc.rpy:393
translate italian getEfromSchool1_7cb675a2:

    # j "Well, come on. Let's go!"
    j "Beh, vieni. Andiamo!"

# game/events_mc.rpy:394
translate italian getEfromSchool1_94a92282:

    # e "Mhm!"
    e "Mhm!"

# game/events_mc.rpy:395
translate italian getEfromSchool1_fef852ff:

    # pc "Yeah..."
    pc "Sì..."

# game/events_mc.rpy:397
translate italian getEfromSchool1_1bcb5e23:

    # n "You take the two back home."
    n "Riporti i due a casa."

# game/events_mc.rpy:406
translate italian afterGetEFromSchool_0070cec2:

    # n "After your arrival you let them do their thing and you go to your room."
    n "Dopo il vostro arrivo li lasciate fare la loro cosa e voi andate nella vostra stanza."

# game/events_mc.rpy:409
translate italian afterGetEFromSchool_b4779b58:

    # pcthink "Damn these girls are crazy..."
    pcthink "Dannazione queste ragazze sono pazze..."

# game/events_mc.rpy:410
translate italian afterGetEFromSchool_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:411
translate italian afterGetEFromSchool_012903ad:

    # pcthink "Alright, I guess it's time to unpack the rest of my stuff."
    pcthink "Bene, immagino che sia ora di scompattare il resto della mia roba."

# game/events_mc.rpy:413
translate italian afterGetEFromSchool_cd88fbb2:

    # j "Hey!"
    j "Ehi!"

# game/events_mc.rpy:415
translate italian afterGetEFromSchool_57148086:

    # pc "Uh, hey [j], what's up?"
    pc "Uh, hey [j], che cos'è?"

# game/events_mc.rpy:416
translate italian afterGetEFromSchool_7fa1bbfe:

    # j "Do you need help?"
    j "Hai bisogno di aiuto?"

# game/events_mc.rpy:417
translate italian afterGetEFromSchool_a1b83240:

    # pc "Err... no, I..."
    pc "Err... no, I..."

# game/events_mc.rpy:419
translate italian afterGetEFromSchool_17e9c83e:

    # j "Fuuuuck, you have a VR headset too?"
    j "Fuuuuck, hai anche un auricolare VR?"

# game/events_mc.rpy:421
translate italian afterGetEFromSchool_ca5a95bf:

    # pc "...yeah..."
    pc "...sì..."

# game/events_mc.rpy:423
translate italian afterGetEFromSchool_b0fa534a:

    # j "Awesome!"
    j "Fantastico!"

# game/events_mc.rpy:425
translate italian afterGetEFromSchool_f9754191:

    # j "Oh hey [e], let's help [pc] \"unpack\", hehe."
    j "Oh hey [e], let's help [pc] \"unpack\", hehe."

# game/events_mc.rpy:426
translate italian afterGetEFromSchool_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:427
translate italian afterGetEFromSchool_5c58ed99:

    # n "[e] nods."
    n "[e] nods."

# game/events_mc.rpy:428
translate italian afterGetEFromSchool_581b1a35:

    # pcthink "Damn [j]. Luckily [e] didn't get the joke..."
    pcthink "Dannazione [j]. Fortunatamente [e] non ha ottenuto la battuta..."

# game/events_mc.rpy:430
translate italian afterGetEFromSchool_d7a797d0:

    # j "Okay, let's start with the boxes over here."
    j "Ok, iniziamo con le caselle da qui."

# game/events_mc.rpy:431
translate italian afterGetEFromSchool_a7c89ecd:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:432
translate italian afterGetEFromSchool_1e2add12:

    # pcthink "Well I guess I have no say in this..."
    pcthink "Beh, immagino di non avere voce in capitolo..."

# game/events_mc.rpy:434
translate italian afterGetEFromSchool_a3af40ea:

    # n "A few minutes later." with dissolve
    n "Pochi minuti dopo." with dissolve

# game/events_mc.rpy:440
translate italian afterGetEFromSchool_7a3851e5:

    # pcthink "Damn, [j] is stretching her ass at me all the time..."
    pcthink "Dannazione, [j] sta allungando il culo a me per tutto il tempo..."

# game/events_mc.rpy:442
translate italian afterGetEFromSchool_1bbe003b:

    # pcthink "...this can't be coincidence."
    pcthink "... questa non può essere una coincidenza."

# game/events_mc.rpy:444
translate italian afterGetEFromSchool_75846895:

    # pc "Huh?{w=1}{nw}"
    pc "Huh?{w=1}{nw}"

# game/events_mc.rpy:447
translate italian afterGetEFromSchool_ecc419db:

    # j "Whaa! Naughty..."
    j "Whaa! Cattivo..."

# game/events_mc.rpy:449
translate italian afterGetEFromSchool_0ab94a3c:

    # j "[e]?"
    j "[e]?"

# game/events_mc.rpy:451
translate italian afterGetEFromSchool_f151517c:

    # pc "Uh..."
    pc "Uh..."

# game/events_mc.rpy:453
translate italian afterGetEFromSchool_8956243a:

    # j "Oh come on [e], don't be so selfish. I'm not going to take him away from you!"
    j "Oh vieni [e], non essere così egoista. Non lo toglierò da te!"

# game/events_mc.rpy:455
translate italian afterGetEFromSchool_7a959c1a:

    # pcthink "Take me away from her?"
    pcthink "Mi porti via da lei?"

# game/events_mc.rpy:457
translate italian afterGetEFromSchool_ae3b81a3:

    # j "...but he's still a man and men like to get a little peek from time to time, right [pc]?"
    j "...ma lui è ancora un uomo e un uomo piace avere un piccolo sbircio di tanto in tanto, giusto [pc]?"

# game/events_mc.rpy:459
translate italian afterGetEFromSchool_a7c89ecd_1:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:463
translate italian afterGetEFromSchool_11ff6de2:

    # j "Oh wow, look at this [e]."
    j "Oh wow, guarda questo [e]."

# game/events_mc.rpy:464
translate italian afterGetEFromSchool_6e3b6836:

    # e "Hehe."
    e "Hehe."

# game/events_mc.rpy:465
translate italian afterGetEFromSchool_ca581621:

    # pcthink "What the..."
    pcthink "Che cosa..."

# game/events_mc.rpy:466
translate italian afterGetEFromSchool_21f579ff:

    # pc "Hey what are you doing with my underwear?"
    pc "Ehi cosa stai facendo con la mia biancheria intima?"

# game/events_mc.rpy:468
translate italian afterGetEFromSchool_05320ff2:

    # j "Oh come on, what's the matter? It's sexy. I bet you look hot in it."
    j "Oh vieni, qual è la questione? È sexy. Scommetto che sembri caldo in esso."

# game/events_mc.rpy:470
translate italian afterGetEFromSchool_62fbd682:

    # j "Where is the other piece? I wanna try it on!"
    j "Dov'è l'altro pezzo? Voglio provarlo!"

# game/events_mc.rpy:471
translate italian afterGetEFromSchool_76308ec7:

    # pcthink "Jeez, that girl..."
    pcthink "Jeez, quella ragazza..."

# game/events_mc.rpy:472
translate italian afterGetEFromSchool_f8c6ed9a:

    # pc "It won't fit anyway, it's obviously not your size."
    pc "Non si adatta comunque, non è ovviamente la tua dimensione."

# game/events_mc.rpy:474
translate italian afterGetEFromSchool_65cf133e:

    # j "Whoops, yeah. I almost forgot that I don't have such a purrfect and sexy body..."
    j "Whoops, sì. Ho quasi dimenticato che non ho un corpo così violento e sexy..."

# game/events_mc.rpy:476
translate italian afterGetEFromSchool_47cb4908:

    # j "Too bad, I guess {b}you{/b} have to show us how it looks then, hehe."
    j "Troppo male, immagino che {b}tu{/b} debbano mostrarci come sembra allora, hehe."

# game/events_mc.rpy:477
translate italian afterGetEFromSchool_7a0cceb2:

    # pcthink "Damn girl, was this your plan?"
    pcthink "Damn girl, era questo il tuo piano?"

# game/events_mc.rpy:479
translate italian afterGetEFromSchool_fec28338:

    # j "Come on [pc], show us your sexy curves! I'm sure [e] wants to see them too."
    j "Vieni [pc], mostraci le tue curve sexy! Sono sicuro che [e] vuole vederle troppo."

# game/events_mc.rpy:480
translate italian afterGetEFromSchool_fa6c08fd:

    # pcthink "Yeah I'm sure about that, judging by what happened last night..."
    pcthink "Sì, ne sono sicuro, a giudicare da ciò che è successo ieri sera..."

# game/events_mc.rpy:482
translate italian afterGetEFromSchool_4909967a:

    # j "Come on, come on, don't be shy."
    j "Avanti, vieni, non essere timido."

# game/events_mc.rpy:484
translate italian afterGetEFromSchool_252f4d06:

    # j "Here I'll show you my titties too!"
    j "Anche qui vi mostrerò le mie tette!"

# game/events_mc.rpy:486
translate italian afterGetEFromSchool_9623bf2a:

    # e "HNNGH!"
    e "HNNGH!"

# game/events_mc.rpy:487
translate italian afterGetEFromSchool_ca581621_1:

    # pcthink "What the..."
    pcthink "Che cosa..."

# game/events_mc.rpy:488
translate italian afterGetEFromSchool_6c63a394:

    # j "Come on [e], you've seen them a lot too, don't act so prudishly..."
    j "Vieni [e], li hai visti anche molto, non agire così pruamente..."

# game/events_mc.rpy:489
translate italian afterGetEFromSchool_296dd5ba:

    # pcthink "A lot?"
    pcthink "Un sacco?"

# game/events_mc.rpy:491
translate italian afterGetEFromSchool_825836ed:

    # j "Ooooh I get it, it's because mine are bigger than yours, right? Don't worry, I bet she will like yours too."
    j "Ooooh lo ottengo, è perché il mio è più grande del tuo, giusto? Non ti preoccupare, scommetto che anche lei piacerà al tuo."

# game/events_mc.rpy:492
translate italian afterGetEFromSchool_6e73906c:

    # e "HNN!!!"
    e "HNN!!!"

# game/events_mc.rpy:493
translate italian afterGetEFromSchool_9285360e:

    # j "Jeez... calm down [e]! It's not like I'm going to take her away from you..."
    j "Jeez... calma [e]! Non è come se la prendessi via da te..."

# game/events_mc.rpy:498
translate italian haremChoice_21fa08ac:

    # j "...and by the way, didn't we agree to always share everything?"
    j "...e a proposito, non abbiamo accettato di condividere sempre tutto?"

# game/events_mc.rpy:499
translate italian haremChoice_f6a70ae5:

    # pcthink "Shit... this girl is so fucking naughty..."
    pcthink "Si merda... questa ragazza è così scopata naughty..."

# game/events_mc.rpy:501
translate italian haremChoice_f42382f7:

    # j "I bet [heshe] wouldn't mind if we did share, hehe... right [pc]?"
    j "Scommetto [heshe!t] non mi dispiacerebbe se fossimo in condivisione, hehe... giusto [pc]?"

# game/events_mc.rpy:502
translate italian haremChoice_b9d784ee:

    # pc "Eh...?"
    pc "Eh...?"

# game/events_mc.rpy:503
translate italian haremChoice_4ce753c9:

    # pcthink "Did she just imply a threesome? WTF! How am I supposed to answer to that?"
    pcthink "Ha appena implicato un trio? WTF! Come devo rispondere a quello?"

# game/events_mc.rpy:504
translate italian haremChoice_a11119d3:

    # pcthink "Damn, if this would be one these adult games, this would probably be the harem choice..."
    pcthink "Dannazione, se questo sarebbe uno di questi giochi per adulti, questo sarebbe probabilmente la scelta harem..."

# game/events_mc.rpy:508
translate italian haremChoice_7d764bd3:

    # pc "Err... well I guess I wouldn't mind..."
    pc "Err... bene immagino che non mi dispiacerebbe..."

# game/events_mc.rpy:510
translate italian haremChoice_6fd70fd4:

    # j "See [e]?"
    j "Vedi [e]?"

# game/events_mc.rpy:512
translate italian haremChoice_fa0b61d7:

    # pc "Hell no! I'm not going to get shared with anyone!"
    pc "Inferno no! Non andrò a condividere con nessuno!"

# game/events_mc.rpy:513
translate italian haremChoice_8be8da66:

    # j "Oh... I expected a different answer..."
    j "Oh... Mi aspettavo una risposta diversa..."

# game/events_mc.rpy:515
translate italian haremChoice_472c2ff4:

    # n "Suddenly the door to your room opens."
    n "All'improvviso si apre la porta della tua stanza."

# game/events_mc.rpy:517
translate italian haremChoice_1e452f8f:

    # m "Hey Hun, I'm sorry for the hassle. I really didn't expect to be so late."
    m "Hey Hun, mi dispiace per il fastidio. Non mi aspettavo davvero di essere così tardi."

# game/events_mc.rpy:519
translate italian haremChoice_5e75a89f:

    # pc "Yeah, don't worry about it."
    pc "Sì, non preoccuparti."

# game/events_mc.rpy:520
translate italian haremChoice_2ac46b68:

    # j "Hey [m]!"
    j "Ehi [m]!"

# game/events_mc.rpy:522
translate italian haremChoice_5c81e00b:

    # m "Oh hey [j], I thought I heard your voice. I didn't expect you today."
    m "Oh hey [j], pensavo di aver sentito la tua voce. Non ti aspettavo oggi."

# game/events_mc.rpy:524
translate italian haremChoice_77725ba0:

    # j "Yeah, we have a... school project to work on..."
    j "Sì, abbiamo un progetto scolastico su cui lavorare..."

# game/events_mc.rpy:526
translate italian haremChoice_295a6916:

    # m "I see, well then, it's better you start working on it, don't you think?"
    m "Vedo bene, allora, è meglio che tu inizi a lavorarci, non pensi?"

# game/events_mc.rpy:528
translate italian haremChoice_29fcb00c:

    # j "Yeah, sure."
    j "Sì, certo."

# game/events_mc.rpy:530
translate italian haremChoice_789ca657:

    # j "Come on [e], let's go!"
    j "Vieni [e], andiamo!"

# game/events_mc.rpy:531
translate italian haremChoice_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:533
translate italian haremChoice_e477e04e:

    # j "Oh I love that shirt by the way [m]. It looks really hot on you!"
    j "Oh amo quella camicia a modo [m]. Sembra davvero caldo su di te!"

# game/events_mc.rpy:534
translate italian haremChoice_2a77b7f1:

    # m "Why thank you!"
    m "Perché grazie!"

# game/events_mc.rpy:536
translate italian haremChoice_2adb6a63:

    # j "I wonder if you're wearing it for someone special, hehe."
    j "Mi chiedo se lo indossi per qualcuno speciale, guarisci."

# game/events_mc.rpy:537
translate italian haremChoice_cd851e02:

    # m "Wh... well don't you two have something to do?"
    m "Perché... beh, voi due non avete qualcosa da fare?"

# game/events_mc.rpy:538
translate italian haremChoice_d4fc2ce6:

    # j "Of course... {size=-10}Run [e]!{/size}"
    j "Certo... {size=-10}Esegui [e]!{/size}"

# game/events_mc.rpy:539
translate italian haremChoice_01b39e2c:

    # e "Khehehe..."
    e "Khehehe..."

# game/events_mc.rpy:547
translate italian haremChoice_1659dccf:

    # pcthink "Damn, did [j] just stare at [m]s tits?!"
    pcthink "Dannazione, [j] ha appena fissato [m]s tits?!"

# game/events_mc.rpy:550
translate italian haremChoice_812f0b3d:

    # m "Humph..."
    m "Humph..."

# game/events_mc.rpy:551
translate italian haremChoice_f477bc32:

    # pcthink "Why is she closing the door?"
    pcthink "Perché sta chiudendo la porta?"

# game/events_mc.rpy:553
translate italian haremChoice_12a1c413:

    # m "I'm sorry, I didn't know about [j]. I hope she didn't cause you any trouble?"
    m "Mi dispiace, non sapevo [j]. Spero che non ti abbia causato alcun problema?"

# game/events_mc.rpy:554
translate italian haremChoice_9ec943e6:

    # pc "Not at all..."
    pc "Non per niente..."

# game/events_mc.rpy:556
translate italian haremChoice_d823b615:

    # m "Oh that's good, I know she can be a bit ...feisty."
    m "Oh che è buono, so che può essere un po'...feisty."

# game/events_mc.rpy:557
translate italian haremChoice_a5975337:

    # pcthink "...what an understatement..."
    pcthink "...che understatement..."

# game/events_mc.rpy:559
translate italian haremChoice_6365ec0f:

    # m "Anyway, are you hungry? I'll make dinner if you want?"
    m "Comunque, hai fame? Farò cena se vuoi?"

# game/events_mc.rpy:560
translate italian haremChoice_a9d9509c:

    # pc "No, I'm fine, thanks."
    pc "No, sto bene, grazie."

# game/events_mc.rpy:562
translate italian haremChoice_6f236419:

    # m "Okay... Oh by the way..."
    m "Okay... Oh a proposito..."

# game/events_mc.rpy:566
translate italian hallPCtalktoEllie_004808c6:

    # n "You go back to your room."
    n "Tornate nella vostra stanza."

# game/events_mc.rpy:569
translate italian hallPCtalktoEllie_5b15a7d6:

    # pcthink "Okay, I guess it's time for some VR porn, hehe..."
    pcthink "Va bene, immagino che sia ora di qualche porno di VR, hehe..."

# game/events_mc.rpy:570
translate italian hallPCtalktoEllie_fec86c60:

    # pcthink "Err, Shit. I wanted to talk to [e] too..."
    pcthink "Err, merda. Volevo anche parlare con [e]..."

# game/events_mc.rpy:571
translate italian hallPCtalktoEllie_fd689d22:

    # pcthink "...but I'm actually quite a bit horny now..."
    pcthink "...ma sono un po' eccitata ora..."

# game/events_mc.rpy:575
translate italian hallPCtalktoEllie_dea0400a:

    # pcthink "Yeah, it's better to clear my mind before she comes over again tonight."
    pcthink "Sì, è meglio sgomberarmi la mente prima di tornare stasera."

# game/events_mc.rpy:577
translate italian hallPCtalktoEllie_f78c2d8f:

    # pc "Hey Ladies, did you miss me?"
    pc "Ehi signore, ti sei mancato?"

# game/events_mc.rpy:581
translate italian hallPCtalktoEllie_ab8c79c4:

    # Girls" "Heeey, elder unrelated roommate..."
    "Ragazze" "Heeey, anziano compagno di camera..."

# game/events_mc.rpy:583
translate italian hallPCtalktoEllie_e564218e:

    # pcthink "Err what the..."
    pcthink "Err cosa..."

# game/events_mc.rpy:589
translate italian hallPCtalktoEllie_d6ec470a:

    # pcthink "Yeah, I really shouldn't wait any longer. I haven't talked to her alone once, since I've been back..."
    pcthink "Sì, non dovrei aspettare più. Non ho parlato con lei da solo una volta, da quando sono tornato..."

# game/events_mc.rpy:591
translate italian hallPCtalktoEllie_7c37fbb0:

    # n "You go to her room."
    n "Tu vai nella sua stanza."

# game/events_mc.rpy:595
translate italian pcroomnight3_421360dd:

    # n "Nothing special happens for the rest of the day."
    n "Non succede nulla di speciale per il resto della giornata."

# game/events_mc.rpy:597
translate italian pcroomnight3_987cdc4b:

    # n "Later that Night." with dissolve
    n "Più tardi quella Notte." with dissolve

# game/events_mc.rpy:601
translate italian pcroomnight3_b97d710a:

    # pc "Yaawn..."
    pc "Yaawn..."

# game/events_mc.rpy:602
translate italian pcroomnight3_7818a9aa:

    # pcthink "It's late... I guess [e] should be coming over soon..."
    pcthink "È tardi... Immagino che [e] dovrebbe arrivare presto..."

# game/events_mc.rpy:603
translate italian pcroomnight3_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:604
translate italian pcroomnight3_b9b155fe:

    # extend "."
    extend "."

# game/events_mc.rpy:605
translate italian pcroomnight3_b9b155fe_1:

    # extend "."
    extend "."

# game/events_mc.rpy:607
translate italian pcroomnight3_1a88aff7:

    # m "{size=-10}[e]? What are you doing up so late? Trying to sneak into your [bs]'s bed again?{/size}"
    m "{size=-10}[e]? Cosa stai facendo così tardi? Cerchi di entrare di nuovo nel letto del tuo [bs!t]?{/size}"

# game/events_mc.rpy:608
translate italian pcroomnight3_962b8eaa:

    # pcthink "Huh? [m] is still awake?"
    pcthink "Huh? [m] è ancora sveglio?"

# game/events_mc.rpy:609
translate italian pcroomnight3_a643ba94:

    # m "{size=-10}You should really let [himher] sleep [e]...{/size}"
    m "{size=-10}Dovresti davvero lasciare [himher!t] dormire [e]...{/size}"

# game/events_mc.rpy:610
translate italian pcroomnight3_2f115d4d:

    # e "{size=-10}...{/size}"
    e "{size=-10}...{/size}"

# game/events_mc.rpy:611
translate italian pcroomnight3_142bf015:

    # m "{size=-10}Listen [e], I know you're really happy that [pc] is back, but [heshe] just moved in and we both don't know if [heshe] intends to stay...{/size}"
    m "{size=-10}Ascolta [e], So che sei davvero felice che [pc] sia tornato, ma [heshe!t] appena spostato e entrambi non sappiamo se [heshe!t] intende rimanere. .{/size}"

# game/events_mc.rpy:612
translate italian pcroomnight3_0e370ad3:

    # m "{size=-10}...also, [heshe]'s been away for a long time, we basically don't know anything about [himher]. Plus, do you even know if [heshe] is okay with you sleeping in [hisher] bed?{/size}"
    m "{size=-10}...anche, [heshe!t]è stato via per molto tempo, fondamentalmente non sappiamo nulla di [himher!t]. Inoltre, sai anche se [heshe!t] va bene con te nel letto [hisher!t] ?{/size}"

# game/events_mc.rpy:613
translate italian pcroomnight3_7e05c46a:

    # pcthink "Damn, are you serious [m]?"
    pcthink "Dannazione, sei serio [m]?"

# game/events_mc.rpy:614
translate italian pcroomnight3_9689d40d:

    # m "{size=-10}Come on [e], go back to your room. If [heshe]'s going to stay, you'll have enough opportunities to cuddle with [himher] in the future...{/size}"
    m "{size=-10}Vieni su [e], torna alla tua stanza. Se [heshe]sta per rimanere, avrai abbastanza opportunità di coccolare con [himher!t] in futuro...{/size}"

# game/events_mc.rpy:615
translate italian pcroomnight3_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:616
translate italian pcroomnight3_a3d517f0:

    # pcthink "Well... I guess I'm going to sleep alone tonight..."
    pcthink "Bene... Immagino che dormirò da solo stasera..."

# game/events_mc.rpy:621
translate italian pcroomnight3_b297f259:

    # m "[bpc], WHAT THE HELL?!"
    m "[bpc], COSA HELL?!"

# game/events_mc.rpy:622
translate italian pcroomnight3_25e7f477:

    # m "Don't tell me you got yourself into a fight AGAIN?!"
    m "Non dirmi che ti sei imbattuto in una lotta AGAIN?!"

# game/events_mc.rpy:623
translate italian pcroomnight3_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:624
translate italian pcroomnight3_a2d15ea3:

    # m "Just what the hell is wrong with you? Why must you constantly embarrass me?"
    m "Proprio quello che l'inferno è sbagliato con te? Perché devi mettermi costantemente in imbarazzo?"

# game/events_mc.rpy:625
translate italian pcroomnight3_eca096dd:

    # m "Can't you be a normal [boygirl] once in your life?"
    m "Non puoi essere un normale [boygirl!t] una volta nella tua vita?"

# game/events_mc.rpy:626
translate italian pcroomnight3_a69377ef_2:

    # pc "..."
    pc "..."

# game/events_mc.rpy:627
translate italian pcroomnight3_6095f49c:

    # m "Oh I guess that's a bit too much to ask with a Dad like yours..."
    m "Oh immagino che sia un po' troppo da chiedere con un papà come il tuo..."

# game/events_mc.rpy:628
translate italian pcroomnight3_92df5d5f:

    # m "I swear, if I get a call from your school or the police again, you'll regret ever being born!"
    m "Ho giurato, se ricevo una chiamata dalla tua scuola o dalla polizia di nuovo, ti pentirai di essere mai nato!"

# game/events_mc.rpy:629
translate italian pcroomnight3_a69377ef_3:

    # pc "..."
    pc "..."

# game/events_mc.rpy:630
translate italian pcroomnight3_151099dc:

    # m "DON'T JUST STAND THERE LIKE THE FOOL YOU ARE, YOU'RE DRIPPING BLOOD ALL OVER THE FLOOR, GO TO THE BATHROOM AND WASH YOUR FACE!"
    m "NON SENTIRE QUI SIMILE LA PESCA DI ARE, LA TUA PROPRIETA BLOOD TUTTO IL FIORO, VO AL BAGNO E LAVORA IL TUO FACE!"

# game/events_mc.rpy:631
translate italian pcroomnight3_a69377ef_4:

    # pc "..."
    pc "..."

# game/events_mc.rpy:632
translate italian pcroomnight3_28aa1485:

    # m "...and don't forget to wipe the blood off the floor when you're done..."
    m "...e non dimenticare di cancellare il sangue dal pavimento quando hai finito..."

# game/events_mc.rpy:633
translate italian pcroomnight3_a69377ef_5:

    # pc "..."
    pc "..."

# game/events_mc.rpy:642
translate italian wakeupDay3_fe60ed33:

    # pcthink "Uhh... not again..."
    pcthink "Uhh... non di nuovo..."

# game/events_mc.rpy:644
translate italian wakeupDay3_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:645
translate italian wakeupDay3_1eec5249:

    # pcthink "...she didn't even care I was hurt..."
    pcthink "...non le importava nemmeno che mi facesse male..."

# game/events_mc.rpy:646
translate italian wakeupDay3_0754dd99:

    # pcthink "Fuck you [m], I'm not going to forgive you so easily!"
    pcthink "Cazzo tu [m], non ti perdonerò così facilmente!"

# game/events_mc.rpy:647
translate italian wakeupDay3_f0cac90d_1:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:648
translate italian wakeupDay3_27d07940:

    # pcthink "At least [e] was worried about me..."
    pcthink "Almeno [e] era preoccupato per me..."

# game/events_mc.rpy:649
translate italian wakeupDay3_c7edfe55:

    # pcthink "I can't even count how many times she stitched me together after I got into a fight."
    pcthink "Non riesco nemmeno a contare quante volte mi ha cucito insieme dopo che sono entrato in una lotta."

# game/events_mc.rpy:650
translate italian wakeupDay3_5fb8e78b:

    # pcthink "She was still so young, but she tried to cheer me up so many times..."
    pcthink "Era ancora così giovane, ma ha cercato di rallegrarmi tante volte..."

# game/events_mc.rpy:651
translate italian wakeupDay3_a2d1e6e6:

    # pcthink "I think she knew that I was just angry..."
    pcthink "Penso che sapesse che ero appena arrabbiato..."

# game/events_mc.rpy:652
translate italian wakeupDay3_f730a80c:

    # pcthink "Angry at [m], angry at the old man, angry at everything and everyone..."
    pcthink "Arrabbiato contro [m], arrabbiato contro il vecchio, arrabbiato contro tutto e tutti..."

# game/events_mc.rpy:653
translate italian wakeupDay3_f0cac90d_2:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:654
translate italian wakeupDay3_d1861303:

    # pcthink "Well I better find the perfect present for her birthday. I have a lot to make up for and only a few hours time."
    pcthink "Meglio trovare il regalo perfetto per il suo compleanno. Ho molto da recuperare e solo poche ore di tempo."

# game/events_mc.rpy:656
translate italian wakeupDay3_cfa20fe0:

    # n "You take a quick shower and make yourself ready to go into the city."
    n "Si prende una rapida doccia e si prepara ad andare in città."

# game/events_mc.rpy:659
translate italian wakeupDay3_e9fe1850:

    # pcthink "Alright, time to go."
    pcthink "Bene, ora di andare."

# game/events_mc.rpy:667
translate italian pcCityDay3_28848481:

    # pcthink "Okay, where should I start..."
    pcthink "Okay, dove dovrei iniziare..."

# game/events_mc.rpy:668
translate italian pcCityDay3_3199d732:

    # pcthink "Uhh... shit, I don't even know what [e] likes... I should have asked [m] or [j]..."
    pcthink "Uhh... merda, non so nemmeno cosa [e] gradisce... avrei dovuto chiedere [m] o [j]..."

# game/events_mc.rpy:669
translate italian pcCityDay3_c1686997:

    # pcthink "Hum..."
    pcthink "Hum..."

# game/events_mc.rpy:670
translate italian pcCityDay3_ed44c29b:

    # pcthink "Wasn't there a cafe somewhere here? I can't think straight without a good coffee in the morning..."
    pcthink "Non c'era un caffè da qualche parte qui? Non riesco a pensare dritto senza un buon caffè al mattino..."

# game/events_mc.rpy:672
translate italian pcCityDay3_37827316:

    # n "You look around for the cafe until you find it."
    n "Si guarda intorno per il caffè fino a quando non lo si trova."

# game/events_mc.rpy:674
translate italian pcCityDay3_d9794f2e:

    # Dude" "Yeah?"
    "Sì?"

# game/events_mc.rpy:675
translate italian pcCityDay3_9ba53076:

    # pcthink "Wasn't there a girl working here last time?"
    pcthink "L'ultima volta non c'era una ragazza che lavorava qui?"

# game/events_mc.rpy:676
translate italian pcCityDay3_3ec4a123:

    # pc "The biggest cup of the strongest coffee you have."
    pc "La tazza più grande del caffè più forte che avete."

# game/events_mc.rpy:677
translate italian pcCityDay3_30b2640a:

    # Dude" "Sure..."
    "Nudo" "Sure..."

# game/events_mc.rpy:678
translate italian pcCityDay3_ec5691e0:

    # pcthink "This guy looks kind of funny in the uniform..."
    pcthink "Questo ragazzo sembra piuttosto divertente nell'uniforme..."

# game/events_mc.rpy:680
translate italian pcCityDay3_37d2b588:

    # Dude" "Here you go..."
    "Dude" "Qui vai..."

# game/events_mc.rpy:681
translate italian pcCityDay3_1376d297:

    # pc "Thanks, keep the change."
    pc "Grazie, mantieni il cambiamento."

# game/events_mc.rpy:682
translate italian pcCityDay3_404de4ef:

    # Dude" "Thanks."
    "Grazie."

# game/events_mc.rpy:684
translate italian pcCityDay3_2d94c374:

    # Girl" "{size=-10}Yeah, bye!{/size}"
    "Ragazza" "{size=-10}Yeah, bye!{/size}"

# game/events_mc.rpy:685
translate italian pcCityDay3_df2388d3:

    # pcthink "That's the barista from last time."
    pcthink "È il barista dell'ultima volta."

# game/events_mc.rpy:687
translate italian pcCityDay3_f6920ce9:

    # Girl" "Oh, you."
    "Ragazza" "Oh, voi."

# game/events_mc.rpy:688
translate italian pcCityDay3_9f72f07c:

    # pc "Me?"
    pc "Io?"

# game/events_mc.rpy:689
translate italian pcCityDay3_deb02910:

    # pcthink "Looks like she's remembering me as well."
    pcthink "Sembra che anche lei mi stia ricordando."

# game/events_mc.rpy:691
translate italian pcCityDay3_d16d473d:

    # Girl" "Ah sorry, haha. You looked really tired last time, that's why I remember you, haha."
    "Ragazza" "Ah mi dispiace, haha. Sembri davvero stanco la volta scorsa, ecco perché ti ricordo, haha."

# game/events_mc.rpy:692
translate italian pcCityDay3_528a0c0a:

    # pc "Yeah that sounds like me... I hope I didn't look too scary?"
    pc "Sì, che suona come me... spero di non sembrare troppo spaventoso?"

# game/events_mc.rpy:693
translate italian pcCityDay3_d7aed506:

    # Girl" "Haha no, not at all. Err... how is your friend?"
    "Ragazza" "Haha no, non affatto. Errare... come è il tuo amico?"

# game/events_mc.rpy:694
translate italian pcCityDay3_0ea4f12e:

    # pc "Friend?"
    pc "Amico?"

# game/events_mc.rpy:695
translate italian pcCityDay3_14a251d1:

    # Girl" "The older woman..."
    "Ragazza" "La donna anziana..."

# game/events_mc.rpy:696
translate italian pcCityDay3_26a6d4b5:

    # Girl" "Oh that sounds mean. I didn't mean that she looks old or something, haha. She's a regular."
    "Ragazza" "Oh che suona significa. Non intendevo che lei sembri vecchia o qualcosa, haha. È una regolare."

# game/events_mc.rpy:697
translate italian pcCityDay3_2643ac94:

    # Girl" "...but she looked really sad when she left the other day. I hope she's alright?"
    "Ragazza" "...ma sembrava davvero triste quando ha lasciato l'altro giorno. Spero che abbia ragione?"

# game/events_mc.rpy:698
translate italian pcCityDay3_b3701ceb:

    # pc "I guess you mean [m]... yeah she's fine."
    pc "Immagino che tu intenda [m]... sì, sta bene."

# game/events_mc.rpy:699
translate italian pcCityDay3_1bd56328:

    # Girl" "Did you break up with her?"
    "Ragazza" "Avete rotto con lei?"

# game/events_mc.rpy:700
translate italian pcCityDay3_c251847b:

    # pc "Break up? Hell no! She's not my girlfriend."
    pc "Rompiti? Inferno no! Non è la mia ragazza."

# game/events_mc.rpy:701
translate italian pcCityDay3_bf799a98:

    # Dude" "Hey [h], stop chattering, I want to go home!"
    "Dude" "Hey [h], smettere di chiacchierare, voglio andare a casa!"

# game/events_mc.rpy:703
translate italian pcCityDay3_897e803a:

    # Girl" "Ugh... Looks like I have to start my shift."
    "Ragazza" "Ugh... Sembra che io debba iniziare il mio turno."

# game/events_mc.rpy:704
translate italian pcCityDay3_0bea89ce:

    # pc "Yeah, too bad."
    pc "Sì, troppo male."

# game/events_mc.rpy:706
translate italian pcCityDay3_6be367f3:

    # Girl" "I'm [h] by the way."
    "Ragazza" "Sono [h] a proposito."

# game/events_mc.rpy:710
translate italian pcCityDay3_1ca71d76:

    # pc "[pc], nice to meet you."
    pc "[pc], bello incontrarti."

# game/events_mc.rpy:711
translate italian pcCityDay3_bdcb3934:

    # h "Nice to meet you too. I hope you visit us more often from now on, we have the best coffee in town and the cutest barista too, haha."
    h "Piacevole di incontrarti anche. Spero che ci visitate più spesso d'ora in poi, abbiamo il miglior caffè in città e anche il barista più carino, haha."

# game/events_mc.rpy:712
translate italian pcCityDay3_6f8d404b:

    # pc "That's two really good reasons to come back. Especially the last one."
    pc "Sono due ottime ragioni per tornare. Soprattutto le ultime."

# game/events_mc.rpy:713
translate italian pcCityDay3_f0e25423:

    # h "Hehe, thanks."
    h "Hehe, grazie."

# game/events_mc.rpy:714
translate italian pcCityDay3_607614bd:

    # Dude" "[h]!"
    "Dude" "[h]!"

# game/events_mc.rpy:716
translate italian pcCityDay3_1799137c:

    # h "Yes, stop crying baby boy!"
    h "Sì, smetta di piangere bambino!"

# game/events_mc.rpy:718
translate italian pcCityDay3_742f2057:

    # h "Well, I have to go. See you soon~"
    h "Beh, devo andare. Ci vediamo presto~"

# game/events_mc.rpy:719
translate italian pcCityDay3_188cbca6:

    # pc "Yeah, seeya."
    pc "Yeah, seeya."

# game/events_mc.rpy:721
translate italian pcCityDay3_1cc9788e:

    # pcthink "Wow, she seemed kind of nervous at first... because of me?"
    pcthink "Wow, sembrava un po' nervosa all'inizio... a causa di me?"

# game/events_mc.rpy:722
translate italian pcCityDay3_f423bdd0:

    # pcthink "Cute..."
    pcthink "Taglia..."

# game/events_mc.rpy:725
translate italian pcCityDay3_b3f60102:

    # pc "Yeah, well I have to go now. Bye..."
    pc "Sì, bene devo andare adesso. Ciao..."

# game/events_mc.rpy:727
translate italian pcCityDay3_47a11eb7:

    # n "You leave the cafe and walk down the street, starting to think about what kind of present [e] would really like to have."
    n "Si lascia il caffè e si cammina per la strada, iniziando a pensare a che tipo di presente [e] vorrebbe davvero avere."

# game/events_mc.rpy:728
translate italian pcCityDay3_7b2c5e7c:

    # n "You wander from window to window, enter some shops, look at jewelry, plushies, clothes and several other things."
    n "Si vaga dalla finestra alla finestra, entrare in alcuni negozi, guardare gioielli, peluche, vestiti e molte altre cose."

# game/events_mc.rpy:729
translate italian pcCityDay3_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:730
translate italian pcCityDay3_878f0222:

    # pcthink "Hum... looked better from far away."
    pcthink "Uomo... sembrava meglio da lontano."

# game/events_mc.rpy:731
translate italian pcCityDay3_f0cac90d_1:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:732
translate italian pcCityDay3_36bb9d3d:

    # pcthink "No... this won't do as well..."
    pcthink "No... questo non funzionerà..."

# game/events_mc.rpy:733
translate italian pcCityDay3_f0cac90d_2:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:734
translate italian pcCityDay3_038399c8:

    # pcthink "No..."
    pcthink "No..."

# game/events_mc.rpy:735
translate italian pcCityDay3_ac7ad650:

    # pcthink "...no..."
    pcthink "...no..."

# game/events_mc.rpy:736
translate italian pcCityDay3_9677f25c:

    # pcthink "Nope, this looks cheap..."
    pcthink "Nope, questo sembra economico..."

# game/events_mc.rpy:737
translate italian pcCityDay3_f0cac90d_3:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:738
translate italian pcCityDay3_a0c75c22:

    # pcthink "Ugh... who buys this shit?"
    pcthink "Ugh... chi acquista questa merda?"

# game/events_mc.rpy:739
translate italian pcCityDay3_f0cac90d_4:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:740
translate italian pcCityDay3_7fc028a8:

    # n "Several shops and after what feels like hours you start to get exhausted."
    n "Diversi negozi e dopo che cosa ti sembra ore si inizia ad esaurirsi."

# game/events_mc.rpy:741
translate italian pcCityDay3_b15dee0c:

    # pcthink "Damn! Why is it so hard to find a good present..."
    pcthink "Dannazione! Perché è così difficile trovare un buon regalo..."

# game/events_mc.rpy:742
translate italian pcCityDay3_78f01fe2:

    # n "You decide to enter one last shop and take a break after that. You enter the shop without looking at what they actually sell."
    n "Si decide di entrare in un ultimo negozio e fare una pausa dopo quello. Si entra nel negozio senza guardare quello che effettivamente vendono."

# game/events_mc.rpy:744
translate italian pcCityDay3_011ebdf7:

    # pcthink "A phone shop?"
    pcthink "Un negozio di telefoni?"

# game/events_mc.rpy:745
translate italian pcCityDay3_d71efe78:

    # pcthink "Well I won't find anything here..."
    pcthink "Beh, non troverò niente qui..."

# game/events_mc.rpy:746
translate italian pcCityDay3_39735d16:

    # pcthink "Or...?"
    pcthink "Oppure...?"

# game/events_mc.rpy:747
translate italian pcCityDay3_da46b3f1:

    # pcthink "Hum... [m] said [e] wouldn't know how to use it, but I highly doubt that."
    pcthink "Uomo... [m] ha detto che [e] non saprebbe come usarlo, ma ne dubito molto."

# game/events_mc.rpy:748
translate italian pcCityDay3_6b328552:

    # pcthink "I don't know any girl her age who doesn't want to have a mobile phone."
    pcthink "Non conosco nessuna ragazza della sua età che non vuole avere un telefono cellulare."

# game/events_mc.rpy:749
translate italian pcCityDay3_05e16121:

    # pcthink "Plus I don't think she has the money to buy a good one herself..."
    pcthink "Inoltre non penso che abbia i soldi per comprare un bravo se stessa..."

# game/events_mc.rpy:750
translate italian pcCityDay3_20fbc89e:

    # pcthink "Yeah, I bet she would be happy to have one, even if she's only using it to send messages and listen to music."
    pcthink "Sì, scommetto che sarebbe felice di averne uno, anche se lo sta usando solo per inviare messaggi e ascoltare musica."

# game/events_mc.rpy:751
translate italian pcCityDay3_53244e6a:

    # pcthink "But I can't buy just {b}some{/b} phone, it has to be a good one. Maybe a Lie-Phone, or the new Moogle phone?"
    pcthink "Ma non riesco a comprare solo {b}un telefono in qualche {/b}, deve essere buono. Forse un Lie-Phone, o il nuovo telefono Moogle?"

# game/events_mc.rpy:752
translate italian pcCityDay3_92f58eb1:

    # pcthink "...and some cute accessories too. Girls her age love this kind of stuff... I think..."
    pcthink "e alcuni accessori carino troppo. Ragazze la sua età amano questo tipo di roba... Penso..."

# game/events_mc.rpy:754
translate italian pcCityDay3_6c247d4a:

    # n "You buy a stupidly expensive mobile phone and some cute accessories.{p}Then you make your way back home."
    n "Compri un telefono cellulare stupidamente costoso e alcuni accessori carini.{p}Poi fai ritorno a casa."

# game/events_mc.rpy:757
translate italian pcCityDay3_f27bff4b:

    # pc "I'm back..."
    pc "Sono tornato..."

# game/events_mc.rpy:758
translate italian pcCityDay3_2819ad57:

    # pcthink "...looks like [m] and [e] aren't back yet."
    pcthink "...sembra [m] e [e] non sono ancora tornati."

# game/events_mc.rpy:760
translate italian pcCityDay3_97439a5b:

    # n "You hide the mobile in your room and after a while waiting for [m] and [e] to return you decide to spend some time watching TV."
    n "Nascondi il cellulare nella tua camera e dopo un po 'di attesa per [m] e [e] per tornare decidi di passare un po 'di tempo a guardare la TV."

# game/events_mc.rpy:762
translate italian pcCityDay3_2d31c06d:

    # Reporter" "...building that burned down a few days ago."
    "Reporter" "...edificio che ha bruciato alcuni giorni fa."

# game/events_mc.rpy:763
translate italian pcCityDay3_a2f0a046:

    # Reporter" "The man responsible for the fire stated that he wanted to protect his wife from becoming corrupted by another man."
    "Reporter" "L'uomo responsabile del fuoco ha dichiarato che voleva proteggere sua moglie dal diventare corrotto da un altro uomo."

# game/events_mc.rpy:764
translate italian pcCityDay3_58185e0f:

    # Reporter" "Police told us that the man believes that some unspecified person is using a magical serum to bend his wife's and other women's will."
    "Reporter" "Police ci ha detto che l'uomo crede che qualche persona non specificata sta usando un siero magico per piegare la volontà di sua moglie e di altre donne."

# game/events_mc.rpy:766
translate italian pcCityDay3_3fc5d9e7:

    # Reporter" "He was sent to a mental hospital..."
    "Reporter" "È stato inviato in un ospedale mentale..."

# game/events_mc.rpy:767
translate italian pcCityDay3_5300c54f:

    # pcthink "Jeez, this guy must have played too many porn games..."
    pcthink "Jeez, questo ragazzo deve aver giocato troppi giochi porno..."

# game/events_mc.rpy:768
translate italian pcCityDay3_4e2d0012:

    # pcthink "Anyway I... huh?"
    pcthink "Comunque io... eh?"

# game/events_mc.rpy:769
translate italian pcCityDay3_8893cd1a:

    # n "*Mobile phone rings*"
    n "*Anelli telefono cellulare*"

# game/events_mc.rpy:771
translate italian pcCityDay3_6509f6ba:

    # pcthink "[m]... what does she want now?"
    pcthink "[m]... cosa vuole adesso?"

# game/events_mc.rpy:773
translate italian pcCityDay3_3b5608b8:

    # pc "Yes..."
    pc "Sì..."

# game/events_mc.rpy:774
translate italian pcCityDay3_3886e06f:

    # m "Hey [pc], it's [m]."
    m "Ehi [pc], è [m]."

# game/events_mc.rpy:775
translate italian pcCityDay3_95d7fa4d:

    # pc "Yeah, I know."
    pc "Sì, lo so."

# game/events_mc.rpy:776
translate italian pcCityDay3_2917b6c2:

    # m "I'm sorry, we are running a bit late, if [t] shows up, could you tell her that we will be home soon?"
    m "Mi dispiace, stiamo correndo un po' tardi, se [t] si presenta, potrebbe dirle che saremo a casa presto?"

# game/events_mc.rpy:777
translate italian pcCityDay3_36c3c199:

    # pc "The therapist? Did you seriously invite her to [e]'s birthday?"
    pc "Il terapeuto? L'hai invitata seriamente al compleanno di [e]?"

# game/events_mc.rpy:778
translate italian pcCityDay3_91a23443:

    # m "No it's..."
    m "No it's..."

# game/events_mc.rpy:779
translate italian pcCityDay3_37d7e05b:

    # m "It's... a session."
    m "E' una sessione."

# game/events_mc.rpy:780
translate italian pcCityDay3_f9ff8ea8:

    # pc "What? That's even worse!"
    pc "Cosa? È ancora peggio!"

# game/events_mc.rpy:781
translate italian pcCityDay3_de7c93ca:

    # m "[pc], please. I know you don't like her, but these sessions are important for [e] and [t] doesn't have much time, so we have to take what we can get."
    m "[pc], per favore. So che non ti piace, ma queste sessioni sono importanti per [e] e [t] non hanno molto tempo, quindi dobbiamo prendere quello che possiamo ottenere."

# game/events_mc.rpy:782
translate italian pcCityDay3_f0293ec3:

    # pc "Geez, [m] this is awful..."
    pc "Geez, [m] questo è terribile..."

# game/events_mc.rpy:783
translate italian pcCityDay3_2e167f81:

    # m "I know, I'm sorry..."
    m "Lo so, mi dispiace..."

# game/events_mc.rpy:784
translate italian pcCityDay3_3e339463:

    # pc "You should be sorry for [e], it's her birthday!"
    pc "Dovresti essere dispiaciuto per [e], è il suo compleanno!"

# game/events_mc.rpy:785
translate italian pcCityDay3_51304096:

    # m "Will you tell her [pc]? Please?"
    m "Le dirai [pc]? Per favore?"

# game/events_mc.rpy:786
translate italian pcCityDay3_f25f4dc9:

    # pc "Humph... yeah fine..."
    pc "Humph... yeah fine..."

# game/events_mc.rpy:787
translate italian pcCityDay3_c26c5d17:

    # m "Thank you, Hun. We'll be home soon."
    m "Grazie, Caccio. Saremo a casa presto."

# game/events_mc.rpy:789
translate italian pcCityDay3_54b7ec6f:

    # pcthink "I can't believe it, she's ruining [e]'s birthday..."
    pcthink "Non riesco a crederci, sta rovinando il compleanno di [e]..."

# game/events_mc.rpy:792
translate italian pcCityDay3_a7b99095:

    # n "*Doorbell rings*"
    n "*Campanella suoneria*"

# game/events_mc.rpy:793
translate italian pcCityDay3_3f1118c4:

    # pcthink "Ugh... speak of the devil..."
    pcthink "Ugh... parla del diavolo..."

# game/events_mc.rpy:795
translate italian pcCityDay3_35ff8356:

    # n "You get up and open the door."
    n "Ti alzi e apri la porta."

# game/events_mc.rpy:799
translate italian ebdPreps_80b6e591:

    # m "This, and that... oh could you take the plate, Hun?"
    m "Questo, e quello... oh si potrebbe prendere il piatto, Hun?"

# game/events_mc.rpy:800
translate italian ebdPreps_79572587:

    # pc "Sure..."
    pc "Certo..."

# game/events_mc.rpy:802
translate italian ebdPreps_845ed577:

    # m "Alright, that's it, everything is ready. Now we only need to wait for [e]."
    m "Va bene, è tutto pronto. Ora dobbiamo solo aspettare [e]."

# game/events_mc.rpy:803
translate italian ebdPreps_d4eacc2f:

    # pc "Is no one else coming?"
    pc "Non sta arrivando nessun altro?"

# game/events_mc.rpy:805
translate italian ebdPreps_d3107ef6:

    # m "Well..."
    m "Bene..."

# game/events_mc.rpy:807
translate italian ebdPreps_e8f66559:

    # m "[e] doesn't have many friends because of... how she is."
    m "[e] non ha molti amici a causa di... com'è."

# game/events_mc.rpy:808
translate italian ebdPreps_5848f0a7:

    # m "...and aunt Jenna died a few years back..."
    m "...e la zia Jenna morì qualche anno indietro..."

# game/events_mc.rpy:809
translate italian ebdPreps_8d4e0c02:

    # pc "Jenna is dead?"
    pc "Jenna è morta?"

# game/events_mc.rpy:811
translate italian ebdPreps_53fdfe12:

    # m "Yes... that's one of the things I wrote you in the letter I sent you..."
    m "Sì... questa è una delle cose che ti ho scritto nella lettera che ti ho inviato..."

# game/events_mc.rpy:812
translate italian ebdPreps_322c09c2:

    # m "I'm sorry, Hun."
    m "I'm sorry, Hun."

# game/events_mc.rpy:813
translate italian ebdPreps_a222d23b:

    # pc "Uhh... yeah, forget it..."
    pc "Uhh... sì, dimenticatelo..."

# game/events_mc.rpy:814
translate italian ebdPreps_d15f5801:

    # pcthink "Damn, I liked aunt Jenna..."
    pcthink "Damn, mi è piaciuta la zia Jenna..."

# game/events_mc.rpy:816
translate italian ebdPreps_57548985:

    # m "Anyway, why don't you sit down and I'll go get the punchbowl?"
    m "Comunque, perché non ti siedi e andrò a prendere il punchbowl?"

# game/events_mc.rpy:818
translate italian ebdPreps_f38ba041:

    # j "I've got it already."
    j "L'ho già."

# game/events_mc.rpy:820
translate italian ebdPreps_debf6129:

    # m "Oh, thanks [j]."
    m "Oh, grazie [j]."

# game/events_mc.rpy:821
translate italian ebdPreps_7addc437:

    # j "Anytime~"
    j "In ogni momento~"

# game/events_mc.rpy:823
translate italian ebdPreps_4deecf0d:

    # pcthink "Geez..."
    pcthink "Geez..."

# game/events_mc.rpy:825
translate italian ebdPreps_fdd4bb46:

    # m "Oh [t], did you finish for today?"
    m "Oh [t], hai finito per oggi?"

# game/events_mc.rpy:827
translate italian ebdPreps_7b962f4b:

    # t "Yes, obviously."
    t "Sì, ovviamente."

# game/events_mc.rpy:828
translate italian ebdPreps_9eed8506:

    # m "Nice, why don't you come and sit down with us for a while?"
    m "Nizza, perché non venite a sedervi con noi per un po'?"

# game/events_mc.rpy:830
translate italian ebdPreps_679e61d6:

    # pcthink "No.fucking.way!"
    pcthink "No.fucking.way!"

# game/events_mc.rpy:831
translate italian ebdPreps_c441437e:

    # pcthink "Looks like [j] thinks the same."
    pcthink "Sembra che [j] pensi lo stesso."

# game/events_mc.rpy:832
translate italian ebdPreps_3a7f8227:

    # m "You should try my punch."
    m "Dovresti provare il mio pugno."

# game/events_mc.rpy:834
translate italian ebdPreps_3e298e1a:

    # t "Sorry, I don't have time and I still have to drive."
    t "Mi dispiace, non ho tempo e devo ancora guidare."

# game/events_mc.rpy:835
translate italian ebdPreps_66ad1b6c:

    # m "Oh, don't worry, there is no alcohol in it."
    m "Oh, non ti preoccupare, non c'è alcool."

# game/events_mc.rpy:836
translate italian ebdPreps_b5fc3376:

    # t "No I..."
    t "No I..."

# game/events_mc.rpy:838
translate italian ebdPreps_09a94aa2:

    # j "Oh you HAVE to try it, I'm sure you've never had punch as good as [m]'s! Hehe."
    j "Oh devi provarlo, sono sicuro che non hai mai avuto un pugno buono come [m]! Hehe."

# game/events_mc.rpy:839
translate italian ebdPreps_d3b0e5fb:

    # pcthink "What the fuck [j]?"
    pcthink "Che cazzo [j]?"

# game/events_mc.rpy:841
translate italian ebdPreps_c9e9538c:

    # m "Why thank you [j]. See [t]? Come on, I'm sure your work can wait for a few minutes."
    m "Perché grazie [j]. Vedi [t]? Avanti, sono sicuro che il tuo lavoro può aspettare qualche minuto."

# game/events_mc.rpy:843
translate italian ebdPreps_1bb01731:

    # t "..."
    t "..."

# game/events_mc.rpy:844
translate italian ebdPreps_5d1b27ce:

    # t "Humph... fine, but just one glass."
    t "Humph... bene, ma solo un bicchiere."

# game/events_mc.rpy:851
translate italian ebd_81b263e7:

    # m "Oh [pc], do you know what got into [t]?"
    m "Oh [pc], sai cosa è entrato in [t]?"

# game/events_mc.rpy:852
translate italian ebd_adb4fc90:

    # pc "Err... no, why? What happened?"
    pc "Err... no, perché? Cos'è successo?"

# game/events_mc.rpy:853
translate italian ebd_08c180df:

    # m "She came into the room, blushing bright red like a tomato, grabbed her bag and left..."
    m "Entrò nella stanza, arrossando rosso brillante come un pomodoro, afferrò la sua borsa e lasciata..."

# game/events_mc.rpy:855
translate italian ebd_483d7c75:

    # j "Yeah, all she said was \"[pc], the perv molested me while I was in the bathroom.\" Hehe."
    j "Sì, tutto quello che lei ha detto era \"[pc], il perv mi ha molestato mentre ero in bagno.\" Hehe."

# game/events_mc.rpy:856
translate italian ebd_310f9987:

    # pc "W-WHAT?!"
    pc "W-WHAT?!"

# game/events_mc.rpy:857
translate italian ebd_dd73012c:

    # m "[j]! That's not funny."
    m "[j]! Non è divertente."

# game/events_mc.rpy:858
translate italian ebd_f5e3fc18:

    # j "Hehe, sorry."
    j "Hehe, mi dispiace."

# game/events_mc.rpy:860
translate italian ebd_f22fccd7:

    # pcthink "Gawd! She was just joking... damn you [j], I almost died."
    pcthink "Gawd! Stava scherzando... ti maledetto [j], quasi sono morto."

# game/events_mc.rpy:862
translate italian ebd_de9a065c:

    # m "All [t] said, was that she has to leave and before I could say anything she was gone..."
    m "Tutto [t] ha detto, era che lei deve partire e prima di poter dire tutto quello che era andato..."

# game/events_mc.rpy:863
translate italian ebd_36d4c18b:

    # pc "Strange..."
    pc "Strano..."

# game/events_mc.rpy:864
translate italian ebd_00670cd8:

    # m "Right?"
    m "Destra?"

# game/events_mc.rpy:865
translate italian ebd_582afdce:

    # m "Anyway, what's up with [e]? Will she be ready soon?"
    m "Comunque, cosa c'è di più con [e]? Sarà pronta presto?"

# game/events_mc.rpy:866
translate italian ebd_1242be4e:

    # pc "Yeah, she's changing right now."
    pc "Sì, sta cambiando in questo momento."

# game/events_mc.rpy:868
translate italian ebd_a75714bf:

    # j "Oh, so you molested her instead?"
    j "Oh, così la hai molestata invece?"

# game/events_mc.rpy:870
translate italian ebd_04348ff1:

    # m "[j]!"
    m "[j]!"

# game/events_mc.rpy:872
translate italian ebd_bb0ed4ca:

    # j "Yeah, sorry [m], you're right, I shouldn't say things like that..."
    j "Sì, mi dispiace [m], hai ragione, non dovrei dire cose del genere..."

# game/events_mc.rpy:874
translate italian ebd_054ce595:

    # j "...It's not like [pc] {b}{i}needs{/i}{/b} to molest her anyway, hehe."
    j "...Non è come [pc] {b}{i}necessità{/i}{/b} per molestarla comunque, hehe."

# game/events_mc.rpy:876
translate italian ebd_c350f789:

    # m "Ugh... you're hopeless [j]."
    m "Ugh... sei disperato [j]."

# game/events_mc.rpy:877
translate italian ebd_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:879
translate italian ebd_b7bc8b83:

    # n "A little bit later." with dissolve
    n "Un po' più tardi." with dissolve

# game/events_mc.rpy:883
translate italian ebd_4ca3b84a:

    # j "...and then you saw the burning apartment building?"
    j "e poi hai visto l'edificio in fiamme appartamenti?"

# game/events_mc.rpy:885
translate italian ebd_c417e350:

    # pc "Yeah."
    pc "Sì."

# game/events_mc.rpy:886
translate italian ebd_f5b6ff60:

    # j "Crazy..."
    j "Crazy..."

# game/events_mc.rpy:888
translate italian ebd_be56fe30:

    # m "I'm so sorry, Hun. But at least you are here with us now. *hic*"
    m "Mi dispiace, Hun. Ma almeno sei qui con noi ora. *hic*"

# game/events_mc.rpy:890
translate italian ebd_e8dd4ad9:

    # j "Yeah, without the fire we would've never met, so cheers to the crazy dude who..."
    j "Sì, senza il fuoco che non avremmo mai incontrato, così tifo al tizio pazzo che..."

# game/events_mc.rpy:892
translate italian ebd_9097c040:

    # m "There you are, we've been waiting for you [e]... Oh you look beautiful!"
    m "Ci sei, ti abbiamo aspettato [e]... Oh ti sembri bello!"

# game/events_mc.rpy:894
translate italian ebd_92ca9df6:

    # j "Wow, [e], you look luscious!"
    j "Wow, [e], sembri lussuoso!"

# game/events_mc.rpy:896
translate italian ebd_cf30de3b:

    # m "I guess now we know what took her so long."
    m "Credo che ora sappiamo cosa la ha impiegata così a lungo."

# game/events_mc.rpy:897
translate italian ebd_fba7b537:

    # j "Hehe."
    j "Hehe."

# game/events_mc.rpy:898
translate italian ebd_72a33094:

    # e "..."
    e "..."

# game/events_mc.rpy:899
translate italian ebd_e612037c:

    # m "..."
    m "..."

# game/events_mc.rpy:900
translate italian ebd_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:902
translate italian ebd_f0a677d2:

    # j "Hey, say something [pc]!"
    j "Ehi, dica qualcosa [pc]!"

# game/events_mc.rpy:903
translate italian ebd_a7c89ecd:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:904
translate italian ebd_4abc8c9c:

    # pc "Sorry, I had to collect my jaw from the floor first, you look stunning [e]."
    pc "Mi dispiace, ho dovuto raccogliere la mia mascella dal pavimento per primo, si guarda mozzafiato [e]."

# game/events_mc.rpy:906
translate italian ebd_72a33094_1:

    # e "..."
    e "..."

# game/events_mc.rpy:907
translate italian ebd_bb3c8ee6:

    # m "Well then, why don't you come over here and sit down with us, [e]..."
    m "Beh, allora, perché non venite qui a sedervi con noi, [e]..."

# game/events_mc.rpy:909
translate italian ebd_628d2bbd:

    # j "...WHAHAHA, [heshe] really fell out of the bed?"
    j "...WHAHAHA, [heshe!t] è davvero caduto dal letto?"

# game/events_mc.rpy:911
translate italian ebd_a702c4e1:

    # m "Yes, it was really funny *hic*"
    m "Sì, è stato davvero divertente *hic*"

# game/events_mc.rpy:912
translate italian ebd_adb99edb:

    # m "And guess what, [heshe] was sleeping completely naked, haha."
    m "E indovinate cosa, [heshe!t] dormiva completamente nudo, haha."

# game/events_mc.rpy:913
translate italian ebd_258a582b:

    # j "NO WAY!"
    j "NESSUN SEMPO!"

# game/events_mc.rpy:914
translate italian ebd_a8dfa9c8:

    # m "It's true, right [pc]?"
    m "È vero, giusto [pc]?"

# game/events_mc.rpy:916
translate italian ebd_5db57f75:

    # j "Aww, I would've loved to see that! Hehe."
    j "Aww, mi sarebbe piaciuto vederlo! Hehe."

# game/events_mc.rpy:918
translate italian ebd_eecf66f6:

    # m "Oh by the way, don't you want to give [e] your presents?"
    m "Oh a proposito, non vuoi dare [e] i tuoi regali?"

# game/events_mc.rpy:920
translate italian ebd_9546c353:

    # j "I already gave her mine yesterday."
    j "Le ho già dato il mio ieri."

# game/events_mc.rpy:922
translate italian ebd_15ca938c:

    # m "Really, what is it?"
    m "Davvero, che cos'è?"

# game/events_mc.rpy:924
translate italian ebd_d0106f24:

    # j "Just some nice clothes and... a secret, hehe."
    j "Solo alcuni bei vestiti e... un segreto, hehe."

# game/events_mc.rpy:926
translate italian ebd_ed817fb8:

    # m "A shecret eh? *hic*"
    m "Un shecret eh? *hic*"

# game/events_mc.rpy:928
translate italian ebd_fd3af669:

    # j "What did you get her [pc]?"
    j "Cosa l'hai ricevuta [pc]?"

# game/events_mc.rpy:929
translate italian ebd_55fc82ce:

    # pc "Oh I have a..."
    pc "Oh ho un..."

# game/events_mc.rpy:930
translate italian ebd_36080486:

    # pcthink "Shit, I can't give it to her in front of [m]. She still thinks [e] has no idea how electronics work..."
    pcthink "Merda, non posso darle davanti a [m]. Lei pensa ancora [e] non ha idea di come funziona l'elettronica..."

# game/events_mc.rpy:932
translate italian ebd_7581e536:

    # m "Well?"
    m "Bene?"

# game/events_mc.rpy:933
translate italian ebd_9879879d:

    # pc "Err... well, haha, it's a secret too..."
    pc "Err... beh, haha, è anche un segreto..."

# game/events_mc.rpy:934
translate italian ebd_448719c3:

    # m "Huh? Why do you all have secret presentsh... did I miss the joke here? *hic*"
    m "Huh? Perché avete tutti un presentatore segreto... mi sono perso la battuta qui? *hic*"

# game/events_mc.rpy:936
translate italian ebd_105c0c39:

    # j "Naaw, don't worry, we just don't want to destroy the surprise, hehe."
    j "Naaw, non preoccuparti, semplicemente non vogliamo distruggere la sorpresa, hehe."

# game/events_mc.rpy:938
translate italian ebd_4e935e33:

    # m "Oh... okay... *hic*"
    m "Oh... okay... *hic*"

# game/events_mc.rpy:940
translate italian ebd_47c298c2:

    # m "...and then I deshided to *hic... shtop drinking and shange my life... it wash the besht deshision I've ever made..."
    m "...e poi ho deshided a *hic... shtop bere e shange la mia vita... lavare la deshision assediata che ho mai fatto..."

# game/events_mc.rpy:942
translate italian ebd_c9a195dc:

    # m "Oh nooo, the besht deshision wash to get [pc] back... *hic*"
    m "Oh nooo, il deshision assortito lavaggio per ottenere [pc] indietro... *hic*"

# game/events_mc.rpy:943
translate italian ebd_1af29da9:

    # j "Yeah, that was a great decision, hehe."
    j "Sì, quella è stata una grande decisione, hehe."

# game/events_mc.rpy:944
translate italian ebd_a951b22b:

    # m "Ugh..."
    m "Ugh..."

# game/events_mc.rpy:946
translate italian ebd_cc241df9:

    # m "I shink I need to pee..."
    m "Ho bisogno di spezzettare..."

# game/events_mc.rpy:948
translate italian ebd_06a6d364:

    # pc "Whoa!"
    pc "Choa!"

# game/events_mc.rpy:949
translate italian ebd_0c127202:

    # m "Whups..."
    m "Whup..."

# game/events_mc.rpy:950
translate italian ebd_6e3b6836:

    # e "Hehe."
    e "Hehe."

# game/events_mc.rpy:951
translate italian ebd_fe5aa5c4:

    # pc "Are you okay [m]?"
    pc "Stai bene [m]?"

# game/events_mc.rpy:952
translate italian ebd_3001f0b2:

    # m "S-shorry, [pc]..."
    m "S-shorry, [pc]..."

# game/events_mc.rpy:953
translate italian ebd_a35e971a:

    # m "Ugh... I don't feel sho well..."
    m "Ugh... Non mi sento bene..."

# game/events_mc.rpy:954
translate italian ebd_3d64b039:

    # pcthink "Shit, she definitely had enough punch for today..."
    pcthink "Merda, ha sicuramente avuto abbastanza pugno per oggi..."

# game/events_mc.rpy:955
translate italian ebd_d9543283:

    # pc "Maybe you should lie down for a while."
    pc "Forse dovresti sdraiarti per un po'."

# game/events_mc.rpy:956
translate italian ebd_c0175e8c:

    # m "...but it's [e]'sh birthday... *hic*"
    m "...ma è [e]'sh compleanno... *hic*"

# game/events_mc.rpy:957
translate italian ebd_2c10d7d0:

    # pc "Doesn't help if you feel bad [m]."
    pc "Non aiuta se ti senti male [m]."

# game/events_mc.rpy:958
translate italian ebd_e442a3ed:

    # m "Uhh... everything is shpinning..."
    m "Uhh... tutto è shpinning..."

# game/events_mc.rpy:960
translate italian ebd_35fc80fb:

    # j "You should help her to the bedroom, [pc]. Hehe."
    j "Dovresti aiutarla nella camera da letto, [pc]. Hehe."

# game/events_mc.rpy:962
translate italian ebd_8c916250:

    # m "But I don't want to go to bed yet... *hic*"
    m "Ma non voglio ancora andare a letto... *hic*"

# game/events_mc.rpy:963
translate italian ebd_6a46278c:

    # pc "Come on [m], I'll help you..."
    pc "Avanti [m], ti aiuterò..."

# game/events_mc.rpy:964
translate italian ebd_64543edd:

    # m "Ugh... okay... i-if I must..."
    m "Ugh... okay... i-if devo che..."

# game/events_mc.rpy:970
translate italian ebd2_307d7ef6:

    # j "Wow, I've never seen [m] {b}this{/b} drunk before!"
    j "Wow, non ho mai visto [m] {b}questo{/b} ubriaco prima!"

# game/events_mc.rpy:971
translate italian ebd2_d8890d8b:

    # e "Heh..."
    e "Heh..."

# game/events_mc.rpy:973
translate italian ebd2_4705691f:

    # pc "Well I did... several times..."
    pc "Beh, ho fatto... più volte..."

# game/events_mc.rpy:974
translate italian ebd2_91eb52ce:

    # pcthink "...and she was completely different this time..."
    pcthink "ed era completamente diversa questa volta..."

# game/events_mc.rpy:975
translate italian ebd2_e8aaaaae:

    # j "She will be alright... right?"
    j "Lei avrà bene... giusto?"

# game/events_mc.rpy:976
translate italian ebd2_fef852ff:

    # pc "Yeah..."
    pc "Sì..."

# game/events_mc.rpy:977
translate italian ebd2_d4ffef5b:

    # j "Great! Now let's start the {i}real{/i} party, right [e]? Hehe..."
    j "Ottimo! Ora iniziamo il {i}vero {/i} party, giusto [e]? Hehe..."

# game/events_mc.rpy:979
translate italian ebd2_72a33094:

    # e "..."
    e "..."

# game/events_mc.rpy:980
translate italian ebd2_0ab94a3c:

    # j "[e]?"
    j "[e]?"

# game/events_mc.rpy:981
translate italian ebd2_c11599d9:

    # e "...ehh..."
    e "...ehh..."

# game/events_mc.rpy:984
translate italian ebd2_72a33094_1:

    # e "..."
    e "..."

# game/events_mc.rpy:985
translate italian ebd2_e23650ca:

    # j "Awww damn it..."
    j "Awww maledetta..."

# game/events_mc.rpy:986
translate italian ebd2_5a4fe1da:

    # pc "Looks like the party is over before it even started..."
    pc "Sembra che la festa sia finita prima ancora di iniziare..."

# game/events_mc.rpy:987
translate italian ebd2_c77a48de:

    # j "AAAAaaaawwww... stupid [j]!"
    j "AAAAaaaawwww... stupido [j]!"

# game/events_mc.rpy:988
translate italian ebd2_b4fa3fc0:

    # j "..."
    j "..."

# game/events_mc.rpy:989
translate italian ebd2_54326bbc:

    # j "I'm sorry..."
    j "I'm sorry..."

# game/events_mc.rpy:990
translate italian ebd2_03240a86:

    # j "I wanted this to be something special for [e] and now everything is fucked..."
    j "Volevo che questo fosse qualcosa di speciale per [e] e ora tutto è scopato..."

# game/events_mc.rpy:991
translate italian ebd2_2955da2c:

    # pc "Well, you tried... and I guess this is a birthday she will remember..."
    pc "Beh, hai provato... e immagino che questo sia un compleanno che ricorderà..."

# game/events_mc.rpy:992
translate italian ebd2_d1ed6d17:

    # j "If she remembers anything at all..."
    j "Se ricorda qualcosa a tutti..."

# game/events_mc.rpy:993
translate italian ebd2_b4fa3fc0_1:

    # j "..."
    j "..."

# game/events_mc.rpy:994
translate italian ebd2_84db82c7:

    # pc "I'm going to take her to bed..."
    pc "La prenderò a letto..."

# game/events_mc.rpy:995
translate italian ebd2_87d3c8fc:

    # j "Yeah..."
    j "Sì..."

# game/events_mc.rpy:997
translate italian ebd2_2d4a91d0:

    # pcthink "I guess I was right..."
    pcthink "Immagino che avevo ragione..."

# game/events_mc.rpy:998
translate italian ebd2_5009a718:

    # pcthink "...it didn't turn out well..."
    pcthink "... non si è spento bene..."

# game/events_mc.rpy:1002
translate italian ebd2_65ee5a5c:

    # pc "Good night [e]."
    pc "Buona notte [e]."

# game/events_mc.rpy:1003
translate italian ebd2_5916f987:

    # e "Hnn..."
    e "Hnn..."

# game/events_mc.rpy:1010
translate italian ebdEnd_017fb70c:

    # pcthink "Well, back home..."
    pcthink "Beh, torna a casa..."

# game/events_mc.rpy:1012
translate italian ebdEnd_692b88a9:

    # pcthink "Home..."
    pcthink "Home..."

# game/events_mc.rpy:1013
translate italian ebdEnd_229bcf50:

    # pcthink "I've never felt at home in this house..."
    pcthink "Non mi sono mai sentito a casa in questa casa..."

# game/events_mc.rpy:1015
translate italian ebdEnd_eff3a57b:

    # pcthink "Hum... I guess [m] isn't going to clear the table anytime soon..."
    pcthink "Uomo... Immagino che [m] non ripulirà il tavolo in qualsiasi momento presto..."

# game/events_mc.rpy:1016
translate italian ebdEnd_fd163d05:

    # pcthink "At least there's no huge pile of junk to clean up like after a normal party..."
    pcthink "Almeno non c'è un enorme mucchio di spazzatura per ripulire come dopo una festa normale..."

# game/events_mc.rpy:1018
translate italian ebdEnd_4e202750:

    # n "You clear the table and decide to go to bed."
    n "Schiarisci il tavolo e decidi di andare a letto."

# game/events_mc.rpy:1020
translate italian ebdEnd_49c5868b:

    # pcthink "What a \"party\"..."
    pcthink "Che \"partito\"..."

# game/events_mc.rpy:1021
translate italian ebdEnd_41ca071c:

    # pcthink "That must have been the worst birthday party ever..."
    pcthink "Che deve essere stata la festa di compleanno peggiore mai..."

# game/events_mc.rpy:1022
translate italian ebdEnd_ca21d5ae:

    # pcthink "Poor [e], I hope she{w=1.0}{nw}"
    pcthink "Poor [e], I hope she{w=1.0}{nw}"

# game/events_mc.rpy:1025
translate italian ebdEnd_4e0bec6e:

    # pc "Whoa [e]!"
    pc "Whoa [e]!"

# game/events_mc.rpy:1027
translate italian ebdEnd_e792b68b:

    # e "Uhhh..."
    e "Uhhh..."

# game/events_mc.rpy:1029
translate italian ebdEnd_442ac6e9:

    # pcthink "Wow she's really drunk..."
    pcthink "Wow è davvero ubriaca..."

# game/events_mc.rpy:1032
translate italian ebdEnd_ddf68bfe:

    # pc "[e]?"
    pc "[e]?"

# game/events_mc.rpy:1033
translate italian ebdEnd_0e681e16:

    # e "*snore*"
    e "*russare*"

# game/events_mc.rpy:1034
translate italian ebdEnd_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1035
translate italian ebdEnd_c52114ef:

    # pc "Oh well... good night [e]..."
    pc "Oh bene... buona notte [e]..."

# game/events_mc.rpy:1047
translate italian afterEBD_a479e7bc:

    # pc "*Yaaawn*"
    pc "*Yaaawn*"

# game/events_mc.rpy:1049
translate italian afterEBD_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1052
translate italian afterEBD_f12901fd:

    # pcthink "[e]..."
    pcthink "[e]..."

# game/events_mc.rpy:1053
translate italian afterEBD_c8badc00:

    # pcthink "When did she become so attached to me?"
    pcthink "Quando è diventata così attaccata a me?"

# game/events_mc.rpy:1054
translate italian afterEBD_4979b763:

    # pcthink "She's was always a bit clingy when we were younger, but this is... different..."
    pcthink "Era sempre un po' aggrappata quando eravamo più giovani, ma questo è... diverso..."

# game/events_mc.rpy:1055
translate italian afterEBD_be5aaa98:

    # pcthink "Could she really be in love with me, like [j] said?"
    pcthink "Potrebbe davvero essere innamorata di me, come [j] ha detto?"

# game/events_mc.rpy:1056
translate italian afterEBD_a0d3b905:

    # pcthink "...with her own [bs]?"
    pcthink "...con il suo [bs!t]?"

# game/events_mc.rpy:1057
translate italian afterEBD_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1058
translate italian afterEBD_7e43320d:

    # pcthink "...and why doesn't she talk?"
    pcthink "...e perché non parla?"

# game/events_mc.rpy:1059
translate italian afterEBD_580a09c1:

    # pcthink "Or does she?"
    pcthink "O vero?"

# game/events_mc.rpy:1060
translate italian afterEBD_6911a2a4:

    # pcthink "Could she be talking to [j]?"
    pcthink "Potrebbe parlare con [j]?"

# game/events_mc.rpy:1061
translate italian afterEBD_00702135:

    # pcthink "I'd understand it if she doesn't want to talk to [m]... or that therabitch..."
    pcthink "Lo capirei se lei non vuole parlare con [m]... o con quel therabitch..."

# game/events_mc.rpy:1062
translate italian afterEBD_ec023f64:

    # pcthink "...but why wouldn't she want to talk to me?"
    pcthink "...ma perché non vorrebbe parlare con me?"

# game/events_mc.rpy:1063
translate italian afterEBD_b23e951c:

    # pcthink "What the hell happened after I moved out?!"
    pcthink "Che diavolo è successo dopo che sono uscito?!"

# game/events_mc.rpy:1066
translate italian afterEBD_7027498e:

    # e "Hnnn..."
    e "Hnnn..."

# game/events_mc.rpy:1067
translate italian afterEBD_0e7a8064:

    # pcthink "Damn..."
    pcthink "Dannazione..."

# game/events_mc.rpy:1069
translate italian afterEBD_b90183ff:

    # pcthink "Well, time to get up. I need to use the bathroom."
    pcthink "Beh, il tempo per alzarmi. Ho bisogno di usare il bagno."

# game/events_mc.rpy:1071
translate italian afterEBD_8dbc43ac:

    # pcthink "I guess I'll let her sleep..."
    pcthink "Immagino che la lascerò dormire..."

# game/events_mc.rpy:1073
translate italian afterEBD_38a7142f:

    # pcthink "I hope [m] isn't up already..."
    pcthink "Spero che [m] non sia già..."

# game/events_mc.rpy:1079
translate italian officeFirst_658f3f89:

    # n "On your way to the office it starts to rain."
    n "Sulla strada verso l'ufficio inizia a piovere."

# game/events_mc.rpy:1080
translate italian officeFirst_8e6b2543:

    # pcthink "Shit, lucky me!"
    pcthink "Merda, fortunatemi!"

# game/events_mc.rpy:1081
translate italian officeFirst_bc2ee6f5:

    # n "You can see a woman running towards the entrance of the building in which you suspect the office to be located."
    n "È possibile vedere una donna che corre verso l'ingresso dell'edificio in cui si sospetta l'ufficio da trovare."

# game/events_mc.rpy:1083
translate italian officeFirst_7af06681:

    # pc "Hey! Hey you!"
    pc "Ehi! Ehi tu!"

# game/events_mc.rpy:1085
translate italian officeFirst_4f3b0408:

    # Woman" "Y-yes?!"
    "Donna" "Y-sì?!"

# game/events_mc.rpy:1086
translate italian officeFirst_51a6f7e0:

    # pc "Is this Atomic Aeronautics?"
    pc "È questa Aeronautica atomica?"

# game/events_mc.rpy:1087
translate italian officeFirst_eeaceb23:

    # Woman" "Y-yes!"
    "Donna" "Y-sì!"

# game/events_mc.rpy:1088
translate italian officeFirst_b6ac243e:

    # pc "I'm [pc]... [pc] [pcsure]."
    pc "Sono [pc]... [pc] [pcsure]."

# game/events_mc.rpy:1089
translate italian officeFirst_ba58eaa8:

    # Woman" "pc "[pcsure]?"
    "Donna" "[pc] [pcsure]?"

# game/events_mc.rpy:1090
translate italian officeFirst_5cf9f3a1:

    # pc "Yes, I'm starting work here tomorrow!"
    pc "Sì, sto iniziando a lavorare qui domani!"

# game/events_mc.rpy:1091
translate italian officeFirst_c9aca35f:

    # Woman" "O-oh, that's you... please, let's go inside!"
    "Donna" "O-oh, sei tu... per favore, andiamo dentro!"

# game/events_mc.rpy:1093
translate italian officeFirst_d97311e5:

    # You follow her."
    "La segui."

# game/events_mc.rpy:1095
translate italian officeFirst_e9edfff6:

    # pc "Finally out of the rain..."
    pc "Finalmente fuori dalla pioggia..."

# game/events_mc.rpy:1096
translate italian officeFirst_ea7019d0:

    # Woman" "Why are you here?"
    "Donna" "Perché sei qui?"

# game/events_mc.rpy:1097
translate italian officeFirst_225fadf5:

    # pc "Huh?"
    pc "Huh?"

# game/events_mc.rpy:1098
translate italian officeFirst_2f424692:

    # Woman" "I-I mean, you're starting tomorrow, why are you here today?"
    "Donna" "I-Intendo dire, state iniziando domani, perché siete qui oggi?"

# game/events_mc.rpy:1099
translate italian officeFirst_10609a33:

    # pc "Well... it's been a while since I left town, and I don't know this area, so I thought it might be a good idea to have a look before I start."
    pc "Bene... è passato un po' da quando ho lasciato la città, e non conosco questa zona, così ho pensato che potrebbe essere una buona idea avere uno sguardo prima di iniziare."

# game/events_mc.rpy:1100
translate italian officeFirst_0651871c:

    # Woman" "I see."
    "Donna" "Io vedere."

# game/events_mc.rpy:1101
translate italian officeFirst_0368c379:

    # pc "I could ask you the same question by the way. It's Sunday, what are you doing here?"
    pc "Potrei farti la stessa domanda a proposito. È domenica, cosa stai facendo qui?"

# game/events_mc.rpy:1102
translate italian officeFirst_f2fbbb51:

    # Woman" "I... C.J... the boss asked me to bring in some papers."
    "Donna" "I... C.J... il capo mi ha chiesto di portare in alcuni documenti."

# game/events_mc.rpy:1103
translate italian officeFirst_c7965c80:

    # pc "He's here, too?"
    pc "Anche lui è qui?"

# game/events_mc.rpy:1104
translate italian officeFirst_225cc38a:

    # Woman" "Yes, like almost every Sunday."
    "Donna" "Sì, come quasi ogni domenica."

# game/events_mc.rpy:1105
translate italian officeFirst_cb272a23:

    # pcthink "WTF, he must be a workaholic..."
    pcthink "WTF, egli deve essere un workaholic..."

# game/events_mc.rpy:1106
translate italian officeFirst_332ec2ad:

    # pc "Every Sunday?"
    pc "Ogni domenica?"

# game/events_mc.rpy:1108
translate italian officeFirst_202424ff:

    # Woman" "Yes... I can tell him that you are here."
    "Donna Sì... Posso dirgli che sei qui."

# game/events_mc.rpy:1109
translate italian officeFirst_c3f1eb0b:

    # pc "Yeah, why not. Can I have a look around?"
    pc "Sì, perché no. Posso avere uno sguardo intorno?"

# game/events_mc.rpy:1110
translate italian officeFirst_04e2f270:

    # Woman" "Uhm... I can show you a bit, why don't you follow me."
    "Donna" "Uhm... Posso mostrarvi un po ', perché non mi seguite."

# game/events_mc.rpy:1111
translate italian officeFirst_d688a87b:

    # pc "Yeah, sure. What's your name by the way?"
    pc "Sì, certo. Qual è il tuo nome a proposito?"

# game/events_mc.rpy:1112
translate italian officeFirst_dbc67763:

    # Woman" "Stephanie..."
    "Donna" "Stephanie..."

# game/events_mc.rpy:1113
translate italian officeFirst_f7e59657:

    # pc "Well, nice to meet you, Stephanie."
    pc "Beh, bello incontrarti, Stephanie."

# game/events_mc.rpy:1117
translate italian officeFirst_d84438d3:

    # st "This is the fabrication and testing hall."
    st "Questa è la sala di fabbricazione e di prova."

# game/events_mc.rpy:1119
translate italian officeFirst_73c10355:

    # pc "Whoa, I didn't know that you're actually building stuff here..."
    pc "Whoa, non sapevo che stai costruendo roba qui..."

# game/events_mc.rpy:1120
translate italian officeFirst_57cdd18c:

    # st "This is the company headquarters, all our prototypes are built and tested here."
    st "Questa è la sede dell'azienda, tutti i nostri prototipi sono costruiti e testati qui."

# game/events_mc.rpy:1121
translate italian officeFirst_96746b87:

    # pc "That's really impressive."
    pc "È davvero impressionante."

# game/events_mc.rpy:1125
translate italian officeFirst_9e5a74fe:

    # pc "Damn, this building is larger than I'd expected it to be..."
    pc "Dannazione, questo edificio è più grande di quanto mi aspettassi che sarà..."

# game/events_mc.rpy:1128
translate italian officeFirst_8502ad06:

    # st "C.J, I have the papers you asked for."
    st "C.J, Ho i documenti che hai chiesto."

# game/events_mc.rpy:1129
translate italian officeFirst_8fd26fd2:

    # boss "Oh good, thanks, [st]."
    boss "Oh bene, grazie, [st]."

# game/events_mc.rpy:1130
translate italian officeFirst_95047118:

    # st "There is something else..."
    st "C'è qualcos'altro..."

# game/events_mc.rpy:1132
translate italian officeFirst_9b1a9288:

    # st "Our new employee..."
    st "Il nostro nuovo dipendente..."

# game/events_mc.rpy:1134
translate italian officeFirst_b04d1381:

    # boss "Oh, I see, you must be [pc]. I didn't expect you today."
    boss "Oh, vedo, tu devi essere [pc]. Non ti aspettavo oggi."

# game/events_mc.rpy:1135
translate italian officeFirst_e671bf7c:

    # pc "Err... yeah, I actually just wanted to have a look at the office, but then it started to rain and Stephanie was nice enough to let me in."
    pc "Err... sì, in realtà volevo solo dare un'occhiata all'ufficio, ma poi ha cominciato a piovere e Stephanie è stato abbastanza bello da lasciarmi entrare."

# game/events_mc.rpy:1136
translate italian officeFirst_e38743f9:

    # boss "Anyway, nice to meet you."
    boss "Comunque, bello incontrarti."

# game/events_mc.rpy:1138
translate italian officeFirst_749df471:

    # boss "[st], why don't you go and get yourself and [pc] a towel to get dry with."
    boss "[st], perché non vai a prendere te stesso e [pc] un asciugamano con cui asciugarsi."

# game/events_mc.rpy:1139
translate italian officeFirst_5dc56ca1:

    # st "Yes, [boss]."
    st "Sì, [boss]."

# game/events_mc.rpy:1141
translate italian officeFirst_8261ab61:

    # boss "You should have called me, I'd have let you in."
    boss "Avresti dovuto chiamarmi, ti avrei lasciato entrare."

# game/events_mc.rpy:1142
translate italian officeFirst_7dd1a646:

    # pc "I didn't know you're here."
    pc "Non sapevo che sei qui."

# game/events_mc.rpy:1144
translate italian officeFirst_befea34a:

    # boss "Ha! I'm always here. My wife is happy when I leave her alone and I'm happy when I don't have to see her... A drink?"
    boss "Ciao! Sono sempre qui. Mia moglie è felice quando la lascio da sola e sono felice quando non devo vederla... Un drink?"

# game/events_mc.rpy:1145
translate italian officeFirst_4be4d14b:

    # pc "Err... no, thanks."
    pc "Err... no, grazie."

# game/events_mc.rpy:1146
translate italian officeFirst_56c830b3:

    # boss "Well I'll take one."
    boss "Beh, ne prenderò una."

# game/events_mc.rpy:1148
translate italian officeFirst_1f2241b6:

    # boss "Actually, it's good that you're here today, so we can go through all the details while it's quiet."
    boss "In realtà, è bene che tu sia qui oggi, quindi possiamo passare attraverso tutti i dettagli mentre è tranquillo."

# game/events_mc.rpy:1149
translate italian officeFirst_667747fd:

    # boss "I guess [st] showed you around a bit?"
    boss "Immagino che [st] ti abbia mostrato circa un po'?"

# game/events_mc.rpy:1150
translate italian officeFirst_29ffe3c1:

    # pc "Yeah, it's quite impressive."
    pc "Sì, è abbastanza impressionante."

# game/events_mc.rpy:1151
translate italian officeFirst_9491da86:

    # boss "Ha! That's nothing. We're just doing the rough tasks here, but I've seen your work, now {b}that's{/b} impressive. You're a true artist."
    boss "Ciao! Non è niente. Stiamo solo facendo i compiti difficili qui, ma ho visto il tuo lavoro, ora {b}che è impressionante. Sei un vero artista."

# game/events_mc.rpy:1152
translate italian officeFirst_dc740ee7:

    # pc "Thanks, I do my best."
    pc "Grazie, faccio del mio meglio."

# game/events_mc.rpy:1154
translate italian officeFirst_9ccf7cb1:

    # boss "That's why I hired you. Now let's sit down, there are a few things I need from you asap!"
    boss "Ecco perché ti ho assunto. Ora siamoci giù, ci sono alcune cose di cui ho bisogno da te!"

# game/events_mc.rpy:1156
translate italian officeFirst_aee865c1:

    # n "After a quick introduction he tells you about a new project he's planning, and your role in it. It turns out that there is not much time left, you only have one week from start to finish for your part of the project."
    n "Dopo una rapida introduzione ti racconta di un nuovo progetto che sta pianificando e il tuo ruolo in esso. Si scopre che non c'è molto tempo a disposizione, avete solo una settimana dall'inizio alla fine per la vostra parte del progetto."

# game/events_mc.rpy:1157
translate italian officeFirst_3fad21d5:

    # n "A while later."
    n "Un po' più tardi."

# game/events_mc.rpy:1159
translate italian officeFirst_ab03fde0:

    # boss "...and if you need anything, be sure to call or just come by. Even if I'm not here, [st] is always there to help."
    boss "...e se hai bisogno di qualcosa, assicurati di chiamare o semplicemente di venire. Anche se non sono qui, [st] è sempre lì per aiutarti."

# game/events_mc.rpy:1160
translate italian officeFirst_4047cc0a:

    # pc "Sure. By the way, is she your secretary?"
    pc "Certo. A proposito, lei è la tua segretaria?"

# game/events_mc.rpy:1161
translate italian officeFirst_25aa3f3b:

    # boss "Yes and no. She's actually more than just a secretary. In fact, she could handle the company on her own if she'd just be a bit more confident, but that woman is way too shy, I tell ya..."
    boss "Sì e no. In realtà è più di una semplice segretaria. In realtà, potrebbe gestire l'azienda da solo se sarebbe solo un po 'più fiducioso, ma quella donna è decisamente troppo timida, lo dico ya..."

# game/events_mc.rpy:1162
translate italian officeFirst_6510b358:

    # pc "Yeah, I've noticed that."
    pc "Sì, l'ho notato."

# game/events_mc.rpy:1163
translate italian officeFirst_9726a64d:

    # boss "Don't worry, she'll open up to you over time. That's just the way she is."
    boss "Non ti preoccupare, ti aprirà nel tempo. È così che è."

# game/events_mc.rpy:1164
translate italian officeFirst_0fb9e488:

    # pc "I hope so... errm... can I ask you a personal question?"
    pc "Spero così... errm... posso farti una domanda personale?"

# game/events_mc.rpy:1165
translate italian officeFirst_a9b94d3f:

    # boss "There is nothing going on between us, so if you're interested in her, just go for it."
    boss "Non c'è nulla che sta accadendo tra di noi, quindi se sei interessato a lei, basta andare per questo."

# game/events_mc.rpy:1166
translate italian officeFirst_df0e83ab:

    # pc "Eh... haha, actually I was wondering what C.J. stands for?"
    pc "Eh... haha, in realtà mi stavo chiedendo per che cosa stia C.J?"

# game/events_mc.rpy:1167
translate italian officeFirst_8ae5dbe3:

    # boss "My name?"
    boss "Il mio nome?"

# game/events_mc.rpy:1168
translate italian officeFirst_86a016ab:

    # pc "Yeah, it's always just C.J. in every paper I've seen so far."
    pc "Sì, è sempre solo il C.J. in ogni carta che ho visto finora."

# game/events_mc.rpy:1169
translate italian officeFirst_e52b1fe5:

    # boss "Colin-James Olafsson is the name, and yes, I have some Viking ancestry. One of them even sailed with the children of Eirik the Red."
    boss "Colin-James Olafsson è il nome, e sì, ho qualche ascendenza vichinga. Uno di loro ha anche navigato con i figli di Eirik il Rosso."

# game/events_mc.rpy:1170
translate italian officeFirst_30438830:

    # pc "Wow, that's interesting!"
    pc "Wow, è interessante!"

# game/events_mc.rpy:1171
translate italian officeFirst_11a7017c:

    # boss "Anyway, I won't keep you any longer. We both have a lot of work to do. See ya next week."
    boss "Comunque, non ti terrò più. Abbiamo entrambi molto lavoro da fare. Vedi ya la settimana prossima."

# game/events_mc.rpy:1172
translate italian officeFirst_cb70ec2a:

    # pc "Yeah, goodbye."
    pc "Sì, arrivederci."

# game/events_mc.rpy:1174
translate italian officeFirst_3bde9f79:

    # n "On your way back home."
    n "Tornando a casa."

# game/events_mc.rpy:1175
translate italian officeFirst_a1967dee:

    # *Ding*"
    "*Ding*"

# game/events_mc.rpy:1177
translate italian officeFirst_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh?"

# game/events_mc.rpy:1178
translate italian officeFirst_88dffbc9:

    # pcthink "A message from [e]? HA! I knew it!"
    pcthink "Un messaggio da [e]? HA! Lo sapevo!"

# game/events_mc.rpy:1180
translate italian officeFirst_62ddcc97:

    # pcthink "Heh... cute dress..."
    pcthink "Heh... vestito carino..."

# game/events_mc.rpy:1181
translate italian officeFirst_e81b5770:

    # pcthink "Is that her tongue sticking out?"
    pcthink "La sua lingua sta attaccando?"

# game/events_mc.rpy:1183
translate italian officeFirst_195d3dfd:

    # pcthink "Yep, it is... damn she's so cute."
    pcthink "Sì, è... maledetta è così carina."

# game/events_mc.rpy:1187
translate italian officeFirst_53981479:

    # Cop" "Ouuch!"
    "Cop" "Ouuch!"

# game/events_mc.rpy:1188
translate italian officeFirst_26b08e4a:

    # pc "Fuck!"
    pc "Fuck!"

# game/events_mc.rpy:1191
translate italian officeFirst_228fb161:

    # Cop" "YOU!"
    "Cop" "YOU!"

# game/events_mc.rpy:1192
translate italian officeFirst_95710a31:

    # pc "Oh, no..."
    pc "Oh, no..."

# game/events_mc.rpy:1193
translate italian officeFirst_17f6ad68:

    # pcthink "Not this guy again..."
    pcthink "Non è più questo ragazzo..."

# game/events_mc.rpy:1195
translate italian officeFirst_4acfd17a:

    # Cop" "So-so, not watching where we're going, are we?"
    "Cop" "So-so, non guardare dove stiamo andando, siamo?"

# game/events_mc.rpy:1196
translate italian officeFirst_25d64821:

    # pc "I doubt that {b}you{/b} were looking, idiot..."
    pc "Dubito che {b}tu {/b} stavi guardando, idiota..."

# game/events_mc.rpy:1197
translate italian officeFirst_261aa3ea:

    # pcthink "Shit, I shouldn't have said that!"
    pcthink "Merda, non avrei dovuto dirlo!"

# game/events_mc.rpy:1199
translate italian officeFirst_9a256965:

    # Cop" "What was that? Insulting an officer? W-wait... you ran into me, that was obviously an attack! Ha! Now I got you!"
    "Cop" "Cos'era quello? Isolare un ufficiale? W-wait... ti sei imbattuto in me, che era ovviamente un attacco! Ciao! Ora ti ho preso!"

# game/events_mc.rpy:1200
translate italian officeFirst_2ea675f7:

    # pc "What? That's bullshit and you know it!"
    pc "Cosa? È una stronzata e lo sapete!"

# game/events_mc.rpy:1202
translate italian officeFirst_f40d5809:

    # Cop" "Hands where I can see them, turn around, asshole!"
    "Cop" "Mani dove posso vederli, girare intorno, stroncato!"

# game/events_mc.rpy:1203
translate italian officeFirst_5fdae6d7:

    # pcthink "Fuck... what an idiot..."
    pcthink "Fuck... che idiota..."

# game/events_mc.rpy:1208
translate italian officeFirst_59a0c788:

    # pcthink "Damn... I had hoped to never see this cell again..."
    pcthink "Dannazione... Avevo sperato di non vedere più questa cella..."

# game/events_mc.rpy:1209
translate italian officeFirst_5e6ec546:

    # pcthink "Fuck that goddamn wannabe Sherlock! What the fuck is his problem anyway!"
    pcthink "Cazzo quel maledetto wannabe Sherlock! Che cazzo è il suo problema comunque!"

# game/events_mc.rpy:1211
translate italian officeFirst_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1213
translate italian officeFirst_53fca454:

    # pcthink "How many times have I seen this cell from the inside?"
    pcthink "Quante volte ho visto questa cella dall'interno?"

# game/events_mc.rpy:1214
translate italian officeFirst_2c00bbfc:

    # pcthink "Shit, I hope they let me go soon, I don't want to spend the night here..."
    pcthink "Merda, spero che mi abbiano lasciato andare presto, non voglio passare la notte qui..."

# game/events_mc.rpy:1216
translate italian officeFirst_f0cac90d_1:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1218
translate italian officeFirst_2d9183c8:

    # pcthink "Especially not in wet clothes..."
    pcthink "Soprattutto non in vestiti bagnati..."

# game/events_mc.rpy:1220
translate italian officeFirst_cb9aaae9:

    # pcthink "...what's up with her?"
    pcthink "...Cosa c'è con lei?"

# game/events_mc.rpy:1222
translate italian officeFirst_3e8305e8:

    # pcthink "Damn, I hope [m] and [e] don't find out I'm in here."
    pcthink "Dannazione, spero [m] e [e] non scoprirò che sono qui dentro."

# game/events_mc.rpy:1225
translate italian officeFirst_3307dfad:

    # Prostitute" "Hey there, handsome. I haven't seen you here before."
    "Prostitute" "Hey lì, bello. Non ti ho visto qui prima."

# game/events_mc.rpy:1227
translate italian officeFirst_0429c5f5:

    # Prostitute" "Hey there, beautiful. I haven't seen you here before."
    "Prostitute" "Ehi lì, bello. Non ti ho visto qui prima."

# game/events_mc.rpy:1229
translate italian officeFirst_9f4df712:

    # pc "Ugh... yeah, you're a few years late..."
    pc "Ugh... sì, sei in ritardo di qualche anno..."

# game/events_mc.rpy:1230
translate italian officeFirst_a4202cfa:

    # Prostitute" "Huh? What do you mean, honey?"
    "Prostitute" "Huh? Cosa intendi, miele?"

# game/events_mc.rpy:1231
translate italian officeFirst_2df6c4d3:

    # pc "...forget it."
    pc "...dimenticatelo."

# game/events_mc.rpy:1234
translate italian officeFirst_996f0cca:

    # Prostitute" "But it's never too late to have a bit fun, don't you think, sexy?"
    "Prostitute" "Ma non è mai troppo tardi per divertirsi, non pensate, sexy?"

# game/events_mc.rpy:1236
translate italian officeFirst_f9decee2:

    # pc "We are in a holding cell already, do you want to get in even more trouble?"
    pc "Siamo già in una cella di tenuta, vuoi ottenere ancora più problemi?"

# game/events_mc.rpy:1237
translate italian officeFirst_fa635850:

    # Prostitute" "No risk, no fun."
    "Prostitute" "Nessun rischio, nessun divertimento."

# game/events_mc.rpy:1239
translate italian officeFirst_82943c22:

    # Prostitute" "Come on, sexy, I'll make it worth your time!"
    "Prostitute" "Avanti, sexy, farò vale la pena il vostro tempo!"

# game/events_mc.rpy:1240
translate italian officeFirst_3151510c:

    # oc_ "Apart you two!"
    oc_ "A parte due!"

# game/events_mc.rpy:1242
translate italian officeFirst_df4ac260:

    # Prostitute" "Ughh, shit!"
    "Prostitute" "Ughh, merda!"

# game/events_mc.rpy:1243
translate italian officeFirst_bf282ccb:

    # oc_ "Sam, aren't you in enough trouble already?"
    oc_ "Sam, non hai già abbastanza problemi?"

# game/events_mc.rpy:1245
translate italian officeFirst_086a64ed:

    # Prostitute" "Fuck off, old man, you're not my dad!"
    "Prostitute" "Scopata fuori, vecchio uomo, non sei mio papà!"

# game/events_mc.rpy:1246
translate italian officeFirst_90d23aed:

    # oc_ "And you're lucky that I'm not..."
    oc_ "E sei fortunato che non sia..."

# game/events_mc.rpy:1247
translate italian officeFirst_9df8f015:

    # Prostitute" "Pfft..."
    "Prostitute" "Pfft..."

# game/events_mc.rpy:1248
translate italian officeFirst_88d25c77:

    # oc_ "Wait a minute..."
    oc_ "Aspetta un minuto..."

# game/events_mc.rpy:1249
translate italian officeFirst_e654f32d:

    # oc_ "[pc]?"
    oc_ "[pc]?"

# game/events_mc.rpy:1251
translate italian officeFirst_225fadf5_1:

    # pc "Huh?"
    pc "Huh?"

# game/events_mc.rpy:1252
translate italian officeFirst_9d393639:

    # pcthink "Is that... Johnson?!"
    pcthink "È questo... Johnson?!"

# game/events_mc.rpy:1254
translate italian officeFirst_24da5bb9:

    # oc "What the hell are {b}you{/b} doing here? I thought I'd never see you again?!"
    oc "Che diavolo hai {b}che stai facendo qui? Ho pensato che non ti avrei mai rivisto?!"

# game/events_mc.rpy:1255
translate italian officeFirst_cbbfc35c:

    # pc "Yeah, likewise..."
    pc "Sì, allo stesso modo..."

# game/events_mc.rpy:1256
translate italian officeFirst_423514d7:

    # oc "Come on, you know what I mean. When did you come back?"
    oc "Avanti, sai cosa voglio dire. Quando sei tornato?"

# game/events_mc.rpy:1257
translate italian officeFirst_3d6db029:

    # pc "About a week ago..."
    pc "Circa una settimana fa..."

# game/events_mc.rpy:1258
translate italian officeFirst_60faf072:

    # oc "A week?!"
    oc "Una settimana?!"

# game/events_mc.rpy:1259
translate italian officeFirst_caaed063:

    # oc "A fuckin' week and you're in a cell already?"
    oc "Una settimana di fuckin e sei già in una cella?"

# game/events_mc.rpy:1260
translate italian officeFirst_55b427e9:

    # pc "Hey, it's not my fault this time. I've changed a lot since back then."
    pc "Ehi, questa volta non è colpa mia. Ho cambiato molto da allora."

# game/events_mc.rpy:1261
translate italian officeFirst_602ec11f:

    # pcthink "Wow, that sounds like [m]..."
    pcthink "Wow, che suona come [m]..."

# game/events_mc.rpy:1262
translate italian officeFirst_cd85b673:

    # oc "...and you think I believe that? Seriously, you should know better, [pc]."
    oc "...e pensate che io credo? Seriamente, dovreste sapere meglio, [pc]."

# game/events_mc.rpy:1263
translate italian officeFirst_272dc344:

    # pc "It's true. There is this idiot... shit, I mean a co-worker of yours who is on me constantly, for whatever weird reason..."
    pc "È vero. C'è questo idiota... merda, intendo un tuo collaboratore che è su di me costantemente, per qualsiasi strano motivo..."

# game/events_mc.rpy:1264
translate italian officeFirst_63b4d994:

    # oc "Who?"
    oc "Chi?"

# game/events_mc.rpy:1265
translate italian officeFirst_c657bee8:

    # pc "I don't know his name. He wasn't here back then... He's quite short, clumsy, {i}out of shape{/i}... and he thinks he's the new white \"Shaft\" or something..."
    pc "Non conosco il suo nome. Non era qui allora. È abbastanza breve, goffo, {i}fuori forma{/i}... e pensa di essere il nuovo \"Alto\" bianco o qualcosa..."

# game/events_mc.rpy:1266
translate italian officeFirst_fde66fd5:

    # oc "...[w]."
    oc "...[w]."

# game/events_mc.rpy:1267
translate italian officeFirst_e71f2f2e:

    # oc "So that's why he wanted to have access to the old files."
    oc "Quindi è per questo che voleva avere accesso ai vecchi file."

# game/events_mc.rpy:1268
translate italian officeFirst_5ebb18b5:

    # pc "Seriously, what's his problem?"
    pc "Seriamente, qual è il suo problema?"

# game/events_mc.rpy:1269
translate italian officeFirst_dcf7b7fd:

    # oc "..."
    oc "..."

# game/events_mc.rpy:1271
translate italian officeFirst_c2974f5c:

    # oc "Come on, let's get you outta here."
    oc "Avanti, cerchiamo di farti uscire qui."

# game/events_mc.rpy:1273
translate italian officeFirst_ca88187e:

    # You follow him to his office where he quickly fills out all the necessary paperwork."
    "Lo seguite fino al suo ufficio, dove riempie rapidamente tutti i documenti necessari."

# game/events_mc.rpy:1275
translate italian officeFirst_a3dfc68d:

    # oc "Alright, that's it, you're free to go. Make sure that's the last time I have to see you here!"
    oc "Bene, è così, sei libero di andare. Assicurati che sia l'ultima volta che devo vederti qui!"

# game/events_mc.rpy:1277
translate italian officeFirst_2af139c3:

    # pc "If it was up to me I wouldn't be here in the first place... but seriously, thanks."
    pc "Se fosse a me non sarei qui in primo luogo... ma seriamente, grazie."

# game/events_mc.rpy:1279
translate italian officeFirst_57276ab8:

    # w "Oi! What the..."
    w "Oi! Che cosa..."

# game/events_mc.rpy:1280
translate italian officeFirst_102c0d27:

    # oc "Ugh... bad timing..."
    oc "Ugh... errato tempo..."

# game/events_mc.rpy:1281
translate italian officeFirst_b568e141:

    # w "Johnson, what's going on here? What are you doing with my prisoner?"
    w "Johnson, cosa sta succedendo qui? Cosa stai facendo con il mio prigioniero?"

# game/events_mc.rpy:1283
translate italian officeFirst_9c60277f:

    # oc "First of all, [heshe] is not {b}your{/b} prisoner..."
    oc "Prima di tutto, [heshe!t] non è {b}il tuo prigioniero {/b}..."

# game/events_mc.rpy:1284
translate italian officeFirst_91fb484b:

    # oc "Secondly, you know as much as I do, that your accusations won't stand a chance in court. You should be happy that I'm doing the paperwork."
    oc "In secondo luogo, sapete quanto me, che le vostre accuse non avranno alcuna possibilità in tribunale. Dovreste essere felici che sto facendo i documenti."

# game/events_mc.rpy:1286
translate italian officeFirst_d4377c33:

    # w "T-that's... that's harassment! Y-you only want to bully me!"
    w "T-that's... è molestia! Y-you only want to bully me!"

# game/events_mc.rpy:1287
translate italian officeFirst_d581dd55:

    # oc "Sure, [w]... why don't you go back on the streets and watch the traffic..."
    oc "Certo, [w]... perché non tornate per le strade e guardate il traffico..."

# game/events_mc.rpy:1288
translate italian officeFirst_c97e1600:

    # w "{b}You{/b}... I know that you're in cahoots over something... That one closed case, it's obvious that you're hiding something and I'm going to prove it!"
    w "{b}Tu{/b}... So che sei in cahoots su qualcosa... Quel caso chiuso, è ovvio che stai nascondendo qualcosa e lo dimostrerò!"

# game/events_mc.rpy:1289
translate italian officeFirst_5ee56c6d:

    # w "Say goodbye to your pension, Johnson!"
    w "Dite addio alla vostra pensione, Johnson!"

# game/events_mc.rpy:1291
translate italian officeFirst_e61485cd:

    # oc "Jeez..."
    oc "Jeez..."

# game/events_mc.rpy:1292
translate italian officeFirst_c46df379:

    # pc "That doesn't sound good... this guy has some serious issues..."
    pc "Che non sembra buono... questo ragazzo ha alcuni problemi seri..."

# game/events_mc.rpy:1294
translate italian officeFirst_57259f13:

    # oc "Don't worry about him, nobody here takes him seriously, and the case was closed a long time ago."
    oc "Non preoccuparti di lui, nessuno qui lo prende sul serio, e il caso è stato chiuso molto tempo fa."

# game/events_mc.rpy:1295
translate italian officeFirst_3b866ef1:

    # pc "I don't want you to get into trouble because of me."
    pc "Non voglio che tu ti guai a causa di me."

# game/events_mc.rpy:1296
translate italian officeFirst_a68577fe:

    # oc "Better worry about yourself. Make sure that I don't have to see you again before my retirement."
    oc "Meglio preoccuparsi di te stesso. Assicurati di non doverti rivederti ancora prima della mia pensione."

# game/events_mc.rpy:1297
translate italian officeFirst_b121344f:

    # pc "Heh, can't be long, old man."
    pc "Heh, non può essere lungo, anziano."

# game/events_mc.rpy:1298
translate italian officeFirst_dc1eedda:

    # oc "Ha...{w=1} Ha...{w=1} Now move ya ass outta here!"
    oc "Ha...{w=1} Ha...{w=1} Ora muovi ya ass outta qui!"

# game/events_mc.rpy:1299
translate italian officeFirst_59114b30:

    # pc "I'm gone already... thanks, Johnson..."
    pc "Sono già andato... grazie, Johnson..."

# game/events_mc.rpy:1301
translate italian officeFirst_647852c1:

    # n "Your way back home is quite unspectacular, but you can't help but to think about how many times [oc] saved your ass when you were younger..."
    n "Il tuo ritorno a casa è piuttosto inspettacolare, ma non puoi fare a meno di pensare a quante volte [oc] hai salvato il culo quando eri più giovane..."

# game/events_mc.rpy:1302
translate italian officeFirst_34b397f1:

    # n "A bit later."
    n "Un po' più tardi."

# game/events_mc.rpy:1308
translate italian dinnerDay4Home_235607d8:

    # m "So, how was your day, Hun? You've been away longer than I thought you would be."
    m "Allora, com'era il tuo giorno, Hun? Sei stato via più a lungo di quanto pensassi."

# game/events_mc.rpy:1309
translate italian dinnerDay4Home_b8ee6247:

    # pc "Uh, yeah nothing special... I met my new boss and we had a long conversation about work..."
    pc "Uh, sì, niente di speciale... Ho incontrato il mio nuovo capo e abbiamo avuto una lunga conversazione sul lavoro..."

# game/events_mc.rpy:1311
translate italian dinnerDay4Home_3697c268:

    # m "Oh really? He was there on a Sunday?"
    m "Oh veramente? Era lì la domenica?"

# game/events_mc.rpy:1312
translate italian dinnerDay4Home_85b8611b:

    # pc "Yeah, I was surprised as well."
    pc "Sì, anch'io ero sorpreso."

# game/events_mc.rpy:1313
translate italian dinnerDay4Home_9b505c74:

    # m "Now I understand why you took so long."
    m "Ora capisco perché hai impiegato così tanto tempo."

# game/events_mc.rpy:1314
translate italian dinnerDay4Home_a6b055a9:

    # pcthink "Why would she be interested in how long I'm away? I didn't even mention how long I'd be gone..."
    pcthink "Perché sarebbe interessata a quanto tempo sono lontano? Non ho nemmeno accennato per quanto tempo avrei finito..."

# game/events_mc.rpy:1316
translate italian dinnerDay4Home_c819dd93:

    # m "Oh but, he doesn't expect you to work on weekends, too, does he?"
    m "Oh ma, lui non si aspetta che tu lavori anche nei fine settimana, vero?"

# game/events_mc.rpy:1317
translate italian dinnerDay4Home_8ffc4c76:

    # pc "No... as I said, I'm only expected to be there a few days per week."
    pc "No... come ho detto, mi aspettavo solo qualche giorno alla settimana."

# game/events_mc.rpy:1318
translate italian dinnerDay4Home_afe32509:

    # pc "In fact, since we settled everything today, I don't even need to go there next week, but will start working from here tomorrow."
    pc "Infatti, dal momento che oggi abbiamo risolto tutto, non ho nemmeno bisogno di andare lì la prossima settimana, ma inizierà a lavorare da qui domani."

# game/events_mc.rpy:1320
translate italian dinnerDay4Home_63078123:

    # m "Oh, that's nice. But don't forget to take enough breaks in between."
    m "Oh, è bello. Ma non dimenticate di fare abbastanza pause nel mezzo."

# game/events_mc.rpy:1321
translate italian dinnerDay4Home_7961594a:

    # pc "Yeah, sure, I'm not a workaholic, [m]..."
    pc "Sì, certo, non sono uno stacanovista, [m]..."

# game/events_mc.rpy:1323
translate italian dinnerDay4Home_d408d503:

    # m "That's good to hear."
    m "È bello sentire."

# game/events_mc.rpy:1324
translate italian dinnerDay4Home_cb688308:

    # pcthink "Jeez, is she just acting or does she really want me to stay home..."
    pcthink "Jeez, è lei solo recitare o lei vuole davvero che io rimanga a casa..."

# game/events_mc.rpy:1326
translate italian dinnerDay4Home_d9a23300:

    # pcthink "...and why would she want me to be here?"
    pcthink "...e perché vorrebbe che fossi qui?"

# game/events_mc.rpy:1328
translate italian dinnerDay4Home_72a33094:

    # e "..."
    e "..."

# game/events_mc.rpy:1329
translate italian dinnerDay4Home_f423bdd0:

    # pcthink "Cute..."
    pcthink "Taglia..."

# game/events_mc.rpy:1334
translate italian homeday4night_8cf29fc6:

    # n "At night..."
    n "Di notte..."

# game/events_mc.rpy:1336
translate italian homeday4night_88d5a7a4:

    # pcthink "Shit, maybe I should have used the opportunity to ask [m] some more questions when she was under the influence of the pills..."
    pcthink "Merda, forse avrei dovuto approfittare dell'opportunità di fare [m] altre domande quando lei era sotto l'influenza delle pillole..."

# game/events_mc.rpy:1337
translate italian homeday4night_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1338
translate italian homeday4night_815350c3:

    # pcthink "...maybe tomorrow..."
    pcthink "...forse domani..."

# game/events_mc.rpy:1339
translate italian homeday4night_f0cac90d_1:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1340
translate italian homeday4night_9c027654:

    # pcthink "Hum... it's really late..."
    pcthink "Uomo... è davvero tardi..."

# game/events_mc.rpy:1341
translate italian homeday4night_9ffe0b3c:

    # pcthink "[e] should be here by now..."
    pcthink "[e] dovrebbe essere qui ora..."

# game/events_mc.rpy:1342
translate italian homeday4night_f0cac90d_2:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1344
translate italian homeday4night_eb300fc6:

    # pcthink "I wonder if she's embarrassed about what happened in her room earlier today..."
    pcthink "Mi chiedo se lei sia imbarazzata di ciò che è successo nella sua stanza oggi..."

# game/events_mc.rpy:1346
translate italian homeday4night_8a587bc9:

    # pcthink "Nah... I don't think so..."
    pcthink "Nah... Non credo così..."

# game/events_mc.rpy:1347
translate italian homeday4night_98679eca:

    # pcthink "She wasn't really shy about it, and she {b}did{/b} return the kiss after all..."
    pcthink "Non era davvero timida e lei {b}ha fatto {/b} restituire il bacio dopo tutto..."

# game/events_mc.rpy:1348
translate italian homeday4night_45b128ac:

    # pcthink "It's probably [m] who told her to stay in her room again, even though... I haven't heard her talking to [e]..."
    pcthink "È probabilmente [m] che le ha detto di rimanere nuovamente nella sua stanza, anche se... non l'ho sentita parlare con [e]..."

# game/events_mc.rpy:1350
translate italian homeday4night_f0cac90d_3:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1352
translate italian homeday4night_5b73bbbd:

    # pcthink "Anyway, I better get some sleep..."
    pcthink "In ogni caso, meglio dormire un po'..."

# game/events_mc.rpy:1357
translate italian homeday4night_956ae696:

    # m "[bpc]! WHAT THE HELL DID YOU DO THIS TIME?" with hpunch
    m "[bpc]! COSA HELL DIDI FARE QUESTO TEMPO?" with hpunch

# game/events_mc.rpy:1358
translate italian homeday4night_5bd3bf2f:

    # oc "Calm down, [m]. It doesn't help to yell at [himher]."
    oc "Calmati, [m]. Non aiuta a urlare a [himher!t]."

# game/events_mc.rpy:1359
translate italian homeday4night_8aee846a:

    # m "WHAT DO YOU KNOW? YOU DON'T HAVE TO LIVE WITH THIS..."
    m "COSA CONOSCIUTE? NON HAI VIVERE CON QUESTE..."

# game/events_mc.rpy:1360
translate italian homeday4night_97824f31:

    # oc "CALM DOWN!"
    oc "CALM DOWN!"

# game/events_mc.rpy:1361
translate italian homeday4night_42a34ab2:

    # m "Humph... fine..."
    m "Humph... fine..."

# game/events_mc.rpy:1362
translate italian homeday4night_7feca2c9:

    # m "So what did [heshe] do this time?"
    m "Che cosa ha fatto [heshe!t] questa volta?"

# game/events_mc.rpy:1363
translate italian homeday4night_3c2af74d:

    # oc "[m], you're not making this any better! Sit down and think about who you're talking to!"
    oc "[m], non sei in grado di migliorarlo! Siediti e pensi a chi stai parlando!"

# game/events_mc.rpy:1364
translate italian homeday4night_e612037c:

    # m "..."
    m "..."

# game/events_mc.rpy:1365
translate italian homeday4night_99f84bee:

    # m "I'm sorry, [oc]... [heshe]'s just..."
    m "Mi dispiace, [oc]... [heshe!t]è solo..."

# game/events_mc.rpy:1366
translate italian homeday4night_149911e6:

    # m "I just can't stand [himher] any longer..."
    m "Non posso più sopportare [himher!t]..."

# game/events_mc.rpy:1375
translate italian d5morning_03946767:

    # pc "Yaaawn..."
    pc "Yaaawn..."

# game/events_mc.rpy:1376
translate italian d5morning_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1377
translate italian d5morning_ebe882ba:

    # pcthink "Damn dreams..."
    pcthink "Dannazione sogni..."

# game/events_mc.rpy:1378
translate italian d5morning_8f182d39:

    # pcthink "...I think that day was the last time I got into a fight with those two assholes from school again..."
    pcthink "...Penso che quella giornata sia stata l'ultima volta che sono entrato in una lotta con quei due stronzi da scuola di nuovo..."

# game/events_mc.rpy:1379
translate italian d5morning_2179c941:

    # pcthink "I broke the nose of one of them and hit the other one with a cane."
    pcthink "Ho rotto il naso di uno di loro e ho colpito l'altro con una canna."

# game/events_mc.rpy:1380
translate italian d5morning_117c625f:

    # pcthink "...I can't say I regret it, but it was really dumb..."
    pcthink "... non posso dire che me ne rammarico, ma è stato davvero stupido..."

# game/events_mc.rpy:1381
translate italian d5morning_247fdc1b:

    # pcthink "Without [oc] I would've ended up in jail..."
    pcthink "Senza [oc] avrei finito in prigione..."

# game/events_mc.rpy:1383
translate italian d5morning_b0477cd7:

    # pcthink "I guess I wasn't the nicest guy around that time..."
    pcthink "Immagino di non essere stato il ragazzo più bello intorno a quel tempo..."

# game/events_mc.rpy:1385
translate italian d5morning_6ddfdaa0:

    # pcthink "I guess I wasn't the nicest girl around that time..."
    pcthink "Immagino di non essere la ragazza più bella intorno a quel tempo..."

# game/events_mc.rpy:1386
translate italian d5morning_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1388
translate italian d5morning_9f541c54:

    # pcthink "[e] didn't come over..."
    pcthink "[e] non è finito..."

# game/events_mc.rpy:1389
translate italian d5morning_c1686997:

    # pcthink "Hum..."
    pcthink "Hum..."

# game/events_mc.rpy:1391
translate italian d5morning_453b4052:

    # n "You get up and make yourself ready for breakfast."
    n "Ti alzi e ti prepari per la colazione."

# game/events_mc.rpy:1393
translate italian d5morning_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1394
translate italian d5morning_e4a69cb1:

    # pcthink "[e] didn't look at me once yet..."
    pcthink "[e] non mi ha ancora guardato una volta..."

# game/events_mc.rpy:1395
translate italian d5morning_72a33094:

    # e "..."
    e "..."

# game/events_mc.rpy:1396
translate italian d5morning_367e6ba5:

    # pc "Is everything alright, [e]?"
    pc "Va tutto bene, [e]?"

# game/events_mc.rpy:1399
translate italian d5morning_52d6b73f:

    # e "Hn..."
    e "Hn..."

# game/events_mc.rpy:1404
translate italian d5morning_744f56ce:

    # n "A bit later..."
    n "Un po' più tardi..."

# game/events_mc.rpy:1406
translate italian d5morning_4a9406ec:

    # m "We are leaving, I've made you some extra food for the day, it's in the fridge."
    m "Stiamo partendo, ti ho fatto un po' di cibo in più per la giornata, è in frigo."

# game/events_mc.rpy:1407
translate italian d5morning_24aeb086:

    # pc "Thanks, [m]."
    pc "Grazie, [m]."

# game/events_mc.rpy:1408
translate italian d5morning_952a7e5d:

    # m "If you need something else, whatever it might be, don't hesitate and give me a call, okay, Hun?"
    m "Se hai bisogno di qualcos'altro, qualunque cosa sia, non esitare e darmi una chiamata, ok, Hun?"

# game/events_mc.rpy:1409
translate italian d5morning_61f77a4c:

    # pc "Sure."
    pc "Certo."

# game/events_mc.rpy:1411
translate italian d5morning_ec1a6290:

    # pcthink "Why is she so nice to me today? Huh? ...what's up with [e]?!"
    pcthink "Perché è così gentile con me oggi? Huh? ...Cosa c'è con [e]?!"

# game/events_mc.rpy:1413
translate italian d5morning_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh?"

# game/events_mc.rpy:1415
translate italian d5morning_06a6d364:

    # pc "Whoa!"
    pc "Choa!"

# game/events_mc.rpy:1418
translate italian d5morning_58feae78:

    # pcthink "Wow, [e]..."
    pcthink "Wow, [e]..."

# game/events_mc.rpy:1420
translate italian d5morning_72a33094_1:

    # e "..."
    e "..."

# game/events_mc.rpy:1421
translate italian d5morning_4cd516c4:

    # pc "Heh... you should probably go, or you'll be late... We can talk later, okay?"
    pc "Heh... probabilmente dovresti andare, o sarai in ritardo... Possiamo parlare più tardi, okay?"

# game/events_mc.rpy:1422
translate italian d5morning_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:1424
translate italian d5morning_5ab369aa:

    # pcthink "Wow, that came out of nowhere..."
    pcthink "Wow, che è uscito dal nulla..."

# game/events_mc.rpy:1426
translate italian d5morning_20ad7ae7:

    # pcthink "Err... Looks like I'm not the only one who's surprised."
    pcthink "Err... Sembra che io non sia l'unico che sia sorpreso."

# game/events_mc.rpy:1427
translate italian d5morning_e612037c:

    # m "..."
    m "..."

# game/events_mc.rpy:1429
translate italian d5morning_4f0f31e7:

    # pcthink "Phew... [m] didn't say a word, but that look on her face... I wonder what she was thinking."
    pcthink "Phew... [m] non ha detto una parola, ma quel guardare sul suo viso... Mi chiedo cosa stava pensando."

# game/events_mc.rpy:1435
translate italian d5morning_f5618bbd:

    # pcthink "Anyway, I should probably start working on the project for [boss], I have a lot to do and I don't have that much time..."
    pcthink "In ogni caso, probabilmente dovrei iniziare a lavorare sul progetto per [boss], Ho molto da fare e non ho così tanto tempo..."

# game/events_mc.rpy:1437
translate italian d5morning_a76f8e5c:

    # n "You spend the rest of the morning and afternoon working on the project, only taking short breaks to go to the bathroom or get some snacks from the kitchen."
    n "Trascorrete il resto del mattino e del pomeriggio a lavorare al progetto, prendere solo brevi pause per andare in bagno o prendere alcuni spuntini dalla cucina."

# game/events_mc.rpy:1439
translate italian d5morning_8f21880a:

    # pcthink "Ugh, this won't work either..."
    pcthink "Ugh, nemmeno questo funzionerà..."

# game/events_mc.rpy:1440
translate italian d5morning_43080636:

    # pcthink "Damn, where is the error?"
    pcthink "Dannazione, dov'è l'errore?"

# game/events_mc.rpy:1441
translate italian d5morning_2ab2f07a:

    # pcthink "...gaaawd, I need a break..."
    pcthink "...gaaawd, ho bisogno di una pausa..."

# game/events_mc.rpy:1443
translate italian d5morning_4dc0d086:

    # pcthink "I think I've heard [e] and [m] coming back a while ago and it sounded like there was someone else as well..."
    pcthink "Penso di aver sentito [e] e [m] tornare un po 'fa e sembrava che c'era anche qualcun altro..."

# game/events_mc.rpy:1445
translate italian d5morning_c9c7cbd9:

    # pcthink "Maybe I should just stop for today..."
    pcthink "Forse dovrei solo fermarmi per oggi..."

# game/events_mc.rpy:1449
translate italian porn_vr_4_f834cc31:

    # n "A while later..."
    n "Un po' più tardi..."

# game/events_mc.rpy:1451
translate italian porn_vr_4_7cd59846:

    # Voice" "Mphh... mphhh..."
    "Voce" "Mphh... mphh..."

# game/events_mc.rpy:1456
translate italian porn_vr_4_594132e1:

    # pcthink "Gawd, I love these girls!"
    pcthink "Gawd, Amo queste ragazze!"

# game/events_mc.rpy:1458
translate italian porn_vr_4_010b482c:

    # pcthink "Too bad there's no girl on girl action..."
    pcthink "Peccato che non ci sia una ragazza sull'azione della ragazza..."

# game/events_mc.rpy:1460
translate italian porn_vr_4_e499de78:

    # Girl 2" "Are you about to cum babe?"
    "Ragazza 2" "Stai per cum babe?"

# game/events_mc.rpy:1461
translate italian porn_vr_4_794a11fa:

    # pcthink "Hehe, yeah spray that load all over them!"
    pcthink "Hehe, sì, spruzzare quel carico su di loro!"

# game/events_mc.rpy:1465
translate italian porn_vr_4_029af3e9:

    # pc "What!? Are you fucking serious? That's even worse than in porn games..."
    pc "Cosa!? Stai scopando seriamente? È anche peggio che nei giochi porno..."

# game/events_mc.rpy:1467
translate italian porn_vr_4_90a099af:

    # pcthink "Fuck..."
    pcthink "Fuck..."

# game/events_mc.rpy:1468
translate italian porn_vr_4_a14752ab:

    # pcthink "...that was... unsatisfying..."
    pcthink "...che era... insoddisfacente..."

# game/events_mc.rpy:1470
translate italian porn_vr_4_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1471
translate italian porn_vr_4_4bcfd45d:

    # pcthink "Hum... shit, isn't it dinner time already?"
    pcthink "Uomo... merda, non è già ora di cena?"

# game/events_mc.rpy:1472
translate italian porn_vr_4_bd478faf:

    # pcthink "I guess I'll have a look..."
    pcthink "Immagino che avrò un'occhiata..."

# game/events_mc.rpy:1474
translate italian porn_vr_4_91617c2e:

    # pcthink "It's funny how quick I got used to [m] making dinner every evening... or breakfast..."
    pcthink "E 'divertente quanto veloce mi sono abituato a [m] fare cena ogni sera... o colazione..."

# game/events_mc.rpy:1477
translate italian porn_vr_4_1fd0abf0:

    # pcthink "Huh? Isn't [m] making dinner today?"
    pcthink "Huh? Non sta [m] cenando oggi?"

# game/events_mc.rpy:1479
translate italian porn_vr_4_43ebd0bd:

    # pcthink "[e]?"
    pcthink "[e]?"

# game/events_mc.rpy:1484
translate italian d6morning_180d7c36:

    # pc "YAAAAWN..."
    pc "YAAAAWN..."

# game/events_mc.rpy:1487
translate italian d6morning_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1488
translate italian d6morning_ca66ede7:

    # pcthink "[e] is gone again..."
    pcthink "[e] è andato di nuovo..."

# game/events_mc.rpy:1490
translate italian d6morning_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1492
translate italian d6morning_2edac4fd:

    # pc "Wait!"
    pc "Aspetta!"

# game/events_mc.rpy:1493
translate italian d6morning_04f5c868:

    # pcthink "Did that really happen last night?"
    pcthink "È successo davvero ieri sera?"

# game/events_mc.rpy:1494
translate italian d6morning_71a5f304:

    # pcthink "...did we really...?"
    pcthink "... abbiamo davvero...?"

# game/events_mc.rpy:1495
translate italian d6morning_a69377ef_2:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1496
translate italian d6morning_4cc63f49:

    # pcthink "T-that's impossible, she's my little sister..."
    pcthink "T-questo è impossibile, lei è la mia sorellina..."

# game/events_mc.rpy:1497
translate italian d6morning_b4c72b6d:

    # pcthink "Err, I mean, she's not, but still..."
    pcthink "Err, voglio dire, non è, ma ancora..."

# game/events_mc.rpy:1499
translate italian d6morning_2eece04e:

    # pcthink "...and wouldn't she be here if we..."
    pcthink "...e non sarebbe qui se noi..."

# game/events_mc.rpy:1500
translate italian d6morning_d022dadb:

    # pcthink "Wait, what time is it?"
    pcthink "Aspetta, che ora è?"

# game/events_mc.rpy:1503
translate italian d6morning_e072fbd9:

    # pcthink "Ugh, shit, I overslept... dammit!"
    pcthink "Ugh, merda, ho dormito troppo... dammit!"

# game/events_mc.rpy:1504
translate italian d6morning_0e85c611:

    # pcthink "I better get up, I still have a lot of work to do..."
    pcthink "Meglio alzarmi, ho ancora molto lavoro da fare..."

# game/events_mc.rpy:1506
translate italian d6morning_690b379a:

    # n "You get up, make yourself ready and start working on the project again for a while."
    n "Si alza, si prepara e inizia a lavorare di nuovo sul progetto per un po'."

# game/events_mc.rpy:1508
translate italian d6morning_9ef2adad:

    # pcthink "There we go... this should work nicely..."
    pcthink "Ci andiamo... questo dovrebbe funzionare piacevolmente..."

# game/events_mc.rpy:1509
translate italian d6morning_263d2d8e:

    # pc "Ugh...yaaawn..."
    pc "Ugh...yaaawn..."

# game/events_mc.rpy:1510
translate italian d6morning_4cdbbc35:

    # pcthink "I could use a break..."
    pcthink "Potrei usare una pausa..."

# game/events_mc.rpy:1511
translate italian d6morning_8581051f:

    # pcthink "Maybe I should take a walk... get some fresh air... get a coffee..."
    pcthink "Forse dovrei fare una passeggiata... prendere un po' di aria fresca... prendere un caffè..."

# game/events_mc.rpy:1513
translate italian d6morning_56f01959:

    # pcthink "...coffee... I could visit the girl... [h] at the cafe."
    pcthink "...caffè... Potrei visitare la ragazza... [h] al caffè."

# game/events_mc.rpy:1515
translate italian d6morning_7bc8bca3:

    # pcthink "Oh right, she gave me her number, I totally forgot about that!"
    pcthink "Oh giusto, mi ha dato il suo numero, mi sono completamente dimenticato!"

# game/events_mc.rpy:1517
translate italian d6morning_88320126:

    # pcthink "Hum... should I call her?"
    pcthink "Uomo... la dovrei chiamare?"

# game/events_mc.rpy:1522
translate italian d6morning_be1e64d5:

    # pcthink "Yeah, she's cute... and..."
    pcthink "Sì, è carina... e... e..."

# game/events_mc.rpy:1523
translate italian d6morning_fa2a0c40:

    # pcthink "... she seems to be the only normal girl I've met since I'm back home..."
    pcthink "... sembra essere l'unica ragazza normale che ho incontrato da quando sono tornato a casa..."

# game/events_mc.rpy:1525
translate italian d6morning_19b50a19:

    # pcthink "I'm going to call her, let's see where this is going."
    pcthink "La chiamerò, vediamo dove sta andando."

# game/events_mc.rpy:1527
translate italian d6morning_a69377ef_3:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1529
translate italian d6morning_227eb657:

    # h "Yes?"
    h "Sì?"

# game/events_mc.rpy:1530
translate italian d6morning_be61a613:

    # pc "Hey, it's me, [pc], the one from the..."
    pc "Ehi, sono io, [pc], quello del..."

# game/events_mc.rpy:1531
translate italian d6morning_7e9d795c:

    # h "Oh, hey, [pc]! Hi! How are you?! How have you been?"
    h "Oh, hey, [pc]! Ciao! Come stai?! Come sei stato?"

# game/events_mc.rpy:1532
translate italian d6morning_f67a852d:

    # pc "Err, I'm good, how are you?"
    pc "Err, sono bravo, come stai?"

# game/events_mc.rpy:1533
translate italian d6morning_7fa6ffc2:

    # h "Good, I'm really good, I've been waiting for... for you to show up at the cafe again, haha."
    h "Bene, sono davvero bravo, ho aspettato... che tu ti presenti di nuovo al caffè, haha."

# game/events_mc.rpy:1534
translate italian d6morning_02b345a1:

    # pc "Well I was about to go for a walk and get a coffee, are you there?"
    pc "Beh, stavo per andare a fare una passeggiata e prendere un caffè, sei lì?"

# game/events_mc.rpy:1535
translate italian d6morning_63ebac69:

    # h "Yes! Of course! Oh you mean at the cafe, right? Haha."
    h "Sì! Certo! Oh intendi al caffè, giusto? Haha."

# game/events_mc.rpy:1536
translate italian d6morning_42feb7d0:

    # pc "Yeah. Aren't you working today?"
    pc "Sì. Non stai lavorando oggi?"

# game/events_mc.rpy:1537
translate italian d6morning_7746410b:

    # h "I-I am, haha. Of course, I'll be there, just come over whenever you want!"
    h "Io sono, haha. Certo, sarò lì, appena arrivato ogni volta che vuoi!"

# game/events_mc.rpy:1538
translate italian d6morning_812a8bad:

    # pc "Whenever I want?"
    pc "Ogni volta che voglio?"

# game/events_mc.rpy:1539
translate italian d6morning_30ed2430:

    # h "Yes, sure! Ehm... I mean, during working hours of course, haha... I mean... yes working hours... haha."
    h "Sì, sicuro! Ehm... Voglio dire, durante le ore di lavoro naturalmente, haha... Voglio dire... sì orario di lavoro... haha."

# game/events_mc.rpy:1540
translate italian d6morning_37bc2c3c:

    # pc "Heh, alright, I'll be there soon, reserve a table for me."
    pc "Heh, bene, sarò lì presto, riservarmi un tavolo per me."

# game/events_mc.rpy:1542
translate italian d6morning_d3c3bf69:

    # h "Of course, sir! Haha."
    h "Certo, signore! Haha."

# game/events_mc.rpy:1544
translate italian d6morning_bad8f7ec:

    # h "Of course, ma'am! Haha."
    h "Certo, ma'am! Haha."

# game/events_mc.rpy:1545
translate italian d6morning_7683043e:

    # pc "Haha, later, [h]."
    pc "Haha, più tardi, [h]."

# game/events_mc.rpy:1546
translate italian d6morning_c047c425:

    # h "Later, [pc]."
    h "Più tardi, [pc]."

# game/events_mc.rpy:1548
translate italian d6morning_5329c3c2:

    # n "You wait for a second before hanging up, and you're sure that she didn't hang up before you."
    n "Si aspetta un secondo prima di appendersi, e siete sicuri che lei non si è agganciata davanti a voi."

# game/events_mc.rpy:1550
translate italian d6morning_b4ec99ba:

    # pcthink "Damn, she's cute."
    pcthink "Dannazione, è carina."

# game/events_mc.rpy:1553
translate italian d6morning_9cbcc245:

    # pcthink "Nah, I just want to get a coffee..."
    pcthink "Nah, voglio solo prendere un caffè..."

# game/events_mc.rpy:1554
translate italian d6morning_731706d8:

    # pcthink "Alright, let's go!"
    pcthink "Va bene, andiamo!"

# game/events_mc.rpy:1563
translate italian d6_backhome_87c6d927:

    # pcthink "Alright..."
    pcthink "Bene..."

# game/events_mc.rpy:1565
translate italian d6_backhome_d7e55587:

    # pcthink "Time to get back to work... well, at least for a bit, we don't want to overdo it."
    pcthink "È tempo di tornare al lavoro... beh, almeno per un po', non vogliamo esagerare."

# game/events_mc.rpy:1568
translate italian d6_backhome_33b1335e:

    # pcthink "Huh? Why is my computer on?"
    pcthink "Huh? Perché il mio computer è acceso?"

# game/events_mc.rpy:1569
translate italian d6_backhome_1e9727dd:

    # pcthink "Didn't I shut it down?"
    pcthink "Non l'ho chiuso?"

# game/events_mc.rpy:1571
translate italian d6_backhome_ca581621:

    # pcthink "What the..."
    pcthink "Che cosa..."

# game/events_mc.rpy:1572
translate italian d6_backhome_0e861919:

    # pcthink "Did I...?"
    pcthink "Ho fatto...?"

# game/events_mc.rpy:1573
translate italian d6_backhome_128634ca:

    # pcthink "No I didn't search for this!"
    pcthink "No Non ho cercato questo!"

# game/events_mc.rpy:1574
translate italian d6_backhome_d4c69f41:

    # pcthink "Did [e] use my computer again?!"
    pcthink "Hai [e] di nuovo usato il mio computer?!"

# game/events_mc.rpy:1575
translate italian d6_backhome_ba2f9118:

    # pcthink "Incest, huh?"
    pcthink "Incesto, eh?"

# game/events_mc.rpy:1576
translate italian d6_backhome_81c2eddc:

    # pcthink "Interesting..."
    pcthink "Interessante..."

# game/events_mc.rpy:1577
translate italian d6_backhome_2624f60c:

    # pcthink "A little bit late now though..."
    pcthink "Ancora un po' tardi adesso..."

# game/events_mc.rpy:1578
translate italian d6_backhome_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1594
translate italian d7_wakeup_8bf6dc88:

    # n "*Knock* *Knock*"
    n "*Knock* *Knock*"

# game/events_mc.rpy:1597
translate italian d7_wakeup_f151517c:

    # pc "Uh..."
    pc "Uh..."

# game/events_mc.rpy:1599
translate italian d7_wakeup_910fd392:

    # j "Unn..."
    j "Unn..."

# game/events_mc.rpy:1601
translate italian d7_wakeup_6376492b:

    # m "{size=-10}[e], are you there? Come on, we need to get going soon!{/size}"
    m "{size=-10}[e], sei lì? Dai, abbiamo bisogno di andare presto!{/size}"

# game/events_mc.rpy:1602
translate italian d7_wakeup_8d4b0c8b:

    # j "Oh shit!"
    j "Oh merda!"

# game/events_mc.rpy:1604
translate italian d7_wakeup_dabeb1e2:

    # n "*Knock* *Knock* *Knock*"
    n "*Knock* *Knock* *Knock*"

# game/events_mc.rpy:1606
translate italian d7_wakeup_1824a53c:

    # j "We totally overslept."
    j "Abbiamo completamente dormito."

# game/events_mc.rpy:1607
translate italian d7_wakeup_4865e636:

    # pc "Yeah, damn it!"
    pc "Sì, maledetevi!"

# game/events_mc.rpy:1608
translate italian d7_wakeup_2c2639dc:

    # m "{size=-10}[pc]?{/size}"
    m "{size=-10}[pc]?{/size}"

# game/events_mc.rpy:1610
translate italian d7_wakeup_ab857d04:

    # n " *Knock* *Knock*"
    n " *Knock* *Knock*"

# game/events_mc.rpy:1612
translate italian d7_wakeup_dc75fae1:

    # m "{size=-10}I'm coming in!{/size}"
    m "{size=-10}Sto arrivando!{/size}"

# game/events_mc.rpy:1613
translate italian d7_wakeup_62dd52c5:

    # j "Now we have a problem..."
    j "Ora abbiamo un problema..."

# game/events_mc.rpy:1614
translate italian d7_wakeup_7a9c7117:

    # e "Unn..."
    e "Unn..."

# game/events_mc.rpy:1615
translate italian d7_wakeup_225fadf5:

    # pc "Huh?"
    pc "Huh?"

# game/events_mc.rpy:1617
translate italian d7_wakeup_cb44f5d6:

    # pc "What the?! [e]?" with hpunch
    pc "Che cosa?! [e]?" with hpunch

# game/events_mc.rpy:1618
translate italian d7_wakeup_267417d2:

    # pcthink "Shit, shit, shit, this is bad! I need to do something!"
    pcthink "Merda, merda, merda, questo è male! Devo fare qualcosa!"

# game/events_mc.rpy:1628
translate italian d7_wakeup_9c8aa1a8:

    # j "WHAA!"
    j "WHAA!"

# game/events_mc.rpy:1631
translate italian d7_wakeup_611f0752:

    # m "What the!?"
    m "Che cosa!?"

# game/events_mc.rpy:1632
translate italian d7_wakeup_4ef843b7:

    # j "Auu..."
    j "Auu..."

# game/events_mc.rpy:1633
translate italian d7_wakeup_9b3d3223:

    # m "[j]?!"
    m "[j]?!"

# game/events_mc.rpy:1635
translate italian d7_wakeup_a9ccd390:

    # e "NN!"
    e "NN!"

# game/events_mc.rpy:1637
translate italian d7_wakeup_dcb808e1:

    # m "[e]?!"
    m "[e]?!"

# game/events_mc.rpy:1638
translate italian d7_wakeup_fc74700e:

    # pcthink "Oh shit, that was stupid!"
    pcthink "Oh merda, che era stupido!"

# game/events_mc.rpy:1643
translate italian d7_wakeup_c39c25e8:

    # pcthink "Right, I can't hide both of em!"
    pcthink "Destra, non riesco a nascondere entrambi em!"

# game/events_mc.rpy:1644
translate italian d7_wakeup_c55aed56:

    # e "Ehh?!"
    e "Ehh?!"

# game/events_mc.rpy:1645
translate italian d7_wakeup_1bb9fdc8:

    # pc "{size=-10}Hush, [e]!{/size}"
    pc "{size=-10}Hush, [e]!{/size}"

# game/events_mc.rpy:1646
translate italian d7_wakeup_8bee63b5:

    # j "WHAAA!"
    j "WHAAA!"

# game/events_mc.rpy:1649
translate italian d7_wakeup_4d1fb2a5:

    # pcthink "Oh shit, I forgot that [j] is naked."
    pcthink "Oh merda, ho dimenticato che [j] è nudo."

# game/events_mc.rpy:1651
translate italian d7_wakeup_9b3d3223_1:

    # m "[j]?!"
    m "[j]?!"

# game/events_mc.rpy:1652
translate italian d7_wakeup_15afcc2b:

    # j "Uhh... morning [m]... hehe..."
    j "Uhh... mattina [m]... he..."

# game/events_mc.rpy:1653
translate italian d7_wakeup_70c45725:

    # m "What on earth is going on here?"
    m "Cosa sta succedendo sulla terra qui?"

# game/events_mc.rpy:1654
translate italian d7_wakeup_9e0cbd4c:

    # pc "Err... this is not what it's... uhh..."
    pc "Err... questo non è quello che è... uhhh..."

# game/events_mc.rpy:1655
translate italian d7_wakeup_a08e5c45:

    # pcthink "Shit!"
    pcthink "Merda!"

# game/events_mc.rpy:1656
translate italian d7_wakeup_745d2ec1:

    # pc "Well it {b}{i}is{/i}{/b} what it looks like..."
    pc "Beh, {b}{i}è{/i}{/b} come..."

# game/events_mc.rpy:1657
translate italian d7_wakeup_9e1b2e8a:

    # m "Why didn't you tell me?!"
    m "Perché non me l'hai detto?!"

# game/events_mc.rpy:1658
translate italian d7_wakeup_24359880:

    # j "Sorry, that's my fault..."
    j "Scusa, è colpa mia..."

# game/events_mc.rpy:1659
translate italian d7_wakeup_38fa5992:

    # m "Uhm..."
    m "Uhm..."

# game/events_mc.rpy:1660
translate italian d7_wakeup_c630047d:

    # pcthink "Shit, I hope she didn't see [e]!"
    pcthink "Merda, spero che non abbia visto [e]!"

# game/events_mc.rpy:1661
translate italian d7_wakeup_e13f5324:

    # m "You... you haven't seen [e], have you?"
    m "Tu... non hai visto [e], vero?"

# game/events_mc.rpy:1662
translate italian d7_wakeup_3aa1a87d:

    # pcthink "Phew..."
    pcthink "Phew..."

# game/events_mc.rpy:1663
translate italian d7_wakeup_fda5f912:

    # pc "Err... no... no I haven't..."
    pc "Err... no... no ho..."

# game/events_mc.rpy:1665
translate italian d7_wakeup_76a7757e:

    # j "I think I've heard her opening her door a while ago."
    j "Penso di averla sentita aprire la sua porta un po' fa."

# game/events_mc.rpy:1667
translate italian d7_wakeup_10b9afd3:

    # m "Umm... okay..."
    m "Umm... okay..."

# game/events_mc.rpy:1668
translate italian d7_wakeup_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1669
translate italian d7_wakeup_7e3552b4:

    # j "Hehe..."
    j "Hehehe..."

# game/events_mc.rpy:1670
translate italian d7_wakeup_e7673a6d:

    # m "Does she... know about..."
    m "Lei lo sappia..."

# game/events_mc.rpy:1671
translate italian d7_wakeup_ea4be5c8:

    # m "...this?"
    m "...questo?"

# game/events_mc.rpy:1672
translate italian d7_wakeup_bdcd7a62:

    # j "Umm... yeah, I told her, hehe."
    j "Umm... sì, le ho detto, hehe."

# game/events_mc.rpy:1673
translate italian d7_wakeup_58714e34:

    # m "...okay..."
    m "...okay..."

# game/events_mc.rpy:1674
translate italian d7_wakeup_b4fa3fc0:

    # j "..."
    j "..."

# game/events_mc.rpy:1675
translate italian d7_wakeup_2637c315:

    # m "Uhh... right, well I... I'll make some breakfast..."
    m "Uhh... destra, beh, io... farò una colazione..."

# game/events_mc.rpy:1680
translate italian d7_wakeup_ca33a6dd:

    # j "[e], you better hurry!"
    j "[e], è meglio sbrigarsi!"

# game/events_mc.rpy:1682
translate italian d7_wakeup_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:1684
translate italian d7_wakeup_79c731b2:

    # n "[e] jumps up and quickly runs out the balcony."
    n "[e] salta e chiude rapidamente il balcone."

# game/events_mc.rpy:1686
translate italian d7_wakeup_7fe431b0:

    # pc "Huh? [e], wait!"
    pc "Huh? [e], aspetta!"

# game/events_mc.rpy:1688
translate italian d7_wakeup_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1689
translate italian d7_wakeup_fdacea98:

    # pc "Damn, I hope [m] doesn't see her like this..."
    pc "Dannazione, spero che [m] non la veda così..."

# game/events_mc.rpy:1691
translate italian d7_wakeup_b4fa3fc0_1:

    # j "..."
    j "..."

# game/events_mc.rpy:1694
translate italian d7_wakeup_f78b1475:

    # m "Are you awake, Hun?"
    m "Sei sveglio, Caccio?"

# game/events_mc.rpy:1695
translate italian d7_wakeup_6804df5a:

    # pc "...ugh... Yeah, I'm awake..."
    pc "...ugh... Sì, sono sveglio..."

# game/events_mc.rpy:1697
translate italian d7_wakeup_f422d8e0:

    # m "Good morning, Hun."
    m "Buongiorno, Caccio."

# game/events_mc.rpy:1698
translate italian d7_wakeup_f17dbea8:

    # pc "...what's up, [m]?"
    pc "...cos'è, [m]?"

# game/events_mc.rpy:1700
translate italian d7_wakeup_90c961dc:

    # m "Oh, I thought I might ask if you want some breakfast? I hope that's okay? Did I wake you up, Hun?"
    m "Oh, pensavo di poter chiedere se vuoi fare una colazione? Spero che vada bene? Ti ho svegliato, Hun?"

# game/events_mc.rpy:1702
translate italian d7_wakeup_6e81f568:

    # m "Well, breakfast is ready, and you have some work to do, right?"
    m "Beh, la colazione è pronta, e hai un po' di lavoro da fare, vero?"

# game/events_mc.rpy:1703
translate italian d7_wakeup_2ff46816:

    # pc "Uhn... I'm coming..."
    pc "Uhn... Sto arrivando..."

# game/events_mc.rpy:1704
translate italian d7_wakeup_672f5c8a:

    # m "Alright~"
    m "Alright~"

# game/events_mc.rpy:1706
translate italian d7_wakeup_e308e089:

    # pcthink "Damn... she's almost acting as if she's my real mother..."
    pcthink "Dannazione... si sta quasi comportando come se fosse mia vera madre..."

# game/events_mc.rpy:1716
translate italian d7_breakfast_1a15478a:

    # pcthink "Looks like [j] is gone already..."
    pcthink "Sembra che [j] sia già finito..."

# game/events_mc.rpy:1717
translate italian d7_breakfast_58626add:

    # m "So, what are your plans today, Hun?"
    m "Quali sono i tuoi piani oggi, Hun?"

# game/events_mc.rpy:1718
translate italian d7_breakfast_2c634384:

    # pc "Err... work, I guess..."
    pc "Err... lavoro, immagino..."

# game/events_mc.rpy:1719
translate italian d7_breakfast_2cbba7f5:

    # m "Oh, I bet you're making good progress?"
    m "Oh, scommetto che stai facendo buoni progressi?"

# game/events_mc.rpy:1720
translate italian d7_breakfast_7816c0f8:

    # pc "Yeah, kind of."
    pc "Sì, del genere."

# game/events_mc.rpy:1721
translate italian d7_breakfast_d408d503:

    # m "That's good to hear."
    m "È bello sentire."

# game/events_mc.rpy:1723
translate italian d7_breakfast_d8728d98:

    # n "You continue to make small talk for a bit until [m] and [e] get ready to leave..."
    n "Continui a fare una piccola chiacchierata per un po' fino a [m] e [e] preparati a partire..."

# game/events_mc.rpy:1726
translate italian d7_breakfast_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1727
translate italian d7_breakfast_e612037c:

    # m "..."
    m "..."

# game/events_mc.rpy:1728
translate italian d7_breakfast_b4fa3fc0:

    # j "..."
    j "..."

# game/events_mc.rpy:1730
translate italian d7_breakfast_7b551003:

    # pcthink "Shit, nobody is saying anything..."
    pcthink "Merda, nessuno sta dicendo niente..."

# game/events_mc.rpy:1731
translate italian d7_breakfast_a29c1aed:

    # pcthink "Looks like I really fucked up..."
    pcthink "Sembra che io abbia davvero scopato..."

# game/events_mc.rpy:1732
translate italian d7_breakfast_52d0cc61:

    # pcthink "Everybody seems to be mad at me..."
    pcthink "Sembra che tutti siano arrabbiati con me..."

# game/events_mc.rpy:1733
translate italian d7_breakfast_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1734
translate italian d7_breakfast_bb8fff34:

    # pc "So, uh..."
    pc "Così, uh..."

# game/events_mc.rpy:1739
translate italian d7_breakfast_801242f5:

    # pc "...about what happened..."
    pc "...su quello che è accaduto..."

# game/events_mc.rpy:1742
translate italian d7_breakfast_971f3699:

    # pc "Looks like it's going to be a nice day today..."
    pc "Sembra che sarà una bella giornata oggi..."

# game/events_mc.rpy:1745
translate italian d7_breakfast_594f9961:

    # m "Humph... don't you think you should apologize to someone?"
    m "Humph... non pensi che dovresti scusarti con qualcuno?"

# game/events_mc.rpy:1748
translate italian d7_breakfast_91375c36:

    # j "Pff, is that all you have to say?"
    j "Pff, è che tutto quello che dovete dire?"

# game/events_mc.rpy:1750
translate italian d7_breakfast_58a1eaba:

    # m "That was not nice, [pc]."
    m "Non è stato bello, [pc]."

# game/events_mc.rpy:1752
translate italian d7_breakfast_b1fed97d:

    # j "Yeah, that really hurt."
    j "Sì, quello veramente ferito."

# game/events_mc.rpy:1753
translate italian d7_breakfast_9b940997:

    # pc "I'm sorry... I was surprised and just reacted..."
    pc "Mi dispiace... Sono rimasto sorpreso e appena reagitato..."

# game/events_mc.rpy:1754
translate italian d7_breakfast_87c73e4e:

    # j "...humpf... so it was \"just\" a reaction, huh?"
    j "...humpf... quindi era \"giusto\" una reazione, eh?"

# game/events_mc.rpy:1755
translate italian d7_breakfast_5959fb60:

    # pc "Yes, it wasn't on purpose!"
    pc "Sì, non era di scopo!"

# game/events_mc.rpy:1756
translate italian d7_breakfast_e7062f61:

    # j "Humph..."
    j "Humph..."

# game/events_mc.rpy:1759
translate italian d7_breakfast_9f9cc93e:

    # j "So uhh... I hope you don't mind me having spent the night here, [m]."
    j "Così uhhh... Spero che non ti dispiace di aver trascorso la notte qui, [m]."

# game/events_mc.rpy:1760
translate italian d7_breakfast_e612037c_1:

    # m "..."
    m "..."

# game/events_mc.rpy:1762
translate italian d7_breakfast_294a0cae:

    # m "Well... you should have asked me first."
    m "Bene... mi avresti dovuto chiedere prima."

# game/events_mc.rpy:1764
translate italian d7_breakfast_15208453:

    # j "I know, but... it was kinda unexpected."
    j "Lo so, ma... era un po' inaspettato."

# game/events_mc.rpy:1766
translate italian d7_breakfast_0dc14203:

    # m "Unexpected?"
    m "Inaspettato?"

# game/events_mc.rpy:1767
translate italian d7_breakfast_176cf8c8:

    # j "Yeah, hehe."
    j "Sì, hehe."

# game/events_mc.rpy:1768
translate italian d7_breakfast_e612037c_2:

    # m "..."
    m "..."

# game/events_mc.rpy:1770
translate italian d7_breakfast_941a283f:

    # pcthink "Hum... looks like [j] doesn't want to tell her what really happened..."
    pcthink "Uomo... assomiglia a [j] non vuole dirle cosa è realmente accaduto..."

# game/events_mc.rpy:1772
translate italian d7_breakfast_f8ed8d86:

    # j "But I'll stay at home tonight, so [pc]'s bed will be free, hehe."
    j "Ma rimarrò a casa stasera, quindi il letto di [pc]sarà libero."

# game/events_mc.rpy:1773
translate italian d7_breakfast_c648d533:

    # pcthink "Uh-Oh..."
    pcthink "Uh-Oh..."

# game/events_mc.rpy:1775
translate italian d7_breakfast_c946a8f3:

    # j "[e] can have her big [bs] all for herself again, or maybe even..."
    j "[e] può avere il suo grande [bs!t] tutto per se stessa di nuovo, o forse..."

# game/events_mc.rpy:1777
translate italian d7_breakfast_04348ff1:

    # m "[j]!"
    m "[j]!"

# game/events_mc.rpy:1778
translate italian d7_breakfast_aeeb5622:

    # m "That's is not funny!"
    m "Non è divertente!"

# game/events_mc.rpy:1779
translate italian d7_breakfast_4686d521:

    # j "Sorry..."
    j "Sorry..."

# game/events_mc.rpy:1780
translate italian d7_breakfast_54a75af6:

    # pcthink "I knew it..."
    pcthink "Lo sapevo..."

# game/events_mc.rpy:1782
translate italian d7_breakfast_03d4e094:

    # n "A little bit later."
    n "Un po' più tardi."

# game/events_mc.rpy:1785
translate italian d7_breakfast_cf0b512e:

    # pcthink "Well... everybody is gone..."
    pcthink "Bene... tutti sono andati..."

# game/events_mc.rpy:1786
translate italian d7_breakfast_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:1787
translate italian d7_breakfast_324d7900:

    # pcthink "Hum, I guess I should get some work done..."
    pcthink "Hum, immagino che dovrei ottenere un po 'di lavoro fatto..."

# game/events_mc.rpy:1789
translate italian d7_breakfast_5f3edcc8:

    # n "An hour later."
    n "Un'ora dopo."

# game/events_mc.rpy:1791
translate italian d7_breakfast_a69377ef_2:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1792
translate italian d7_breakfast_b9b155fe:

    # extend "."
    extend "."

# game/events_mc.rpy:1793
translate italian d7_breakfast_b9b155fe_1:

    # extend "."
    extend "."

# game/events_mc.rpy:1794
translate italian d7_breakfast_b9b155fe_2:

    # extend "."
    extend "."

# game/events_mc.rpy:1795
translate italian d7_breakfast_6cbcbb85:

    # pcthink "Naaaaaaaaw... I can't think... not a single line of code done..."
    pcthink "Naaaaaaaaw... Non riesco a pensare... non una singola riga di codice fatta..."

# game/events_mc.rpy:1796
translate italian d7_breakfast_7923630c:

    # pcthink "I'm tired, I'm bored... I can't work like this."
    pcthink "Sono stanco, sono annoiato... non posso lavorare così."

# game/events_mc.rpy:1797
translate italian d7_breakfast_01f6d6fa:

    # pcthink "I need to do something else..."
    pcthink "Ho bisogno di fare qualcos'altro..."

# game/events_mc.rpy:1798
translate italian d7_breakfast_a69377ef_3:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1799
translate italian d7_breakfast_dadf699b:

    # pcthink "I'll go for a walk..."
    pcthink "Andrò a fare una passeggiata..."

# game/events_mc.rpy:1805
translate italian d7_evening_5838427b:

    # pcthink "Damn, that was embarrassing... everyone was staring at me..."
    pcthink "Dannazione, che era imbarazzante... tutti mi stavano fissando..."

# game/events_mc.rpy:1806
translate italian d7_evening_3e9379b4:

    # pcthink "I don't get it, why would you ever need so much toilet paper?"
    pcthink "Non lo ottengo, perché mai avrai bisogno di tanta carta igienica?"

# game/events_mc.rpy:1807
translate italian d7_evening_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1808
translate italian d7_evening_6ea063df:

    # pcthink "Anyway, I better get some work done now..."
    pcthink "In ogni caso, faccio meglio a fare un po' di lavoro ora..."

# game/events_mc.rpy:1812
translate italian d7_home_f68a2112:

    # n "Later..."
    n "Più tardi..."

# game/events_mc.rpy:1814
translate italian d7_home_efeb7a86:

    # pcthink "Alright, that's better. At least {b}some{/b} progress..."
    pcthink "Va bene, è meglio. Almeno {b}in qualche {/b} progresso..."

# game/events_mc.rpy:1817
translate italian d7_home_32ead6f8:

    # pcthink "Huh?"
    pcthink "Huh?"

# game/events_mc.rpy:1826
translate italian d7_home_02970987:

    # pcthink "Oh shit! It's the boss!"
    pcthink "Oh merda! È il capo!"

# game/events_mc.rpy:1845
translate italian d7_home_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1846
translate italian d7_home_b9b155fe:

    # extend "."
    extend "."

# game/events_mc.rpy:1847
translate italian d7_home_b9b155fe_1:

    # extend "."
    extend "."

# game/events_mc.rpy:1848
translate italian d7_home_b9b155fe_2:

    # extend "."
    extend "."

# game/events_mc.rpy:1908
translate italian d7_home_eabfa31f:

    # pcthink "So [st] told him about my visit... I wonder why they were talking about me."
    pcthink "Così [st] gli ha detto della mia visita... Mi chiedo perché stavano parlando di me."

# game/events_mc.rpy:1913
translate italian d8_morning_8bf6dc88:

    # n "*Knock* *Knock*"
    n "*Knock* *Knock*"

# game/events_mc.rpy:1915
translate italian d8_morning_e2ee86ba:

    # pc "Uhh..."
    pc "Uhh..."

# game/events_mc.rpy:1917
translate italian d8_morning_a2c50bf9:

    # m "Time to get up, [e]..."
    m "È ora di alzarsi, [e]..."

# game/events_mc.rpy:1919
translate italian d8_morning_c0150f59:

    # e "Uhnn..."
    e "Uhnn..."

# game/events_mc.rpy:1921
translate italian d8_morning_ea7b0f4c:

    # pcthink "...of course she snuck over again..."
    pcthink "...naturalmente si è staccata di nuovo..."

# game/events_mc.rpy:1924
translate italian d8_morning_81b740a4:

    # m "... and [pc] as well."
    m "... e [pc] pure."

# game/events_mc.rpy:1926
translate italian d8_morning_2ed3e3d9:

    # m "... do you want to have breakfast with us, [pc]?"
    m "... vuoi fare colazione con noi, [pc]?"

# game/events_mc.rpy:1927
translate italian d8_morning_6faca216:

    # pc "*YAAAWN*... yeah..."
    pc "*YAAAWN*... sì..."

# game/events_mc.rpy:1928
translate italian d8_morning_2f1ef192:

    # pcthink "... [m] really {b}has{/b} changed..."
    pcthink "... [m] realmente {b}ha cambiato{/b} ..."

# game/events_mc.rpy:1931
translate italian d8_morning_23fe4ea7:

    # n "You and [m] engage in some small talk again, you can't help but wonder if this is how a normal family would have their breakfast..."
    n "Tu e [m] fai qualche piccola chiacchierata di nuovo, non si può fare a meno di chiedersi se questo è come una famiglia normale avrebbe fatto la loro colazione..."

# game/events_mc.rpy:1933
translate italian d8_morning_848f5afd:

    # m "See you later, Hun."
    m "Ci vediamo più tardi, Hun."

# game/events_mc.rpy:1934
translate italian d8_morning_19392efe:

    # pc "Later..."
    pc "Più tardi..."

# game/events_mc.rpy:1935
translate italian d8_morning_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1937
translate italian d8_morning_f2c88496:

    # pcthink "Shit... I'm almost starting to enjoy the time here..."
    pcthink "Merda... Sto quasi iniziando a godermi il tempo qui..."

# game/events_mc.rpy:1938
translate italian d8_morning_6cafa954:

    # pcthink "[m] really changed, but I'm sure she's still hiding something from me."
    pcthink "[m] è cambiato davvero, ma sono sicuro che mi sta ancora nascondendo qualcosa."

# game/events_mc.rpy:1939
translate italian d8_morning_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:1940
translate italian d8_morning_0eb791b9:

    # pcthink "I wonder why she didn't mention anything about [e] sleeping in my bed again..."
    pcthink "Mi chiedo perché non abbia parlato di [e] dormire di nuovo nel mio letto..."

# game/events_mc.rpy:1941
translate italian d8_morning_61a4ffe9:

    # pcthink "Maybe she's starting to accept it?"
    pcthink "Forse sta iniziando ad accettarlo?"

# game/events_mc.rpy:1942
translate italian d8_morning_4e48822d:

    # pcthink "...or she just gave up trying..."
    pcthink "...o lei ha appena rinunciato a provare..."

# game/events_mc.rpy:1944
translate italian d8_morning_10f6ec45:

    # pcthink "Well, whatever, I have work to do..."
    pcthink "Beh, qualunque cosa, ho lavoro da fare..."

# game/events_mc.rpy:1946
translate italian d8_morning_e6f0cf97:

    # n "A few hours later..."
    n "Qualche ora dopo..."

# game/events_mc.rpy:1952
translate italian d8_eveningWJada_ae86d761:

    # pcthink "Hum, I just hope [m] remembers when the pills wear off..."
    pcthink "Hum, spero solo [m] ricorda quando le pillole usurano..."

# game/events_mc.rpy:1953
translate italian d8_eveningWJada_a0bd8abc:

    # pcthink "Maybe I should keep everyone together until the pills wear off. She {b}{i}has{/i}{/b} to remember when I don't give her the chance to think about anything else..."
    pcthink "Forse dovrei tenere tutti insieme fino a quando le pillole non si esauriscono. Lei {b}{i}non ha {/i}{/b} da ricordare quando non le do la possibilità di pensare a nient'altro..."

# game/events_mc.rpy:1955
translate italian d8_eveningWJada_fdc42a74:

    # pc "Hey, how about we celebrate this with a... err... with something, haha."
    pc "Ehi, come si celebra questo con un... err... con qualcosa, haha."

# game/events_mc.rpy:1957
translate italian d8_eveningWJada_66934b11:

    # j "Yeah, why not, hehe."
    j "Sì, perché no, è lui."

# game/events_mc.rpy:1959
translate italian d8_eveningWJada_e4c2420d:

    # m "Oh, that's a great idea, I'll make us some drinks!"
    m "Oh, questa è una grande idea, ci farò qualche bevanda!"

# game/events_mc.rpy:1961
translate italian d8_eveningWJada_a3d1076c:

    # j "Uhh..."
    j "Uhh..."

# game/events_mc.rpy:1963
translate italian d8_eveningWJada_537ee46a:

    # j "...but better nothing alcoholic... hehe...heh..."
    j "...ma meglio niente di alcolico... he...heh..."

# game/events_mc.rpy:1965
translate italian d8_eveningWJada_99771c90:

    # m "Huh? Of course not, you know that we won't have alcohol in this house, honey."
    m "Huh? Naturalmente no, sapete che non avremo alcool in questa casa, miele."

# game/events_mc.rpy:1967
translate italian d8_eveningWJada_6ab4742f:

    # j "Uhh... yeah, hehe..."
    j "Uhh... sì, lui..."

# game/events_mc.rpy:1969
translate italian d8_eveningWJada_084f5294:

    # j "{size=-10}Did she just call me honey?{/size}"
    j "{size=-10}Mi ha appena chiamato miele?{/size}"

# game/events_mc.rpy:1970
translate italian d8_eveningWJada_dbc63618:

    # pc "{size=-10}Yeah, it sounded like it...{/size}"
    pc "{size=-10}Sì, sembrava così...{/size}"

# game/events_mc.rpy:1971
translate italian d8_eveningWJada_4796f34c:

    # j "Weird..."
    j "Strano..."

# game/events_mc.rpy:1974
translate italian d8_eveningWJada_7c3855bb:

    # j "...and then she was like \"What the hell was that?!\", haha."
    j "...e poi era come \"Che diavolo era quello?!\", haha."

# game/events_mc.rpy:1976
translate italian d8_eveningWJada_ef50611b:

    # pc "Damn, that must have looked funny."
    pc "Dannazione, che deve essere sembrata divertente."

# game/events_mc.rpy:1978
translate italian d8_eveningWJada_ba0e7088:

    # j "Yeah it did! Haha."
    j "Sì, lo ha fatto! Haha."

# game/events_mc.rpy:1980
translate italian d8_eveningWJada_20c435f6:

    # pc "You better not do that here though, as funny as it sounds..."
    pc "È meglio non farlo qui, però, così divertente come suona..."

# game/events_mc.rpy:1982
translate italian d8_eveningWJada_585d60ca:

    # j "Don't worry, I promise I won't cause you any trouble."
    j "Non ti preoccupare, ti prometto che non ti causerò alcun problema."

# game/events_mc.rpy:1984
translate italian d8_eveningWJada_95be680b:

    # pc "Good, otherwise I'd have to send you back to that creepy old perv."
    pc "Bene, altrimenti dovrei rimandarti a quel vecchio perv. raccapricciante."

# game/events_mc.rpy:1986
translate italian d8_eveningWJada_8e79494f:

    # j "...and we don't want that, hehe."
    j "...e non lo vogliamo, hehe."

# game/events_mc.rpy:1988
translate italian d8_eveningWJada_072dc244:

    # j "Thanks again by the way, [m]. It's really nice of you to let me stay here..."
    j "Grazie ancora a proposito, [m]. È davvero bello di voi lasciarmi stare qui..."

# game/events_mc.rpy:1990
translate italian d8_eveningWJada_e612037c:

    # m "..."
    m "..."

# game/events_mc.rpy:1991
translate italian d8_eveningWJada_a7ecf1ab:

    # pc "[m]?!"
    pc "[m]?!"

# game/events_mc.rpy:1993
translate italian d8_eveningWJada_487a1850:

    # m "Hm?"
    m "Hm?"

# game/events_mc.rpy:1994
translate italian d8_eveningWJada_83ad7b2f:

    # pcthink "Shit, looks like the pills are going to wear off..."
    pcthink "Merda, sembra che le pillole stanno per uscire..."

# game/events_mc.rpy:1995
translate italian d8_eveningWJada_52f9ab04:

    # pc "[j] said thank you for letting her stay."
    pc "[j] ha detto grazie per aver lasciato il suo soggiorno."

# game/events_mc.rpy:1997
translate italian d8_eveningWJada_2302705b:

    # m "Letting her..."
    m "Lasciandolo..."

# game/events_mc.rpy:1999
translate italian d8_eveningWJada_e8055ccc:

    # j "Yeah, thanks, [m]. You're a life saver!"
    j "Sì, grazie, [m]. Sei un salvatore di vita!"

# game/events_mc.rpy:2000
translate italian d8_eveningWJada_0c86cf15:

    # pcthink "I better try to remind her about the talk we had..."
    pcthink "Tento meglio a ricordarle il discorso che avevamo..."

# game/events_mc.rpy:2002
translate italian d8_eveningWJada_bb068418:

    # pc "Yeah, now [e] has her BFF here, and err... we \"have her close\" ..."
    pc "Sì, ora [e] ha il suo BFF qui, e errori... noi \"abbiamo il suo vicino\" ..."

# game/events_mc.rpy:2003
translate italian d8_eveningWJada_a0667ccf:

    # pc "Err...to keep an eye on her!"
    pc "Err...per tenerla d'occhio!"

# game/events_mc.rpy:2005
translate italian d8_eveningWJada_1de1e8bb:

    # j "Hey, I'm sitting right here you know."
    j "Ehi, sono seduto proprio qui lo sai."

# game/events_mc.rpy:2006
translate italian d8_eveningWJada_8372f8a0:

    # pc "Of course I know."
    pc "Naturalmente lo so."

# game/events_mc.rpy:2007
translate italian d8_eveningWJada_3fa6b1c9:

    # e "Khehehe."
    e "Khehehe."

# game/events_mc.rpy:2009
translate italian d8_eveningWJada_9c7e54c5:

    # j "Aww, stop laughing [e], they will probably monitor us 24/7!"
    j "Aww, smetta di ridere [e], probabilmente ci monitoreranno 24/7!"

# game/events_mc.rpy:2010
translate italian d8_eveningWJada_bf277c84:

    # pc "Haha, every second."
    pc "Haha, ogni secondo."

# game/events_mc.rpy:2013
translate italian d8_eveningWJada_91a2ea44:

    # j "Dick!"
    j "Dick!"

# game/events_mc.rpy:2015
translate italian d8_eveningWJada_f45321f9:

    # j "Bitch!"
    j "Bitch!"

# game/events_mc.rpy:2016
translate italian d8_eveningWJada_ab0407ce:

    # pc "Haha."
    pc "Haha."

# game/events_mc.rpy:2018
translate italian d8_eveningWJada_f87d08f5:

    # m "[j], I don't want to hear any swear words in this house, so if you want to stay here for a while, you better stop that!"
    m "[j], Non voglio sentire parole di giuramento in questa casa, quindi se si vuole rimanere qui per un po ', è meglio fermarlo!"

# game/events_mc.rpy:2020
translate italian d8_eveningWJada_b8cd6cc7:

    # j "Uhh... sorry, no swear words anymore..."
    j "Uhh... scusa, nessuna parola di giuramento più..."

# game/events_mc.rpy:2021
translate italian d8_eveningWJada_e659ac4d:

    # pcthink "Phew, looks like it worked..."
    pcthink "Phew, sembra che abbia funzionato..."

# game/events_mc.rpy:2023
translate italian d8_eveningWJada_f68a2112:

    # n "Later..."
    n "Più tardi..."

# game/events_mc.rpy:2024
translate italian d8_eveningWJada_9a87cbd7:

    # e "HNNN!"
    e "HNNN!"

# game/events_mc.rpy:2025
translate italian d8_eveningWJada_cddc6e66:

    # pc "Right! Go right!"
    pc "A destra! Va bene!"

# game/events_mc.rpy:2026
translate italian d8_eveningWJada_f8f4d1e2:

    # j "I know!"
    j "Lo so!"

# game/events_mc.rpy:2028
translate italian d8_eveningWJada_a9ccd390:

    # e "NN!"
    e "NN!"

# game/events_mc.rpy:2029
translate italian d8_eveningWJada_99551eb4:

    # pc "Watch out, behind you!"
    pc "Attenzione, dietro di te!"

# game/events_mc.rpy:2031
translate italian d8_eveningWJada_b8fed04a:

    # j "Fuuuuuck!"
    j "Fuuuuuck!"

# game/events_mc.rpy:2032
translate italian d8_eveningWJada_a4fe781c:

    # j "Aww damn it, I almost got her..."
    j "La maledetta Aww, l'ho quasi ottenuta..."

# game/events_mc.rpy:2033
translate italian d8_eveningWJada_49f7755e:

    # pc "Welp..."
    pc "Welp..."

# game/events_mc.rpy:2036
translate italian d8_eveningWJada_8bf6dc88:

    # n "*Knock* *Knock*"
    n "*Knock* *Knock*"

# game/events_mc.rpy:2039
translate italian d8_eveningWJada_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:2040
translate italian d8_eveningWJada_20b8f121:

    # pc "Yes?"
    pc "Sì?"

# game/events_mc.rpy:2042
translate italian d8_eveningWJada_96772528:

    # m "Hey, are you having fun?"
    m "Ehi, ti stai divertendo?"

# game/events_mc.rpy:2044
translate italian d8_eveningWJada_3863cbbf:

    # j "Naaaw... I just lost..."
    j "Naaaw... Ho appena perso..."

# game/events_mc.rpy:2045
translate italian d8_eveningWJada_42fb089f:

    # m "That's... well..."
    m "Ciao... bene..."

# game/events_mc.rpy:2047
translate italian d8_eveningWJada_9603b05a:

    # m "Anyway, [pc], I'm going to bed and I just wanted to make sure that..."
    m "Comunque, [pc], sto andando a letto e volevo solo assicurarmi che..."

# game/events_mc.rpy:2049
translate italian d8_eveningWJada_a177bee0:

    # m "...well, I'm sure it isn't necessary, but I wanted to remind you that [e] has her own room and I expect [j] to sleep in [e]'s room, too."
    m "... ell, sono sicuro che non è necessario, ma volevo ricordarvi che [e] ha la sua stanza e mi aspetto [j] di dormire nella stanza di [e], anche."

# game/events_mc.rpy:2051
translate italian d8_eveningWJada_3f91d725:

    # m "Umm... well, I... I just wanted to remind you that this is {b}{i}your{/i}{/b} room, [e] has her own room and bed and [j] is her best friend so..."
    m "Umm... beh, Io... Volevo solo ricordarti che questa è {b}{i}la tua{/i}{/b} stanza, [e] ha la sua camera e letto e [j] è la sua migliore amica così..."

# game/events_mc.rpy:2052
translate italian d8_eveningWJada_3c8e055f:

    # m "I..."
    m "I..."

# game/events_mc.rpy:2053
translate italian d8_eveningWJada_4d699985:

    # m "I just wanted to make sure that you all remember that."
    m "Volevo solo fare in modo che voi tutti lo ricordate."

# game/events_mc.rpy:2054
translate italian d8_eveningWJada_8095c2c4:

    # pcthink "Jeez, is she serious now?"
    pcthink "Jeez, è seria adesso?"

# game/events_mc.rpy:2055
translate italian d8_eveningWJada_b3c9e471:

    # j "Oh don't worry [m]..."
    j "Oh non preoccuparti [m]..."

# game/events_mc.rpy:2057
translate italian d8_eveningWJada_ca366517:

    # j "Of course we remember that..."
    j "Naturalmente lo ricordiamo..."

# game/events_mc.rpy:2059
translate italian d8_eveningWJada_c3c54b71:

    # j "[e] and I will of course sleep in [e]'s room."
    j "[e] e naturalmente dormirò nella stanza di [e]."

# game/events_mc.rpy:2060
translate italian d8_eveningWJada_225fadf5:

    # pc "Huh?"
    pc "Huh?"

# game/events_mc.rpy:2061
translate italian d8_eveningWJada_d593d4b0:

    # e "Eh?"
    e "Eh?"

# game/events_mc.rpy:2062
translate italian d8_eveningWJada_a4b0f5dd:

    # m "Oh really?!"
    m "Oh davvero?!"

# game/events_mc.rpy:2064
translate italian d8_eveningWJada_58163bb5:

    # j "Of course, and I'll make sure that everyone stays in their respective beds."
    j "Naturalmente, e mi assicurerò che tutti rimangano nei rispettivi letti."

# game/events_mc.rpy:2066
translate italian d8_eveningWJada_01fc1be6:

    # m "That's, umm... well thanks [j]..."
    m "Questo, umm... benissimo grazie [j]..."

# game/events_mc.rpy:2067
translate italian d8_eveningWJada_38c7e193:

    # pcthink "Wow, [j] really wants to make a good impression..."
    pcthink "Wow, [j] vuole davvero fare una buona impressione..."

# game/events_mc.rpy:2068
translate italian d8_eveningWJada_943ce365:

    # m "Okay then... have a good night everyone..."
    m "Va bene allora... avere una buona notte tutti..."

# game/events_mc.rpy:2069
translate italian d8_eveningWJada_9c35c852:

    # j "Good night, [m]."
    j "Buona notte, [m]."

# game/events_mc.rpy:2071
translate italian d8_eveningWJada_ac74160d:

    # j "Well, I guess that means we should go to bed, huh, [e]?"
    j "Beh, immagino che significa che dovremmo andare a letto, ehh, [e]?"

# game/events_mc.rpy:2073
translate italian d8_eveningWJada_1da7971d:

    # e "Unh..."
    e "Unh..."

# game/events_mc.rpy:2075
translate italian d8_eveningWJada_66aa8c20:

    # j "Oh come on [e], I don't want to get kicked out again!"
    j "Oh vieni il [e], non voglio essere espulso di nuovo!"

# game/events_mc.rpy:2077
translate italian d8_eveningWJada_8ba6ae53:

    # pc "Well, I guess she's right, let's play along, okay?"
    pc "Beh, immagino che lei abbia ragione, giochiamo bene, okay?"

# game/events_mc.rpy:2082
translate italian d8_eveningWJada_755b8a4e:

    # n "You expect a simple goodnight kiss, but right when your lips meet, you can feel [e]'s tongue sliding into your mouth..."
    n "Ti aspetti un semplice bacio notturno, ma proprio quando le tue labbra si incontrano, puoi sentire la lingua di [e]scivolare nella tua bocca..."

# game/events_mc.rpy:2087
translate italian d8_eveningWJada_7828051b:

    # j "Hehe, cute!"
    j "Hehe, carino!"

# game/events_mc.rpy:2089
translate italian d8_eveningWJada_e63cf01d:

    # j "Good night, [pc]."
    j "Buona notte, [pc]."

# game/events_mc.rpy:2090
translate italian d8_eveningWJada_c0d50c6d:

    # pc "Night, [j], night, [e]."
    pc "Notte, [j], notte, [e]."

# game/events_mc.rpy:2093
translate italian d8_eveningWJada_7d716a37:

    # j "Night..."
    j "Notte..."

# game/events_mc.rpy:2095
translate italian d8_eveningWJada_b9e1b26a:

    # j "{size=-15}[pcmd]{/size}."
    j "{size=-15}[pcmd]{/size}."

# game/events_mc.rpy:2096
translate italian d8_eveningWJada_77b0399a:

    # n "*Smooch*"
    n "*Smooch*"

# game/events_mc.rpy:2098
translate italian d8_eveningWJada_6e5441be:

    # pc "Night, [j]..."
    pc "Notte, [j]..."

# game/events_mc.rpy:2100
translate italian d8_eveningWJada_e8a48adf:

    # pc "Heh, night, sweetie."
    pc "Heh, notte, dolcezza."

# game/events_mc.rpy:2102
translate italian d8_eveningWJada_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:2105
translate italian d8_eveningWJada_f2ead045:

    # j "Good night, [pcmd]..."
    j "Buona notte, [pcmd]..."

# game/events_mc.rpy:2110
translate italian d8_eveningWJada_2d849fbb:

    # pc "Heh... good night, babygirl."
    pc "Heh... buona notte, babygirl."

# game/events_mc.rpy:2112
translate italian d8_eveningWJada_2f6d0e57:

    # pc "Night, sweetie."
    pc "Notte, dolce."

# game/events_mc.rpy:2113
translate italian d8_eveningWJada_8de614b1:

    # e "Khehe!"
    e "Khehe!"

# game/events_mc.rpy:2115
translate italian d8_eveningWJada_744f56ce:

    # n "A bit later..."
    n "Un po' più tardi..."

# game/events_mc.rpy:2117
translate italian d8_eveningWJada_e6315939:

    # j "{size=-10}Hahaha... yes!{/size}"
    j "{size=-10}Hahaha... sì!{/size}"

# game/events_mc.rpy:2118
translate italian d8_eveningWJada_7ae9f4d0:

    # n "You can hear [j]'s voice through the wall."
    n "Puoi sentire la voce di [j]attraverso il muro."

# game/events_mc.rpy:2119
translate italian d8_eveningWJada_ab504d42:

    # j "{size=-10}Do you think...{/size}"
    j "{size=-10}Pensi...{/size}"

# game/events_mc.rpy:2120
translate italian d8_eveningWJada_08da4a08:

    # pcthink "Looks like [e] and [j] are still awake..."
    pcthink "Sembra [e] e [j] sono ancora svegli..."

# game/events_mc.rpy:2121
translate italian d8_eveningWJada_4de159dd:

    # j "{size=-10}Haha, nooo, I'll help you, but that's too...{/size}"
    j "{size=-10}Haha, nooo, ti aiuterò, ma anche questo...{/size}"

# game/events_mc.rpy:2122
translate italian d8_eveningWJada_305b9374:

    # j "{size=-10}...yes!{/size}"
    j "{size=-10}...sì!{/size}"

# game/events_mc.rpy:2123
translate italian d8_eveningWJada_cfbbd79b:

    # j "{size=-10}...really like [himher]!{/size}"
    j "{size=-10}...davvero come [himher!t]!{/size}"

# game/events_mc.rpy:2124
translate italian d8_eveningWJada_75fc793d:

    # pcthink "Damn, I can't understand all of it."
    pcthink "Dannazione, non riesco a capirlo."

# game/events_mc.rpy:2125
translate italian d8_eveningWJada_9f00adb1:

    # j "{size=-10}...really sure about it?{/size}"
    j "{size=-10}...sei sicuro di esso?{/size}"

# game/events_mc.rpy:2126
translate italian d8_eveningWJada_19b7b455:

    # n "From this point on you can only hear some whispering, but can't understand a word."
    n "Da questo punto in poi puoi sentire solo qualche sussurro, ma non puoi capire una parola."

# game/events_mc.rpy:2127
translate italian d8_eveningWJada_66e2dfd5:

    # pcthink "Hum... I wonder what they were talking about..."
    pcthink "Uomo... Mi chiedo di cosa stavano parlando..."

# game/events_mc.rpy:2135
translate italian d9_wakeup_0b3abc60:

    # n "*Sniff*"
    n "*Sniff*"

# game/events_mc.rpy:2136
translate italian d9_wakeup_ba0ce6f0:

    # n "*Wimper*"
    n "*Wimper*"

# game/events_mc.rpy:2137
translate italian d9_wakeup_e78beedd:

    # pc "Huh? What's wrong, [m]?"
    pc "Huh? Cosa c'è di sbagliato, [m]?"

# game/events_mc.rpy:2138
translate italian d9_wakeup_62545805:

    # m "*Sniff* ...oh..."
    m "*Sniff* ...oh..."

# game/events_mc.rpy:2139
translate italian d9_wakeup_a398b817:

    # m "I'm sorry, [pc], it's nothing..."
    m "Mi dispiace, [pc], non è niente..."

# game/events_mc.rpy:2140
translate italian d9_wakeup_4c4f5296:

    # pc "Where's Dad?"
    pc "Dov'è il papà?"

# game/events_mc.rpy:2141
translate italian d9_wakeup_fbcdc9b6:

    # m "*Sniff* ...he didn't tell you?"
    m "*Sniff* ... non te l'ha detto?"

# game/events_mc.rpy:2142
translate italian d9_wakeup_37638e4d:

    # pc "Uh-uh."
    pc "Uh-uh."

# game/events_mc.rpy:2143
translate italian d9_wakeup_8083b61b:

    # m "He got suspended and is attending the hearing... [oc] just picked him up..."
    m "È stato sospeso e sta frequentando l'udienza... [oc] appena scelto lui su..."

# game/events_mc.rpy:2144
translate italian d9_wakeup_5a07187a:

    # pc "Why did he get suspended?"
    pc "Perché è stato sospeso?"

# game/events_mc.rpy:2145
translate italian d9_wakeup_507892dd:

    # m "*Sniff* ...I don't know, he didn't tell me... he was probably drunk again..."
    m "*Sniff* ... non lo so, non mi ha detto... probabilmente era nuovamente ubriaco..."

# game/events_mc.rpy:2146
translate italian d9_wakeup_f3c083a8:

    # pc "Does that mean he will be home more often from now on?"
    pc "Questo significa che sarà a casa più spesso d'ora?"

# game/events_mc.rpy:2147
translate italian d9_wakeup_02e42931:

    # m "*Sniff* ...yes..."
    m "*Sniff* ...sì..."

# game/events_mc.rpy:2148
translate italian d9_wakeup_930497c3:

    # m "[oc] said that your Dad will probably stay home for a while from now on..."
    m "[oc] ha detto che il tuo papà probabilmente rimarrà a casa per un po' di tempo..."

# game/events_mc.rpy:2149
translate italian d9_wakeup_dd2d8f0d:

    # pc "Did [oc] get suspended too?"
    pc "Anche [oc] è stato sospeso?"

# game/events_mc.rpy:2150
translate italian d9_wakeup_e5fc8ba9:

    # m "No, but he has to attend the hearing too."
    m "No, ma anche lui deve partecipare all'udienza."

# game/events_mc.rpy:2161
translate italian d9_breakfast_f136f8bf:

    # pc "Morning everyone."
    pc "Mattina tutti."

# game/events_mc.rpy:2162
translate italian d9_breakfast_c32695b8:

    # m "Good morning, hun."
    m "Buongiorno, caccia."

# game/events_mc.rpy:2164
translate italian d9_breakfast_c7687275:

    # m "Nice shirt."
    m "Bella camicia."

# game/events_mc.rpy:2165
translate italian d9_breakfast_4cb533b6:

    # pc "Thanks..."
    pc "Grazie..."

# game/events_mc.rpy:2167
translate italian d9_breakfast_ea9caa26:

    # m "Oh by the way [j], I didn't see you bring any of your stuff yesterday?"
    m "Oh a proposito [j], Non ti ho visto portare nessuna delle tue cose ieri?"

# game/events_mc.rpy:2169
translate italian d9_breakfast_ffd273b8:

    # j "Yeah, most of it is still in the apartment..."
    j "Sì, la maggior parte di esso è ancora nell'appartamento..."

# game/events_mc.rpy:2171
translate italian d9_breakfast_4d1e2a17:

    # m "I see, you should get it here soon if you plan to stay for a bit longer..."
    m "Vedo che dovresti arrivare qui presto se hai intenzione di rimanere un po' più a lungo..."

# game/events_mc.rpy:2173
translate italian d9_breakfast_87d3c8fc:

    # j "Yeah..."
    j "Sì..."

# game/events_mc.rpy:2175
translate italian d9_breakfast_dbf3eae9:

    # m "It will surely help to make you feel at home."
    m "Sicuramente ti aiuterà a farti sentire a casa."

# game/events_mc.rpy:2177
translate italian d9_breakfast_80206a6d:

    # j "Oh thanks, I feel at home already, I mean, the only place I've ever had a breakfast like this is here."
    j "Oh grazie, mi sento già a casa, voglio dire, l'unico posto in cui ho mai fatto colazione come questa è qui."

# game/events_mc.rpy:2179
translate italian d9_breakfast_5ac27665:

    # m "Really? You don't eat breakfast together with your mom?"
    m "Davvero? Non fai colazione insieme a tua mamma?"

# game/events_mc.rpy:2181
translate italian d9_breakfast_aa83e1d2:

    # j "No, but I always liked it when I was staying here overnight. It feels like... having a real family."
    j "No, ma mi è sempre piaciuto quando stavo soggiornando qui per tutta la notte. Mi sembra che abbia una vera famiglia."

# game/events_mc.rpy:2183
translate italian d9_breakfast_9b154c9e:

    # m "I didn't know that it meant so much to you..."
    m "Non sapevo che intendeva molto per te..."

# game/events_mc.rpy:2185
translate italian d9_breakfast_9a1d349c:

    # j "It kinda makes me feel like having a real family..."
    j "Mi fa sentire come avere una vera famiglia..."

# game/events_mc.rpy:2186
translate italian d9_breakfast_0a005ee3:

    # pcthink "Damn, I know how she feels..."
    pcthink "Dannazione, so come si sente..."

# game/events_mc.rpy:2188
translate italian d9_breakfast_d8ea254e:

    # j "By the way, [pc], did you ever call [m] Mom?"
    j "A proposito, [pc], hai mai chiamato [m] Moma?"

# game/events_mc.rpy:2189
translate italian d9_breakfast_fcf41317:

    # pc "Eh?"
    pc "Eh?"

# game/events_mc.rpy:2191
translate italian d9_breakfast_472b2611:

    # m "[j] you know that I'm not [hisher] mom."
    m "[j] sai che non sono [hisher!t] mamma."

# game/events_mc.rpy:2192
translate italian d9_breakfast_3fa6b1c9:

    # e "Khehehe."
    e "Khehehe."

# game/events_mc.rpy:2194
translate italian d9_breakfast_d9c4138e:

    # m "...I mean... I wouldn't mind..."
    m "... Intendo dire... Non mi dispiace..."

# game/events_mc.rpy:2195
translate italian d9_breakfast_a7c89ecd:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:2200
translate italian d9_breakfast_30efd40d:

    # pc "Well, I guess I could..."
    pc "Beh, immagino che potrei..."

# game/events_mc.rpy:2202
translate italian d9_breakfast_d93364d6:

    # m "Oh, but you don't need to if you feel uncomfortable about it."
    m "Oh, ma non è necessario se ti senti a disagio."

# game/events_mc.rpy:2203
translate italian d9_breakfast_803c4437:

    # pc "Nah, it's okay..."
    pc "Nah, va bene..."

# game/events_mc.rpy:2204
translate italian d9_breakfast_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:2205
translate italian d9_breakfast_c8768c83:

    # pc "...[mmom]."
    pc "...[mmom]."

# game/events_mc.rpy:2207
translate italian d9_breakfast_e612037c:

    # m "..."
    m "..."

# game/events_mc.rpy:2209
translate italian d9_breakfast_86d648c6:

    # m "Umm..."
    m "Umm..."

# game/events_mc.rpy:2211
translate italian d9_breakfast_beb41611:

    # m "A-Anyway... [j]. Me and [e] will go shopping and pick up some groceries after breakfast, why don't you come with us so we can get your stuff on our way back?"
    m "A-Anyway... [j]. Io e [e] andremo a fare shopping e prendere alcuni generi alimentari dopo la colazione, perché non vieni con noi in modo che possiamo ottenere la tua roba sulla nostra via di ritorno?"

# game/events_mc.rpy:2213
translate italian d9_breakfast_4de61af5:

    # j "Yeah, why not."
    j "Sì, perché no."

# game/events_mc.rpy:2216
translate italian d9_breakfast_63bae69e:

    # pc "...well about your stuff in the apartment, [j]..."
    pc "...bene sulle tue cose nell'appartamento, [j]..."

# game/events_mc.rpy:2217
translate italian d9_breakfast_227f3a09:

    # j "Hm?"
    j "Hm?"

# game/events_mc.rpy:2219
translate italian d9_breakfast_99a47862:

    # m "Right, you can come and help me and [e] with the groceries after breakfast, we can get your stuff on our way back."
    m "Destra, puoi venire ad aiutarmi e [e] con i generi alimentari dopo la prima colazione, possiamo ottenere la tua roba sulla nostra strada di ritorno."

# game/events_mc.rpy:2221
translate italian d9_breakfast_a3d1076c:

    # j "Uhh..."
    j "Uhh..."

# game/events_mc.rpy:2223
translate italian d9_breakfast_402d4708:

    # j "...well, okay."
    j "...beh, va bene."

# game/events_mc.rpy:2224
translate italian d9_breakfast_82b74462:

    # pcthink "Hmm... I have the feeling that she had some other plans..."
    pcthink "Hmm... Ho la sensazione che lei abbia avuto alcuni altri piani..."

# game/events_mc.rpy:2226
translate italian d9_breakfast_ff62b093:

    # n "After everyone is done eating you're surprised to see [j] helping with the dishes instantly."
    n "Dopo che tutti hanno finito di mangiare, sei sorpreso di vedere [j] aiutare con i piatti istantaneamente."

# game/events_mc.rpy:2227
translate italian d9_breakfast_d64ac769:

    # pcthink "Hum... she's full of surprises..."
    pcthink "Uomo... è piena di sorprese..."

# game/events_mc.rpy:2228
translate italian d9_breakfast_7f827474:

    # n "Since there's nothing left to do, you decide go back to your room and do some work..."
    n "Dal momento che non c'è nulla da fare, decidi di tornare nella tua stanza e fare un po' di lavoro..."

# game/events_mc.rpy:2232
translate italian d9_breakfast_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:2233
translate italian d9_breakfast_b9b155fe:

    # extend "."
    extend "."

# game/events_mc.rpy:2234
translate italian d9_breakfast_b9b155fe_1:

    # extend "."
    extend "."

# game/events_mc.rpy:2235
translate italian d9_breakfast_b9b155fe_2:

    # extend "."
    extend "."

# game/events_mc.rpy:2236
translate italian d9_breakfast_5531c1c8:

    # pcthink "Hum... that's weird..."
    pcthink "Uomo... è strano..."

# game/events_mc.rpy:2237
translate italian d9_breakfast_0b7d0499:

    # pcthink "...something doesn't fit..."
    pcthink "...qualcosa non funziona..."

# game/events_mc.rpy:2238
translate italian d9_breakfast_4a57ea09:

    # m "[pc]?"
    m "[pc]?"

# game/events_mc.rpy:2240
translate italian d9_breakfast_9455801f:

    # pc "Hm?"
    pc "Hm?"

# game/events_mc.rpy:2242
translate italian d9_breakfast_924d2dc2:

    # m "Didn't you hear me knocking?"
    m "Non mi hai sentito bussare?"

# game/events_mc.rpy:2244
translate italian d9_breakfast_395b9705:

    # pc "Uh... no, what's up?"
    pc "Uh... no, che cos'è?"

# game/events_mc.rpy:2246
translate italian d9_breakfast_bb3b7b86:

    # m "We're leaving, do you need anything from the city?"
    m "Stiamo partendo, hai bisogno di qualcosa dalla città?"

# game/events_mc.rpy:2248
translate italian d9_breakfast_7940353d:

    # pc "Hm? Ah, no... nothing."
    pc "Hm? Ah, no... niente."

# game/events_mc.rpy:2250
translate italian d9_breakfast_10781970:

    # m "Okay, see you later then."
    m "Ok, a vederti più tardi allora."

# game/events_mc.rpy:2252
translate italian d9_breakfast_df3a4c48:

    # pc "Yeah, later..."
    pc "Sì, più tardi..."

# game/events_mc.rpy:2253
translate italian d9_breakfast_ca388340:

    # pc "[mmom]."
    pc "[mmom]."

# game/events_mc.rpy:2261
translate italian d9_breakfast_c660d414:

    # pcthink "Ugh... calling her mom feels really weird..."
    pcthink "Ugh... chiamando sua mamma si sente davvero strana..."

# game/events_mc.rpy:2262
translate italian d9_breakfast_c7677361:

    # pcthink "Why am I even doing this... I don't trust her god dammit..."
    pcthink "Perché sto anche facendo questo... non mi fido del suo dio dammit..."

# game/events_mc.rpy:2264
translate italian d9_breakfast_930a282a:

    # pcthink "Shit... better get back to work... I don't want to think about stuff like that..."
    pcthink "Merda... meglio tornare al lavoro... Non voglio pensare a cose del genere..."

# game/events_mc.rpy:2266
translate italian d9_breakfast_a8c6cc8a:

    # pc "Hum..."
    pc "Hum..."

# game/events_mc.rpy:2267
translate italian d9_breakfast_186419d4:

    # pcthink "...there seems to be something missing..."
    pcthink "...sembra mancare qualcosa..."

# game/events_mc.rpy:2268
translate italian d9_breakfast_c1686997:

    # pcthink "Hum..."
    pcthink "Hum..."

# game/events_mc.rpy:2269
translate italian d9_breakfast_b9b155fe_3:

    # extend "."
    extend "."

# game/events_mc.rpy:2270
translate italian d9_breakfast_b9b155fe_4:

    # extend "."
    extend "."

# game/events_mc.rpy:2271
translate italian d9_breakfast_b9b155fe_5:

    # extend "."
    extend "."

# game/events_mc.rpy:2277
translate italian d9_breakfast_a69377ef_2:

    # pc "..."
    pc "..."

# game/events_mc.rpy:2278
translate italian d9_breakfast_af934448:

    # st "...yes?"
    st "...sì?"

# game/events_mc.rpy:2279
translate italian d9_breakfast_c7479b17:

    # pcthink "As I thought, she's working at the weekend again..."
    pcthink "Come pensavo, sta lavorando di nuovo nel fine settimana..."

# game/events_mc.rpy:2280
translate italian d9_breakfast_01a67768:

    # pc "Hey, [st], I'm just working on the project, but I think I'm missing some files, could you check the files you sent me?"
    pc "Hey, [st], Sto solo lavorando al progetto, ma penso che mi manchino alcuni file, potresti controllare i file che mi hai inviato?"

# game/events_mc.rpy:2281
translate italian d9_breakfast_3994b364:

    # st "Uhm... yes, give me a minute..."
    st "Uhm... sì, dammi un minuto..."

# game/events_mc.rpy:2282
translate italian d9_breakfast_38710895:

    # pc "Okay, thanks."
    pc "Okay, grazie."

# game/events_mc.rpy:2283
translate italian d9_breakfast_a69377ef_3:

    # pc "..."
    pc "..."

# game/events_mc.rpy:2284
translate italian d9_breakfast_b9b155fe_6:

    # extend "."
    extend "."

# game/events_mc.rpy:2285
translate italian d9_breakfast_b9b155fe_7:

    # extend "."
    extend "."

# game/events_mc.rpy:2286
translate italian d9_breakfast_b9b155fe_8:

    # extend "."
    extend "."

# game/events_mc.rpy:2287
translate italian d9_breakfast_e8152094:

    # st "This... might be a problem..."
    st "Questo potrebbe essere un problema..."

# game/events_mc.rpy:2288
translate italian d9_breakfast_a2a6fb65:

    # pc "What do you mean?"
    pc "Che cosa intendi?"

# game/events_mc.rpy:2289
translate italian d9_breakfast_21f4c37c:

    # st "It looks like [boss] forgot to include the latest data, but I'll have to check some older files to make sure, there's definitely something missing though."
    st "Sembra che [boss] abbia dimenticato di includere i dati più recenti, ma dovrò controllare alcuni file più vecchi per essere sicuro, c'è sicuramente qualcosa che manca però."

# game/events_mc.rpy:2290
translate italian d9_breakfast_9ac1eb9c:

    # pc "Okay... sounds like it'll take a while?"
    pc "Okay... sembra che ci vorrà un po'?"

# game/events_mc.rpy:2291
translate italian d9_breakfast_dbed7976:

    # st "Yes..."
    st "Sì..."

# game/events_mc.rpy:2292
translate italian d9_breakfast_01ed9b97:

    # pc "Hum... okay, well it looks like I'll have an actual weekend then."
    pc "Uomo... ok, beh, sembra che avrò un vero fine settimana allora."

# game/events_mc.rpy:2293
translate italian d9_breakfast_d9eaa6bf:

    # st "Sorry... I'll send them to you as soon as I can."
    st "Mi dispiace... Le invierò il prima possibile."

# game/events_mc.rpy:2294
translate italian d9_breakfast_0e17cfdd:

    # pc "Yeah no sweat, it's the weekend we shouldn't be working in the first place..."
    pc "Sì, nessun sudore, è il fine settimana che non dovremmo lavorare in primo luogo..."

# game/events_mc.rpy:2295
translate italian d9_breakfast_2384fd4b:

    # st "..."
    st "..."

# game/events_mc.rpy:2296
translate italian d9_breakfast_203650b4:

    # pc "Thinking about it, I don't have much else to do, so maybe I'll come over later."
    pc "Pensandoci, non ho molto altro da fare, quindi forse tornerò più tardi."

# game/events_mc.rpy:2297
translate italian d9_breakfast_2384fd4b_1:

    # st "..."
    st "..."

# game/events_mc.rpy:2298
translate italian d9_breakfast_bcbd1c8f:

    # st "Okay."
    st "Okay."

# game/events_mc.rpy:2299
translate italian d9_breakfast_64b6a4cc:

    # pc "Well... maybe later then."
    pc "Bene... forse più tardi."

# game/events_mc.rpy:2300
translate italian d9_breakfast_32c6a094:

    # st "L-later..."
    st "L-più tardi..."

# game/events_mc.rpy:2306
translate italian d9_breakfast_77ddecae:

    # pcthink "Hum... I don't get that woman..."
    pcthink "Uomo... Non ricevo quella donna..."

# game/events_mc.rpy:2307
translate italian d9_breakfast_54475726:

    # pcthink "Anyway, what am I going to do now?"
    pcthink "In ogni caso, cosa farò adesso?"

# game/events_mc.rpy:2308
translate italian d9_breakfast_e28e8ec7:

    # n "*BZZZZZ*"
    n "*BZZZZZ*"

# game/events_mc.rpy:2310
translate italian d9_breakfast_4ab6197a:

    # pcthink "Huh? Did she forget anything?"
    pcthink "Huh? Ha dimenticato qualcosa?"

# game/events_mc.rpy:2312
translate italian d9_breakfast_f0cb153b:

    # pc "What's up [st], did you forget anything?"
    pc "Che cos'è [st], hai dimenticato qualcosa?"

# game/events_mc.rpy:2313
translate italian d9_breakfast_54d61c82:

    # ma "Hello, this is [ma] Shelter."
    ma "Ciao, questo è [ma] Rifugio."

# game/events_mc.rpy:2314
translate italian d9_breakfast_62612790:

    # pc "Ma..."
    pc "Ma..."

# game/events_mc.rpy:2315
translate italian d9_breakfast_87dc5ca4:

    # pcthink "Oh right, the office secretary from the rental company."
    pcthink "Oh destra, la segretaria di ufficio della società di noleggio."

# game/events_mc.rpy:2316
translate italian d9_breakfast_6d96d7b7:

    # pc "Sorry I thought you were my co-worker, I just talked to her."
    pc "Mi dispiace che tu fossi il mio collaboratore, ho appena parlato con lei."

# game/events_mc.rpy:2317
translate italian d9_breakfast_26dfc1f2:

    # ma "No problem, do you want me to call again another time?"
    ma "Nessun problema, vuoi che io chiami un'altra volta?"

# game/events_mc.rpy:2318
translate italian d9_breakfast_4145ad23:

    # pc "No, it's okay, [ma], what do you need?"
    pc "No, va bene, [ma], che cosa ti serve?"

# game/events_mc.rpy:2319
translate italian d9_breakfast_2717beb3:

    # ma "I have some good and some bad news for you, which one would you like to hear first?"
    ma "Ho qualche buona e qualche cattiva notizia per voi, quale vorreste sentire per primo?"

# game/events_mc.rpy:2322
translate italian d9_breakfast_f1f284ff:

    # ma "We are ready to refund your deposit for the apartment, all I need is your signature on a few documents."
    ma "Siamo pronti a rimborsare il tuo deposito per l'appartamento, tutto ciò di cui ho bisogno è la tua firma su alcuni documenti."

# game/events_mc.rpy:2323
translate italian d9_breakfast_62b8cef0:

    # pc "Okay, so what's the bad news then?"
    pc "Okay, allora qual è la brutta notizia?"

# game/events_mc.rpy:2324
translate italian d9_breakfast_e124b86c:

    # ma "I still don't have any news on another apartment."
    ma "Non ho ancora notizie su un altro appartamento."

# game/events_mc.rpy:2325
translate italian d9_breakfast_4b3a0d7e:

    # pc "Well, that's no big problem for now, I have somewhere to stay for a while."
    pc "Beh, questo non è un grosso problema per ora, ho da qualche parte da rimanere per un po'."

# game/events_mc.rpy:2326
translate italian d9_breakfast_f0d78308:

    # ma "Good to hear."
    ma "Buono a sentire."

# game/events_mc.rpy:2327
translate italian d9_breakfast_1eca72ab:

    # ma "Do you want me to send you the documents via email?"
    ma "Vuoi che ti invii i documenti via email?"

# game/events_mc.rpy:2330
translate italian d9_breakfast_e124b86c_1:

    # ma "I still don't have any news on another apartment."
    ma "Non ho ancora notizie su un altro appartamento."

# game/events_mc.rpy:2331
translate italian d9_breakfast_4b3a0d7e_1:

    # pc "Well, that's no big problem for now, I have somewhere to stay for a while."
    pc "Beh, questo non è un grosso problema per ora, ho da qualche parte da rimanere per un po'."

# game/events_mc.rpy:2332
translate italian d9_breakfast_bb1c37cf:

    # ma "Oh, that's good to hear."
    ma "Oh, è bello sentire."

# game/events_mc.rpy:2333
translate italian d9_breakfast_2ad7efbe:

    # pc "So what's the good news?"
    pc "Qual è la buona notizia?"

# game/events_mc.rpy:2334
translate italian d9_breakfast_f1f284ff_1:

    # ma "We are ready to refund your deposit for the apartment, all I need is your signature on a few documents."
    ma "Siamo pronti a rimborsare il tuo deposito per l'appartamento, tutto ciò di cui ho bisogno è la tua firma su alcuni documenti."

# game/events_mc.rpy:2335
translate italian d9_breakfast_16838e2a:

    # pc "Good, can you send them over?"
    pc "Bene, puoi mandarli finito?"

# game/events_mc.rpy:2336
translate italian d9_breakfast_bd7e709d:

    # ma "Sure."
    ma "Certo."

# game/events_mc.rpy:2337
translate italian d9_breakfast_843e8754:

    # ma "Do you want me to send them via email?"
    ma "Vuoi che le invii via email?"

# game/events_mc.rpy:2341
translate italian d9_leavehome_0b3883cc:

    # pc "I sadly don't have a printer here..."
    pc "Purtroppo non ho una stampante qui..."

# game/events_mc.rpy:2342
translate italian d9_leavehome_6f8c5996:

    # ma "Oh, well I can send them by post, but you could also come over during business hours, our office is in the same city."
    ma "Oh, bene posso mandarli per posta, ma si potrebbe anche venire durante gli orari di lavoro, il nostro ufficio è nella stessa città."

# game/events_mc.rpy:2343
translate italian d9_leavehome_25c8e98b:

    # pc "Okay, I could come over Monday, what's the address?"
    pc "Va bene, potrei venire lunedì, qual è l'indirizzo?"

# game/events_mc.rpy:2344
translate italian d9_leavehome_2cbd6059:

    # ma "It's... well, actually..."
    ma "E... beh, in realtà..."

# game/events_mc.rpy:2345
translate italian d9_leavehome_e35ac279:

    # pc "Hmm?"
    pc "Hmm?"

# game/events_mc.rpy:2346
translate italian d9_leavehome_069ecdd9:

    # ma "We usually don't accept customers on sunday, but I wanted to do some overtime anyway, so you could come over after closing time if you want."
    ma "Di solito non accettiamo i clienti la domenica, ma volevo comunque fare qualche straordinario, così si potrebbe venire oltre dopo il tempo di chiusura se si desidera."

# game/events_mc.rpy:2347
translate italian d9_leavehome_9dbc81eb:

    # pc "Hmm, yeah sounds good, I was thinking about going to the city anyway."
    pc "Hmm, sì suona bene, stavo pensando di andare in città comunque."

# game/events_mc.rpy:2348
translate italian d9_leavehome_490058b6:

    # ma "Okay, the address is..."
    ma "Okay, l'indirizzo è..."

# game/events_mc.rpy:2350
translate italian d9_leavehome_08d9627c:

    # n "You note the address and even though it's still a lot of time until closing time, you get ready to leave..."
    n "Si nota l'indirizzo e anche se è ancora un sacco di tempo fino all'orario di chiusura, si è pronti a partire..."

# game/events_mc.rpy:2355
translate italian d9_backhome_pcroom_f834cc31:

    # n "A while later..."
    n "Un po' più tardi..."

# game/events_mc.rpy:2359
translate italian d9_backhome_pcroom_e4ab4b10:

    # j "The movie is better than I thought, if only the screen was bigger..."
    j "Il film è migliore di quanto pensassi, se solo lo schermo era più grande..."

# game/events_mc.rpy:2361
translate italian d9_backhome_pcroom_7ccf276c:

    # j "How come you don't have a TV?"
    j "Come mai non hai una TV?"

# game/events_mc.rpy:2363
translate italian d9_backhome_pcroom_a289ebbc:

    # pc "I had one, but it broke when I was packing my stuff before moving."
    pc "Ne avevo uno, ma si è rotto quando stavo imballando la mia roba prima di muoversi."

# game/events_mc.rpy:2365
translate italian d9_backhome_pcroom_42ce01d2:

    # j "Oh shit, that sucks, you really should get a new one."
    j "Oh merda, che succhia, si dovrebbe davvero ottenerne uno nuovo."

# game/events_mc.rpy:2367
translate italian d9_backhome_pcroom_e0db8102:

    # pc "Yeah, you're right..."
    pc "Sì, hai ragione..."

# game/events_mc.rpy:2369
translate italian d9_backhome_pcroom_46f8b8fb:

    # j "Hey, looks like he's going to bang the princess, hehe."
    j "Ehi, sembra che stia per scopare la principessa, hehe."

# game/events_mc.rpy:2370
translate italian d9_backhome_pcroom_9c76afe8:

    # pcthink "[j]... always naughty..."
    pcthink "[j]... sempre cattivo..."

# game/events_mc.rpy:2372
translate italian d9_backhome_pcroom_63365631:

    # j "Night, [pcmd]."
    j "Notte, [pcmd]."

# game/events_mc.rpy:2373
translate italian d9_backhome_pcroom_4376661d:

    # pc "Night you two."
    pc "Notte voi due."

# game/events_mc.rpy:2375
translate italian d9_backhome_pcroom_8664ba0b:

    # pcthink "Well, I guess I'm going to bed too..."
    pcthink "Beh, immagino che andrò a letto troppo..."

# game/events_mc.rpy:2377
translate italian d9_backhome_pcroom_215e1221:

    # pcthink "I better file the papers somewhere before I forget about it..."
    pcthink "Ho meglio a archiviare i documenti da qualche parte prima di dimenticarlo..."

# game/events_mc.rpy:2379
translate italian d9_backhome_pcroom_787daedf:

    # pcthink "What's this? That's not my name..."
    pcthink "Cos'è questo? Non è il mio nome..."

# game/events_mc.rpy:2380
translate italian d9_backhome_pcroom_8c991f22:

    # pcthink "Did she hand me the wrong papers?"
    pcthink "Mi ha consegnato i documenti sbagliati?"

# game/events_mc.rpy:2381
translate italian d9_backhome_pcroom_071734fa:

    # pcthink "Shit, I better call her tomorrow..."
    pcthink "Merda, meglio chiamarla domani..."

# game/events_mc.rpy:2382
translate italian d9_backhome_pcroom_ac7ec890:

    # pcthink "Oh damn, I don't have her number."
    pcthink "Oh dannato, non ho il suo numero."

# game/events_mc.rpy:2385
translate italian d9_backhome_pcroom_905de5b0:

    # pcthink "Huh? [h] tried to call me?!"
    pcthink "Huh? [h] ha provato a chiamarmi?!"

# game/events_mc.rpy:2386
translate italian d9_backhome_pcroom_d7f47e07:

    # pcthink "Oh shit, I wanted to text her..."
    pcthink "Oh merda, volevo scriverla..."

# game/events_mc.rpy:2387
translate italian d9_backhome_pcroom_328d0f51:

    # pcthink "Wait, tomorrow is the date!"
    pcthink "Aspetta, domani è la data!"

# game/events_mc.rpy:2388
translate italian d9_backhome_pcroom_379d93ec:

    # pcthink "Dammit, I totally forgot about that."
    pcthink "Dammit, ne ho completamente dimenticato."

# game/events_mc.rpy:2390
translate italian d9_backhome_pcroom_b15122a3:

    # pcthink "Alright, I'll call her, I hope she's still awake..."
    pcthink "Bene, la chiamerò, spero che lei sia ancora sveglia..."

# game/events_mc.rpy:2393
translate italian d9_backhome_pcroom_4eeeb2c1:

    # pcthink "Maybe I should send her a message instead..."
    pcthink "Forse invece dovrei inviarle un messaggio..."

# game/events_mc.rpy:2395
translate italian d9_backhome_pcroom_10363c10:

    # h "Yes, yes, I'm awake! Ah... I mean... hi [pc]!"
    h "Sì, sì, sono sveglio! Ah... Voglio dire... ciao [pc]!"

# game/events_mc.rpy:2396
translate italian d9_backhome_pcroom_87896395:

    # pc "Oh shit, did I wake you up? I'm sorry..."
    pc "Oh merda, ti ho svegliato? Mi dispiace..."

# game/events_mc.rpy:2397
translate italian d9_backhome_pcroom_71f0a118:

    # h "Nono, I'm totally awake, it's fine, really!"
    h "Nono, sono totalmente sveglio, va bene, davvero!"

# game/events_mc.rpy:2398
translate italian d9_backhome_pcroom_b4ec99ba:

    # pcthink "Damn, she's cute."
    pcthink "Dannazione, è carina."

# game/events_mc.rpy:2400
translate italian d9_backhome_pcroom_32a88a20:

    # pc "I just saw that you tried to call me and thought I'd call back."
    pc "Ho appena visto che hai provato a chiamarmi e ho pensato che avrei richiamato."

# game/events_mc.rpy:2401
translate italian d9_backhome_pcroom_7abd5452:

    # h "Oh, haha, I was just... uhm... since you didn't text me..."
    h "Oh, haha, ero solo... uhm... dal momento che non mi hai scritto..."

# game/events_mc.rpy:2403
translate italian d9_backhome_pcroom_baec15ef:

    # pc "Yeah, sorry about that, it was a really busy week. Lots of stuff happened."
    pc "Sì, mi dispiace di questo, è stata una settimana davvero occupata. Molte cose sono accadute."

# game/events_mc.rpy:2405
translate italian d9_backhome_pcroom_7bbac3ba:

    # h "It did for sure!"
    h "Lo ha fatto di sicuro!"

# game/events_mc.rpy:2406
translate italian d9_backhome_pcroom_f560ac96:

    # h "So do you still want to go on a date tomorrow?"
    h "Quindi vuoi ancora andare a un appuntamento domani?"

# game/events_mc.rpy:2407
translate italian d9_backhome_pcroom_7e06d964:

    # pc "Yes, of course!"
    pc "Sì, ovviamente!"

# game/events_mc.rpy:2408
translate italian d9_backhome_pcroom_fb659fe9:

    # h "{size=-20}Yes!{/size}... ahem... good, but uhm... the lock down basically disrupted all my plans..."
    h "{size=-20}Sì!{/size}... ahem... buono, ma uhm... il blocco giù fondamentalmente ha interrotto tutti i miei piani..."

# game/events_mc.rpy:2409
translate italian d9_backhome_pcroom_9bebaa59:

    # pc "Lock down?"
    pc "Bloccare?"

# game/events_mc.rpy:2410
translate italian d9_backhome_pcroom_7c618874:

    # h "You didn't get the news yet? All public areas like restaurants, bars and so on are closed from tomorrow on."
    h "Non hai ancora ricevuto la notizia? Tutte le aree pubbliche come ristoranti, bar e così via sono chiuse da domani."

# game/events_mc.rpy:2411
translate italian d9_backhome_pcroom_a7483d27:

    # pc "What?! Why?"
    pc "Cosa?! Perché?"

# game/events_mc.rpy:2412
translate italian d9_backhome_pcroom_e570a7e4:

    # h "Uhh... you don't watch TV?"
    h "Uhh... non guardi la TV?"

# game/events_mc.rpy:2413
translate italian d9_backhome_pcroom_52dde854:

    # pc "Not much lately..."
    pc "Non molto recentemente..."

# game/events_mc.rpy:2414
translate italian d9_backhome_pcroom_641e39b8:

    # h "You've really been busy, haven't you?"
    h "Sei stato davvero occupato, vero?"

# game/events_mc.rpy:2415
translate italian d9_backhome_pcroom_429b9a1d:

    # pc "Yeah... sorry."
    pc "Sì... mi dispiace."

# game/events_mc.rpy:2416
translate italian d9_backhome_pcroom_3f39ef32:

    # h "It's okay, it's nice that you didn't forget about me."
    h "Va bene, è bello che tu non ti sei dimenticato di me."

# game/events_mc.rpy:2418
translate italian d9_backhome_pcroom_35c9fc02:

    # pc "How could I forget such a cute girl."
    pc "Come potrei dimenticare una ragazza così carina."

# game/events_mc.rpy:2419
translate italian d9_backhome_pcroom_d4e6c9d6:

    # h "Haha, thanks!"
    h "Haha, grazie!"

# game/events_mc.rpy:2420
translate italian d9_backhome_pcroom_8ddf7669:

    # pc "So where do you want to go tomorrow if everything's closed?"
    pc "Dove vuoi andare domani se tutto è chiuso?"

# game/events_mc.rpy:2421
translate italian d9_backhome_pcroom_a39d5d94:

    # h "Oh I have an idea already, but I won't tell, hehe."
    h "Oh ho già un'idea, ma non dirò, hehe."

# game/events_mc.rpy:2422
translate italian d9_backhome_pcroom_84edfdb0:

    # pc "Oh come on, that's mean."
    pc "Oh vieni, è meschino."

# game/events_mc.rpy:2423
translate italian d9_backhome_pcroom_121e0175:

    # h "Hehe... well, there is a nice place I'd like to go."
    h "Hehe... beh, c'è un bel posto che vorrei andare."

# game/events_mc.rpy:2424
translate italian d9_backhome_pcroom_227113d8:

    # pc "Where is it?"
    pc "Dov'è questo?"

# game/events_mc.rpy:2425
translate italian d9_backhome_pcroom_f6f5d842:

    # h "Hmm... I'll tell you this much: It's a quiet and beautiful place."
    h "Mmm... ve lo dirò molto: è un posto tranquillo e bello."

# game/events_mc.rpy:2426
translate italian d9_backhome_pcroom_40891c8f:

    # pc "Quiet and beautiful, huh?"
    pc "Tranquillo e bello, eh?"

# game/events_mc.rpy:2427
translate italian d9_backhome_pcroom_ba1b1223:

    # h "Yes, I think you'll like it."
    h "Sì, penso che ti piacerà."

# game/events_mc.rpy:2428
translate italian d9_backhome_pcroom_70cd033b:

    # pc "I bet!"
    pc "Scommetto!"

# game/events_mc.rpy:2430
translate italian d9_backhome_pcroom_e2a2f85a:

    # pc "Do I need a coat and tie?"
    pc "Ho bisogno di un cappotto e una cravatta?"

# game/events_mc.rpy:2432
translate italian d9_backhome_pcroom_b4c4f045:

    # pc "Do I need a fancy dress?"
    pc "Ho bisogno di un vestito elegante?"

# game/events_mc.rpy:2433
translate italian d9_backhome_pcroom_b39a23b4:

    # h "Haha, no! You'll look good anyway..."
    h "Haha, no! Sarai comunque bravo..."

# game/events_mc.rpy:2434
translate italian d9_backhome_pcroom_723709d4:

    # pc "Now you're making me blush."
    pc "Ora mi stai facendo arrossire."

# game/events_mc.rpy:2435
translate italian d9_backhome_pcroom_84e2e925:

    # h "Hehe... {size=-20}*yaaaaawn*{/size}"
    h "Hehehe... {size=-20}*yaaaaawn*{/size}"

# game/events_mc.rpy:2436
translate italian d9_backhome_pcroom_feedf2db:

    # pc "Well, I can't wait, but I better let you sleep now."
    pc "Beh, non posso aspettare, ma è meglio lasciarti dormire adesso."

# game/events_mc.rpy:2437
translate italian d9_backhome_pcroom_cd6fd29c:

    # h "Oh I'm not tired, really!"
    h "Oh non sono stanco, davvero!"

# game/events_mc.rpy:2438
translate italian d9_backhome_pcroom_034c3085:

    # pc "Heh, we'll have a lot of time to talk tomorrow, and you know what?"
    pc "Heh, avremo molto tempo per parlare domani, e sai cosa?"

# game/events_mc.rpy:2439
translate italian d9_backhome_pcroom_ff932e82:

    # h "What?"
    h "Cosa?"

# game/events_mc.rpy:2440
translate italian d9_backhome_pcroom_54362f51:

    # pc "It will be even better, because we can see each other."
    pc "Sarà ancora meglio, perché possiamo vederci a vicenda."

# game/events_mc.rpy:2441
translate italian d9_backhome_pcroom_419d2aa8:

    # h "Haha, right! And maybe even..."
    h "Haha, giusto! E forse anche..."

# game/events_mc.rpy:2442
translate italian d9_backhome_pcroom_182bc93e:

    # h "Touch... haha!"
    h "Tocco... haha!"

# game/events_mc.rpy:2443
translate italian d9_backhome_pcroom_09f0e787:

    # pc "Haha, you're cute! Good night and sweet dreams, [h]!"
    pc "Haha, sei carino! Buona notte e dolci sogni, [h]!"

# game/events_mc.rpy:2444
translate italian d9_backhome_pcroom_79363e4c:

    # h "You too! Sweet dreams, [pc]! See you tomorrow!"
    h "Anche tu! Sogni dolci, [pc]! Ci vediamo domani!"

# game/events_mc.rpy:2445
translate italian d9_backhome_pcroom_806f8391:

    # pc "See you tomorrow, [h]!"
    pc "Ci vediamo domani, [h]!"

# game/events_mc.rpy:2447
translate italian d9_backhome_pcroom_a84cf61e:

    # pcthink "Wow, she's so cute..."
    pcthink "Wow, è così carina..."

# game/events_mc.rpy:2449
translate italian d9_backhome_pcroom_276756bd:

    # pcthink "I'm curious where she wants to go tomorrow..."
    pcthink "Sono curioso dove vuole andare domani..."

# game/events_mc.rpy:2451
translate italian d9_backhome_pcroom_8cbec36f:

    # pcthink "I guess I'll have to go to her office again..."
    pcthink "Immagino che dovrò andare di nuovo nel suo ufficio..."

# game/events_mc.rpy:2452
translate italian d9_backhome_pcroom_2b1354a9:

    # pcthink "Anyway, time to sleep."
    pcthink "Comunque, il tempo di dormire."

# game/events_mc.rpy:2460
translate italian d9dream_99c802ff:

    # Voice" "*Sniff*"
    "Voce *Sniff*"

# game/events_mc.rpy:2461
translate italian d9dream_8f56933b:

    # n "You can hear a weeping noise."
    n "Puoi sentire un rumore da piangere."

# game/events_mc.rpy:2462
translate italian d9dream_e701d572:

    # pc "Dad?"
    pc "Papà?"

# game/events_mc.rpy:2463
translate italian d9dream_9aef7dc5:

    # Dad" "Oh hey, [pc]... *sniff*"
    "Papà" "Oh hey, [pc]... *sniff*"

# game/events_mc.rpy:2464
translate italian d9dream_e6e5c075:

    # n "He wipes tears from his eyes. You can see him holding a photo of your mom in his hands."
    n "Si asciuga le lacrime dagli occhi. Puoi vederlo tenendo in mano una foto di tua mamma."

# game/events_mc.rpy:2465
translate italian d9dream_84c70355:

    # Dad" "You're back early..."
    "Papà" "Sei tornato presto..."

# game/events_mc.rpy:2466
translate italian d9dream_96535a88:

    # pc "Are you crying, Dad?"
    pc "Stai piangendo, papà?"

# game/events_mc.rpy:2467
translate italian d9dream_e24b33bd:

    # Dad" "No... *sniff* ...must've gotten something in my eye..."
    "Papà" "No... *sniff* ...deve aver ottenuto qualcosa nei miei occhi..."

# game/events_mc.rpy:2468
translate italian d9dream_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:2469
translate italian d9dream_da7061f8:

    # Dad" "Do you miss your mom, [pc]?"
    "Papà" "Ti manca tua mamma, [pc]?"

# game/events_mc.rpy:2470
translate italian d9dream_3b5608b8:

    # pc "Yes..."
    pc "Sì..."

# game/events_mc.rpy:2471
translate italian d9dream_234a4777:

    # Dad" "Hm... you should forget about her..."
    "Papà" "Hm... dovresti dimenticare di lei..."

# game/events_mc.rpy:2472
translate italian d9dream_0628eca5:

    # pc "I don't want to forget her!"
    pc "Non voglio dimenticarla!"

# game/events_mc.rpy:2473
translate italian d9dream_d262fbf2:

    # Dad" "But it's better for you... for both of us..."
    "Papà" "Ma è meglio per voi... per entrambi noi..."

# game/events_mc.rpy:2474
translate italian d9dream_85c0867b:

    # Dad" "...and do yourself a favor and never fall in love..."
    "Papà" "...e farti un favore e non innamorarti mai..."

# game/events_mc.rpy:2475
translate italian d9dream_a69377ef_1:

    # pc "..."
    pc "..."

# game/events_mc.rpy:2476
translate italian d9dream_785d229f:

    # n "He puts the photo of your mom into a drawer."
    n "Mette la foto di tua mamma in un cassetto."

# game/events_mc.rpy:2477
translate italian d9dream_f363bd8e:

    # pc "Don't you love [m], Dad?"
    pc "Non ami [m], papà?"

# game/events_mc.rpy:2478
translate italian d9dream_51070d0f:

    # Dad" "[m]?! Pff..."
    "Papà" "[m]?! Pff..."

# game/events_mc.rpy:2479
translate italian d9dream_14cfced5:

    # Dad" "That b... she doesn't even know how to cook a proper meal, let alone how to suck a..."
    "Papà" "Che b... non sa nemmeno come cucinare un pasto corretto, figuriamoci come succhiare..."

# game/events_mc.rpy:2480
translate italian d9dream_8854a3cf:

    # Dad" "Uch... damn it..."
    "Papà" "Uch... maledetto..."

# game/events_mc.rpy:2481
translate italian d9dream_d582bfa7:

    # Dad" "Just always remember that she's not your mom, even if she tries to be... she's useless and she'll never be your mom!"
    "Papà" "Basta ricordare sempre che non è tua mamma, anche se lei cerca di essere... è inutile e lei non sarà mai tua mamma!"

# game/events_mc.rpy:2482
translate italian d9dream_5d7119e2:

    # pc "...okay..."
    pc "...okay..."

# game/events_mc.rpy:2483
translate italian d9dream_155a3712:

    # Dad" "Now go back to your room or go out and play or whatever you do..."
    "Papà" "Ora torna nella tua stanza o esci a giocare o qualsiasi cosa facci..."

# game/events_mc.rpy:2484
translate italian d9dream_880b4aa8:

    # pc "Okay..."
    pc "Okay..."

# game/events_mc.rpy:2485
translate italian d9dream_465f1434:

    # n "You're about to leave the room..."
    n "Stai per lasciare la stanza..."

# game/events_mc.rpy:2486
translate italian d9dream_14e7ad76:

    # Dad" "[pc]?"
    "Papà" "[pc]?"

# game/events_mc.rpy:2487
translate italian d9dream_bbe33639:

    # pc "Yes, Dad?"
    pc "Sì, papà?"

# game/events_mc.rpy:2488
translate italian d9dream_0b01384d:

    # Dad" "I'm sorry that I..."
    "Papà" "Mi dispiace che..."

# game/events_mc.rpy:2489
translate italian d9dream_503d5266:

    # Dad" "...sometimes..."
    "Papà" "...A volte..."

# game/events_mc.rpy:2490
translate italian d9dream_cde1b3be:

    # Dad" "..."
    "Papà" "..."

# game/events_mc.rpy:2491
translate italian d9dream_b283b4e3:

    # Dad" "Never mind, just go..."
    "Papà non importa, basta andare..."

# game/events_mc.rpy:2497
translate italian d10wakeup_e2ee86ba:

    # pc "Uhh..."
    pc "Uhh..."

# game/events_mc.rpy:2498
translate italian d10wakeup_bfc6ca53:

    # pc "{cps=10}...{/cps}"
    pc "{cps=10}...{/cps}"

# game/events_mc.rpy:2499
translate italian d10wakeup_debe3bfd:

    # pcthink "These dreams..."
    pcthink "Questi sogni..."

# game/events_mc.rpy:2500
translate italian d10wakeup_1c3a6ac3:

    # pcthink "...I hate them..."
    pcthink "... li odio..."

# game/events_mc.rpy:2501
translate italian d10wakeup_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:2502
translate italian d10wakeup_015a74bc:

    # pcthink "I think that was the only time ever I saw the old man crying..."
    pcthink "Penso che sia stata l'unica volta in cui ho visto il vecchio piangere..."

# game/events_mc.rpy:2503
translate italian d10wakeup_8fd5c2f4:

    # pcthink "He didn't even cry at Mom's funeral..."
    pcthink "Non ha nemmeno pianto al funerale della mamma..."

# game/events_mc.rpy:2504
translate italian d10wakeup_f0cac90d_1:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:2505
translate italian d10wakeup_853f810c:

    # pcthink "Shit, time to get up."
    pcthink "Merda, tempo per alzarsi."

# game/events_mc.rpy:2507
translate italian d10wakeup_550133db:

    # n "You get up and take a shower."
    n "Ti alzi e fai una doccia."

# game/events_mc.rpy:2511
translate italian d10pcroom_11ffdab3:

    # pcthink "Alright, what to do now... hmm..."
    pcthink "Bene, cosa fare ora... hmm..."

# game/events_mc.rpy:2514
translate italian d10pcroom_82548f9b:

    # j "Morning!"
    j "Mattina!"

# game/events_mc.rpy:2515
translate italian d10pcroom_c3dd90d4:

    # pc "Mornin' [j], what's up?"
    pc "Mornin' [j], che cos'è?"

# game/events_mc.rpy:2516
translate italian d10pcroom_ba46d8da:

    # j "Follow me, I want to show you something!"
    j "Seguimi, voglio mostrarti qualcosa!"

# game/events_mc.rpy:2517
translate italian d10pcroom_9c6c5efc:

    # pc "Huh? What is it?"
    pc "Huh? Che cos'è?"

# game/events_mc.rpy:2519
translate italian d10pcroom_e2c8352b:

    # j "Just come, it's a surprise!"
    j "Appena venuto, è una sorpresa!"

# game/events_mc.rpy:2520
translate italian d10pcroom_76c0b06f:

    # pc "Eh... fine..."
    pc "Eh... fine..."

# game/events_mc.rpy:2522
translate italian d10pcroom_2aea1f34:

    # n "You follow her to [e]'s room."
    n "La segui nella stanza di [e]."

# game/events_mc.rpy:2529
translate italian d10johnson_225fadf5:

    # pc "Huh?"
    pc "Huh?"

# game/events_mc.rpy:2531
translate italian d10johnson_c97cde28:

    # pc "What's going on here? [oc]?!"
    pc "Cosa succede qui? [oc]?!"

# game/events_mc.rpy:2533
translate italian d10johnson_30d49bc7:

    # m "Oh [pc], there was this policeman here while you were gone..."
    m "Oh [pc], c'era questo poliziotto qui mentre eri andato..."

# game/events_mc.rpy:2535
translate italian d10johnson_3f113d39:

    # oc "Looks like [w] still doesn't want to give up."
    oc "Sembra che [w] non voglia ancora arrendersi."

# game/events_mc.rpy:2537
translate italian d10johnson_89238e9a:

    # pc "[w]? Oh no, not that guy again."
    pc "[w]? Oh no, non più quel ragazzo."

# game/events_mc.rpy:2539
translate italian d10johnson_cf23b743:

    # j "You know that guy?"
    j "Tu sai quel tipo?"

# game/events_mc.rpy:2540
translate italian d10johnson_72964e10:

    # pc "Sadly, yes. What happened, what did he want?"
    pc "Purtroppo, sì. Che cos'è successo, che cosa voleva?"

# game/events_mc.rpy:2541
translate italian d10johnson_32b17776:

    # j "He was asking weird questions and acted like we're all gangsters..."
    j "Stava facendo domande strane e ha agito come siamo tutti gangsters..."

# game/events_mc.rpy:2543
translate italian d10johnson_9f733bc4:

    # oc "[m] called me after he was gone."
    oc "[m] mi ha chiamato dopo che era andato."

# game/events_mc.rpy:2545
translate italian d10johnson_a731af6d:

    # m "I didn't know what to do, I was confused and scared."
    m "Non sapevo cosa fare, ero confuso e spaventato."

# game/events_mc.rpy:2547
translate italian d10johnson_b6f21aca:

    # oc "It's okay, [m], he can't do anything."
    oc "Va bene, [m], non può fare nulla."

# game/events_mc.rpy:2549
translate italian d10johnson_a57c8079:

    # pc "Jeez, [oc], isn't there anything you can do to stop him? Do we have to get an injunction?"
    pc "Jeez, [oc], non c'è nulla che puoi fare per fermarlo? Dobbiamo ottenere un'ingiunzione?"

# game/events_mc.rpy:2551
translate italian d10johnson_3bf3dfc8:

    # oc "It's not that easy, especially not with all the shit you've done in the past."
    oc "Non è così facile, soprattutto non con tutta la merda che hai fatto in passato."

# game/events_mc.rpy:2553
translate italian d10johnson_c8912b9c:

    # pc "Oh come on, [oc], it's been years now."
    pc "Oh vieni, [oc], sono passati anni adesso."

# game/events_mc.rpy:2555
translate italian d10johnson_0c42f081:

    # oc "Yeah, but not long enough to be removed from your file, if there's anything he can link to you, any old case, even if it's just a suspicion, he has all the rights to go after it."
    oc "Sì, ma non abbastanza a lungo per essere rimosso dal tuo file, se c'è qualcosa che può collegare a te, qualsiasi vecchio caso, anche se è solo un sospetto, ha tutti i diritti di andare dopo."

# game/events_mc.rpy:2557
translate italian d10johnson_a01f7be6:

    # pc "Shit! So we can't do anything?"
    pc "Ciao! Quindi non possiamo fare niente?"

# game/events_mc.rpy:2559
translate italian d10johnson_5af31d80:

    # oc "Nobody really takes him seriously and I told the chief about your case, so as long as [w] doesn't find any hard evidence for something, he won't be able to do much more than asking questions and be as annoying as always."
    oc "Nessuno lo prende davvero sul serio e ho detto al capo del tuo caso, fino a quando [w] non trova alcuna prova dura per qualcosa, non sarà in grado di fare molto di più che fare domande ed essere fastidioso come sempre."

# game/events_mc.rpy:2561
translate italian d10johnson_d9907ef1:

    # m "Will he come back?"
    m "Ritornerà?"

# game/events_mc.rpy:2563
translate italian d10johnson_cd8a6e2e:

    # oc "Possibly, but you can safely ignore him. He knows he can't do anything without getting himself into trouble."
    oc "Forse, ma puoi tranquillamente ignorarlo. Egli sa che non può fare nulla senza mettersi nei guai."

# game/events_mc.rpy:2565
translate italian d10johnson_c33ac23e:

    # oc "Just don't do anything he can use as pretence."
    oc "Semplicemente non fare nulla che possa usare come pretesa."

# game/events_mc.rpy:2567
translate italian d10johnson_6f057b0a:

    # m "Of course not!"
    m "Certamente no!"

# game/events_mc.rpy:2569
translate italian d10johnson_dd58edec:

    # oc "That was more or less directed at [pc]..."
    oc "Questo è stato più o meno diretto a [pc]..."

# game/events_mc.rpy:2571
translate italian d10johnson_c9a0e6ec:

    # oc "...and [j]."
    oc "...e [j]."

# game/events_mc.rpy:2573
translate italian d10johnson_d1743dfa:

    # pc "Wait, you know each other?"
    pc "Aspetta, ti conosci a vicenda?"

# game/events_mc.rpy:2575
translate italian d10johnson_4042c389:

    # j "Eh...haha..."
    j "Eh...haha..."

# game/events_mc.rpy:2577
translate italian d10johnson_e9043f27:

    # oc "Let's just say I wasn't surprised to find her here..."
    oc "Diciamo solo che non ero sorpreso di trovarla qui..."

# game/events_mc.rpy:2579
translate italian d10johnson_aaf54574:

    # m "[j], I didn't know you had trouble with the law?"
    m "[j], Non sapevo che aveste avuto problemi con la legge?"

# game/events_mc.rpy:2581
translate italian d10johnson_5122a028:

    # j "It was an accident, I swear!"
    j "È stato un incidente, ho giurato!"

# game/events_mc.rpy:2583
translate italian d10johnson_df701526:

    # oc "Don't worry about it, [m], she's just a little trouble maker, I'm sure [pc] can handle her."
    oc "Non preoccuparti, [m], è solo un piccolo produttore di guai, sono sicuro che [pc] può gestirla."

# game/events_mc.rpy:2584
translate italian d10johnson_9fbca821:

    # j "Huh?"
    j "Huh?"

# game/events_mc.rpy:2586
translate italian d10johnson_15060167:

    # oc "Anyway, my wife is waiting, so I better take my leave. Call me if there's any trouble again."
    oc "In ogni caso, mia moglie sta aspettando, quindi è meglio prendere il mio congedo. Chiamami se c'è di nuovo qualche problema."

# game/events_mc.rpy:2588
translate italian d10johnson_0b42741a:

    # m "Thanks, [oc], give her my regards."
    m "Grazie, [oc], date a lei i miei saluti."

# game/events_mc.rpy:2590
translate italian d10johnson_449844e5:

    # oc "Sure."
    oc "Certo."

# game/events_mc.rpy:2592
translate italian d10johnson_c9095921:

    # oc "...and you stay clear of trouble."
    oc "...e si rimane al riparo dai problemi."

# game/events_mc.rpy:2593
translate italian d10johnson_3639171b:

    # pc "Eh? Sure, I'll try my best."
    pc "Eh? Certo, proverò il mio meglio."

# game/events_mc.rpy:2595
translate italian d10johnson_522fc3ca:

    # oc "{size=-10}...and take good care of [j], she needs someone like you to look after her.{/size}"
    oc "{size=-10}...e prendersi cura di [j], ha bisogno di qualcuno come te per prendersi cura di lei.{/size}"

# game/events_mc.rpy:2597
translate italian d10johnson_36bb3adf:

    # pc "I know, thanks, [oc]."
    pc "Lo so, grazie, [oc]."

# game/events_mc.rpy:2600
translate italian d10johnson_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:2602
translate italian d10johnson_f57abf22:

    # m "So, I think there's something we should talk about, [j]... but first..."
    m "Quindi, penso che ci sia qualcosa di cui dovremmo parlare, [j]... ma prima..."

# game/events_mc.rpy:2604
translate italian d10johnson_f6a949de:

    # m "...who is that [w] guy, [pc]?"
    m "...chi è quel [w] ragazzo, [pc]?"

# game/events_mc.rpy:2606
translate italian d10johnson_1299eaec:

    # n "You explain the situation, how you met [w] on your very first day at the burning apartment and all the other occasions when you bumped into each other. How he suspected you to be a criminal for no reason..."
    n "Spieghi la situazione, come hai incontrato [w] il tuo primo giorno presso l'appartamento in fiamme e tutte le altre occasioni in cui ti sei imbattuto l'un l'altro. Come egli sospettava di essere un criminale per nessuna ragione..."

# game/events_mc.rpy:2608
translate italian d10johnson_113d07fe:

    # m "My god, I hope he doesn't dig too deep..."
    m "Mio dio, spero che non scava troppo in profondità..."

# game/events_mc.rpy:2610
translate italian d10johnson_640bba00:

    # e "Nh!"
    e "Nh!"

# game/events_mc.rpy:2612
translate italian d10johnson_0bc33d60:

    # pc "As [oc] said, he can't do anything as long as we don't give him a reason."
    pc "Come [oc] ha detto, non può fare nulla finché non gli diamo una ragione."

# game/events_mc.rpy:2614
translate italian d10johnson_09fa3a44:

    # j "What an annoying guy."
    j "Che tipo fastidioso."

# game/events_mc.rpy:2616
translate italian d10johnson_78fa2646:

    # m "Oh right, isn't there something you wanted to tell us, [j]? How did you and [oc] come to know each other?"
    m "Oh giusto, non c'è qualcosa che volevi dirci, [j]? Come hai fatto tu e [oc] a conoscersi?"

# game/events_mc.rpy:2618
translate italian d10johnson_3773ac01:

    # j "Eh... it's nothing, really!"
    j "Eh... non è niente, veramente!"

# game/events_mc.rpy:2620
translate italian d10johnson_3225628c:

    # m "[j], if you want to live with us, then you should at least be honest !"
    m "[j], se vuoi vivere con noi, allora dovresti almeno essere onesto !"

# game/events_mc.rpy:2623
translate italian d10johnson_22eff034:

    # m "I mean..."
    m "Voglio dire..."

# game/events_mc.rpy:2625
translate italian d10johnson_ac734b6c:

    # m "Don't you agree, [pc]?"
    m "Non sei d'accordo, [pc]?"

# game/events_mc.rpy:2626
translate italian d10johnson_36106d45:

    # pc "Hm? Well, yeah, she's right, [j]."
    pc "Hm? Beh, sì, ha ragione, [j]."

# game/events_mc.rpy:2628
translate italian d10johnson_26f32268:

    # j "Hmph... fine..."
    j "Hmph... fine..."

# game/events_mc.rpy:2629
translate italian d10johnson_2a972d71:

    # pcthink "This is probably going to be about the bike she took from her mom's ex..."
    pcthink "Questo probabilmente sarà circa la moto che ha preso dalla mamma ex..."

# game/events_mc.rpy:2631
translate italian d10johnson_5d7b2792:

    # n "As expected, [j] talks about the motorcycle, but then she goes on about the argument she had with her Mom, explaining that it wasn't the first time, then about what happened in school and even bits and pieces about things that happened in the past."
    n "Come previsto, [j] parla della moto, ma poi prosegue la discussione che ha avuto con la mamma, spiegando che non è stata la prima volta, poi su quello che è successo a scuola e anche pezzi e pezzi su cose che sono accadute in passato."

# game/events_mc.rpy:2632
translate italian d10johnson_fb0503dc:

    # n "All in all it's nothing too dramatic, but it seems like she just needed a little push to go all out. She talks about her childhood, how she's never seen her real Dad, how most of her Mom's boyfriends were assholes or didn't care about her..."
    n "Tutto sommato non è niente di troppo drammatico, ma sembra che lei abbia solo bisogno di un po 'di spinta per uscire tutto. Lei parla della sua infanzia, come lei non ha mai visto il suo vero papà, come la maggior parte dei fidanzati di sua mamma erano stronzi o non si preoccupava di lei..."

# game/events_mc.rpy:2633
translate italian d10johnson_365476fa:

    # n "After roughly an hour..."
    n "Dopo circa un'ora..."

# game/events_mc.rpy:2635
translate italian d10johnson_b4fa3fc0:

    # j "..."
    j "..."

# game/events_mc.rpy:2636
translate italian d10johnson_e612037c:

    # m "..."
    m "..."

# game/events_mc.rpy:2638
translate italian d10johnson_72a33094:

    # e "..."
    e "..."

# game/events_mc.rpy:2639
translate italian d10johnson_a7c89ecd:

    # pc "Err..."
    pc "Err..."

# game/events_mc.rpy:2641
translate italian d10johnson_88ec1494:

    # pc "Well, now that you're here with us, you don't have to worry about that anymore."
    pc "Beh, ora che sei qui con noi, non devi più preoccuparti di questo."

# game/events_mc.rpy:2643
translate italian d10johnson_ee170f72:

    # m "Absolutely, let the past be the past and focus on the future."
    m "Assolutamente, lasciate che il passato sia il passato e concentratevi sul futuro."

# game/events_mc.rpy:2645
translate italian d10johnson_9810575c:

    # j "Eh, sorry, I didn't mean to tell my whole life story!"
    j "Eh, mi dispiace, non avevo intenzione di raccontare tutta la mia storia di vita!"

# game/events_mc.rpy:2646
translate italian d10johnson_43e4e875:

    # pc "Sometimes things have to be said."
    pc "A volte le cose devono essere dette."

# game/events_mc.rpy:2648
translate italian d10johnson_e0a84304:

    # j "Maybe, but it feels like I've been talking all evening..."
    j "Forse, ma sembra che io abbia parlato tutta la sera..."

# game/events_mc.rpy:2650
translate italian d10johnson_55dd0a42:

    # m "Oh, right. I should make dinner."
    m "Oh, giusto. Dovrei fare la cena."

# game/events_mc.rpy:2652
translate italian d10johnson_e59f988c:

    # j "I-I'll help!"
    j "Aiuterò!"

# game/events_mc.rpy:2653
translate italian d10johnson_97a38a54:

    # m "You don't need to, [j]."
    m "Non è necessario, [j]."

# game/events_mc.rpy:2654
translate italian d10johnson_7509b543:

    # j "But I want to..."
    j "Ma io voglio..."

# game/events_mc.rpy:2656
translate italian d10johnson_5f7c3303:

    # pc "*Smirk*"
    pc "*Smirk*"

# game/events_mc.rpy:2659
translate italian d10johnson_39681a47:

    # pc "Haha, the new Housekeeper update is funny!"
    pc "Haha, il nuovo aggiornamento Housekeeper è divertente!"

# game/events_mc.rpy:2661
translate italian d10johnson_2d2fab39:

    # pcthink "Too bad it's male MC only..."
    pcthink "Peccato che sia solo MC maschile..."

# game/events_mc.rpy:2663
translate italian d10johnson_f11f8b6b:

    # pc "Hey [e], you should have a look at this!"
    pc "Ehi [e], dovresti dare un'occhiata a questo!"

# game/events_mc.rpy:2664
translate italian d10johnson_72a33094_1:

    # e "..."
    e "..."

# game/events_mc.rpy:2666
translate italian d10johnson_a9ffb6ca:

    # pc "...or just stay there and look cute."
    pc "o semplicemente stare lì e guardare carino."

# game/events_mc.rpy:2668
translate italian d10johnson_ff1cafc9:

    # pc "...or just stay there looking sexy."
    pc "o semplicemente stare lì sembra sexy."

# game/events_mc.rpy:2670
translate italian d10johnson_3fa6b1c9:

    # e "Khehehe."
    e "Khehehe."

# game/events_mc.rpy:2671
translate italian d10johnson_affa0beb:

    # pc "...hmm..."
    pc "...hmm..."

# game/events_mc.rpy:2672
translate italian d10johnson_bf217bfc:

    # pc "You know, I think [j] is right, I really should get a new TV."
    pc "Sai, penso che [j] abbia ragione, dovrei davvero avere una nuova TV."

# game/events_mc.rpy:2674
translate italian d10johnson_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:2675
translate italian d10johnson_84395206:

    # pc "By the way, why don't you have your own TV?"
    pc "A proposito, perché non hai la tua TV?"

# game/events_mc.rpy:2676
translate italian d10johnson_4e286571:

    # e "Hm..."
    e "Hm..."

# game/events_mc.rpy:2677
translate italian d10johnson_36c9a7bf:

    # pc "...and why does [m] think you're bad with electronics?"
    pc "...e perché [m] pensa di essere brutto con l'elettronica?"

# game/events_mc.rpy:2679
translate italian d10johnson_a0a4da04:

    # e "Uhh..."
    e "Uhh..."

# game/events_mc.rpy:2683
translate italian d10johnson_00b9e6c9:

    # j "Time to stop drooling over each other, dinner is readyyyyy~!"
    j "È ora di smettere di cadere l'un l'altro, la cena è prontayyyyyy~!"

# game/events_mc.rpy:2685
translate italian d10johnson_9a8ef00c:

    # n "You go have dinner and it tastes surprisingly well, even though you half expected it to be a disaster since [j] helped with it." with dissolve
    n "Si va a cena e ha un sapore sorprendentemente bene, anche se la metà si aspettava che fosse un disastro dal [j] aiutato con esso." with dissolve

# game/events_mc.rpy:2686
translate italian d10johnson_3855fdcb:

    # n "It's quite late already, so everyone quickly goes to bed after dinner, you decide do the same."
    n "È già abbastanza tardi, quindi tutti vanno rapidamente a letto dopo cena, si decide di fare lo stesso."

# game/events_mc.rpy:2701
translate italian d10nightsex_38cd2de2:

    # Voice" "Wakey wakey." with slowdissolve
    "Voce" "Wakey wakey." with slowdissolve

# game/events_mc.rpy:2702
translate italian d10nightsex_9f4f5a57:

    # pc "Uhh... is it morning already?"
    pc "Uhh... è già mattina?"

# game/events_mc.rpy:2703
translate italian d10nightsex_9bdc0426:

    # Voice" "No, Silly, it's still night!"
    "Voce" "No, Silly, è ancora notte!"

# game/events_mc.rpy:2704
translate italian d10nightsex_109cf5a7:

    # Voice" "Khehehe."
    "Voce" "Khehehe."

# game/events_mc.rpy:2706
translate italian d10nightsex_61e4f5ac:

    # pc "Huh?!"
    pc "Huh?!"

# game/events_mc.rpy:2708
translate italian d10nightsex_2391c076:

    # pc "[j]!?"
    pc "[j]!?"

# game/events_mc.rpy:2710
translate italian d10nightsex_6df89eaf:

    # pc "[e], what are you..."
    pc "[e], che cosa sei..."

# game/events_mc.rpy:2711
translate italian d10nightsex_3af9c0d7:

    # pc "Aren't you supposed to sleep in your bed?"
    pc "Non dovresti dormire nel tuo letto?"

# game/events_mc.rpy:2713
translate italian d10nightsex_72a33094:

    # e "..."
    e "..."

# game/events_mc.rpy:2714
translate italian d10nightsex_b74916c3:

    # j "Well, we're not sleeping..."
    j "Beh, non stiamo dormendo..."

# game/events_mc.rpy:2716
translate italian d10nightsex_7a3f2b13:

    # j "We're just here to say good night, hehe."
    j "Siamo qui solo per dire buona notte, hehe."

# game/events_mc.rpy:2717
translate italian d10nightsex_de5fe506:

    # pc "I see..."
    pc "Vedo che..."

# game/events_mc.rpy:2723
translate italian d10nightsex_aadd0180:

    # e "*Smootch*"
    e "*Smootch*"

# game/events_mc.rpy:2725
translate italian d10nightsex_bae972de:

    # pc "Oh, I think I like where this is going!"
    pc "Oh, penso che mi piaccia dove sta andando questo!"

# game/events_mc.rpy:2727
translate italian d10nightsex_6c679a4b:

    # j "So, are we allowed to stay a little longer, [pcmd]?"
    j "Dunque, ci è permesso rimanere un po' più a lungo, [pcmd]?"

# game/events_mc.rpy:2728
translate italian d10nightsex_41979139:

    # pc "How could I possibly say no."
    pc "Come potrei dire di no."

# game/events_mc.rpy:2729
translate italian d10nightsex_53154678:

    # j "Hmm, thank you, [pcmd]."
    j "Hmm, grazie, [pcmd]."

# game/events_mc.rpy:2731
translate italian d10nightsex_8bcba63c:

    # j "*Smootch*"
    j "*Smootch*"

# game/events_mc.rpy:2733
translate italian d10nightsex_8bcba63c_1:

    # j "*Smootch*"
    j "*Smootch*"

# game/events_mc.rpy:2734
translate italian d10nightsex_aadd0180_1:

    # e "*Smootch*"
    e "*Smootch*"

# game/events_mc.rpy:2736
translate italian d10nightsex_08a84e50:

    # pcthink "Damn, this is so hot!"
    pcthink "Dannazione, questo è così caldo!"

# game/events_mc.rpy:2749
translate italian d10nightsex_86321039:

    # j "You or me?"
    j "Tu o me?"

# game/events_mc.rpy:2751
translate italian d10nightsex_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:2753
translate italian d10nightsex_7ad69330:

    # j "You decide."
    j "Tu decidi."

# game/events_mc.rpy:2755
translate italian d10nightsex_bfd83a51:

    # pc "Decide what?"
    pc "Decidi cosa?"

# game/events_mc.rpy:2757
translate italian d10nightsex_54c5f8f0:

    # j "Whose pussy you want to lick, hehe."
    j "Di chi vuoi leccare, hehe."

# game/events_mc.rpy:2758
translate italian d10nightsex_96184459:

    # pc "Oh, I see."
    pc "Oh, vedo."

# game/events_mc.rpy:2760
translate italian d10nightsex_d8629ddd:

    # pc "Why not both?"
    pc "Perché non entrambi?"

# game/events_mc.rpy:2762
translate italian d10nightsex_46ae205e:

    # j "You can't do us both at the same time."
    j "Non puoi farci entrambi allo stesso tempo."

# game/events_mc.rpy:2764
translate italian d10nightsex_ea206375:

    # pc "Ha! Are you sure about that?"
    pc "Ciao! Sei sicuro?"

# game/events_mc.rpy:2767
translate italian d10nightsex_042cf2c5:

    # j "Haha, okay, who do you want to taste first then?"
    j "Haha, ok, chi vuoi assaggiare prima allora?"

# game/events_mc.rpy:2769
translate italian d10nightsex_f8a565cd:

    # j "Haha, but who gets your dick first?"
    j "Haha, ma chi ottiene il tuo cazzo prima?"

# game/events_mc.rpy:2771
translate italian d10nightsex_66c3f992:

    # pc "Tough question."
    pc "Domanda dura."

# game/events_mc.rpy:2775
translate italian d10nightfun_0fcb1868:

    # j "Awww..."
    j "Awww..."

# game/events_mc.rpy:2778
translate italian d10nightfun_fd2b49ba:

    # e "Mmm..."
    e "Mmm..."

# game/events_mc.rpy:2780
translate italian d10nightfun_ba55f128:

    # e "...nnnaah."
    e "...nnnaah."

# game/events_mc.rpy:2789
translate italian d10nightfun_3b76887f:

    # j "Yay!"
    j "Yay!"

# game/events_mc.rpy:2794
translate italian d10nightfun_03dc513d:

    # j "But leave me some pussy, okay, [e]?"
    j "Ma lasciami una figa, ok, [e]?"

# game/events_mc.rpy:2795
translate italian d10nightfun_36eeb504:

    # e "Kehehe."
    e "Kehehe."

# game/events_mc.rpy:2798
translate italian d10nle_d2992ced:

    # pc "Come here, [e]."
    pc "Vieni qui, [e]."

# game/events_mc.rpy:2800
translate italian d10nle_9d52e2e8:

    # j "I'll take that yummy pussy then."
    j "Allora prenderò quella figa magica."

# game/events_mc.rpy:2810
translate italian d10nle_b43efd55:

    # j "Mhhh... I love how you taste, [pcmd]!"
    j "Mhhh... Amo come ti gusti, [pcmd]!"

# game/events_mc.rpy:2811
translate italian d10nle_2d99ee61:

    # pc "Nhhh, and I love how you work your tongue!"
    pc "Nhhh, e amo come si lavora la lingua!"

# game/events_mc.rpy:2812
translate italian d10nle_380637e2:

    # j "Just don't come too early, you two!"
    j "Semplicemente non venite troppo presto, voi due!"

# game/events_mc.rpy:2813
translate italian d10nle_7f627bb0:

    # e "Hnnn... hn!"
    e "Hnnn... hn!"

# game/events_mc.rpy:2814
translate italian d10nle_86446217:

    # pc "I can't guarantee for anything if you go on like this, [j]."
    pc "Non posso garantire nulla se continui così, [j]."

# game/events_mc.rpy:2815
translate italian d10nle_02ea632c:

    # j "Hehe... mphh... I love... mhh... your pussy, [pcmd]."
    j "Hehe... mphh... Mi piace... mhh... la tua figa, [pcmd]."

# game/events_mc.rpy:2816
translate italian d10nle_8829d4f3:

    # j "I just... mhh... can't hold back!"
    j "Semplicemente... mhh... non posso trattenerti!"

# game/events_mc.rpy:2820
translate italian md10nle_3b7ae288:

    # pc "[j] come over here, I want to taste you as well!"
    pc "[j] venite qui, voglio assaggiarvi pure!"

# game/events_mc.rpy:2821
translate italian md10nle_3b76887f:

    # j "Yay!"
    j "Yay!"

# game/events_mc.rpy:2825
translate italian md10nle_e45fc757:

    # j "Here I cum, [pcmd]!"
    j "Here I cum, [pcmd]!"

# game/events_mc.rpy:2826
translate italian md10nle_76ae973b:

    # pc "Heh, not yet!"
    pc "Heh, non ancora!"

# game/events_mc.rpy:2829
translate italian md10nle_256cb5d1:

    # e "Nhh... nhhh..."
    e "Nhh... nhhh..."

# game/events_mc.rpy:2830
translate italian md10nle_04eac808:

    # j "[e]... mhh... hold on... mphh."
    j "[e]... mhh... tieni premuto su... mphh."

# game/events_mc.rpy:2831
translate italian md10nle_c3d61fdd:

    # e "Nhh... haaa..."
    e "Nhh... haaa..."

# game/events_mc.rpy:2832
translate italian md10nle_539e527d:

    # j "Hold on, [e], hold on..."
    j "Tieni premuto su, [e], tieni premuto..."

# game/events_mc.rpy:2836
translate italian md10nle_ace490b7:

    # e "Haaaa!"
    e "Haaaa!"

# game/events_mc.rpy:2838
translate italian md10nle_ed6b3306:

    # e "Haaa..."
    e "Haaa..."

# game/events_mc.rpy:2839
translate italian md10nle_6ac5ba71:

    # j "Dammit, [e]."
    j "Dammit, [e]."

# game/events_mc.rpy:2845
translate italian d10nlj_b4fa3fc0:

    # j "..."
    j "..."

# game/events_mc.rpy:2853
translate italian d10nlj_6db3b035:

    # j "Hnnn... I just... gnnnh... love your tongue... haaa...!"
    j "Hnnnn... Semplicemente... gnnnh... ama la tua lingua... haaa...!"

# game/events_mc.rpy:2854
translate italian d10nlj_743809c6:

    # j "Do you... hnn... like how I taste, [pcmd]?"
    j "Ti piace... hnn... ti piace come gusto, [pcmd]?"

# game/events_mc.rpy:2855
translate italian d10nlj_82adeed6:

    # pc "I love it!"
    pc "Mi piace!"

# game/events_mc.rpy:2856
translate italian d10nlj_e9278008:

    # j "Hnn... yes, [pcmd], right there."
    j "Hnn... sì, [pcmd], proprio lì."

# game/events_mc.rpy:2863
translate italian md10nlj_556b7777:

    # j "Your turn, [e]!"
    j "Il tuo turno, [e]!"

# game/events_mc.rpy:2864
translate italian md10nlj_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:2867
translate italian md10nlj_a8361128:

    # e "Mhhh..."
    e "Mhhh..."

# game/events_mc.rpy:2868
translate italian md10nlj_59cd70f8:

    # j "Gaawd... [pcmd]... nhh... I love it... hnnn."
    j "Gaawd... [pcmd]... nhh... Mi piace... hnnn."

# game/events_mc.rpy:2869
translate italian md10nlj_367bea97:

    # j "Hold on, hold on... fuck!"
    j "Attenersi, tieni premuto... fuck!"

# game/events_mc.rpy:2873
translate italian md10nlj_4f2be3b6:

    # j "Haaaa!"
    j "Haaaa!"

# game/events_mc.rpy:2875
translate italian md10nlj_ec25329c:

    # j "Haaa... your tongue feels amazing, [pcmd]."
    j "Haaa... la tua lingua è incredibile, [pcmd]."

# game/events_mc.rpy:2881
translate italian d10nfe_5916f987:

    # e "Hnn..."
    e "Hnn..."

# game/events_mc.rpy:2883
translate italian d10nfe_57867369:

    # e "Nhhhaa..."
    e "Nhhhaa..."

# game/events_mc.rpy:2886
translate italian d10nfe_b4fa3fc0:

    # j "..."
    j "..."

# game/events_mc.rpy:2890
translate italian d10nfe_fee914df:

    # j "Go on, [e]."
    j "Vai su, [e]."

# game/events_mc.rpy:2899
translate italian d10nfe_a4b12779:

    # e "Haa... haaa..."
    e "Haa... haaa..."

# game/events_mc.rpy:2900
translate italian d10nfe_31c6bd65:

    # j "Damn, [e], don't go too fast yet!"
    j "Dannazione, [e], non andare ancora troppo veloce!"

# game/events_mc.rpy:2902
translate italian d10nfe_8c2d61de:

    # j "I want some fun, too..."
    j "Anche io voglio un po' di divertimento..."

# game/events_mc.rpy:2904
translate italian d10nfe_2b66b452:

    # j "If you don't mind, [pcmd]."
    j "Se non ti dispiace, [pcmd]."

# game/events_mc.rpy:2905
translate italian d10nfe_651eb3f3:

    # pc "Get get over here, Babygirl!"
    pc "Ottenere superare qui, Babygirl!"

# game/events_mc.rpy:2906
translate italian d10nfe_3b76887f:

    # j "Yay!"
    j "Yay!"

# game/events_mc.rpy:2908
translate italian d10nfe_c2622bd7:

    # j "I hope you don't mind, [e]."
    j "Spero che non ti dispiaccia [e]."

# game/events_mc.rpy:2910
translate italian d10nfe_72a33094:

    # e "..."
    e "..."

# game/events_mc.rpy:2918
translate italian d10nfe_4d63df1b:

    # j "Hnn... how are you holding up, [e]?"
    j "Hnn... come ti stai trattenendo, [e]?"

# game/events_mc.rpy:2919
translate italian d10nfe_5983978b:

    # e "Nhhh!...hnnn..."
    e "Nhhh!...hnnn..."

# game/events_mc.rpy:2921
translate italian d10nfe_15da9523:

    # j "Ohhh, damn, [pcmd]... this feels great!"
    j "Ohhh, dannato, [pcmd]... questo si sente grande!"

# game/events_mc.rpy:2922
translate italian d10nfe_805a228c:

    # e "Hnn... nnhhh..."
    e "Hnn... nnhhh..."

# game/events_mc.rpy:2923
translate italian d10nfe_1e48254c:

    # j "Nhhh... yes, there... haaaa!"
    j "Nhhh... sì, lì... haaaa!"

# game/events_mc.rpy:2924
translate italian d10nfe_b8b3efba:

    # e "Haaa... nnhhh..."
    e "Haaa... nnhhh..."

# game/events_mc.rpy:2925
translate italian d10nfe_9ce693ec:

    # pcthink "Shit, [e] is tightening up..."
    pcthink "Merda, [e] sta stringendo..."

# game/events_mc.rpy:2932
translate italian md10nfe_f4d25661:

    # j "Now's my turn, [e]."
    j "Ora è il mio turno, [e]."

# game/events_mc.rpy:2933
translate italian md10nfe_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:2936
translate italian md10nfe_256cb5d1:

    # e "Nhh... nhhh..."
    e "Nhh... nhhh..."

# game/events_mc.rpy:2937
translate italian md10nfe_aeff8489:

    # j "Gaawd... [e]... nhh... hold on... hnnn."
    j "Gaawd... [e]... nhh... hold on... hnnn."

# game/events_mc.rpy:2938
translate italian md10nfe_c3d61fdd:

    # e "Nhh... haaa..."
    e "Nhh... haaa..."

# game/events_mc.rpy:2939
translate italian md10nfe_367bea97:

    # j "Hold on, hold on... fuck!"
    j "Attenersi, tieni premuto... fuck!"

# game/events_mc.rpy:2943
translate italian md10nfe_ace490b7:

    # e "Haaaa!"
    e "Haaaa!"

# game/events_mc.rpy:2945
translate italian md10nfe_ed6b3306:

    # e "Haaa..."
    e "Haaa..."

# game/events_mc.rpy:2951
translate italian d10nfj_5ff0b171:

    # j "Right there."
    j "Proprio lì."

# game/events_mc.rpy:2953
translate italian d10nfj_08681c4a:

    # j "Haa... yes!"
    j "Haa... sì!"

# game/events_mc.rpy:2954
translate italian d10nfj_bf503bc9:

    # pc "[e] come here."
    pc "[e] vieni qui."

# game/events_mc.rpy:2956
translate italian d10nfj_52d6b73f:

    # e "Hn..."
    e "Hn..."

# game/events_mc.rpy:2966
translate italian d10nfj_2e7b609d:

    # e "Hnnn... nhnnn!"
    e "Hnnn... nhnnn!"

# game/events_mc.rpy:2967
translate italian d10nfj_b26a3110:

    # j "Ohh gaaaawd, [pcmd], I love your dick!"
    j "Ohh gaaaawd, [pcmd], Amo il tuo cazzo!"

# game/events_mc.rpy:2968
translate italian d10nfj_dec085be:

    # j "Shit, hnn... [pcmd]."
    j "Merda, hnn... [pcmd]."

# game/events_mc.rpy:2969
translate italian d10nfj_c5f2357d:

    # e "Nhhh..."
    e "Nhhh..."

# game/events_mc.rpy:2970
translate italian d10nfj_2381c1d1:

    # j "[pcmd], [pcmd]!"
    j "[pcmd], [pcmd]!"

# game/events_mc.rpy:2978
translate italian md10nfj_256cb5d1:

    # e "Nhh... nhhh..."
    e "Nhh... nhhh..."

# game/events_mc.rpy:2979
translate italian md10nfj_aeff8489:

    # j "Gaawd... [e]... nhh... hold on... hnnn."
    j "Gaawd... [e]... nhh... hold on... hnnn."

# game/events_mc.rpy:2980
translate italian md10nfj_c3d61fdd:

    # e "Nhh... haaa..."
    e "Nhh... haaa..."

# game/events_mc.rpy:2981
translate italian md10nfj_367bea97:

    # j "Hold on, hold on... fuck!"
    j "Attenersi, tieni premuto... fuck!"

# game/events_mc.rpy:2985
translate italian md10nfj_ace490b7:

    # e "Haaaa!"
    e "Haaaa!"

# game/events_mc.rpy:2987
translate italian md10nfj_ed6b3306:

    # e "Haaa..."
    e "Haaa..."

# game/events_mc.rpy:2992
translate italian d10nend_93cccb90:

    # pc "I'm getting close!"
    pc "Mi sto avvicinando!"

# game/events_mc.rpy:2996
translate italian d10nend_06ff9bda:

    # j "[e], I think it's time for the grand finale."
    j "[e], penso che sia il momento per il finale grande."

# game/events_mc.rpy:2997
translate italian d10nend_9bee5006:

    # e "Hn!"
    e "Hn!"

# game/events_mc.rpy:2998
translate italian d10nend_38132b9b:

    # pc "Hey, why did you stop?"
    pc "Ehi, perché ti sei fermato?"

# game/events_mc.rpy:3000
translate italian d10nend_7ec0328a:

    # j "Like we agreed?"
    j "Ti siamo acconsentiti?"

# game/events_mc.rpy:3001
translate italian d10nend_61bb715b:

    # e "Mh-hm!"
    e "Mh-hm!"

# game/events_mc.rpy:3003
translate italian d10nend_9942717c:

    # j "Come on, get up, [pcmd]."
    j "Avanti, alzati, [pcmd]."

# game/events_mc.rpy:3004
translate italian d10nend_225fadf5:

    # pc "Huh?"
    pc "Huh?"

# game/events_mc.rpy:3006
translate italian d10nend_ad9342e9:

    # pc "What are you up to?"
    pc "A cosa stai facendo?"

# game/events_mc.rpy:3007
translate italian d10nend_405f0f47:

    # j "The grant finale! You on top of [e]."
    j "Il finanziamento finale! Sei in cima a [e]."

# game/events_mc.rpy:3008
translate italian d10nend_fd5d6251:

    # pc "Hmm... sounds good."
    pc "Hmm... suona bene."

# game/events_mc.rpy:3010
translate italian d10nend_3ea2c48b:

    # pc "Let's try this."
    pc "Proviamo questo."

# game/events_mc.rpy:3011
translate italian d10nend_9a273c4d:

    # j "Mmh... looks sexy! I'll watch!"
    j "Mmh... sembra sexy! Guarderò!"

# game/events_mc.rpy:3016
translate italian d10nend_0d66a17d:

    # j "Damn, this looks so hot!"
    j "Dannazione, questo sembra così caldo!"

# game/events_mc.rpy:3018
translate italian d10nend_d3597349:

    # j "You like it, [e]?"
    j "Ti piace, [e]?"

# game/events_mc.rpy:3019
translate italian d10nend_4563a8a3:

    # e "Hn!... nnhh..."
    e "Hn!... nnhh..."

# game/events_mc.rpy:3020
translate italian d10nend_a4e732ed:

    # j "Haha, obvious."
    j "Haha, ovvio."

# game/events_mc.rpy:3025
translate italian d10nend_9455801f:

    # pc "Hm?"
    pc "Hm?"

# game/events_mc.rpy:3027
translate italian d10nend_b4fa3fc0:

    # j "..."
    j "..."

# game/events_mc.rpy:3028
translate italian d10nend_19e9c6c1:

    # pc "Heh, come closer..."
    pc "Heh, avvicinati..."

# game/events_mc.rpy:3033
translate italian d10nend_cd975a4b:

    # j "Hnn, yes, [pcmd]!"
    j "Hnn, sì, [pcmd]!"

# game/events_mc.rpy:3038
translate italian d10nend_a2d0a918:

    # pc "Damn, I'm close, [e], are you ready?"
    pc "Dannazione, sono vicino, [e], sei pronto?"

# game/events_mc.rpy:3039
translate italian d10nend_7348351a:

    # e "Hnn!"
    e "Hnn!"

# game/events_mc.rpy:3040
translate italian d10nend_7fc31305:

    # j "Oh, [pcmd], I'm close, too!"
    j "Oh, [pcmd], anche io sono vicino!"

# game/events_mc.rpy:3041
translate italian d10nend_6ba83f13:

    # e "Hnnn... hnnnn..."
    e "Hnnn... hnnnnn..."

# game/events_mc.rpy:3043
translate italian d10nend_463e8cb9:

    # pc "Shit!"
    pc "Merda!"

# game/events_mc.rpy:3044
translate italian d10nend_c3b6f930:

    # j "Nhhh... fill her, [pcmd]! Fill her with your hot, juicy cum!"
    j "Nhhh... riempila [pcmd]! Riempila con il tuo sperma caldo e succoso!"

# game/events_mc.rpy:3045
translate italian d10nend_d4f1256f:

    # e "Hnnn... nhaaa..." with flash
    e "Hnnn... nhaaa..." with flash

# game/events_mc.rpy:3046
translate italian d10nend_b52f4e4b:

    # j "Fill your little sister's baby room with your seed, [pcmd]!"
    j "Riempi la stanza del bambino della tua sorella con il tuo seme, [pcmd]!"

# game/events_mc.rpy:3047
translate italian d10nend_0cf7346a:

    # pc "Holy shit!" with flash
    pc "Merda santa!" with flash

# game/events_mc.rpy:3049
translate italian d10nend_f2a00302:

    # e "Haaaaa!"
    e "Haaaaa!"

# game/events_mc.rpy:3052
translate italian d10nend_aeed5ca7:

    # pc "Gawd!"
    pc "Gawd!"

# game/events_mc.rpy:3055
translate italian d10nend_9ec09061:

    # pc "Damn..."
    pc "Dannazione..."

# game/events_mc.rpy:3058
translate italian d10nend_94e1dbb5:

    # pc "...phew..."
    pc "...phew..."

# game/events_mc.rpy:3059
translate italian d10nend_ff62eb80:

    # j "Mmmh, [pcmd] that was awesome!"
    j "Mmmh, [pcmd] è stato fantastico!"

# game/events_mc.rpy:3063
translate italian d10nend_ad9342e9_1:

    # pc "What are you up to?"
    pc "A cosa stai facendo?"

# game/events_mc.rpy:3065
translate italian d10nend_1bba5b0e:

    # j "You'll love it, promise!"
    j "Lo amerai, promessa!"

# game/events_mc.rpy:3067
translate italian d10nend_e4bf7a13:

    # pc "Now I'm curious."
    pc "Ora sono curioso."

# game/events_mc.rpy:3069
translate italian d10nend_3f66b558:

    # pc "Oh, I think I know..."
    pc "Oh, penso di sapere..."

# game/events_mc.rpy:3075
translate italian d10nend_e93f2588:

    # j "Do you like it, [pcmd]?"
    j "Ti piace, [pcmd]?"

# game/events_mc.rpy:3087
translate italian d10nend_e7217a9b:

    # pc "Hmmm... suck on my clit, [e]."
    pc "Hmmm... succhiare il mio clit, [e]."

# game/events_mc.rpy:3088
translate italian d10nend_a8361128:

    # e "Mhhh..."
    e "Mhhh..."

# game/events_mc.rpy:3089
translate italian d10nend_0c5d6f71:

    # pc "How could I not like it, with two beautiful, hot girls..."
    pc "Come non mi piace, con due belle ragazze calde..."

# game/events_mc.rpy:3091
translate italian d10nend_103333d8:

    # j "Mhh... do you like...mphh... getting eaten out by your... mhhh... little girls, [pcmd]?"
    j "Mhh... ti piace...mphh... farsi mangiare fuori dalla tua... mhhh... bambine, [pcmd]?"

# game/events_mc.rpy:3092
translate italian d10nend_a4dee7a9:

    # pc "It feels great, girls!"
    pc "Si sente benissimo, ragazze!"

# game/events_mc.rpy:3093
translate italian d10nend_ca813d77:

    # j "Mhhh... your skin is so smooth..."
    j "Mhhh... la tua pelle è così liscia..."

# game/events_mc.rpy:3094
translate italian d10nend_e4c5adeb:

    # e "Mnhh..."
    e "Mnhh..."

# game/events_mc.rpy:3095
translate italian d10nend_18b80caa:

    # j "Do you want to cum into your little sister's mouth, [pcmd]!"
    j "Vuoi ingoiare nella bocca della tua sorellina, [pcmd]!"

# game/events_mc.rpy:3096
translate italian d10nend_9ec09061_1:

    # pc "Damn..."
    pc "Dannazione..."

# game/events_mc.rpy:3097
translate italian d10nend_29c12796:

    # j "Look at her...mhhh... right between your legs... mmph... tasting your juicy, wet pussy..."
    j "Guardate...mhhh... a destra tra le gambe... mmph... assaggiare la vostra figa succosa, bagnata..."

# game/events_mc.rpy:3098
translate italian d10nend_82da1106:

    # pc "Damn, you look so sexy down there..."
    pc "Dannazione, si guarda così sexy laggiù..."

# game/events_mc.rpy:3099
translate italian d10nend_d9173080:

    # j "Let us taste all your yummy pussy juice... mmphh..."
    j "Facciamo assaggiare tutti i vostri succhi di figa... mmphh..."

# game/events_mc.rpy:3100
translate italian d10nend_a24b49bc:

    # pc "Shit..." with flash
    pc "Shit..." with flash

# game/events_mc.rpy:3101
translate italian d10nend_fe9c36e3:

    # j "Cum for us, [pcmd], mhh... cum for us!"
    j "Cum for us, [pcmd], mhh... cum for us!"

# game/events_mc.rpy:3102
translate italian d10nend_a1c8f658:

    # pc "Oh fuck! Don't stop, don't stop!" with flash
    pc "Oh scopata! Non fermarti, non fermarti!" with flash

# game/events_mc.rpy:3103
translate italian d10nend_808c8846:

    # j "Yes, [pcmd], cum for us!"
    j "Sì, [pcmd], cum per noi!"

# game/events_mc.rpy:3107
translate italian d10nend_dbd30717:

    # pc "FUCK!"
    pc "FUCK!"

# game/events_mc.rpy:3109
translate italian d10nend_cad09cce:

    # e "*Lick*"
    e "*Lick*"

# game/events_mc.rpy:3111
translate italian d10nend_98805c19:

    # e "*Smooch*"
    e "*Smooch*"

# game/events_mc.rpy:3113
translate italian d10nend_fa9126f2:

    # j "Gaaawd, we need to do this again some day."
    j "Gaaawd, abbiamo bisogno di farlo di nuovo un giorno."

# game/events_mc.rpy:3114
translate italian d10nend_c417e350:

    # pc "Yeah."
    pc "Sì."

# game/events_mc.rpy:3116
translate italian d10nend_49eceec2:

    # j "How about tomorrow night?"
    j "Che ne dici di domani sera?"

# game/events_mc.rpy:3117
translate italian d10nend_472fb1cf:

    # pc "Haha, sure."
    pc "Haha, certo."

# game/events_mc.rpy:3118
translate italian d10nend_51daeb63:

    # j "And the night after tomorrow?"
    j "E la notte dopo domani?"

# game/events_mc.rpy:3119
translate italian d10nend_3fa6b1c9:

    # e "Khehehe."
    e "Khehehe."

# game/events_mc.rpy:3120
translate italian d10nend_29ebc440:

    # pc "How about every night?"
    pc "Che ne dici di ogni notte?"

# game/events_mc.rpy:3121
translate italian d10nend_02818a2e:

    # j "Hehe, oh [pcmd], you're so naughty..."
    j "Hehe, oh [pcmd], sei così cattivo..."

# game/events_mc.rpy:3122
translate italian d10nend_c176eb1f:

    # pc "Hey, it's you two who came over to keep me from sleeping."
    pc "Ehi, siete voi due che siete venuti per impedirmi di dormire."

# game/events_mc.rpy:3123
translate italian d10nend_00309318:

    # j "You didn't seem to mind it much."
    j "Sembra che non ti dispiacesse molto."

# game/events_mc.rpy:3124
translate italian d10nend_611ff6e2:

    # pc "Speaking of which, maybe you two should go back to your room, [e] needs to get up early for school, right [e]?"
    pc "Parlando di cui, forse due dovreste tornare alla vostra stanza, [e] ha bisogno di alzarsi presto per la scuola, a destra [e]?"

# game/events_mc.rpy:3126
translate italian d10nend_95918891:

    # e "*Snooze*"
    e "*Snooze*"

# game/events_mc.rpy:3127
translate italian d10nend_9ec09061_2:

    # pc "Damn..."
    pc "Dannazione..."

# game/events_mc.rpy:3129
translate italian d10nend_2c0c9e9d:

    # j "I guess we're going to sleep here tonight."
    j "Immagino che stiamo per dormire qui stasera."

# game/events_mc.rpy:3130
translate italian d10nend_f312f230:

    # pc "Well, let's hope [m] doesn't notice anything."
    pc "Beh, speriamo che [m] non si accorga nulla."

# game/events_mc.rpy:3132
translate italian d10nend_d92e8f46:

    # j "We really need to make her change her mind, so she's okay with us sleeping here instead of [e]'s room."
    j "Abbiamo davvero bisogno di farla cambiare idea, quindi lei va bene con noi a dormire qui invece della stanza di [e]."

# game/events_mc.rpy:3133
translate italian d10nend_f9f62fc1:

    # pc "Maybe she'll change her mind in the future."
    pc "Forse cambierà idea in futuro."

# game/events_mc.rpy:3135
translate italian d10nend_48207abb:

    # j "I'm sure you can convince her, hehe."
    j "Sono sicuro che puoi convincerla, hehe."

# game/events_mc.rpy:3136
translate italian d10nend_462cc5bd:

    # pc "Maybe... wait, was that some kind of suggestion again?"
    pc "Forse... aspettare, è stato che qualche tipo di suggerimento di nuovo?"

# game/events_mc.rpy:3138
translate italian d10nend_1e448d60:

    # j "Hehe, good night, [pcmd]."
    j "Hehe, buona notte, [pcmd]."

# game/events_mc.rpy:3139
translate italian d10nend_510163bd:

    # pc "Oh well, whatever... Night, [j]..."
    pc "Oh bene, qualsiasi... Notte, [j]..."

# game/events_mc.rpy:3149
translate italian d11wakeup_a68105ea:

    # m "[pc], do you want to eat breakfast with us?"
    m "[pc], vuoi fare colazione con noi?"

# game/events_mc.rpy:3151
translate italian d11wakeup_8e3eb455:

    # pc "YAAAAAaaaawn..."
    pc "YAAAAAaaaawn..."

# game/events_mc.rpy:3154
translate italian d11wakeup_d9fd0b2d:

    # pcthink "Looks like they got up in time..."
    pcthink "Sembra che si siano alzati nel tempo..."

# game/events_mc.rpy:3156
translate italian d11wakeup_4f79c49f:

    # pcthink "Looks like [e] didn't come over again..."
    pcthink "Sembra che [e] non sia tornato più..."

# game/events_mc.rpy:3158
translate italian d11wakeup_4a57ea09:

    # m "[pc]?"
    m "[pc]?"

# game/events_mc.rpy:3160
translate italian d11wakeup_3538051c:

    # pc "Yeah, I'm coming..."
    pc "Sì, sto arrivando..."

# game/events_mc.rpy:3162
translate italian d11wakeup_b4ec8360:

    # n "You get up, take a quick shower and then join the others for breakfast. [j] and [m] seem to have been engaged in a conversation for a while..."
    n "Ti alzi, fai una doccia veloce e poi unisciti agli altri per colazione. [j] e [m] sembrano essere stati impegnati in una conversazione per un po'..."

# game/events_mc.rpy:3165
translate italian d11wakeup_2add307d:

    # j "...and then it all went down from there..."
    j "...e poi tutto è sceso da lì..."

# game/events_mc.rpy:3167
translate italian d11wakeup_8b73d057:

    # m "I see... say, [j], for how long are you suspended from school again?"
    m "Vedo che... diciamo, [j], per quanto tempo sei sospeso di nuovo dalla scuola?"

# game/events_mc.rpy:3169
translate italian d11wakeup_1a481b63:

    # j "This week still."
    j "Questa settimana ancora."

# game/events_mc.rpy:3171
translate italian d11wakeup_f0f7d53a:

    # m "I see, any plans for today?"
    m "Vedo, qualche piano per oggi?"

# game/events_mc.rpy:3173
translate italian d11wakeup_abc21079:

    # j "Uhh... no, not really."
    j "Uhh... no, non veramente."

# game/events_mc.rpy:3175
translate italian d11wakeup_c177f7ea:

    # m "Do you have any plans for the future? Maybe you could look for a job, it's not that long anymore until school is out."
    m "Hai dei piani per il futuro? Forse potresti cercare un lavoro, non è più così tanto finché la scuola non è fuori."

# game/events_mc.rpy:3177
translate italian d11wakeup_70f903af:

    # j "I... don't know. I will probably end up as a waitress anyway..."
    j "Io... non lo so. Probabilmente finirò comunque come cameriera..."

# game/events_mc.rpy:3179
translate italian d11wakeup_8c91b24d:

    # m "You don't have any plans for the future?"
    m "Non hai piani per il futuro?"

# game/events_mc.rpy:3181
translate italian d11wakeup_6d0ab809:

    # j "It's not like I have much to chose from."
    j "Non è come se avessi molto da scegliere."

# game/events_mc.rpy:3182
translate italian d11wakeup_844c942c:

    # pc "I didn't know what to do either."
    pc "Neanche io sapevo cosa fare."

# game/events_mc.rpy:3184
translate italian d11wakeup_227f3a09:

    # j "Hm?"
    j "Hm?"

# game/events_mc.rpy:3185
translate italian d11wakeup_8151da85:

    # pc "After school... I didn't have a degree, I had no idea what to do, nor did I care at the time... and look at me now, I'm basically self-employed and do what I like."
    pc "Dopo la scuola... non avevo una laurea, non avevo idea di cosa fare, né mi importavo all'epoca. . – e guardatemi ora, sono fondamentalmente autonomo e faccio quello che mi piace."

# game/events_mc.rpy:3187
translate italian d11wakeup_3aa4d591:

    # m "You don't have a degree?"
    m "Non hai una laurea?"

# game/events_mc.rpy:3189
translate italian d11wakeup_c2b4ba68:

    # pc "I didn't, you know how I left... I got my degree a few years later, but until then I still had many different jobs."
    pc "Non l'ho fatto, sapete come sono partita... ho ottenuto la mia laurea qualche anno dopo, ma fino ad allora ho ancora avuto molti lavori diversi."

# game/events_mc.rpy:3190
translate italian d11wakeup_add3d30b:

    # pc "I'm not saying that a degree doesn't matter, it does, but just because you don't have a degree doesn't mean you can't do anything. There are still a lot of opportunities in life."
    pc "Non sto dicendo che un grado non importa, lo fa, ma solo perché non hai un grado non significa che non puoi fare nulla. Ci sono ancora molte opportunità nella vita."

# game/events_mc.rpy:3192
translate italian d11wakeup_19c1e4e2:

    # m "I see..."
    m "Vedo che..."

# game/events_mc.rpy:3193
translate italian d11wakeup_a69377ef:

    # pc "..."
    pc "..."

# game/events_mc.rpy:3195
translate italian d11wakeup_cb5a7a19:

    # m "So, what are your plans for today?"
    m "Quindi, quali sono i tuoi piani per oggi?"

# game/events_mc.rpy:3197
translate italian d11wakeup_577d4c6e:

    # pc "Today is the deadline for the project I was working on, so I'll have to present it at work today."
    pc "Oggi è la scadenza per il progetto su cui stavo lavorando, quindi dovrò presentarlo oggi al lavoro."

# game/events_mc.rpy:3199
translate italian d11wakeup_38ab289d:

    # m "I see, sounds like it'll be a busy day for you."
    m "Vedo, suona come sarà una giornata occupata per te."

# game/events_mc.rpy:3201
translate italian d11wakeup_92e2e8a0:

    # pc "Yeah, probably..."
    pc "Sì, probabilmente..."

# game/events_mc.rpy:3203
translate italian d11wakeup_d7508814:

    # n "After breakfast, [e] and [m] get ready to leave for the day. To everyone's surprise, [j] demands to take care of the dishes... you decide to look through the project one more time before you leave for town."
    n "Dopo la colazione, [e] e [m] preparatevi a partire per la giornata. Con la sorpresa di tutti, [j] richieste di prendersi cura dei piatti... si decide di guardare attraverso il progetto ancora una volta prima di partire per la città."

# game/events_mc.rpy:3208
translate italian d11bhpcroom_d6d25b7b:

    # pcthink "Hmm... what to do now..."
    pcthink "Hmm... cosa fare ora..."

# game/events_mc.rpy:3209
translate italian d11bhpcroom_f0cac90d:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:3210
translate italian d11bhpcroom_a2266564:

    # pcthink "Maybe I should look for a good TV to buy..."
    pcthink "Forse dovrei cercare una buona TV da comprare..."

# game/events_mc.rpy:3211
translate italian d11bhpcroom_f7367be0:

    # pcthink "I mean, it looks like I'll stay..."
    pcthink "Intendo dire che sembra che resterò..."

# game/events_mc.rpy:3212
translate italian d11bhpcroom_35ad042e:

    # pcthink "Right?"
    pcthink "Destra?"

# game/events_mc.rpy:3213
translate italian d11bhpcroom_f0cac90d_1:

    # pcthink "..."
    pcthink "..."

# game/events_mc.rpy:3215
translate italian d11bhpcroom_40a6633b:

    # pcthink "Damn, my life really took a 180 over the last two weeks..."
    pcthink "Dannazione, la mia vita ha davvero preso un 180 nelle ultime due settimane..."

# game/events_mc.rpy:3218
translate italian d11bhpcroom_3dd86183:

    # j "Whatcha doing?"
    j "Che cosa fa?"

# game/events_mc.rpy:3220
translate italian d11bhpcroom_953fabce:

    # pc "I was just about to look for a TV online."
    pc "Stavo per cercare una TV online."

# game/events_mc.rpy:3222
translate italian d11bhpcroom_dc67dfba:

    # j "Finally! That's great! Can I help?"
    j "Finalmente! È fantastico! Posso aiutare?"

# game/events_mc.rpy:3223
translate italian d11bhpcroom_79572587:

    # pc "Sure..."
    pc "Certo..."

# game/events_mc.rpy:3225
translate italian d11bhpcroom_a7b99095:

    # n "*Doorbell rings*"
    n "*Campanella suoneria*"

# game/events_mc.rpy:3227
translate italian d11bhpcroom_9455801f:

    # pc "Hm?"
    pc "Hm?"

# game/events_mc.rpy:3228
translate italian d11bhpcroom_f7a2c366:

    # j "Are you waiting for somebody?"
    j "Stai aspettando qualcuno?"

# game/events_mc.rpy:3229
translate italian d11bhpcroom_3cdf5875:

    # pc "No, not that I remember..."
    pc "No, non che ricordo..."

# game/events_mc.rpy:3231
translate italian d11bhpcroom_0b75a0f5:

    # j "Oh no, I hope it's not that weird cop again..."
    j "Oh no, spero che non sia più così strano poliziotto..."

translate italian strings:

    # game/events_mc.rpy:249
    old "Watch TV in the living room"
    new "Guarda la TV nel soggiorno"

    # game/events_mc.rpy:505
    old "{i}(Damn, I love the idea...){/i}"
    new "{i}(Damn, amo l'idea...){/i}"

    # game/events_mc.rpy:505
    old "{i}(There is no way I would ever do that!){/i}"
    new "{i}(Non c'è modo di farlo mai!){/i}"

    # game/events_mc.rpy:572
    old "She will come over tonight anyway... gimme porn!"
    new "Lei verrà questa sera comunque... gimme porno!"

    # game/events_mc.rpy:572
    old "I should really talk to her now!"
    new "Dovrei davvero parlare con lei adesso!"

    # game/events_mc.rpy:707
    old "Introduce yourself."
    new "Introduci te stesso."

    # game/events_mc.rpy:707
    old "Say good bye and leave."
    new "Dite il buon addio e partite."

    # game/events_mc.rpy:1518
    old "Call her?"
    new "Chiamatela?"

    # game/events_mc.rpy:1518
    old "Call her!"
    new "Chiamatela!"

    # game/events_mc.rpy:1518
    old "Nah, not now."
    new "Nah, non adesso."

    # game/events_mc.rpy:1620
    old "Push [j] out of the bed."
    new "Spingi [j] fuori dal letto."

    # game/events_mc.rpy:1620
    old "Hide [e] under the blanket."
    new "Nascondi [e] sotto la coperta."

    # game/events_mc.rpy:1735
    old "Tell them you're sorry."
    new "Di' loro che ti dispiace."

    # game/events_mc.rpy:1735
    old "Talk about the weather."
    new "Parla del tempo."

    # game/events_mc.rpy:2197
    old "Call her Mom."
    new "Chiama la sua mamma."

    # game/events_mc.rpy:2197
    old "Change the topic."
    new "Cambia l'argomento."

    # game/events_mc.rpy:2320
    old "The good news."
    new "Le buone notizie."

    # game/events_mc.rpy:2320
    old "The bad news."
    new "Le brutte notizie."

    # game/events_mc.rpy:2772
    old "[e]."
    new "[e]."

    # game/events_mc.rpy:2772
    old "[j]."
    new "[j]."

    # game/events_mc.rpy:2817
    old "Switch to [j]."
    new "Passa a [j]."

    # game/events_mc.rpy:2817
    old "Go on, [e]."
    new "Vai su, [e]."

    # game/events_mc.rpy:2817
    old "Cum."
    new "Cum."

    # game/events_mc.rpy:2857
    old "Switch to [e]."
    new "Passa a [e]."

    # game/events_mc.rpy:2857
    old "Go on, [j]."
    new "Vai su, [j]."

    # game/events_mc.rpy:2926
    old "Switch to fuck [j]."
    new "Passa al cazzo [j]."

    # game/events_mc.rpy:2971
    old "Switch to fuck [e]."
    new "Passa al cazzo [e]."

